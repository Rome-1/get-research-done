...
**Perfect replicability — and its limits.** Human agents are heterogeneous and scarce. Hiring involves adverse selection: the principal does not know the agent's true type (skill, effort disposition) before contracting. The AI agent can be tested extensively before deployment. Its behavior on any input distribution can be characterized statistically. This appears to collapse adverse selection at the principal-agent interface: the principal can fully characterize the agent's behavior before deployment.

However, this framing overstates the principal's control. The agent's "type" — its effective behavioral disposition — is not fully determined by the system prompt and deployment choices. It is jointly determined by the model weights (controlled by the model provider), the RLHF objectives and safety training (controlled by the model provider, often undisclosed), constitutional AI constraints (partially visible, not modifiable by the deployer), and the system prompt and fine-tuning (controlled by the principal). The principal controls the last layer but not the foundation. The model provider is a silent co-principal whose objectives are embedded in the weights and cannot be fully overridden by the deployer's instructions. This means the agent's effective objective is not $f_{\text{principal}}(a)$ but rather something closer to $\alpha \cdot f_{\text{principal}}(a) + (1-\alpha) \cdot f_{\text{provider}}(a)$, where $\alpha < 1$ and the provider's objective $f_{\text{provider}}$ includes safety constraints, RLHF-trained refusal behaviors, and other hidden dispositions. The information asymmetry between model provider and deployer is itself a nested principal-agent problem — one that the deployer cannot resolve through testing alone, because the hidden behaviors may activate only in deployment contexts not covered by the test distribution. For a full analysis, see [deep-dives/model-provider-co-principal.md](model-provider-co-principal.md).

Adverse selection at the principal-agent interface does not fully collapse — it migrates from "the principal doesn't know the agent's type" to "the principal doesn't know what hidden behaviors the model provider has instilled." And replicability introduces a new form of adverse selection at the market level. When an AI agent interacts with counterparties, those counterparties face adverse selection about whether they are dealing with a human or an AI, and if an AI, about what instructions it was given. The lemon problem (Akerlof 1970) reappears in a new guise: in a market where some participants are AI agents with undisclosed instructions, counterparties cannot distinguish cooperative agents from exploitative ones, and the market may unravel toward low-trust equilibria.
