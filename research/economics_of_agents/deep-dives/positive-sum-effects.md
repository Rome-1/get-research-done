...
**The current trajectory is concerning.** As documented in the [prediction markets analysis](prediction-markets-attention.md), the foundation model landscape is consolidating: a small number of model families (GPT, Claude, Gemini, Llama derivatives) account for the vast majority of deployed agents. Open-weight models increase nominal diversity but often share training data and architectural patterns. The effective diversity of AI agent strategies is likely *decreasing* even as the raw number of deployed agents increases. If this trend continues, the negative-sum effects will increasingly dominate in information-sensitive markets (prediction markets, price discovery, credit assessment) even as the positive-sum effects persist in markets where diversity matters less (transaction cost reduction, matching efficiency). Monitoring and promoting genuine architectural diversity — not just the number of competing products — is therefore a first-order policy priority.
