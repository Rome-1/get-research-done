...
- **Observability streams** (`GPD/observability/`): Historical event logs are primarily machine-local telemetry. They travel with git, but `GPD/observability/current-execution.json` contributes to the portable bounded-segment resume state only when its `resume_file` still resolves to a repo-local path on the current machine. If a live execution snapshot's `resume_file` is non-project, missing, or otherwise unusable, the snapshot remains advisory continuity context only: it stays visible as live execution state but does not create a resumable `current_execution` candidate or set `resume_mode="bounded_segment"`. A recorded `session.resume_file` is handled separately.
