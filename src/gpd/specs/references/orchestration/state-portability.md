...
The current continuation architecture separates execution provenance from bounded-resume authority. The portable execution story now has an append-only lineage record plus a derived execution head, while `gpd init resume` computes the canonical continuation view by reading `state.json.continuation` first and only consulting compatibility surfaces when the canonical bounded segment is missing or incomplete. The canonical public resume surface centers on `active_resume_kind`, `active_resume_origin`, `active_resume_pointer`, `active_bounded_segment`, `derived_execution_head`, `continuity_handoff_file`, `recorded_continuity_handoff_file`, `missing_continuity_handoff_file`, and `resume_candidates`. Legacy alias names stay nested as compatibility-only cues inside `compat_resume_surface.segment_candidates` and the `session` mirror; they do not compete with the canonical fields. Nested compatibility-only cues remain available for migration and compatibility intake, but the public top-level resume payload does not promote them alongside canonical fields. `session`, `.continue-here.md`, and `DERIVATION-STATE.md` are supporting projections only; they may mirror canonical continuation or provenance, but they do not compete with it:
...
- **Task level**: bounded execution state, active resume pointer, interrupted agents, and live review gates (the current raw envelope still exposes nested compatibility-only cues such as `current_execution`, `execution_resume_file`, and `segment_candidates` under `compat_resume_surface`)
...
2. **Resume artifacts**: Canonical `state.json.continuation.bounded_segment` is the primary durable bounded-segment state. A derived execution head contributes a bounded-segment candidate only when that canonical field is absent and its `resume_file` still resolves to a portable repo-local path. When `session.resume_file` resolves to an existing repo-local file, it surfaces as a non-resumable recorded handoff candidate, and it becomes `continuity_handoff_file` / `active_resume_pointer` only as the fallback pointer when no usable derived execution head exists. If the recorded handoff is repo-local but missing, resume surfaces a missing recorded handoff artifact plus an advisory recorded handoff candidate with `status: "missing"`. In the current raw envelope those compatibility cues still appear as nested legacy aliases `session_resume_file` and `missing_session_resume_file` under `compat_resume_surface`. A derived head without a portable usable resume file remains advisory continuity context only.
...
The resume workflow ranks three semantic recovery families into `resume_candidates`: bounded execution snapshot, recorded handoff, and interrupted agent. In the current raw envelope those families still appear as the nested compatibility aliases `current_execution`, `session_resume_file`, and `interrupted_agent` inside `compat_resume_surface.segment_candidates`. The recorded handoff source can appear either as a non-resumable `handoff` candidate or as an advisory `missing` candidate. It does not synthesize additional candidates from auto-checkpoints, filesystem drift, or machine-change state. A `.continue-here.md` file by itself is only a temporary continuation handoff artifact; it becomes part of the canonical continuation view only when reached through the ranked runtime surfaces. `current_execution` remains the compatibility fallback whenever canonical continuation is missing or incomplete; when canonical `state.json.continuation` is complete, it stays the source of truth first.
...
Current public behavior therefore distinguishes canonical continuation authority, continuity mirrors, and the derived execution head, while keeping legacy alias names nested as compatibility-only cues:
