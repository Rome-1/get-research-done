...
Keep `uncertainty_markers` explicit in the `contract_results` ledger, and use concrete non-blank entries in `weakest_anchors` and `disconfirming_observations` that name the actual unresolved evidence gap or contrary observation. Do not use filler placeholders.
