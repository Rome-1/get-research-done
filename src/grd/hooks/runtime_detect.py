"""Runtime detection for GRD hooks.
...
from grd.core.constants import ENV_GRD_ACTIVE_RUNTIME, PLANNING_DIR_NAME, TODOS_DIR_NAME
from grd.hooks.install_metadata import install_scope_from_manifest, load_install_manifest_runtime_status
...
def _explicit_runtime_override() -> str | None:
    """Return an explicit runtime override supplied by GRD-owned shell surfaces."""
    return normalize_runtime_name(os.environ.get(ENV_GRD_ACTIVE_RUNTIME))
...
def _has_grd_install(
    config_dir: Path,
) -> bool:
    """Return True when *config_dir* has stable markers of a GRD install."""
...
def _runtime_dir_has_grd_install(
    runtime: str,
    *,
    cwd: Path | None = None,
    home: Path | None = None,
    ...
    """Return whether *runtime* has a concrete GRD install in the requested locations."""
    resolved_cwd = cwd or Path.cwd()
    resolved_home = home or Path.home()

    if include_local and _has_grd_install(_local_runtime_dir(runtime, resolved_cwd), cwd=resolved_cwd, home=resolved_home):
        return True
    if include_global and _has_grd_install(_global_runtime_dir(runtime, home=resolved_home), cwd=resolved_cwd, home=resolved_home):
        return True
    return False
...
def _detect_runtime_install_target(
    ...
    *,
    ...
    if _has_grd_install(local_dir, cwd=resolved_cwd, home=resolved_home):
    ...
    if _has_grd_install(global_dir, cwd=resolved_cwd, home=resolved_home):
        return RuntimeInstallTarget(
...
def resolve_effective_runtime(
    *,
    ...
    require_grd_install: bool = False,
    ...
    if override_runtime is not None and not require_grd_install:
        ...
        return EffectiveRuntimeResolution(
            ...
            source=SOURCE_ENV,
            has_grd_install=install_target is not None,
            install_scope=None if install_target is None else install_target.install_scope,
        )

    if override_runtime is not None and require_grd_install:
        install_target = _detect_runtime_install_target(override_runtime, cwd=resolved_cwd, home=resolved_home)
        if install_target is not None:
            return EffectiveRuntimeResolution(
                runtime=override_runtime,
                source=SOURCE_ENV,
                has_grd_install=True,
                install_scope=install_target.install_scope,
            )
        return EffectiveRuntimeResolution()

    if not require_grd_install:
...
def detect_active_runtime_with_grd_install(*, cwd: Path | None = None, home: Path | None = None) -> str:
    """Detect the active runtime only when that runtime also has a GRD install."""
    preferred_runtime = detect_active_runtime(cwd=cwd, home=home)
    return resolve_effective_runtime(
        cwd=cwd,
        home=home,
        preferred_runtime=preferred_runtime,
        require_grd_install=True,
    ).runtime


def detect_runtime_for_grd_use(*, cwd: Path | None = None, home: Path | None = None) -> str:
    """Return the runtime GRD should target for installed surfaces.

    Prefer a runtime that both appears active and has a concrete GRD install.
    ...
    """
    installed_runtime = detect_active_runtime_with_grd_install(cwd=cwd, home=home)
...
def detect_install_scope(
    ...
    *,
    ...
    resolved_runtime = runtime or detect_runtime_for_grd_use(cwd=cwd, home=home)
...
def _resolved_priority_runtime(
    ...
    *,
    ...
    return detect_runtime_for_grd_use(cwd=cwd, home=home)
...
def should_consider_update_cache_candidate(
    ...
    *,
    ...
    and a different runtime currently has a live GRD install. This prevents stale
    ...
    if manifest_state == "missing":
        return False
    if not _has_grd_install(candidate_config_dir, cwd=cwd, home=home):
        return False
    ...
    if not _runtime_dir_has_grd_install(active_installed_runtime, cwd=cwd, home=home):
        return True
...
def get_grd_install_dirs(
    *, prefer_active: bool = False, cwd: Path | None = None, home: Path | None = None
    ...
    resolved_cwd = cwd or Path.cwd()
    resolved_home = home or Path.home()
    prioritized_runtime = detect_runtime_for_grd_use(cwd=resolved_cwd, home=resolved_home)
    ...
    if prioritized_runtime in runtime_names:
        for runtime_dir, _scope in _ordered_runtime_dirs_for_lookup(
            ...
            dirs.append(runtime_dir / GRD_INSTALL_DIR_NAME)

    for runtime in runtime_names:
        if runtime == prioritized_runtime:
            continue
        dirs.append(_local_runtime_dir(runtime, resolved_cwd) / GRD_INSTALL_DIR_NAME)
        dirs.append(_global_runtime_dir(runtime, home=resolved_home) / GRD_INSTALL_DIR_NAME)
    return _unique_paths(dirs)
