...
from grd.core.constants import TODOS_DIR_NAME
from grd.hooks.install_metadata import (
    config_dir_has_complete_install,
    install_scope_from_manifest,
    installed_runtime,
)
from grd.hooks.runtime_lookup import normalize_runtime_hint, resolve_runtime_lookup_dir
...
def _prefer_runtime_lookup_cwd(
    *,
    ...
    fallback preserves ancestor-local installs for existing GRD projects.
    """
...
def resolve_hook_lookup_context(
    *,
    ...
    from grd.hooks.runtime_detect import (
        ...
        detect_local_runtime_with_grd_install,
    ...
    detected_runtime_hint = normalize_runtime_hint(detect_runtime_for_grd_use(cwd=resolved_cwd, home=resolved_home))
    ...
    active_runtime = normalize_runtime_hint(detect_active_runtime_with_grd_install(cwd=lookup_cwd, home=resolved_home))
    ...
    prioritized_runtime = (
        ...
        else normalize_runtime_hint(detect_runtime_for_grd_use(cwd=lookup_cwd, home=resolved_home))
    )
    if active_runtime is None:
        local_runtime = normalize_runtime_hint(
            detect_local_runtime_with_grd_install(cwd=lookup_cwd, home=resolved_home)
        )
