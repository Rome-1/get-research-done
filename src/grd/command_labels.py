"""Shared normalization for runtime-native GRD command labels."""
...
from grd.adapters.runtime_catalog import RuntimeDescriptor

CANONICAL_COMMAND_PREFIX = "grd:"
CANONICAL_SKILL_PREFIX = "grd-"
...
def canonical_command_label(label: str) -> str:
    """Return the canonical ``grd:`` command label for *label*."""
...
def canonical_skill_label(label: str) -> str:
    """Return the canonical ``grd-`` skill label for *label*."""
...
def runtime_command_surface_pattern() -> re.Pattern[str]:
    """Return a regex that matches any runtime-native GRD command surface."""
