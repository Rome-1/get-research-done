"""Version and source-checkout resolution helpers for GRD.
...
module can instead resolve the checkout's ``src/grd`` tree and its
...
def _is_checkout_root(root: Path) -> bool:
    """Return whether *root* looks like a GRD source checkout."""
    src_gpd = root / "src" / "grd"
    return (
...
def _find_checkout_root(start: Path) -> Path | None:
    """Walk upward from *start* to find the nearest GRD checkout root."""
...
def checkout_root(start: Path | None = None) -> Path | None:
    """Resolve the active GRD checkout root, preferring *start* when provided."""
...
def resolve_install_grd_root(start: Path | None = None) -> Path:
    """Return the GRD source tree to use for installs.

    When invoked from inside a source checkout, prefer that checkout's
    ``src/grd`` tree. Otherwise fall back to the running package directory.
    """
...
def version_for_grd_root(grd_root: Path) -> str | None:
    """Return the source version associated with *grd_root*, if it is a checkout."""
    root = _find_checkout_root(grd_root)
    if root is None:
        return None
...
try:
    __version__ = version("get-research-done")
