...
Each mapper agent explores a focus area and **writes documents directly** to `.grd/research-map/`. The orchestrator only receives confirmations, keeping context usage minimal.
...
Output: .grd/research-map/ folder with 7 structured documents about the research project state.
</objective>

<execution_context>
@{GRD_INSTALL_DIR}/workflows/map-research.md
</execution_context>
...
- Before /grd:new-project (existing research with prior work) - creates research map first
- After /grd:new-project (fresh research direction) - updates research map as work evolves
...
<process>
1. Check if .grd/research-map/ already exists (offer to refresh or skip)
2. Create .grd/research-map/ directory structure
3. Spawn 4 parallel grd-research-mapper agents:
