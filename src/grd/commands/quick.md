---
name: grd:quick
description: Execute a quick research task with GRD guarantees (atomic commits, state tracking) but skip optional agents
argument-hint: ""
...
**Use full workflow (`/grd:plan-phase` + `/grd:execute-phase`) for:**
...
- If a quick task reveals unexpected complexity, stop and promote it to a full phase with `/grd:add-phase` or `/grd:insert-phase`

</inline_guidance>

<process>
Execute the quick workflow from @{GRD_INSTALL_DIR}/workflows/quick.md end-to-end.
