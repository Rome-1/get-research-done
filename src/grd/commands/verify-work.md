...
review-contract:
  ...
  schema_version: 1
  required_outputs:
    - ".grd/phases/XX-name/{phase}-VERIFICATION.md"
  required_evidence:
    - roadmap
    - phase summaries
    - artifact files
...
Output: `.grd/phases/XX-name/{phase}-VERIFICATION.md` tracking all check results. This workflow is only valid once the phase has reached the `phase_executed` state. If issues are found, return diagnosed gaps with severity classification and verified fix plans ready for `/grd:execute-phase`.
...
Read the file at {GRD_INSTALL_DIR}/workflows/verify-work.md — this contains the complete step-by-step instructions. Do NOT improvise. Follow the workflow file exactly.
...
**For deeper focused analysis**, use the dedicated commands: `/grd:dimensional-analysis` (unit consistency), `/grd:limiting-cases` (known limit recovery), or `/grd:numerical-convergence` (convergence testing).
  </process>
