...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/parameter-sweep.md
</execution_context>
...
<process>
Execute the parameter-sweep workflow from @{GRD_INSTALL_DIR}/workflows/parameter-sweep.md end-to-end.
...
A sweep that varies a physical parameter should also verify that the numerical computation at each point is converged. A phase diagram computed on an unconverged grid is meaningless. Run `/grd:numerical-convergence` on representative points.
