...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/progress.md
</execution_context>
...
Read the file at {GRD_INSTALL_DIR}/workflows/progress.md — this contains the complete step-by-step instructions. Do NOT improvise. Follow the workflow file exactly.
...
If `project_exists` is false (no `.grd/` directory):

```
No planning structure found.

Run /grd:new-project to start a new research project.
```
...
**Generate progress bar from grd, then present rich status report:**
...
```bash
ls -1 .grd/phases/[current-phase-dir]/*-PLAN.md 2>/dev/null | wc -l
ls -1 .grd/phases/[current-phase-dir]/*-SUMMARY.md 2>/dev/null | wc -l
...
grep -l -E "status: (gaps_found|diagnosed|human_needed|expert_needed)" .grd/phases/[current-phase-dir]/*-VERIFICATION.md 2>/dev/null
```
...
for plan in .grd/phases/[current-phase-dir]/*-PLAN.md; do
...
- `/grd:list-phase-assumptions {phase}` -- see assumptions before planning
...
- `/grd:verify-work {Z}` -- validation check before continuing
...
**Also available:**
- `/grd:verify-work` -- validation check before completing milestone
