...
review-contract:
  ...
  schema_version: 1
  required_outputs:
    - "GRD/knowledge/reviews/{knowledge_id}-R{round_suffix}-REVIEW.md"
...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/review-knowledge.md
@{GRD_INSTALL_DIR}/templates/knowledge-schema.md
@{GRD_INSTALL_DIR}/templates/knowledge.md
...
@GRD/STATE.md
@GRD/knowledge/
@GRD/knowledge/reviews/

Resolve the target deterministically from the explicit argument:

- an exact `GRD/knowledge/{knowledge_id}.md` path
...
Follow `@{GRD_INSTALL_DIR}/workflows/review-knowledge.md` exactly.
</process>

<success_criteria>
- [ ] Review target resolved exactly from path or canonical knowledge_id
- [ ] Review artifact written under `GRD/knowledge/reviews/`
