...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/literature-review.md
</execution_context>
...
```bash
REVIEWER_MODEL=$(grd resolve-model grd-literature-reviewer)
```
...
```bash
ls .grd/literature/*.md 2>/dev/null
```
...
## 3. Spawn grd-literature-reviewer Agent
...
<output>
Write to: .grd/literature/{slug}-REVIEW.md
...
<prior_state>
Review file path: .grd/literature/{slug}-REVIEW.md
...
- [ ] grd-literature-reviewer spawned with structured search protocol
