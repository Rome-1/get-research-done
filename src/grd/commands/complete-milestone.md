...
0. **Check for audit:**
   ...
   - Look for `.grd/v{{version}}-MILESTONE-AUDIT.md`
   ...
   unsupported claims). Run `/grd:plan-milestone-gaps` to create phases that
...
4. **Archive milestone:**
   ...
   - Create `.grd/milestones/v{{version}}-ROADMAP.md`
...
5. **Archive requirements:**
   ...
   - Create `.grd/milestones/v{{version}}-REQUIREMENTS.md`
   ...
   - Delete `.grd/REQUIREMENTS.md` (fresh one created for next milestone)

5b. **Promote patterns:**

   - Check INSIGHTS.md for confirmed patterns (2+ phases, clear detection/prevention)
   - Run `grd pattern search` to check for duplicates
   - Suggest `grd pattern add` for promotion candidates
...
8. **Offer next steps:**
   - `/grd:new-milestone` — start next research milestone (questioning -> literature review -> requirements -> roadmap)

</process>

<success_criteria>

- Milestone archived to `.grd/milestones/v{{version}}-ROADMAP.md`
- Requirements archived to `.grd/milestones/v{{version}}-REQUIREMENTS.md`
- `.grd/REQUIREMENTS.md` deleted (fresh for next milestone)
