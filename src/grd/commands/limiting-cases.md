...
```bash
cat .grd/research-map/FORMALISM.md 2>/dev/null | grep -A 20 "Known Limiting Cases"
cat .grd/research-map/VALIDATION.md 2>/dev/null | grep -A 30 "Limiting Cases"
```

</context>

<execution_context>
@{GRD_INSTALL_DIR}/workflows/limiting-cases.md
</execution_context>
...
**Pre-flight check:**
```bash
CONTEXT=$(grd --raw validate command-context limiting-cases "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Follow the limiting-cases workflow: @{GRD_INSTALL_DIR}/workflows/limiting-cases.md
