...
description: Research how to tackle a phase (standalone - usually use /grd:plan-phase instead)
argument-hint: "[phase]"
...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/research-phase.md
...
```bash
INIT=$(grd init phase-op "$ARGUMENTS")
```
...
```bash
ls .grd/phases/${PHASE}-*/RESEARCH.md 2>/dev/null
```
...
cat .grd/phases/${PHASE}-*/*-CONTEXT.md 2>/dev/null
grep -A30 "### Decisions" .grd/STATE.md 2>/dev/null
```
...
<prior_state>
Research file path: .grd/phases/${PHASE}-{slug}/${PHASE}-RESEARCH.md
