...
```bash
cat .grd/research-map/ARCHITECTURE.md 2>/dev/null | grep -A 20 "Predictions"
...
Treat `.grd/**` as internal provenance only. Discover predictions and reusable comparison inputs from stable workspace directories such as `artifacts/`, `results/`, `data/`, `figures/`, `simulations/`, or `paper/`.

</context>

<execution_context>
@{GRD_INSTALL_DIR}/workflows/compare-experiment.md
</execution_context>
...
**Pre-flight check:**
```bash
CONTEXT=$(grd --raw validate command-context compare-experiment "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Follow the compare-experiment workflow: @{GRD_INSTALL_DIR}/workflows/compare-experiment.md
</process>
