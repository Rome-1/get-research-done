...
Produces RESEARCH.md (with `depth: quick`) that informs subsequent planning via /grd:plan-phase.
...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/discover.md
</execution_context>
...
<process>
Execute the discover workflow from @{GRD_INSTALL_DIR}/workflows/discover.md end-to-end.

## Step 0: Validate Context

```bash
CONTEXT=$(grd --raw validate command-context discover "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
