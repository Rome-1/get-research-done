...
3. `/grd:plan-phase <N>` — Plan a research phase
4. `/grd:execute-phase <N>` — Execute phase plans
5. `/grd:verify-work [phase]` — Verify research results
6. `/grd:progress` — Check status and get next action
7. `/grd:complete-milestone` — Archive completed milestone
8. `/grd:help --all` — Full command reference
...
- Creates `.grd/AUTHOR-RESPONSE{round_suffix}.md` for structured issue tracking plus `.grd/paper/REFEREE_RESPONSE{round_suffix}.md` for the journal-facing response letter
...
Usage: `/grd:compare-results results/summary.csv experiment.csv`
...
Set during `/grd:new-project` and adjustable later with `/grd:settings`:
