...
argument-hint: "[topic | source file | arXiv ID | GRD/knowledge/K-*.md]"
...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/digest-knowledge.md
</execution_context>
...
```bash
ls GRD/knowledge/*.md 2>/dev/null | head -10
```

Check for related source inputs when you already know the target family:

```bash
ls GRD/literature/*.md 2>/dev/null | head -10
ls GRD/research-map/*.md 2>/dev/null | head -10
```
...
```bash
CONTEXT=$(grd --raw validate command-context digest-knowledge "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi

INIT=$(grd --raw init progress --include state,roadmap,config)
```
...
If the user is asking to promote a doc to `stable`, approve it, or mutate an existing stable target, route them to `grd:review-knowledge` instead of pretending `digest-knowledge` can own that lifecycle step.

## 2. Resolve The Target

Prefer deterministic resolution over implicit repair:

1. reuse an explicit `GRD/knowledge/K-*.md` path if provided
...
If the resolved target exists and is `stable` or `superseded`, do not repurpose it here; route the user to `grd:review-knowledge` for promotion, revision, or replacement guidance.
...
If the requested action belongs to review, approval, or stable-state mutation, explicitly point to `grd:review-knowledge` rather than overloading this wrapper.
</objective>
```
