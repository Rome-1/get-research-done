...
<objective>
Export GRD observability session logs and execution traces to files for external review, sharing, debugging, or archival.

Reads session event streams from `GRD/observability/sessions/` and trace logs from `GRD/traces/`, applies optional filters, and writes the results to `GRD/exports/logs/` (or a custom directory).
...
- `--output-dir <path>`: Write files to a custom directory instead of `GRD/exports/logs/`
</objective>

<execution_context>
@{GRD_INSTALL_DIR}/workflows/export-logs.md
</execution_context>
...
<process>
Execute the export-logs workflow from @{GRD_INSTALL_DIR}/workflows/export-logs.md end-to-end.

## Step 1: Validate project

Confirm a GRD project exists and observability data is present.
...
```bash
grd --raw observe export $ARGUMENTS
```
