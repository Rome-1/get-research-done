...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/sensitivity-analysis.md
</execution_context>
...
```bash
CONTEXT=$(grd --raw validate command-context sensitivity-analysis "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Execute the sensitivity-analysis workflow from @{GRD_INSTALL_DIR}/workflows/sensitivity-analysis.md end-to-end.
...
Save to:
- Phase target: `.grd/phases/XX-name/SENSITIVITY.md`
- Project-wide: `.grd/analysis/sensitivity-{target}.md`
