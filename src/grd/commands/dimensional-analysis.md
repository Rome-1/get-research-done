...
```bash
cat .grd/research-map/FORMALISM.md 2>/dev/null | grep -A 10 "Unit System"
cat .grd/research-map/FORMALISM.md 2>/dev/null | grep -A 20 "Notation and Conventions"
```

</context>

<execution_context>
@{GRD_INSTALL_DIR}/workflows/dimensional-analysis.md
</execution_context>
...
**Pre-flight check:**
```bash
CONTEXT=$(grd --raw validate command-context dimensional-analysis "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Follow the dimensional-analysis workflow: @{GRD_INSTALL_DIR}/workflows/dimensional-analysis.md

**For comprehensive verification** (dimensional analysis + limiting cases + symmetries + convergence), use `/grd:verify-work`.
</process>
