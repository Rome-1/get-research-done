...
<objective>
Set the concrete runtime-native model strings GRD should use for `tier-1`, `tier-2`, and `tier-3`.

Keep this command narrow and direct:

- It changes only `model_overrides.<runtime>` in `GRD/config.json`
- It does **not** change `model_profile`, autonomy, review cadence, budgets, workflow toggles, or git settings
- Use `grd:set-profile` when you want to change the abstract research profile
- Use `grd:settings` when you want the broader unattended/configuration flow
...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/set-tier-models.md
</execution_context>

<process>
1. Read and follow `@{GRD_INSTALL_DIR}/workflows/set-tier-models.md`.
...
4. Keep the distinction explicit:
   - `grd:set-profile` = abstract profile / agent tier behavior
   - `grd:set-tier-models` = concrete runtime model ids for `tier-1`, `tier-2`, `tier-3`
   - `grd:settings` = broad unattended/configuration bundle
</process>
