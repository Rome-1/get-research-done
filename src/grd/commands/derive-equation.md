...
Provide the equation or topic to derive as an argument (e.g., `/grd:derive-equation "effective mass from self-energy"`). If no argument is given, you will be asked what to derive.
...
- For theorem-bearing derivations, spawns `grd-check-proof` as a separate proof critic and fails closed without its audit
...
<execution_context>
@{GRD_INSTALL_DIR}/workflows/derive-equation.md
</execution_context>

<process>
Execute the derive-equation workflow from @{GRD_INSTALL_DIR}/workflows/derive-equation.md end-to-end.
Preserve all workflow gates (assumption statement, notation, step-by-step derivation, verification, documentation).

The workflow will:
1. Set up the derivation context (conventions, starting point, target), including canonical result lookup via `grd result search` and direct stored-result inspection via `grd result show "{result_id}"` when the target already has a known registry entry
