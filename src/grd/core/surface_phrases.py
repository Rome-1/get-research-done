...
def workflow_preset_surface_note() -> str:
    from grd.core.workflow_presets import list_workflow_presets
    ...
    return (
        ...
        f"({preset_labels}), `grd presets show <preset>` to preview one bundle, "
        "and `grd presets apply <preset> --dry-run` to preview the changed knobs "
        "before writing them."
    )
