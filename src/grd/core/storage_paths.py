"""Project-local storage-path policy helpers for GRD.
...
from grd.core.constants import PLANNING_DIR_NAME, SCRATCH_DIR_NAME, ProjectLayout
...
class StoragePathError(GRDError, ValueError):
...
class ProjectStorageLayout:
    ...
    def __init__(self, root: Path, grd_dir: str = PLANNING_DIR_NAME) -> None:
        ...
        self.grd = ProjectLayout(self.root, grd_dir=grd_dir).grd.resolve(strict=False)
    ...
    def scratch_dir(self) -> Path:
        return (self.grd / SCRATCH_DIR_NAME).resolve(strict=False)
    ...
    def classify(self, path: Path | str) -> StorageClass:
        ...
        if _is_relative_to(resolved, self.grd):
    ...
    def validate_internal_output(self, path: Path | str) -> Path:
        ...
        if classification != StorageClass.INTERNAL_DURABLE:
            raise StoragePathError(
                f"Internal durable outputs must stay under {self.grd}, got {resolved} ({classification})."
            )
        return resolved
