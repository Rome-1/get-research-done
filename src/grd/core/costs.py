...
from grd.adapters.runtime_catalog import get_hook_payload_policy, get_runtime_capabilities
...
from grd.core.observability import get_current_session_id
...
from grd.core.utils import atomic_write, file_lock, safe_read_file
...
def _project_state_usage_attribution(project_root: Path | None, *, session_id: str | None) -> _UsageAttribution:
    ...
    ``project_root`` here is the resolved GRD project scope used for
...
def record_usage_from_runtime_payload(
    ...
    *,
    ...
    while ``project_root`` identifies the resolved GRD project scope used for
    ...
    # Keep the runtime's raw workspace path separate from the resolved GRD
...
def build_cost_summary(
    ...
    *,
    ...
    try:
        ...
        detected_runtime = detect_runtime_for_grd_use(cwd=resolved_project_root)
    ...
    if not project_records:
        if active_runtime_capabilities.get("telemetry_completeness") == "none" and active_runtime:
            guidance.append(
                f"{active_runtime} does not currently expose a GRD-managed usage telemetry collection path, so `grd cost` may remain empty even when work runs."
            )
            ...
            guidance.append(
                "No measured usage telemetry is recorded for this workspace yet. GRD records usage only when the runtime emits token or cost payloads."
            )
