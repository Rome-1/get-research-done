"""Git operations for GRD — commit and pre-commit checks.
...
def _load_active_convention_lock(cwd: Path) -> tuple[object | None, bool]:
    ...
    from grd.core.conventions import ConventionLock, is_bogus_value
...
def _check_assert_conventions(
    ...
    *,
    ...
    from grd.core.conventions import check_assertions, required_assertion_keys
...
@instrument_grd_function("git_ops.pre_commit_check")
...
@instrument_grd_function("git_ops.commit")
def cmd_commit(
    cwd: Path,
    message: str,
    files: list[str] | None = None,
) -> CommitResult:
    """Stage specified files (or all .grd/ changes) and create a git commit.

    Args:
        cwd: Project root directory.
        message: Commit message.
        files: Specific files to stage. If empty/None, stages all .grd/ changes.
