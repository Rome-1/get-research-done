"""Centralized constants for the GRD package.
...
from grd.core.return_contract import REQUIRED_RETURN_FIELDS, VALID_RETURN_STATUSES
...
__all__ = [
    ...
    "ENV_GRD_DISABLE_CHECKOUT_REEXEC",
    "ENV_GRD_DEBUG",
...
# These define the on-disk layout of a GRD project's .grd/ directory.

PLANNING_DIR_NAME = ".grd"
"""Top-level GRD metadata directory inside a project root."""
...
"""Subdirectory under .grd/ for execution trace JSONL files."""

OBSERVABILITY_DIR_NAME = "observability"
"""Subdirectory under .grd/ for local session/event observability logs."""
...
"""Subdirectory under GRD/ containing append-only lineage projections."""
...
"""Subdirectory under .grd/ for todo items."""

LITERATURE_DIR_NAME = "literature"
"""Subdirectory under .grd/ for literature review files."""

KNOWLEDGE_DIR_NAME = "knowledge"
"""Subdirectory under GRD/ for knowledge documents."""

RESEARCH_MAP_DIR_NAME = "research-map"
"""Subdirectory under .grd/ for theory/research map files."""

RECENT_PROJECTS_DIR_NAME = "recent-projects"
"""Subdirectory under the home GRD data root for recent-project discovery state."""
...
"""Subdirectory under the home GRD data root for machine-local usage/cost records."""
...
"""Subdirectory under .grd/ for transient scratch files."""
...
# All env vars that GRD reads.

ENV_PATTERNS_ROOT = "GRD_PATTERNS_ROOT"
"""Override for pattern library root directory."""

ENV_DATA_DIR = "GRD_DATA_DIR"
"""Override for GRD data directory (patterns default to {data_dir}/learned-patterns)."""

ENV_MAX_INCLUDE_CHARS = "GRD_MAX_INCLUDE_CHARS"
"""Override for maximum characters when reading files for context."""

ENV_GRD_ACTIVE_RUNTIME = "GRD_ACTIVE_RUNTIME"
"""Explicit runtime override for GRD-owned shell invocations."""

ENV_GRD_DISABLE_CHECKOUT_REEXEC = "GRD_DISABLE_CHECKOUT_REEXEC"
...
"""Files that must exist in .grd/ for a valid project."""

REQUIRED_PLANNING_DIRS: tuple[str, ...] = (PHASES_DIR_NAME,)
"""Directories that must exist in .grd/ for a valid project."""

OPTIONAL_PLANNING_FILES: tuple[str, ...] = (CONFIG_FILENAME, CONVENTIONS_FILENAME)
"""Files that are checked but not required in .grd/."""
...
"""Default character limit for file includes (overridable via GRD_MAX_INCLUDE_CHARS)."""
...
"""Allowed values for grd_return.status in summary files."""

REQUIRED_RETURN_FIELDS: tuple[str, ...] = ("status", "files_written", "issues", "next_actions")
"""Fields that must be present in a grd_return YAML block."""
...
class ProjectLayout:
    ...
    def __init__(self, root: Path, grd_dir: str = PLANNING_DIR_NAME) -> None:
        self.root = root
        self.grd = root / grd_dir

    # ── Top-level GRD files ───────────────────────────────────────────────

    @property
    def state_json(self) -> Path:
        return self.grd / STATE_JSON_FILENAME

    @property
    def state_md(self) -> Path:
        return self.grd / STATE_MD_FILENAME

    @property
    def roadmap(self) -> Path:
        return self.grd / ROADMAP_FILENAME

    @property
    def project_md(self) -> Path:
        return self.grd / PROJECT_FILENAME

    @property
    def config_json(self) -> Path:
        return self.grd / CONFIG_FILENAME

    @property
    def conventions_md(self) -> Path:
        return self.grd / CONVENTIONS_FILENAME

    @property
    def state_archive(self) -> Path:
        return self.grd / STATE_ARCHIVE_FILENAME

    @property
    def state_json_backup(self) -> Path:
        return self.grd / STATE_JSON_BACKUP_FILENAME

    @property
    def state_intent(self) -> Path:
        return self.grd / STATE_WRITE_INTENT_FILENAME

    @property
    def requirements_md(self) -> Path:
        return self.grd / REQUIREMENTS_FILENAME
    ...
    def analysis_dir(self) -> Path:
        return self.grd / ANALYSIS_DIR_NAME

    @property
    def traces_dir(self) -> Path:
        return self.grd / TRACES_DIR_NAME

    @property
    def observability_dir(self) -> Path:
        return self.grd / OBSERVABILITY_DIR_NAME

    @property
    def lineage_dir(self) -> Path:
        return self.grd / LINEAGE_DIR_NAME
    ...
    def milestones_dir(self) -> Path:
        return self.grd / MILESTONES_DIR_NAME

    @property
    def todos_dir(self) -> Path:
        return self.grd / TODOS_DIR_NAME

    @property
    def literature_dir(self) -> Path:
        return self.grd / LITERATURE_DIR_NAME

    @property
    def knowledge_dir(self) -> Path:
        return self.grd / KNOWLEDGE_DIR_NAME

    @property
    def research_map_dir(self) -> Path:
        return self.grd / RESEARCH_MAP_DIR_NAME

    @property
    def scratch_dir(self) -> Path:
        return self.grd / SCRATCH_DIR_NAME
