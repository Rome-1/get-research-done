...
This module owns the narrow factual contract for deciding whether GRD should
...
from grd.core.recent_projects import list_recent_projects
...
def _recent_project_reentry_reason(
    *,
    ...
    if decision_source == "auto-selected-recent-project":
        return "GRD found the only recoverable recent project on this machine and selected it automatically."
    if decision_source == "ambiguous-recent-projects":
        if resumable_projects_count > 0:
            return f"GRD found {resumable_projects_count} recoverable recent projects on this machine, so you need to choose one."
    ...
    if decision_source == "forced-recent-projects":
        return "GRD will show the recent-project list so you can pick a workspace manually."
    if decision_source == "recent-projects":
        ...
        if available_projects_count > 0:
            return "GRD found recent projects on this machine, but none are ready to reopen automatically."
        if recent_projects_count > 0:
            return "GRD found recent projects on this machine, but none can be reopened automatically."
    return None
