...
from grd.contracts import ComparisonVerdict, ContractResults, ResearchContract, SuggestedContractCheck
...
class FrontmatterParseError(GRDError, ValueError):
    """YAML frontmatter block is syntactically invalid."""


class FrontmatterValidationError(GRDError, ValueError):
...
@instrument_grd_function("frontmatter.validate")
...
@instrument_grd_function("frontmatter.verify_summary")
def verify_summary(
    cwd: Path,
    summary_path: Path,
...
@instrument_grd_function("frontmatter.verify_plan")
...
@instrument_grd_function("frontmatter.verify_phase")
def verify_phase_completeness(cwd: Path, phase: str) -> PhaseCompleteness:
    ...
    Uses lazy import of ``find_phase`` from ``grd.core.phases``
...
@instrument_grd_function("frontmatter.verify_references")
...
@instrument_grd_function("frontmatter.verify_commits")
...
@instrument_grd_function("frontmatter.verify_artifacts")
