...
from grd.core.utils import atomic_write, file_lock, generate_slug
...
def patterns_root(specs_root: Path | None = None) -> Path:
    ...
    Precedence: *specs_root* argument > ``GRD_PATTERNS_ROOT`` env >
    ``GRD_DATA_DIR`` env > ``~/.grd/learned-patterns``.
    """
...
@instrument_grd_function("patterns.add")
def pattern_add(
    *,
    domain: str,
    title: str,
    ...
    with grd_span("patterns.add", **{"grd.pattern_id": pattern_id, "grd.domain": domain}):
...
@instrument_grd_function("patterns.list")
def pattern_list(
    *,
...
@instrument_grd_function("patterns.promote")
def pattern_promote(pattern_id: str, *, root: Path | None = None) -> PatternPromoteResult:
    ...
    with grd_span("patterns.promote", **{"grd.pattern_id": pattern_id, "grd.to": next_level}):
...
@instrument_grd_function("patterns.search")
def pattern_search(query: str, *, root: Path | None = None) -> PatternSearchResult:
    ...
    with grd_span("patterns.search", **{"grd.query": query}):
...
def _load_bootstrap_patterns() -> list[dict[str, object]]:
    ...
    Falls back to empty list if no domain pack or seed file is found.
    """
    import os
