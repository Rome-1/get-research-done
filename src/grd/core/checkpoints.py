...
- one ``.grd/phase-checkpoints/<phase-dir>.md`` file per phase that has summaries
- one ``.grd/CHECKPOINTS.md`` index covering the whole project

The canonical source of truth remains ``.grd/phases/.../*-SUMMARY.md`` and
...
from grd.core.commands import SummaryExtractResult, _normalize_string_list, cmd_summary_extract
...
from grd.core.utils import atomic_write, compare_phase_numbers, safe_read_file
...
def _render_root_index(cwd: Path, entries: list[_CheckpointIndexEntry]) -> str:
    lines = [
        ...
        "Each phase note is generated from the canonical SUMMARY and VERIFICATION artifacts under `.grd/phases/`.",
        "",
...
@instrument_grd_function("checkpoints.sync_phase_checkpoints")
def sync_phase_checkpoints(cwd: Path) -> SyncPhaseCheckpointsResult:
    """Generate the `GRD` checkpoint shelf from phase summaries.
