"""Planner artifact freshness and ``grd_return.files_written`` reconciliation helpers.
...
def resolve_planner_artifact_freshness(
    ...
    if missing_from_return or unexpected_reported_paths:
        return PlannerArtifactFreshnessResult(
            ...
            state="stale",
            detail=_build_detail(
                "planner artifact set does not match grd_return.files_written",
                resolved_cwd,
                missing_from_return=missing_from_return,
                unexpected_reported_paths=unexpected_reported_paths,
            ),
    ...
    return PlannerArtifactFreshnessResult(
        ...
        state="fresh",
        detail=_build_detail(
            "planner artifact(s) are present on disk and reconciled with grd_return.files_written",
            resolved_cwd,
            missing_from_return=(),
...
def _build_detail(
    ...
    *,
    ...
    if missing_from_return:
        parts.append(
            "missing from grd_return.files_written: "
