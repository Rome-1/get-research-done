"""Auto-migrate planning files from workspace root into GRD/."""
...
def migrate_root_planning_files(project_root: Path) -> list[str]:
    """Copy ROADMAP.md and PROJECT.md from root to GRD/ if not already there.

    Idempotent: files already in GRD/ are skipped. Errors are logged as
    ...
    for filename in _MIGRATABLE_FILES:
        ...
        try:
            ...
            shutil.copy2(root_path, grd_path)
