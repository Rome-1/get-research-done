"""GRD configuration loading and model tier system.
...
AGENT_DEFAULT_TIERS: dict[str, ModelTier] = {
    "grd-planner": ModelTier.TIER_1,
    "grd-roadmapper": ModelTier.TIER_1,
    "grd-executor": ModelTier.TIER_2,
    "grd-phase-researcher": ModelTier.TIER_2,
    "grd-project-researcher": ModelTier.TIER_2,
    "grd-research-synthesizer": ModelTier.TIER_2,
    "grd-debugger": ModelTier.TIER_1,
    "grd-research-mapper": ModelTier.TIER_3,
    "grd-verifier": ModelTier.TIER_1,
    "grd-plan-checker": ModelTier.TIER_1,
    "grd-consistency-checker": ModelTier.TIER_1,
    "grd-paper-writer": ModelTier.TIER_2,
    "grd-literature-reviewer": ModelTier.TIER_2,
    "grd-bibliographer": ModelTier.TIER_2,
    "grd-explainer": ModelTier.TIER_2,
    ...
    "grd-experiment-designer": ModelTier.TIER_2,
    "grd-notation-coordinator": ModelTier.TIER_2,
}

# ─── Config Model ───────────────────────────────────────────────────────────────


class GRDProjectConfig(BaseModel):
    """Configuration for a GRD project, loaded from .grd/config.json.

    Named GRDProjectConfig to distinguish it from other shared project
    ...
    phase_branch_template: str = "grd/phase-{phase}-{slug}"
    milestone_branch_template: str = "grd/{milestone}-{slug}"
...
_CONFIG_DEFAULTS = GRDProjectConfig()
...
_EFFECTIVE_CONFIG_SECTIONS: dict[str, Callable[[GRDProjectConfig], dict[str, object]]] = {
    "git": lambda config: {
...
def effective_config_value(config: GRDProjectConfig, key: str) -> tuple[bool, object]:
...
def _model_from_parsed_config(parsed: dict[str, object]) -> GRDProjectConfig:
...
def load_config(project_dir: Path) -> GRDProjectConfig:
    """Load GRD config from .grd/config.json with defaults.
    ...
    try:
        ...
        return _apply_gitignore_commit_docs(project_dir, GRDProjectConfig())
...
def _apply_gitignore_commit_docs(project_dir: Path, config: GRDProjectConfig) -> GRDProjectConfig:
...
@instrument_grd_function("config.resolve_tier")
...
@instrument_grd_function("config.resolve_project_tier")
...
@instrument_grd_function("config.resolve_model")
