"""Next-action intelligence for GRD research projects.
...
from grd.command_labels import canonical_command_label
...
from grd.core.proof_review import (
...
from grd.core.publication_review_paths import (
...
from grd.mcp.paper.models import ArtifactManifest
...
def _load_config(cwd: Path) -> dict[str, object]:
    ...
    :func:`grd.core.config.load_config`. Malformed files and removed keys are
    intentionally surfaced to callers instead of being silently masked here.
    """
    from grd.core.config import load_config as _load_config_canonical
...
def _count_pending_todos(cwd: Path) -> int:
    """Count .md files in .grd/todos/pending/."""
...
def _publication_review_package_allows_submission(cwd: Path, manuscript_entrypoint: Path | None) -> bool:
    ...
    try:
        from grd.core.referee_policy import evaluate_referee_decision
        from grd.mcp.paper.review_artifacts import read_referee_decision, read_review_ledger
...
def _load_state_json_safe(cwd: Path) -> dict[str, object] | None:
    ...
    Tries ``grd.core.state.load_state_json`` if available; falls back to direct read.
    """
    try:
