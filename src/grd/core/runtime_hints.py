...
from grd.core.costs import build_cost_summary, resolve_cost_advisory
from grd.core.observability import derive_execution_visibility, get_current_session_id
from grd.core.project_reentry import (
...
from grd.core.recent_projects import _strict_bool_value, list_recent_projects
from grd.core.recovery_advice import (
    RecoveryAdvice,
    build_recovery_advice,
    serialize_recovery_orientation,
)
from grd.core.resume_surface import (
...
from grd.core.root_resolution import normalize_workspace_hint, resolve_project_roots
...
from grd.core.utils import dedupe_preserve_order
from grd.core.workflow_presets import (
...
def _project_reentry_summary(
    ...
    *,
    ...
    if auto_selected:
        summary = "GRD auto-selected the only recoverable recent project on this machine."
    ...
    if requires_selection:
        return "GRD found multiple recoverable recent projects on this machine, so you need to choose one."
    if (
        ...
        if any(_candidate_bool(candidate, "resumable") for candidate in recent_candidates):
            return "GRD found recent projects on this machine, but none are selected automatically."
        return "GRD found recent projects on this machine, but none are ready to reopen automatically."
...
def _installed_runtime_for_surface(cwd: Path) -> str | None:
    try:
        ...
        runtime_name = detect_runtime_for_grd_use(cwd=cwd)
        if (
