...
Delegates to :mod:`grd.core.config` for configuration loading and model-tier
...
from grd.core.utils import safe_read_file_truncated as _safe_read_file_truncated
...
def _explicit_workspace_layout_context(cwd: Path) -> tuple[Path, dict[str, object]] | None:
    """Return local current-workspace metadata when the caller already targets a GRD layout."""
    ...
    if resolution.walk_up_steps > 0:
        ...
        reason = "workspace carries partial recoverable GRD state"
    else:
        reason = "workspace already points at a GRD project"
...
def _generate_slug(text: str | None) -> str | None:
    ...
    Thin wrapper around :func:`grd.core.utils.generate_slug` that also
...
def _normalize_phase_name(phase: str) -> str:
    ...
    Delegates to :func:`grd.core.utils.phase_normalize`.
    """
...
def _compute_branch_name(
    ...
    if strategy in ("per-phase", "phase") and phase_number:
        template = config.get("phase_branch_template", "grd/phase-{phase}-{slug}")
        return template.replace("{phase}", phase_number).replace("{slug}", phase_slug or "phase")
    if strategy in ("per-milestone", "milestone"):
        template = config.get("milestone_branch_template", "grd/{milestone}-{slug}")
...
def _config_to_dict(cfg: GRDProjectConfig) -> dict:
    """Convert a :class:`GRDProjectConfig` to the plain-dict format used by context callers.
...
def load_config(cwd: Path) -> dict:
    """Load .grd/config.json with defaults.

    Delegates to :func:`grd.core.config.load_config` (the canonical
    implementation) and converts the result to a plain dict for context
    assembly callers.

    Raises :class:`~grd.core.errors.ConfigError` on malformed JSON.
    """
...
def _resolve_model(
    ...
    if active_runtime is None:
        ...
        try:
            from grd.hooks.runtime_detect import RUNTIME_UNKNOWN, normalize_runtime_name
...
def _try_get_milestone_info(cwd: Path) -> dict:
    ...
    from grd.core.phases import get_milestone_info
...
def init_execute_phase(
    ...
    if stage is not None and includes:
        raise ValueError(
            "grd init execute-phase does not allow --include together with --stage; "
    ...
    result: dict[str, object] = {
        # Models
        "executor_model": _resolve_model(cwd, "grd-executor", config),
        "verifier_model": _resolve_model(cwd, "grd-verifier", config),
        # Config flags
    ...
    from grd.core.workflow_staging import load_execute_phase_stage_contract
...
def init_plan_phase(
    ...
    result: dict[str, object] = {
        # Models
        "researcher_model": _resolve_model(cwd, "grd-phase-researcher", config),
        "planner_model": _resolve_model(cwd, "grd-planner", config),
        "checker_model": _resolve_model(cwd, "grd-plan-checker", config),
        # Workflow flags
...
def init_new_project(cwd: Path, stage: str | None = None) -> dict:
    ...
    result = {
        # Models
        "researcher_model": _resolve_model(cwd, "grd-project-researcher", config),
        "synthesizer_model": _resolve_model(cwd, "grd-research-synthesizer", config),
        "roadmapper_model": _resolve_model(cwd, "grd-roadmapper", config),
        # Config
        "commit_docs": config["commit_docs"],
        "autonomy": config["autonomy"],
        "research_mode": config["research_mode"],
...
def init_new_milestone(cwd: Path, stage: str | None = None) -> dict:
    ...
    staged_source["researcher_model"] = _resolve_model(cwd, "grd-project-researcher", config)
    if "synthesizer_model" in required_fields:
        staged_source["synthesizer_model"] = _resolve_model(cwd, "grd-research-synthesizer", config)
    if "roadmapper_model" in required_fields:
        staged_source["roadmapper_model"] = _resolve_model(cwd, "grd-roadmapper", config)
...
def init_phase_op(
    ...
    if stage is not None and includes:
        raise ValueError(
            "grd init phase-op does not allow --include together with --stage; "
            "stage payloads already declare their required context."
        )
    config = load_config(cwd)
...
def init_map_research(cwd: Path, stage: str | None = None) -> dict:
    ...
    result = {
        # Models
        "mapper_model": _resolve_model(cwd, "grd-research-mapper", config),
        # Config
        "commit_docs": config["commit_docs"],
        "autonomy": config["autonomy"],
        "research_mode": config["research_mode"],
