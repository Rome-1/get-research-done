"""JSONL-based execution tracing for GRD agent debugging.

Traces are stored in ``.grd/traces/{phase}-{plan}.jsonl`` as one JSON
...
import grd.core.observability as _observability
from grd.core.constants import ProjectLayout
from grd.core.errors import TraceError
...
@instrument_grd_function("trace.start")
def trace_start(cwd: Path, phase: str, plan: str) -> TraceStartResult:
    ...
    with grd_span("trace.start", **{"grd.phase": phase, "grd.plan": plan}):
...
@instrument_grd_function("trace.log")
def trace_log(cwd: Path, event_type: str, data: dict[str, object] | None = None) -> TraceLogResult:
    ...
    with grd_span("trace.log", **{"grd.trace_event_type": event_type}):
        pass
...
@instrument_grd_function("trace.stop")
def trace_stop(cwd: Path) -> TraceStopResult:
    ...
    with grd_span("trace.stop", **{"grd.phase": active.phase, "grd.plan": active.plan}):
...
@instrument_grd_function("trace.show")
def trace_show(
    cwd: Path,
    *,
    phase: str | None = None,
    ...
    with grd_span("trace.show", **{"grd.phase": phase or "", "grd.plan": plan or ""}):
        # Specific phase + plan
        if phase and plan:
