"""Shared project-root resolution helpers for workspace-aware GRD surfaces.

The resolver keeps two concepts separate:

- the normalized workspace hint supplied by a caller or runtime payload
- the resolved GRD project root used for project-scoped lookups

Resolution prefers an explicit ``project_dir`` only when it can be verified by
walking ancestors for a ``GRD/`` directory. If that verification fails, a
...
class ProjectRootResolution:
    ...
    def verified(self) -> bool:
        """Return whether the result was verified by locating a ``GRD/`` directory."""
...
def _walk_project_root(candidate: Path | None) -> tuple[Path | None, int]:
    """Walk *candidate* and its ancestors until the best ``GRD/`` layout is found."""
    ...
    for steps, path in enumerate((candidate, *candidate.parents)):
        ...
        marker_count = sum(
            ...
            if (layout.grd / name).exists()
        )
        marker_count += sum(1 for name in REQUIRED_PLANNING_DIRS if (layout.grd / name).is_dir())
        if (layout.grd / STATE_JSON_BACKUP_FILENAME).exists():
            marker_count += 1
