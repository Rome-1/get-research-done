...
Create hypothesis branches with /grd:branch-hypothesis <description>
```
...
```bash
git show {branch}:.grd/STATE.md 2>/dev/null
git show {branch}:.grd/hypotheses/{slug}/HYPOTHESIS.md 2>/dev/null
```
...
**If HYPOTHESIS.md is missing:** The branch was created manually without `/grd:branch-hypothesis`. Use the branch name as the description and mark hypothesis metadata as "N/A".
...
```bash
git ls-tree -r --name-only {branch} -- .grd/phases/ | grep SUMMARY.md
```
...
```bash
git ls-tree -r --name-only {branch} -- .grd/phases/ | grep VERIFICATION.md
```
...
Prefer parsing the `git show` output directly in memory. The branch SUMMARY content is already available from `git show`, so do not write it to `.grd/tmp/` just to run a path-based extractor.

If the structure is ambiguous, re-run `git show {branch}:{summary_path} 2>/dev/null` and inspect the frontmatter plus the `## Key Results` and `## Equations Derived` sections directly. Keep branch-summary extraction in memory/stdout only; do not use `.grd/tmp/`, `/tmp`, or another temp root for this step.
...
4. **Export comparison** -- write this comparison to .grd/BRANCH-COMPARISON.md
...
Write the comparison output to `.grd/BRANCH-COMPARISON.md`.

```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/BRANCH-COMPARISON.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs: export branch comparison" --files .grd/BRANCH-COMPARISON.md
```
...
**Also available:**
- `/grd:branch-hypothesis <desc>` -- create another hypothesis branch
- `/grd:progress` -- check overall research progress
