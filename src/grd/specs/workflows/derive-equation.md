...
**If any convention differs from the lock, STOP.** Either the lock needs updating (via `grd convention set`) or the derivation must use the locked convention. Never derive with one convention and combine with locked results using another.
...
Write to `.grd/analysis/derivation-{slug}.md`.
</step>
