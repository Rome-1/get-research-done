...
Produces a Level 2-3 discovery artifact that informs planning: phase-scoped `RESEARCH.md` or standalone `GRD/analysis/discovery-{slug}.md`.
...
NOTE: For comprehensive literature survey ("what do experts know about this"), use /grd:research-phase instead, which produces a full-depth RESEARCH.md. This workflow produces a quick-depth RESEARCH.md (depth: quick) suitable for method selection and landscape scanning.
</purpose>
...
See {GRD_INSTALL_DIR}/templates/research.md for the RESEARCH.md template structure (use `depth: quick` for discovery-level output).
</source_hierarchy>
...
**If no phase specified:** Discovery is standalone; output goes to `.grd/analysis/`. Ensure the directory exists: `mkdir -p .grd/analysis`.
</step>
...
6. **Create RESEARCH.md** using {GRD_INSTALL_DIR}/templates/research.md structure:
...
**Output:** Phase-scoped `${phase_dir}/RESEARCH.md`, or standalone `GRD/analysis/discovery-{slug}.md`
</step>
...
1. **Scope the discovery** using {GRD_INSTALL_DIR}/templates/research.md:
...
6. **Create comprehensive RESEARCH.md:**
   ...
   - Full structure from {GRD_INSTALL_DIR}/templates/research.md
...
**Output:** Phase-scoped `${phase_dir}/RESEARCH.md`, or standalone `GRD/analysis/discovery-{slug}.md` (comprehensive)
</step>
...
Use {GRD_INSTALL_DIR}/templates/research.md.
...
```bash
mkdir -p .grd/analysis
```

Write to `.grd/analysis/discovery-{slug}.md` (where `{slug}` is derived from the discovery topic).
...
- Standalone Level 2-3: GRD/analysis/discovery-{slug}.md
