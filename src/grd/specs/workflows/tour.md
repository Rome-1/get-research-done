<purpose>
Provide a beginner-friendly, read-only tour of the core GRD command surface.
Teach what the main commands do, when to use them, and how GRD behaves in plain
...
`This is a read-only tour of the main GRD commands. It will not change your files.`
...
Then explain the two places beginners will see GRD commands:

- the normal terminal, where you install GRD and run setup / status commands
- the runtime, where you use the GRD command prefix provided for that runtime

When you first use an official GRD term such as `runtime`, `read-only`, or
...
such as `grd resume` here; explain them later in the terminal/runtime distinction.
...
the main capability groups GRD supports once a project is underway:

- project work: `grd:discuss-phase`, `grd:plan-phase`, `grd:execute-phase`, `grd:verify-work`
- writing and review: `grd:write-paper`, `grd:peer-review`, `grd:respond-to-referees`, `grd:arxiv-submission`
- side investigations and preferences: `grd:tangent`, `grd:branch-hypothesis`, `grd:set-profile`, `grd:set-tier-models`, `grd:settings`
...
- The normal terminal is where you install GRD, run `grd --help`, and run
  checks like `grd doctor`.
- The runtime is the AI terminal app where you use the runtime-specific GRD
  commands.
- `grd resume` is the normal-terminal recovery step for reopening the right
  workspace.
...
- `settings` is the guided runtime command for changing autonomy,
  permission-sync behavior, and other GRD preferences after your first
...
- `Use \`grd resume\` first if you need to reopen the project before using \`grd:resume-work\`.`
</step>
...
- `runtime` - the AI terminal app that receives GRD commands
...
- `resume-work` - continue an existing GRD project from where it left off
...
- `If you want the reference list again later, run grd:help.`
...
- [ ] The core GRD entry points are explained in beginner language
