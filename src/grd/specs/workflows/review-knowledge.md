...
- write a deterministic review artifact under `GRD/knowledge/reviews/`
...
Called from `grd:review-knowledge`.
</purpose>
...
```bash
CONTEXT=$(grd --raw validate command-context review-knowledge "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
1. an exact `GRD/knowledge/{knowledge_id}.md` path
...
- `KNOWLEDGE_PATH` = resolved `GRD/knowledge/{knowledge_id}.md`
...
```text
GRD/knowledge/reviews/{knowledge_id}-R{review_round}-REVIEW.md
```
