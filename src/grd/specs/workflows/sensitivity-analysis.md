...
Called from /grd:sensitivity-analysis command. Used to prioritize effort: if parameter A contributes 90% of the uncertainty while parameter B contributes 1%, improving the precision of A has 90x the impact of improving B.
</purpose>
...
If `.grd/analysis/PARAMETERS.md` exists, use it as the parameter registry (see `{GRD_INSTALL_DIR}/templates/parameter-table.md` for the template).
...
- Load project state via `grd CLI init progress --include state,config` and check `intermediate_results` for computed quantities
...
Read from `.grd/STATE.md` to identify active approximations and their controlling parameters. Where structured data is needed, load via `grd CLI init progress --include state,config`.
...
For each active approximation in the project (read from `.grd/STATE.md`; load structured data via `grd CLI init progress --include state,config` if needed):
...
- If standalone: `.grd/analysis/sensitivity-{slug}.md`
...
```bash
PRE_CHECK=$(grd pre-commit-check --files "${REPORT_PATH}" .grd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

grd commit \
  "data(phase-${phase_number}): sensitivity analysis - ${TARGET_QUANTITY}" \
  --files "${REPORT_PATH}" .grd/STATE.md
```

Where `${REPORT_PATH}` is `${phase_dir}/SENSITIVITY-REPORT.md` or `.grd/analysis/sensitivity-{slug}.md` depending on scope.

**Present final results:**

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GRD > SENSITIVITY ANALYSIS COMPLETE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- `.grd/STATE.md` -- updated with uncertainty estimates
...
- **Error propagation:** `/grd:error-propagation` -- trace full error budget through derivation chain
- **Parameter sweep:** `/grd:parameter-sweep` -- map out behavior across parameter range
- **Convergence:** `/grd:numerical-convergence` -- verify numerical error bars at key points
...
- [ ] Project context loaded via `grd CLI init phase-op`
...
- [ ] propagated_uncertainties updated via `grd CLI uncertainty add`
- [ ] Artifacts committed via `grd CLI commit`
