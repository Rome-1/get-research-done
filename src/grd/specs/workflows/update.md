<purpose>
Check for a newer GRD release, show recent release notes, confirm with the user, and reinstall the active runtime with the public bootstrap command.
</purpose>
...
```bash
GRD_INSTALL_DIR="{GRD_INSTALL_DIR}"
GRD_CONFIG_DIR="{GRD_CONFIG_DIR}"
...
- GitHub Releases API: `https://api.github.com/repos/psi-oss/get-research-done/releases`
...
>> **Note:** This reinstalls the current runtime's managed GRD files:
- GRD command files for this runtime will be replaced
- `get-research-done/` will be replaced
- `grd-*` agent files will be replaced

Custom files outside the managed GRD install are preserved.
If you've modified managed GRD files directly, they will be backed up to `grd-local-patches/` and can be reapplied with `/grd:reapply-patches` after the update.
```
...
```bash
rm -f \
  "{GRD_CONFIG_DIR}/cache/grd-update-check.json" \
  "{GRD_GLOBAL_CONFIG_DIR}/cache/grd-update-check.json" \
  "$HOME/.grd/cache/grd-update-check.json"
```
</step>
...
[View releases](https://github.com/psi-oss/get-research-done/releases)
```
</step>
...
Run /grd:reapply-patches to merge your modifications into the new version.
```
