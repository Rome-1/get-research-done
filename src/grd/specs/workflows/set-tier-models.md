...
This creates `GRD/config.json` with defaults if missing and loads current config values.
</step>
...
- `grd:set-profile` changes the abstract research profile and agent tier assignments.
- `grd:set-tier-models` changes only the concrete runtime-native model IDs behind `tier-1`, `tier-2`, and `tier-3`.
- `grd:settings` is the broader unattended/configuration flow when the user also wants autonomy, review cadence, workflow toggles, budgets, or git settings.
...
- Use `grd cost` after runs for the read-only recorded local usage / cost view.
</step>
...
ask_user([
  {
    question: "How should GRD configure concrete tier models for the active runtime?",
    header: "Tier Models",
    multiSelect: false,
    options: [
...
Merge the result into `GRD/config.json`:
...
Use `grd:set-profile <profile>` to change abstract tier assignments.
Use `grd:settings` when you want broader unattended/configuration changes too.

Useful checks from your normal terminal:
- `grd resolve-tier grd-planner`
- `grd resolve-model grd-planner --runtime <runtime>`
- `grd cost`
```

State clearly that future agent spawns in this project will use the updated runtime-specific tier overrides when `grd resolve-model` resolves to them.
</step>
...
- [ ] Confirmation distinguishes `grd:set-profile`, `grd:set-tier-models`, and `grd:settings`
</success_criteria>
