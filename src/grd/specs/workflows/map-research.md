...
Check if .grd/research-map/ already exists using `has_maps` from init context.

If `research_map_dir_exists` is true:

```bash
ls -la .grd/research-map/
```

**If exists:**

```
.grd/research-map/ already exists with these documents:
[List files found]
...
If "Refresh": Delete .grd/research-map/, continue to create_structure
...
Create .grd/research-map/ directory:

```bash
mkdir -p .grd/research-map
```
...
Spawn 4 parallel grd-research-mapper agents.

Use task tool with `subagent_type="grd-research-mapper"`, `model="{mapper_model}"`, `readonly=false`, and `run_in_background=true` for parallel execution.
> **Runtime delegation:** Spawn a subagent for the task below. Adapt the `task()` call to your runtime's agent spawning mechanism. If `model` resolves to `null` or an empty string, omit it so the runtime uses its default model. Always pass `readonly=false` for file-producing agents. If subagent spawning is unavailable, execute these steps sequentially in the main context.

**CRITICAL:** Use the dedicated `grd-research-mapper` agent, NOT `Explore`. The mapper agent writes documents directly.
...
```
First, read {GRD_AGENTS_DIR}/grd-research-mapper.md for your role and instructions.
...
Write these documents to .grd/research-map/:
...
grep -E '(sk-[a-zA-Z0-9]{20,}|sk_live_[a-zA-Z0-9]+|sk_test_[a-zA-Z0-9]+|ghp_[a-zA-Z0-9]{36}|gho_[a-zA-Z0-9]{36}|glpat-[a-zA-Z0-9_-]+|AKIA[A-Z0-9]{16}|xox[baprs]-[a-zA-Z0-9-]+|-----BEGIN.*PRIVATE KEY|eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.)' .grd/research-map/*.md 2>/dev/null && SECRETS_FOUND=true || SECRETS_FOUND=false
```
...
```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/research-map/*.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs: map existing research project" --files .grd/research-map/*.md
```
...
**Also available:**
- Re-run mapping: `/grd:map-research`
- Review specific file: `cat .grd/research-map/FORMALISM.md`
...
- .grd/research-map/ directory created
- 4 parallel grd-research-mapper agents spawned with run_in_background=true
