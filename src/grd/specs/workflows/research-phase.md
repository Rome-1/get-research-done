<purpose>
Research mathematical methods, physical principles, and computational tools needed to approach a phase. Spawns grd-phase-researcher with phase context.
...
```bash
INIT=$(grd init phase-op --include state,config "${PHASE}")
if [ $? -ne 0 ]; then
...
```bash
RESEARCH_MODE=$(echo "$BOOTSTRAP_INIT" | grd json get .research_mode --default balanced)
```
...
@{GRD_INSTALL_DIR}/references/orchestration/model-profile-resolution.md

```bash
RESEARCHER_MODEL=$(grd resolve-model grd-phase-researcher)
```

## Step 1: Validate Phase

```bash
PHASE_INFO=$(grd roadmap get-phase "${phase_number}")
```
...
echo "$PHASE_INFO" | grd json get .section --default ""
cat .grd/REQUIREMENTS.md 2>/dev/null
cat "${phase_dir}/"*-CONTEXT.md 2>/dev/null
# Decisions from grd state snapshot (structured JSON)
grd state snapshot | grd json get .decisions --default "[]"
```
...
if [ $? -ne 0 ]; then
  echo "ERROR: grd initialization failed: $HANDOFF_INIT"
  exit 1
fi
```
...
<output>
Write to: .grd/phases/${PHASE}-{slug}/${PHASE}-RESEARCH.md
</output>",
...
write_scope:
  mode: scoped_write
  allowed_paths:
    - .grd/phases/${PHASE}-{slug}/${PHASE}-RESEARCH.md
expected_artifacts:
  - .grd/phases/${PHASE}-{slug}/${PHASE}-RESEARCH.md
...
Human-readable headings such as `## RESEARCH COMPLETE` and `## CHECKPOINT REACHED` are presentation only; route on `grd_return.status`, `grd_return.files_written`, and the artifact gate.

## Step 5: Handle Return

**If the researcher agent fails to spawn or returns an error:** Report the failure. Offer: 1) Retry with the same context, 2) Execute the research in the main context (slower but reliable), 3) Skip research and proceed to `/grd:plan-phase` directly (planner will work with less context). Do not silently continue without research output.

- **Artifact gate:** If `grd_return.status: completed` but the `expected_artifacts` entry (`RESEARCH.md`) is missing from the phase directory or absent from `grd_return.files_written`, treat the handoff as incomplete. Offer: 1) Retry researcher, 2) Execute the research in the main context, 3) Abort.
- `grd_return.status: completed` -- Display summary, offer: Plan/Dig deeper/Review/Done
- `grd_return.status: checkpoint` -- Present the checkpoint to the user, collect the response, and spawn a fresh continuation handoff. Do not resume the same spawned run.
- `grd_return.status: blocked` or `failed` -- Show attempts, offer: Add context/Try different approach/Manual
...
- [ ] grd-phase-researcher spawned with physics research directives
