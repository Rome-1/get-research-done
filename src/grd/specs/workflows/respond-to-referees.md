...
```bash
CONTEXT=$(grd --raw validate command-context respond-to-referees "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
if [ -n "$ARGUMENTS" ]; then
  REVIEW_PREFLIGHT=$(grd validate review-preflight respond-to-referees "$ARGUMENTS" --strict)
else
  REVIEW_PREFLIGHT=$(grd validate review-preflight respond-to-referees --strict)
fi
if [ $? -ne 0 ]; then
...
```bash
ls .grd/review/REVIEW-LEDGER*.json 2>/dev/null
...
Use that exact suffix for both `.grd/AUTHOR-RESPONSE{round_suffix}.md` and `.grd/paper/REFEREE_RESPONSE{round_suffix}.md`.
</step>
...
- `.grd/AUTHOR-RESPONSE{round_suffix}.md` — structured internal tracker keyed by `REF-*` issues, change locations, and staged review outcomes
- `.grd/paper/REFEREE_RESPONSE{round_suffix}.md` — journal-facing response letter built from the template
...
```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/paper/REFEREE_RESPONSE{round_suffix}.md .grd/AUTHOR-RESPONSE{round_suffix}.md 2>&1) || true
echo "$PRE_CHECK"

grd commit \
  "docs: create referee response structure" \
  --files .grd/paper/REFEREE_RESPONSE{round_suffix}.md .grd/AUTHOR-RESPONSE{round_suffix}.md
```

Keep the two files synchronized for the rest of the workflow: draft issue-by-issue substance in `.grd/AUTHOR-RESPONSE{round_suffix}.md`, and mirror the journal-facing prose into `.grd/paper/REFEREE_RESPONSE{round_suffix}.md`.
...
| C: New calculations | {N} | Create research phases via /grd:add-phase |
...
|----|-------------|-------------|-----------------|
| NC-1 | Referee 1, Comment 3 | Extend to next-to-leading order | /grd:insert-phase {N}.1 |
| NC-2 | Referee 2, Comment 5 | Compare with Monte Carlo results | /grd:add-phase |
...
grd phase add "Referee revision: {description}"
```

The user should run `/grd:plan-phase` and `/grd:execute-phase` for each new phase, then return to `/grd:respond-to-referees` to continue.
...
3. Verify new citations exist in .bib file — for any NEW citations added during revision, verify metadata accuracy (author, year, journal) via `grd pattern search` or web search. Referee-suggested references are usually real but may have wrong metadata.
