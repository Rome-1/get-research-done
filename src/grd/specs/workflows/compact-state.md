...
Triggered automatically when progress.md detects STATE.md exceeds 1500 lines, or manually via `/grd:compact-state`.
</purpose>
...
- {GRD_INSTALL_DIR}/templates/state-archive.md -- Template for STATE-ARCHIVE.md (archive structure, what gets archived, archive format)
</required_reading>
...
**Delegate to grd state compact:**

The grd CLI handles the detailed archival logic:

```bash
RESULT=$(grd state compact)
if [ $? -ne 0 ]; then
...
All archived content is appended to `.grd/STATE-ARCHIVE.md` with a dated header.
...
for SECTION in "Current Position" "Project Reference" "Accumulated Context" "Session"; do
  grep -q "## ${SECTION}" .grd/STATE.md || echo "MISSING: ${SECTION}"
done

# Verify state.json was synced
ls -la .grd/state.json
```
...
**If state.json sync failed:** Do not delete it blindly. Keep `.grd/state.json` (and `.grd/state.json.bak` if present), inspect `grd state validate`, and use `/grd:sync-state` or the recovery step above so JSON-only fields are preserved.
</step>
...
```bash
ls -la .grd/STATE-ARCHIVE.md 2>/dev/null
ARCHIVE_LINES=$(wc -l < .grd/STATE-ARCHIVE.md 2>/dev/null || echo 0)
```
...
grd commit \
  ...
  --files .grd/STATE.md .grd/STATE-ARCHIVE.md .grd/state.json
```
</step>
...
### Archive location:
.grd/STATE-ARCHIVE.md ({archive_lines} total lines)
...
- **grd state compact fails:** Check error output. Common causes: file lock held by another process, corrupt STATE.md parsing. Suggest: `cat .grd/STATE.md | head -5` to verify file is readable.
