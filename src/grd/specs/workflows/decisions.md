<purpose>
Display and search the cumulative decision log from .grd/DECISIONS.md. Supports filtering by phase number or keyword, and presents formatted output with summary statistics.
</purpose>
...
```bash
cat .grd/DECISIONS.md 2>/dev/null
```
...
Decisions are recorded automatically during phase transitions (/grd:progress).
You can also add entries manually to .grd/DECISIONS.md.

---

Would you like to:

1. Check project progress (/grd:progress)
2. View current phase (/grd:show-phase)
```
...
- `/grd:decisions` -> show all entries
- `/grd:decisions 3` (number) -> filter to Phase 3 only
- `/grd:decisions regularization` (text) -> keyword search across all fields
- `/grd:decisions high` -> filter by impact level
...
Try:
- `/grd:decisions` — show all
- `/grd:decisions <phase>` — filter by phase
- `/grd:decisions <keyword>` — search by keyword
```
...
- [ ] Decision log loaded from .grd/DECISIONS.md
