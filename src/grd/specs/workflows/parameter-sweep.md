...
Called from /grd:parameter-sweep command. References wave-based execution patterns from execute-phase.md.
</purpose>
...
```bash
SWEEP_PHASE_DIR=".grd/phases/XX-sweep"
...
Keep plans and SUMMARY files in `${SWEEP_PHASE_DIR}` because they are internal execution records. Write machine-readable sweep datasets to `${SWEEP_ARTIFACT_DIR}`. Do not put point-result JSON under `.grd/phases/**`.
</step>
...
1. **Create wave checkpoint:**
   ...
   ```bash
   WAVE_CHECKPOINT="grd-checkpoint/sweep-${phase_number}-wave-${WAVE_NUM}-$(date +%s)"
...
```bash
PRE_CHECK=$(grd pre-commit-check --files "${SWEEP_ARTIFACT_DIR}/sweep-results.json" "${SWEEP_PHASE_DIR}/SWEEP-SUMMARY.md" .grd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

grd commit \
  "data(phase-${phase_number}): parameter sweep - ${OBSERVABLE} vs ${PARAM_NAME}" \
  --files "${SWEEP_ARTIFACT_DIR}/sweep-results.json" "${SWEEP_PHASE_DIR}/SWEEP-SUMMARY.md" "${SWEEP_ARTIFACT_DIR}/results" .grd/STATE.md
```
...
- **Refine:** `/grd:parameter-sweep {phase} --adaptive` -- add points near interesting features
- **Converge:** `/grd:numerical-convergence {phase}` -- verify convergence at key points
- **Branch:** `/grd:branch-hypothesis` -- investigate features with different methods
