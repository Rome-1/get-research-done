<purpose>
Create a git branch for parallel investigation of an alternative hypothesis or approach. Sets up isolated research state with hypothesis documentation, enabling side-by-side comparison later via /grd:compare-branches. Common in physics when multiple approximation schemes, gauge choices, or derivation pathways need to be compared systematically.
</purpose>
...
Example: `/grd:branch-hypothesis Use dimensional regularization instead of hard cutoff`
...
Usage: /grd:branch-hypothesis <description>
Example: /grd:branch-hypothesis Use dimensional regularization instead of hard cutoff
```
...
```bash
git stash push -m "grd: auto-stash before hypothesis branch ${SLUG}"
```
...
```bash
mkdir -p .grd/hypotheses/${SLUG}
```

Write `.grd/hypotheses/${SLUG}/HYPOTHESIS.md`:
...
```bash
# Get current phase and project state via grd CLI
INIT=$(grd init progress --include roadmap,state)
```
...
```bash
CONTEXT=$(grd --raw validate command-context branch-hypothesis "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
PHASE_DIR=$(grd --raw phase find "$CURRENT_PHASE_NUM")
if [ $? -ne 0 ]; then
...
Use the file_read tool to read `.grd/STATE.md`, `.grd/ROADMAP.md`, and any SUMMARY.md files in the phase directory. If `state_content` and `roadmap_content` were included via `--include`, use those directly instead of re-reading.
...
Review and edit: .grd/hypotheses/{slug}/HYPOTHESIS.md
...
1. Record the hypothesis branch decision via grd CLI:

```bash
grd state add-decision --phase "${CURRENT_PHASE_NUM}" --summary "Created hypothesis branch: ${description}" --rationale "Investigating alternative approach on branch hypothesis/${slug}"
```
...
Compare results with parent branch via `/grd:compare-branches`.
```

3. Treat that note as markdown-only context. It is not part of the structured state schema, so it will not be mirrored into `state.json`, and future JSON-driven state rewrites may replace it. The durable record for the hypothesis is the `grd state add-decision` entry plus `.grd/hypotheses/${SLUG}/HYPOTHESIS.md`.
   </step>

<step name="commit_setup">
Commit the hypothesis setup.

The `commit` CLI command respects `commit_docs` from `.grd/config.json` internally — if `commit_docs` is false, the commit is automatically skipped.

```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/hypotheses/${SLUG}/HYPOTHESIS.md .grd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs: create hypothesis branch for {slug}" --files .grd/hypotheses/${SLUG}/HYPOTHESIS.md .grd/STATE.md
```
</step>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GRD > HYPOTHESIS BRANCH CREATED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Branch:** hypothesis/{slug}
**Description:** {description}
**Parent:** {parent_branch}
**Hypothesis doc:** .grd/hypotheses/{slug}/HYPOTHESIS.md
...
- [ ] `.grd/hypotheses/{slug}/HYPOTHESIS.md` created with metadata
