...
Called from /grd:error-propagation command. Complements /grd:numerical-convergence (which establishes error bars on individual computations) by tracing how those errors combine through multi-step derivation chains.
...
- {GRD_INSTALL_DIR}/domains/{GRD_DOMAIN}/protocols/error-propagation-protocol.md -- Cross-phase uncertainty propagation protocol (verification checks, phase handoff format, catastrophic cancellation detection)
...
```bash
CONTEXT=$(grd --raw validate command-context error-propagation "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Run /grd:new-project first, then complete phases with tracked results.
```
...
If the target is not found in `STATE.md` `intermediate_results`, first use `grd result search` to look for the canonical result by identifier, equation, or description. Once you have the canonical `result_id`, use `grd result show "{result_id}"` for the direct stored-result view before `grd result deps "{result_id}"` to recover the recorded dependency tree before reconstructing it manually. If the target still is not found, check SUMMARY.md files across phases for the quantity name.
</step>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GRD > ERROR PROPAGATION: DEPENDENCY TREE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
If /grd:numerical-convergence has been run, read the convergence report for error estimates. If not, flag these as "numerical uncertainty not yet quantified" and recommend running convergence tests.
...
```bash
TARGET_PHASE_INFO=$(grd phase find "${target_phase_number}")
```
...
```bash
PRE_CHECK=$(grd pre-commit-check --files ${target_phase_dir}/ERROR-BUDGET.md .grd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs: error propagation analysis for {quantity}" --files ${target_phase_dir}/ERROR-BUDGET.md .grd/STATE.md
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GRD > ERROR PROPAGATION: COMPLETE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**Also available:**
- `/grd:numerical-convergence` -- refine error bars on individual computations
- `/grd:verify-work` -- verify the full phase including error analysis
- `/grd:regression-check` -- check that error budgets remain valid after changes
...
- **State files missing:** If STATE.md or state.json does not exist, error immediately with a clear message: "No project state found. Run `/grd:new-project` or `/grd:execute-phase` first to establish project state before running error propagation."
