...
```bash
INIT=$(grd init execute-phase "${PHASE_ARG}")
if [ $? -ne 0 ]; then
...
Before launching any plan, require that the selected `PLAN.md` passes `grd validate plan-preflight <PLAN.md>` when specialized tool requirements are declared.
...
```bash
PHASE_GOAL=$(grd roadmap get-phase "${phase_number}" | grd json get .goal --default "")
PLAN_OBJECTIVES=""
for plan in "$phase_dir"/*-PLAN.md; do
  OBJ=$(grd frontmatter get "$plan" --field objective 2>/dev/null)
...
| **numerical** | `INTER_WAVE_CHECKS+=("convergence_spot_check")` — between waves, scan SUMMARY for convergence metrics and flag regressions. `EXECUTOR_CONTEXT_HINT="code-heavy"` — hint executors to reserve context for code output and numerical tables. `PRE_EXECUTION_SPECIALISTS+=("experiment-designer")` — route to `grd-experiment-designer` before wave 1 when parameter validation is needed. |
...
| **paper-writing** | `PRE_EXECUTION_SPECIALISTS+=("notation-coordinator")` — route to `grd-notation-coordinator` before drafting when the notation glossary must be refreshed. `INTER_WAVE_CHECKS+=("latex_compile")` — compile after each wave to catch LaTeX errors early. `EXECUTOR_CONTEXT_HINT="prose-heavy"` — hint executors to balance equation density with exposition. |
| **formalism** | `CONVENTION_LOCK_REQUIRED=true`. `PRE_EXECUTION_SPECIALISTS+=("notation-coordinator")` — route to `grd-notation-coordinator` before framework setup when conventions need to be locked. `INTER_WAVE_CHECKS+=("identity_scan")`. |
...
```bash
CONV_STATUS=$(grd --raw convention check)
if [ "$CONV_STATUS" != "locked" ] && [ "$CONV_STATUS" != "complete" ]; then
  ...
  echo "  grd convention set"
  echo "  /grd:validate-conventions"
  echo ""
...
for AGENT_TYPE in "${PRE_EXECUTION_AGENTS[@]}"; do
  case "$AGENT_TYPE" in
    notation-coordinator)
      AGENT_MODEL=$(grd resolve-model grd-notation-coordinator)
      # Spawn notation-coordinator to verify/establish conventions
      # task(subagent_type="grd-notation-coordinator", model="{AGENT_MODEL}", readonly=false, ...)
      ;;
    experiment-designer)
      AGENT_MODEL=$(grd resolve-model grd-experiment-designer)
      # Spawn experiment-designer to validate parameter ranges
      # task(subagent_type="grd-experiment-designer", model="{AGENT_MODEL}", readonly=false, ...)
      ;;
  esac
done
```
...
When runtime delegation is available, `grd-check-proof` is the canonical owner of this sibling artifact. The executor may draft the proof and theorem inventory, but it must not self-certify theorem-proof alignment as its own independent redteam.
</step>
...
if ! grd phase validate-waves "$phase_number"; then
...
for plan in "$phase_dir"/*-PLAN.md; do
  if ! grd verify references "$plan"; then
...
```bash
PLAN_INDEX=$(grd phase index "${phase_number}")
```
...
for WAVE_NUM in $(echo "$PLAN_INDEX" | grd json keys .waves); do
  WAVE_PLANS=$(echo "$PLAN_INDEX" | grd json list ".waves[\"$WAVE_NUM\"]")
  for PLAN_ID in $WAVE_PLANS; do
    DEPS=$(grd frontmatter get \
...
```bash
REVIEW_CADENCE=$(echo "$INIT" | grd json get .review_cadence --default adaptive)
RESEARCH_MODE=$(echo "$INIT" | grd json get .research_mode --default balanced)
MAX_UNATTENDED_MINUTES_PER_PLAN=$(echo "$INIT" | grd json get .max_unattended_minutes_per_plan --default 45)
MAX_UNATTENDED_MINUTES_PER_WAVE=$(echo "$INIT" | grd json get .max_unattended_minutes_per_wave --default 90)
CHECKPOINT_AFTER_N_TASKS=$(echo "$INIT" | grd json get .checkpoint_after_n_tasks --default 3)
CHECKPOINT_AFTER_FIRST_RESULT=$(echo "$INIT" | grd json get .checkpoint_after_first_load_bearing_result --default true)
CHECKPOINT_BEFORE_DOWNSTREAM=$(echo "$INIT" | grd json get .checkpoint_before_downstream_dependent_tasks --default true)
```
...
- `branch_later` — recommend `grd:tangent ...` or `grd:branch-hypothesis ...` for explicit follow-up, but do not create new side work during this execution pass
...
1. **Convention lock check (before parallel execution):**
   ...
   - If conventions are unlocked for any field that parallel plans will use, LOCK them first via `grd convention set`
...
2. **Create wave-level checkpoint** before any plan in the wave starts:
   ...
   ```bash
   WAVE_CHECKPOINT="grd-checkpoint/phase-${phase_number}-wave-${WAVE_NUM}-$(date +%s)-$$"
...
4. **Spawn executor agents:**
   ...
   task(
     ...
     readonly=false,
     prompt="First, read {GRD_AGENTS_DIR}/grd-executor.md for your role and instructions.
       ...
       <proof_redteam_gate>
       If this plan is proof-bearing, you must leave behind the proof artifact, theorem inventory, and enough supporting context for the orchestrator to spawn `grd-check-proof`.
       Do NOT self-certify the sibling `{plan_id}-PROOF-REDTEAM.md` artifact as your own independent proof critic when a fresh `grd-check-proof` subagent is available.
       ...
       - Validation ref: {GRD_INSTALL_DIR}/domains/{GRD_DOMAIN}/verification/core/verification-core.md (+ domain-specific verification file)
       ...
       - Config: .grd/config.json (if exists)
	       ...
	       - [ ] Proof-bearing plans leave enough artifact context for the orchestrator to run `grd-check-proof`
...
5a. **For proof-bearing plans, spawn the independent proof critic before accepting the result.**
   ...
   After a proof-bearing executor has written its proof artifact(s) and `SUMMARY.md`, but before the wave-level spot-check accepts the plan, spawn `grd-check-proof` in a fresh context:
...
Then read {GRD_INSTALL_DIR}/templates/proof-redteam-schema.md and {GRD_INSTALL_DIR}/references/verification/core/proof-redteam-protocol.md before writing any proof audit artifact.
   ...
   If `grd-check-proof` cannot be spawned, returns malformed output, or reports `status != passed`, route the plan to `wave_failure_handling`. Do not treat executor self-review as an acceptable substitute.
...
6. **Report completion -- spot-check claims first:**
   ...
   - Validate the grd_return envelope:

     ```bash
     RETURN_CHECK=$(grd --raw validate-return "${SUMMARY_FILE}")
...
9. **Execute checkpoint plans between waves** -- see `<checkpoint_handling>`.
   ...
   **Tangent proposals at the same stop:** if the first result suggests an unexpected but non-blocking alternative path, keep it inside the same review conversation rather than spawning extra work. Resolve it with one of:
   ...
   - `branch_later` — recommend an explicit `grd:tangent ...` or `grd:branch-hypothesis ...` follow-up after the bounded stop
...
10. **Inter-wave verification gate (if more waves remain):**
   ...
   **Determine if gate is enabled from init/context fields only:**
   ...
   wave_summaries=()
   for PLAN_ID in $(echo "$PLAN_INDEX" | grd json list ".waves[\"$WAVE_NUM\"]"); do
   ...
   ```bash
   CONV_CHECK=$(grd --raw convention check)
   ...
   Prefer paths surfaced through SUMMARY `key-files` or contract deliverables. Do not assume durable artifacts live beside the SUMMARY in `.grd/phases/**`.
...
LATER_PLANS=()
for LATER_WAVE in $(echo "$PLAN_INDEX" | grd json keys .waves | awk -v w="$WAVE_NUM" '$1 > w'); do
  for P in $(echo "$PLAN_INDEX" | grd json list ".waves[\"$LATER_WAVE\"]"); do
    LATER_PLANS+=("$P")
  done
done
...
6. **Spawn continuation agent (NOT resume)** using `{GRD_INSTALL_DIR}/templates/continuation-prompt.md` template:
...
for summary in "${phase_dir}"/*-SUMMARY.md; do
  grd summary-extract "$summary" --field one_liner
done
```
</step>
...
Generated figures and plots should live in stable workspace roots such as `artifacts/phases/${phase_number}-${phase_slug}/`, `figures/`, or `paper/figures/`, not under `.grd/phases/**`.
...
```bash
cat {GRD_INSTALL_DIR}/templates/paper/figure-tracker.md
```

**If `.grd/paper/FIGURE_TRACKER.md` already exists:** Append new figures to the existing registry. Do not overwrite existing entries.
...
Write `.grd/paper/FIGURE_TRACKER.md` with:
...
```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/paper/FIGURE_TRACKER.md 2>&1) || true
echo "$PRE_CHECK"

grd commit \
  "docs(phase-${phase_number}): update figure tracker" \
  --files .grd/paper/FIGURE_TRACKER.md
```

**If no figures found:** Skip silently (not all phases produce visual outputs).

**Experimental comparison artifact:** If any plan in this phase compared theoretical predictions with experimental or observational data (PHENO-type objectives, or plans whose SUMMARY mentions "experimental comparison", "pull", "chi-squared", or "theory vs data"), create `.grd/paper/EXPERIMENTAL_COMPARISON.md` using `{GRD_INSTALL_DIR}/templates/paper/experimental-comparison.md`. Populate with comparison tables, pull values, and discrepancy classifications from the plan SUMMARYs. Skip if no experimental comparison was performed.
...
ALL_PLAN_IDS=($(echo "$PLAN_INDEX" | grd json pluck .plans id))
...
Create a recovery section in the phase directory. For physics-specific root cause analysis, consult `{GRD_INSTALL_DIR}/templates/recovery-plan.md`:
...
plans_failed: [{ list }]
plans_skipped: [{ list }]
checkpoint_tags: [{ list of all remaining grd-checkpoint tags for this phase }]
---
...
1. Fix failing plans and re-execute: `/grd:execute-phase {X}` (auto-detects partial completion)
2. Re-plan failed tasks: `/grd:plan-phase {X} --gaps` (creates new plans for unfinished work)
3. Revise phase goal: `/grd:discuss-phase {X}` (rethink approach based on what failed)
4. Continue to next phase: `/grd:plan-phase {X+1}` (if remaining work is non-critical)
```

Commit recovery document:

```bash
PRE_CHECK=$(grd pre-commit-check --files "${RECOVERY_FILE}" .grd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

grd commit \
  "docs(phase-${phase_number}): phase recovery report" \
  --files "${RECOVERY_FILE}" .grd/STATE.md
```
...
{If single plan failed, rest passed:}
  ...
  `/grd:execute-phase {X}` -- will resume from the failed plan
...
{If multiple plans failed in same wave:}
  ...
  `/grd:discuss-phase {X}` -- reassess methodology
...
{If all plans failed:}
  ...
  `/grd:plan-phase {X}` -- re-plan from scratch
──────────────────────────────────────────────────────
```
...
Follow the verification workflow. Read `{GRD_INSTALL_DIR}/workflows/verify-phase.md` using the file_read tool.
...
- Route only on `grd_return.status` and the validated frontmatter; do not use `grep "^status:"`, headings, or marker strings to decide success.
...
Also: `/grd:verify-work {X}` -- manual review first
```

Gap closure cycle: `/grd:plan-phase {X} --gaps` reads VERIFICATION.md -> creates gap plans with `gap_closure: true` -> user runs `/grd:execute-phase {X} --gaps-only` -> automatic re-verification (below).
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GRD > CIRCUIT BREAKER: VERIFICATION LOOP HALTED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
### Suggested Actions
1. `/grd:debug` — Systematic investigation of the persistent failure
2. `/grd:discuss-phase {X}` — Reassess the approach with fresh perspective
...
```bash
VERIFIER_MODEL=$(grd resolve-model grd-verifier)
```
...
task(
  ...
  readonly=false,
 prompt="First, read {GRD_AGENTS_DIR}/grd-verifier.md for your role and instructions.

Reload these canonical verifier surfaces before updating any verdicts:
- {GRD_INSTALL_DIR}/workflows/verify-phase.md
...
- Roadmap: .grd/ROADMAP.md
	...
	Return exactly one typed `grd_return` envelope with `status: completed | checkpoint | blocked | failed`, include `files_written`, and write `{phase_dir}/{phase}-VERIFICATION.md` before returning. Use the verifier's canonical `verification_status: passed | gaps_found | expert_needed | human_needed` inside the structured return or the written report; do not return legacy `passed | gaps_found` text as the routing surface.",
  description="Re-verify Phase {PHASE_NUMBER} after gap closure"
)
```

**If the verifier agent fails to spawn or returns an error:** Proceed without automated re-verification. Note in the phase status that post-gap-closure verification was skipped. The user should run `/grd:verify-work` separately to confirm gaps are closed.

**Handle the verifier response through `grd_return.status`:**

- `grd_return.status: completed`:
  1. Do not accept `grd_return.status: completed` until `{phase_dir}/{phase}-VERIFICATION.md` exists on disk.
  2. The same path appears in `grd_return.files_written`.
  ...
  4. After the artifact gate passes, use the canonical verifier verdict from `grd_return.verification_status` or the written report frontmatter:
...
- `grd_return.status: checkpoint`: stop and surface the checkpoint payload. Do not wait in place for user input inside this run.
- `grd_return.status: blocked` / `grd_return.status: failed`: stop in a blocked state, surface the issues, and keep gap-closure state intact.

**If the verifier output is malformed or omits `grd_return.status`:** Treat it as blocked. Do not infer success from prose headings or untyped legacy routing.
...
task(prompt="First, read {GRD_AGENTS_DIR}/grd-consistency-checker.md for your role and instructions.
...
file_read: .grd/STATE.md, .grd/state.json
file_read: All SUMMARY.md files from phase {PHASE_NUMBER}

Return consistency_status with any issues found.
", subagent_type="grd-consistency-checker", model="{consistency_model}", readonly=false, description="Rapid consistency check")

**If the consistency checker agent fails to spawn or returns an error:** Proceed without cross-phase consistency checking for this wave. Note in the phase status that consistency verification was skipped. The user should run `/grd:validate-conventions` after execution completes to catch any convention drift.

**If INCONSISTENT:** STOP execution. Present issues to user with resolution options:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GRD > CONVENTION INCONSISTENCY DETECTED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**If "Resolve conventions":** Spawn grd-notation-coordinator to fix the conflicts:
...
file_read: All SUMMARY.md files from phase {PHASE_NUMBER}
Load conventions: grd convention list
</project_context>

<output>
1. Update convention lock via grd convention set (if lock is wrong)
...
**If the notation coordinator agent fails to spawn or returns an error:** The consistency issues remain unresolved. Offer: 1) Retry notation coordinator, 2) Resolve conflicts manually by editing CONVENTIONS.md and using `grd convention set`, 3) Force continue with known inconsistencies (log to DECISIONS.md). Do not silently proceed — convention errors compound across phases.

Handle notation-coordinator return:
- **`CONVENTION UPDATE`:** Conventions fixed. Commit CONVENTIONS.md. Then verify `grd convention check` reports `locked` or `complete`, and re-check any phase artifacts flagged for re-execution are still present on disk before continuing. If the lock is still open or a flagged artifact is missing, treat the update as incomplete and keep the phase blocked.
...
**If `grd_return.status: completed`:** Continue to phase completion.
</step>
...
Follow the full transition protocol. Read `{GRD_INSTALL_DIR}/workflows/transition.md` using the file_read tool for PROJECT.md evolution, DECISIONS.md updates, and parallel phase detection.

```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/ROADMAP.md .grd/STATE.md "${phase_dir}"/*-VERIFICATION.md .grd/REQUIREMENTS.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs(phase-${phase_number}): complete phase execution" --files .grd/ROADMAP.md .grd/STATE.md "${phase_dir}"/*-VERIFICATION.md .grd/REQUIREMENTS.md
```
...
Remove all `grd-checkpoint/*` tags for this phase -- they are no longer needed.

```bash
# List all checkpoint tags for this phase
PHASE_TAGS=$(git tag -l "grd-checkpoint/phase-${phase_number}-*")
...
| Condition                               | Action                                             |
| --------------------------------------- | -------------------------------------------------- |
| All plans passed + verification passed  | Delete all `grd-checkpoint/phase-{X}-*` tags       |
...
After phase completion, check the project's autonomy mode. If yolo or balanced with no pending checkpoint, auto-route to the next phase. If supervised, or if a checkpoint requires review, pause with a clear status message showing: current phase completed, why execution paused, exact next command to continue, and key artifacts to review. See `{GRD_INSTALL_DIR}/references/orchestration/continuous-execution.md` for the standard checkpoint protocol.
</continuation_routing>
