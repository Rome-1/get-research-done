<purpose>
Export GRD observability session logs and execution traces to files. Reads session event streams from `GRD/observability/sessions/` and trace logs from `GRD/traces/`, applies optional filters, and writes results to `GRD/exports/logs/` or a custom directory. Supports JSONL (raw), JSON (pretty-printed), and markdown (human-readable report) formats.
</purpose>
...
**Validate GRD project and observability data:**

```bash
grd validate command-context grd:export-logs $ARGUMENTS
```

Check that `GRD/observability/sessions/` exists and contains at least one `.jsonl` file:

```bash
ls GRD/observability/sessions/*.jsonl 2>/dev/null | head -5
```
...
No observability sessions found in GRD/observability/sessions/.

Session logs are recorded automatically during GRD command execution.
Run any GRD command first, then retry grd:export-logs.
```
...
| `--output-dir <path>` | `GRD/exports/logs/` | Custom output directory |
...
RESULT=$(grd --raw observe export $EXPORT_ARGS)
...
**Also available:**
- `grd:export-logs --format markdown` — human-readable report
- `grd:export-logs --last 5` — export only recent sessions
- `grd observe show` — inspect events interactively
- `grd observe sessions` — list available sessions
...
Try broadening filters or run `grd observe sessions` to see available sessions.
```
...
- [ ] GRD project validated and observability data found
