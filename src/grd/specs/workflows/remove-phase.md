...
**Delegate the entire removal operation to grd CLI:**

```bash
RESULT=$(grd phase remove "${target}")
if [ $? -ne 0 ]; then
  echo "Phase removal blocked: $RESULT"
fi
```

If the phase has executed plans (SUMMARY.md files), grd will error. Use `--force` only if the user confirms:

```bash
RESULT=$(grd phase remove "${target}" --force)
```
...
- Don't manually renumber -- use `grd phase remove` which handles all renumbering
