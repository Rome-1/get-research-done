...
```bash
INIT=$(grd --raw init milestone-op)
if [ $? -ne 0 ]; then
...
```bash
milestone_version=$(echo "$INIT" | grd json get .milestone_version --default "")
milestone_name=$(echo "$INIT" | grd json get .milestone_name --default "")
phase_count=$(echo "$INIT" | grd json get .phase_count --default 0)
completed_phases=$(echo "$INIT" | grd json get .completed_phases --default 0)
roadmap_exists=$(echo "$INIT" | grd json get .roadmap_exists --default false)
state_exists=$(echo "$INIT" | grd json get .state_exists --default false)
commit_docs=$(echo "$INIT" | grd json get .commit_docs --default true)
```

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `grd:new-project` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `grd:new-project` first."

Read the model profile for physics-aware smart discuss depth:

```bash
MODEL_PROFILE=$(grd config get model_profile 2>/dev/null || echo "review")
```
...
```bash
DETAIL=$(grd --raw roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GRD ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_GOAL=$(grd --raw roadmap get-phase ${PHASE_NUM} | grd json get .goal --default "")
```
...
- **options:** "Use grd:write-paper" / "Use normal discuss→plan→execute"

On **"Use grd:write-paper":**

```
Skill(skill="grd:write-paper")
```
...
```bash
PHASE_STATE=$(grd --raw init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(grd config get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
- Acceptance signal: Verification via grd:verify-work
...
```bash
grd commit "docs(${padded_phase}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="grd:execute-phase", args="${PHASE_NUM}")
```
...
```bash
PHASE_STATE=$(grd --raw init phase-op ${PHASE_NUM})
PHASE_DIR=$(echo "$PHASE_STATE" | grd json get .phase_dir --default "")
```
...
```
Skill(skill="grd:verify-work", args="${PHASE_NUM}")
```
...
```
Skill(skill="grd:execute-phase", args="${PHASE_NUM} --gaps-only")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `grd:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged. Future milestones may extract this to a separate skill file.
...
```bash
cat GRD/PROJECT.md 2>/dev/null || true
cat GRD/REQUIREMENTS.md 2>/dev/null || true
cat GRD/STATE.md 2>/dev/null || true
cat GRD/CONVENTIONS.md 2>/dev/null || true
```
...
```bash
(find GRD/phases -name "*-CONTEXT.md" 2>/dev/null || true) | sort
```
...
```bash
ls GRD/literature/*.md 2>/dev/null || true
ls GRD/research-map/*.md 2>/dev/null || true
```
...
```bash
find GRD/phases -name "*-SUMMARY.md" 2>/dev/null | sort
```
...
```bash
grd commit "docs(${padded_phase}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
After each phase completes (verification passed or gaps accepted), check convention consistency with prior phases. This is a GRD-specific gate that GSD does not need.

**Check if CONVENTIONS.md exists:**

```bash
ls GRD/CONVENTIONS.md 2>/dev/null
```
...
```
Skill(skill="grd:validate-conventions", args="${PHASE_NUM}")
```
...
```bash
AUDIT_FILE="GRD/v${milestone_version}-MILESTONE-AUDIT.md"
...
```
Skill(skill="grd:plan-milestone-gaps")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run `grd:audit-milestone` to review, then `grd:plan-milestone-gaps` to create fix phases."
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit issues to address. Run `grd:audit-milestone` to review details."
...
```bash
ls GRD/milestones/v${milestone_version}-ROADMAP.md 2>/dev/null || true
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Research milestone archived. Run grd:new-milestone to start the next phase of investigation.
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: grd:autonomous --from {next_phase}
```
...
- [ ] Missing VERIFICATION.md triggers explicit grd:verify-work invocation
...
- [ ] Paper-writing phases detected and routed to grd:write-paper when user confirms
...
- [ ] Audit gap closure offered via grd:plan-milestone-gaps (loops back to phase execution)
...
- [ ] No UI-phase or frontend-specific detection (GRD is physics research, not software)
- [ ] All CLI commands use grd (not gsd-tools.cjs)
- [ ] All file paths use GRD/ (not .planning/)
