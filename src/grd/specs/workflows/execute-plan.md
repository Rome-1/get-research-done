...
- {GRD_INSTALL_DIR}/domains/{GRD_DOMAIN}/protocols/reproducibility.md
- {GRD_INSTALL_DIR}/domains/{GRD_DOMAIN}/protocols/error-propagation-protocol.md -- Cross-phase uncertainty propagation protocol (Uncertainty Budget declaration format, verification checks, phase handoff format)
...
CONFIG_CONTENT=$(echo "$INIT" | grd json get .config_content --default "")
```
...
```bash
grd observe event workflow execute-plan.start --phase "${phase}" --plan "${plan}" 2>/dev/null || true
```
...
```bash
CHECKPOINT_TAG="grd-checkpoint/phase-${phase}-plan-${plan}-$(date +%s)"
...
**Pattern A:** init_agent_tracking -> spawn task(subagent_type="grd-executor", model=executor_model, readonly=false) with prompt: execute plan at [path], all tasks + SUMMARY + structured return envelope, follow deviation/validation rules, **load conventions from `grd convention list` before starting work**, `<autonomy_mode>{AUTONOMY}</autonomy_mode>`, `<review_cadence>{REVIEW_CADENCE}</review_cadence>`, `<bounded_execution>false</bounded_execution>` (only for genuinely low-risk short plans), return: plan name, tasks, SUMMARY path, commit hash, and state updates -> track agent_id -> wait -> update tracking -> report.
...
```bash
if [ ! -f .grd/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > .grd/agent-history.json
fi
if [ -f .grd/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat .grd/current-agent-id.txt)
...
2. Per task:
   - `type="auto"`: Execute derivation/calculation/simulation. Verify done criteria including dimensional checks. Commit (see task_commit). Track hash for Summary.
     ```bash
     grd observe event task task-complete --phase "${phase}" --plan "${plan}" --data "{\"task\":\"${TASK_NUM}\",\"description\":\"${TASK_DESCRIPTION}\"}" 2>/dev/null || true
     grd trace log checkpoint --data "{\"description\":\"Task ${TASK_NUM} done: ${TASK_DESCRIPTION}\"}" 2>/dev/null || true
     ```
     **Required first-result sanity gate:** At the earliest of (a) first quantitative result, (b) first derived core equation, (c) first produced artifact, (d) first benchmark-style comparison, or (e) two completed auto tasks, STOP and check:
     ...
     ```bash
     grd observe event execution gate --action clear --phase "${phase}" --plan "${plan}" \
     ...
     ```bash
     grd observe event execution fanout --action lock --phase "${phase}" --plan "${plan}" \
     ...
     grd observe event execution fanout --action unlock --phase "${phase}" --plan "${plan}" \
       --data "{\"execution\":{\"checkpoint_reason\":\"pre_fanout\"}}" 2>/dev/null || true
     ```
...
3. Run `<verification>` checks including physics validation (see `execute-plan-validation.md`)
   ```bash
   grd observe event verification verification-complete --phase "${phase}" --plan "${plan}" --data "{\"description\":\"${VERIFICATION_RESULT}\"}" 2>/dev/null || true
   grd trace log assertion --data "{\"description\":\"Verification: ${VERIFICATION_RESULT}\"}" 2>/dev/null || true
   ```
...
3. If >80%: save intermediate results and trigger `/grd:pause-work` before quality degrades
...
If pausing mid-plan: commit current work, create `.continue-here.md` with full derivation state, recommend `/clear` + `/grd:resume-work`. See `{GRD_INSTALL_DIR}/references/orchestration/context-budget.md` for budget guidelines.
...
2. If above 75% (or 85% in yolo mode): Proactively trigger pause protocol:
  ...
  - **supervised/balanced:** Suggest `/clear` + `/grd:resume-work`
  - **yolo:** Prepare the bounded resume handoff automatically and continue only if the runtime can spawn the continuation with explicit segment state; otherwise suggest `/clear` + `/grd:resume-work`
...
**2. Choose the exact files for `grd commit --files`** (NEVER commit broad paths like `git add .` or `git add -A`):
...
```bash
grd commit "{type}({phase}-{plan}): {description}" --files "${TASK_FILES[@]}"
```

**6. Pre-commit validation** runs automatically inside `grd commit`. If it fails (invalid frontmatter or serialized NaN/Inf in the checked files), the commit is blocked — fix the errors and retry.
...
grd json set --file "$COMMITS_FILE" --path "task_${TASK_NUM}" --value "${TASK_COMMIT}"
```
...
if ls "${phase_dir}"/plan-commits-seg-*.json 1>/dev/null 2>&1; then
  grd json merge-files --out "${phase_dir}/plan-commits.json" "${phase_dir}"/plan-commits-seg-*.json
...
See `execute-plan-checkpoints.md` for the full checkpoint protocol (display format, types, resume signals) and `{GRD_INSTALL_DIR}/references/orchestration/checkpoints.md` for general checkpoint details.
...
Create `${phase}-${plan}-SUMMARY.md` at `${phase_dir}/`. Use `{GRD_INSTALL_DIR}/templates/summary.md`.
...
**Do NOT write STATE.md directly.** Return state updates in the `grd_return` envelope so the orchestrator (execute-phase.md) can apply them sequentially after each executor finishes. This prevents parallel write conflicts.
...
```bash
grd state advance
grd state update-progress
grd state record-metric \
...
From SUMMARY: Extract decisions and blockers. Include them in the `grd_return` envelope:
...
Include session update in the `grd_return` envelope:
...
PRE_CHECK=$(grd pre-commit-check --files "${phase_dir}/${phase}-${plan}-SUMMARY.md" .grd/STATE.md .grd/ROADMAP.md 2>&1) || true
echo "$PRE_CHECK"
```

If the explicit `PRE_CHECK` command reports issues, treat it as early visibility only. `grd commit` re-runs the same validation on the commit paths and remains the blocking gate, so fix any reported issues before retrying when the commit is rejected.

```bash
grd commit "docs(${phase}-${plan}): complete ${PLAN_NAME} plan" --files "${phase_dir}/${phase}-${plan}-SUMMARY.md" .grd/STATE.md .grd/ROADMAP.md
```
...
```bash
grd observe event workflow execute-plan.complete --phase "${phase}" --plan "${plan}" 2>/dev/null || true
```

Stop execution trace (captures event summary):

```bash
grd trace stop 2>/dev/null || true
```
</step>
