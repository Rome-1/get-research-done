...
```bash
CONTEXT=$(grd --raw validate command-context audit-milestone "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Run /grd:new-project first, then complete phases before auditing.
```
...
```bash
CHECKER_MODEL=$(grd resolve-model grd-consistency-checker)
```
...
Create `.grd/v{version}-MILESTONE-AUDIT.md` with:
...
import json, pathlib
try:
    c = json.loads(pathlib.Path('.grd/config.json').read_text())
...
- All SUMMARY.md files from completed phases in .grd/phases/
...
- .grd/v{milestone_version}-MILESTONE-AUDIT.md (audit results)
...
**If the referee agent fails to spawn or returns an error:** Proceed without mock peer review — note in the audit report that peer review was skipped. The audit is still valid based on consistency checks and phase-level verification. The user should run `/grd:verify-work` separately after the audit.
...
**Report:** .grd/v{version}-MILESTONE-AUDIT.md
...
- cat .grd/v{version}-MILESTONE-AUDIT.md -- see full report
- /grd:complete-milestone {version} -- proceed anyway (accept open questions)
