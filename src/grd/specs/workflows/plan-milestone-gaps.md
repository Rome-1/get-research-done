<purpose>
Create all phases necessary to close research gaps identified by `/grd:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases -- no manual `/grd:add-phase` per gap.
...
ls -t .grd/v*-MILESTONE-AUDIT.md 2>/dev/null | head -1
```
...
PHASES=$(grd phase list)
HIGHEST=$(echo "$PHASES" | grd json get .directories[-1] --default "")
if [ -z "$HIGHEST" ]; then
  echo "ERROR: No existing phases found. Create phases with /grd:plan-phase first."
...
```bash
mkdir -p ".grd/phases/{NN}-{name}"
```

## 8. Commit Roadmap Update

```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/ROADMAP.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs(roadmap): add gap closure phases {N}-{M}" --files .grd/ROADMAP.md
```
...
- `cat .grd/ROADMAP.md` -- see updated roadmap
...
- [ ] Changes committed
- [ ] Researcher knows to run `/grd:plan-phase` next
