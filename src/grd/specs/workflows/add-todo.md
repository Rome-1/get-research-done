<purpose>
Capture an idea, task, or issue that surfaces during a GRD session as a structured todo for later work. Enables "thought -> capture -> continue" flow without losing context. Typical physics research todos include: a subtlety to check, a reference to look up, an alternative approach to explore, a numerical test to run, or a derivation step to verify.
</purpose>
...
**Note:** `add-todo` works even without a project (creates `.grd/todos/` standalone). No project_exists gate needed — todos can be created independently.

Ensure directories exist:

```bash
mkdir -p .grd/todos/pending .grd/todos/done
```
...
- `/grd:add-todo Check unitarity of S-matrix at two loops` -> title = "Check unitarity of S-matrix at two loops"
...
grep -l -i "[key words from title]" .grd/todos/pending/*.md 2>/dev/null
```
...
```bash
slug=$(grd --raw slug "$title")
todo_file=".grd/todos/pending/${date}-${slug}.md"
```
...
```bash
PRE_CHECK=$(grd pre-commit-check --files "${todo_file}" .grd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs: capture todo - ${title}" --files "${todo_file}" .grd/STATE.md
```
