...
1. Extract full milestone details to `.grd/milestones/v[X.Y]-ROADMAP.md`
2. Archive requirements to `.grd/milestones/v[X.Y]-REQUIREMENTS.md`
...
**Use `roadmap analyze` plus the on-disk phase inventory for the same readiness semantics as `grd milestone complete`:**

```bash
ROADMAP=$(grd roadmap analyze)
```

`roadmap analyze` returns the roadmap-backed phase view. Compare that with the on-disk phase directories so the milestone gate sees the same roadmap-plus-disk union that `grd milestone complete` uses. Use this to verify:
...
```bash
if [ -f .grd/config.json ]; then cat .grd/config.json; else echo "WARNING: config.json not found — using defaults"; fi
```
...
for summary in .grd/phases/*/*SUMMARY.md; do
  grd summary-extract "$summary" --field one_liner | grd json get .one_liner --default ""
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `grd milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from summary artifacts (`SUMMARY.md` and `*-SUMMARY.md`).
...
Generate a structured research digest that serves as the primary handoff artifact between milestone completion and paper writing. This digest lives at `.grd/milestones/v[X.Y]/RESEARCH-DIGEST.md` and captures the full research story in a form optimized for downstream consumption by the paper-writing workflow.

**Step 0 -- Ensure digest directory exists:**

```bash
mkdir -p .grd/milestones/v[X.Y]
```

**Step 1 -- Run history-digest for foundation data:**

```bash
DIGEST=$(grd history-digest)
```
...
cat .grd/CONVENTIONS.md

# Convention lock (machine-readable, from state.json)
grd --raw convention list

# Original objectives
cat .grd/REQUIREMENTS.md
```

**Step 3 -- Compose RESEARCH-DIGEST.md:**

Create `.grd/milestones/v[X.Y]/RESEARCH-DIGEST.md` with the following structure:
...
[Timeline of convention changes extracted from `grd --raw convention list`, showing
...
**Delegate archival to grd CLI:**

```bash
ARCHIVE=$(grd milestone complete "v[X.Y]" --name "[Milestone Name]")
```

The CLI handles:

- Creating `.grd/milestones/` directory
...
Verify: `Milestone archived to .grd/milestones/`

**Note:** Phase directories (`.grd/phases/`) are NOT deleted -- they accumulate across milestones as raw research history. Phase numbering continues (v1.0 phases 1-4, v1.1 phases 5-8, etc.).
...
grd --raw pattern list 2>/dev/null
```

**Step 3 — Review insights for promotion candidates:**

Read `.grd/INSIGHTS.md` and identify entries that:
...
```bash
grd --raw pattern search "keyword from insight" 2>/dev/null
```
...
1. **{insight title}** — confirmed in phases {X}, {Y}
   ...
   → `grd pattern add --domain {domain} --category {category} --title "{title}"`
...
if [ -f ".grd/milestones/v${VERSION}-REQUIREMENTS.md" ]; then
  # Use git rm so the deletion is staged for commit (not just a filesystem rm)
  git rm .grd/REQUIREMENTS.md 2>/dev/null || rm .grd/REQUIREMENTS.md
else
  echo "ERROR: Archive not found at .grd/milestones/v${VERSION}-REQUIREMENTS.md. Refusing to delete original."
  exit 1
fi
```
...
```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/milestones/ .grd/ROADMAP.md 2>&1) || true
echo "$PRE_CHECK"

grd commit \
  "chore(milestone-v${VERSION}): reorganize roadmap and retire requirements" \
  --files .grd/milestones/ .grd/ROADMAP.md
```
...
See .grd/MILESTONES.md for full details."
```
...
Commit milestone completion. Only include files that exist (MILESTONE-AUDIT.md is optional — only present if `/grd:audit-milestone` was run beforehand).

Build the file list, conditionally including MILESTONE-AUDIT.md if it exists:

```bash
COMMIT_FILES=".grd/milestones/v[X.Y]-ROADMAP.md .grd/milestones/v[X.Y]-REQUIREMENTS.md .grd/milestones/v[X.Y]/RESEARCH-DIGEST.md .grd/MILESTONES.md .grd/PROJECT.md .grd/STATE.md"

if [ -f ".grd/milestones/v[X.Y]-MILESTONE-AUDIT.md" ]; then
  COMMIT_FILES="$COMMIT_FILES .grd/milestones/v[X.Y]-MILESTONE-AUDIT.md"
fi

PRE_CHECK=$(grd pre-commit-check --files $COMMIT_FILES 2>&1) || true
echo "$PRE_CHECK"

grd commit "chore: complete v[X.Y] research milestone" --files $COMMIT_FILES
```
...
- [ ] Researcher knows next step (/grd:new-milestone)
