...
**Check for existing verification artifact** (e.g., from a prior `/grd:execute-phase` → `verify-phase` run):

```bash
EXISTING_VERIFICATION=$(ls "$phase_dir"/*-VERIFICATION.md 2>/dev/null | head -1)
```

If an existing verification artifact is found (e.g., from a prior `/grd:execute-phase` → `verify-phase` automated run):
...
for PRIOR_SUMMARY in $(ls .grd/phases/*/*SUMMARY.md 2>/dev/null | sort); do
...
```bash
grd validate verification-contract "${phase_dir}/${phase_number}-VERIFICATION.md"

PRE_CHECK=$(grd pre-commit-check --files "${phase_dir}/${phase_number}-VERIFICATION.md" 2>&1) || true
echo "$PRE_CHECK"
