<purpose>
Validate research results through conversational research validation with persistent state. Creates VERIFICATION.md that tracks verification progress, survives /clear, and feeds gaps into /grd:plan-phase --gaps.
...
<template>
@{GRD_INSTALL_DIR}/templates/research-verification.md
</template>
...
Usage: /grd:verify-work <phase-number>
```
...
```bash
if [ -n "${PHASE_ARG}" ]; then
  REVIEW_PREFLIGHT=$(grd validate review-preflight verify-work "${PHASE_ARG}" --strict)
else
  REVIEW_PREFLIGHT=$(grd validate review-preflight verify-work --strict)
fi
if [ $? -ne 0 ]; then
...
```bash
find .grd/phases -name "*-VERIFICATION.md" -type f 2>/dev/null | head -5
```
...
**Check for existing VERIFICATION.md** (e.g., from a prior `/grd:execute-phase` → `verify-phase` run):

```bash
EXISTING_VERIFICATION=$(ls "$phase_dir"/*-VERIFICATION.md 2>/dev/null | head -1)
```

If an existing VERIFICATION.md is found (e.g., from a prior `/grd:execute-phase` → `verify-phase` automated run):
...
If the check passed AND the check name or expected outcome corresponds to a requirement ID (REQ-*) from `.grd/REQUIREMENTS.md`, update the requirement's status:

1. Read `.grd/REQUIREMENTS.md` (skip if file doesn't exist)
...
for PRIOR_SUMMARY in $(ls .grd/phases/*/SUMMARY.md .grd/phases/*/*-SUMMARY.md 2>/dev/null | sort); do
...
```bash
grd validate verification-contract "${phase_dir}/{phase}-VERIFICATION.md"

PRE_CHECK=$(grd pre-commit-check --files "${phase_dir}/{phase}-VERIFICATION.md" 2>&1) || true
echo "$PRE_CHECK"

grd commit "verify({phase}): complete research validation - {passed} passed, {issues} issues" --files "${phase_dir}/{phase}-VERIFICATION.md"
```
...
- `/grd:plan-phase {next}` -- Plan next research phase
...
- Load debug workflow
- Follow @{GRD_INSTALL_DIR}/workflows/debug.md
...
- Validation with diagnoses: .grd/phases/{phase_dir}/{phase}-VERIFICATION.md
- State: .grd/STATE.md
...
<files_to_read>
Read all PLAN.md files in .grd/phases/{phase_dir}/ using the file_read tool.
</files_to_read>
...
**If the plan-checker agent fails to spawn or returns an error:** Proceed without plan verification — the plans will still be executable. Note that plans were not verified and recommend running `/grd:plan-phase --gaps` to re-verify if needed.
...
3. Abandon (exit, researcher runs /grd:plan-phase manually)
