...
```bash
INIT=$(grd init phase-op --include state,roadmap)
```
...
```bash
SOURCE_INFO=$(grd phase find "${SOURCE_PHASE}")
TARGET_INFO=$(grd phase find "${TARGET_PHASE}")
```
...
```bash
SOURCE_GOAL=$(grd roadmap get-phase "${SOURCE_PHASE}" | python3 -c "import sys,json; print(json.loads(sys.stdin.read()).get('goal',''))")
TARGET_GOAL=$(grd roadmap get-phase "${TARGET_PHASE}" | python3 -c "import sys,json; print(json.loads(sys.stdin.read()).get('goal',''))")
```
</step>
...
```bash
grd summary-extract "${SOURCE_DIR}"/*-SUMMARY.md --field conventions
grd summary-extract "${TARGET_DIR}"/*-SUMMARY.md --field conventions
```
...
Wait for user decision. Record decision via grd CLI:

```bash
grd state add-decision --phase "${TARGET_PHASE}" --summary "Convention resolution for merge: chose {chosen}" --rationale "{user rationale}"
```
...
```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/ROADMAP.md .grd/STATE.md .grd/state.json "${TARGET_DIR}" 2>&1) || true
echo "$PRE_CHECK"

grd commit \
  "refactor(phases): merge phase ${SOURCE_PHASE} into phase ${TARGET_PHASE}" \
  --files .grd/ROADMAP.md .grd/STATE.md .grd/state.json "${TARGET_DIR}"
```
</step>
...
**Also available:**
- /grd:show-phase {T} -- inspect merged phase
- /grd:execute-phase {T} -- execute pending plans
- /grd:validate-conventions -- verify convention consistency
