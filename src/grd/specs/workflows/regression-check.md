...
Use `/grd:verify-work <phase>` when a flagged phase needs actual re-verification.
</purpose>
...
Identify completed phase directories under `.grd/phases`.
...
```bash
find .grd/phases -type f -name "*SUMMARY.md" 2>/dev/null | sort
```
...
```bash
find .grd/phases -type f -name "*-VERIFICATION.md" 2>/dev/null | sort
```
...
2. Run `/grd:verify-work <phase>` if an affected phase now needs real re-verification
3. Re-run `/grd:regression-check [phase]` to confirm the artifact-level audit is clean
</step>
