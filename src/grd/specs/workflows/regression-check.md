...
```bash
PROGRESS=$(grd init progress --include state,roadmap,config)
grd roadmap analyze
```
...
```bash
CONTEXT=$(grd --raw validate command-context regression-check "${PHASE_ARG:-}")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
PHASE_COUNT=$(find .grd/phases -name "*-VERIFICATION.md" -type f 2>/dev/null | wc -l | tr -d ' ')
...
```bash
find .grd/phases -name "*-VERIFICATION.md" -type f 2>/dev/null | sort
```

For each file, read frontmatter to check status:

```bash
grd frontmatter get "$VERIF_FILE" --field status
```
...
|-------|---------------------|--------|----------------------|
| 01-setup | .grd/phases/01-setup/01-VERIFICATION.md | passed | 5 |
| 02-derivation | .grd/phases/02-derivation/02-VERIFICATION.md | passed | 7 |
| 03-numerics | .grd/phases/03-numerics/03-VERIFICATION.md | gaps_found | 4 |
```
...
Run /grd:verify-work <phase> to verify a completed phase first.
```
...
targets = [
  {
    ...
    verification_file: ".grd/phases/01-setup/01-VERIFICATION.md",
  ...
  },
  {
    ...
    verification_file: ".grd/phases/02-derivation/02-VERIFICATION.md",
...
4. Check for convention changes since verification:
   ```bash
   find .grd/phases -type f \( -name "SUMMARY.md" -o -name "*-SUMMARY.md" \) -exec grep -l "Convention Change\|unit system\|natural units\|hbar\s*=" {} + 2>/dev/null
   ```
...
1. **Notation consistency:**
   ...
   ```bash
   find .grd/phases -type f \( -name "SUMMARY.md" -o -name "*-SUMMARY.md" \) -exec grep -n "$SYMBOL" {} + 2>/dev/null
   ```
...
```bash
grd convention diff <last-verified-phase> <current-phase>
```
</step>
...
find .grd/phases -type f \( -name "SUMMARY.md" -o -name "*-SUMMARY.md" \) -exec grep -rl "phase.*${PHASE_NUM}\|${PHASE_NAME}" {} + 2>/dev/null
```

</step>

<step name="generate_report">
**Generate `.grd/REGRESSION-REPORT.md`:**
...
3. **Re-run `/grd:regression-check`** -- confirm all regressions resolved
...
Write to `.grd/REGRESSION-REPORT.md`.

Commit:

```bash
PRE_CHECK=$(grd pre-commit-check --files ".grd/REGRESSION-REPORT.md" 2>&1) || true
echo "$PRE_CHECK"

grd commit "verify: regression check - {V}/{M} verified targets hold, {K} regressions" --files ".grd/REGRESSION-REPORT.md"
```
...
```
====================================================
 GRD > REGRESSION CHECK: {K} REGRESSIONS FOUND
====================================================
...
2. `/grd:verify-work {affected_phase}` -- re-verify affected phases
3. `/grd:regression-check` -- confirm all regressions resolved

{If only MAJOR/MINOR:}
1. Review regressions in report
2. `/grd:verify-work {affected_phase}` -- re-verify flagged phases
3. `/grd:regression-check` -- confirm clean
