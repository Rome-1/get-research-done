...
- `.grd/PROJECT.md` -- project title, description, conventions
- `.grd/ROADMAP.md` -- phase structure and status
- `.grd/STATE.md` -- current position

```bash
ROADMAP=$(grd roadmap analyze)
if [ $? -ne 0 ]; then
  echo "ERROR: grd roadmap analyze failed: $ROADMAP"
...
```bash
grd summary-extract {path} --field one_liner --field key_results --field equations --field key_files
```
...
<html>
  ...
  <body>
    <h1>{project_title}</h1>
    <p><em>Generated with Get Research Done (PSI) | {YYYY-MM-DD} | Milestone: {milestone_name}</em></p>
...
Date: {YYYY-MM-DD}
Attribution: Generated with Get Research Done (PSI)
```
...
cp .grd/PROJECT.md exports/PROJECT.md 2>/dev/null
cp .grd/ROADMAP.md exports/ROADMAP.md 2>/dev/null
...
**Also available:**
- `/grd:write-paper` -- draft a full paper from research results
- `/grd:progress` -- check research progress
