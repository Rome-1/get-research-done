...
Debug files use the `.grd/debug/` path (hidden directory with leading dot).
</paths>
...
For each gap, fill the debug subagent prompt template (see `{GRD_INSTALL_DIR}/templates/debug-subagent-prompt.md` for the full template with placeholders, continuation format, and failure protocol) and spawn:
...
- expectation: "Energy is conserved to machine precision"
  ...
  debug_session: .grd/debug/energy-not-conserved.md
```
...
grd commit "docs({phase}): add root causes from diagnosis" --files "${phase_dir}/{phase}-VERIFICATION.md"
```
