...
Example: `/grd:revise-phase 3 "Sign error in vertex correction propagated to all subsequent phases"`
-> phase = 3
-> reason = "Sign error in vertex correction propagated to all subsequent phases"

Example: `/grd:revise-phase 5.1 "Regularization scheme breaks gauge invariance"`
-> phase = 5.1
...
Usage: /grd:revise-phase <phase-number> "<reason for revision>"
Example: /grd:revise-phase 3 "Sign error in vertex correction"
```
...
```bash
INIT=$(grd init phase-op "${target_phase}")
if [ $? -ne 0 ]; then
...
Usage: /grd:revise-phase <phase-number> "<reason>"
```
...
- `.grd/ROADMAP.md` -- full roadmap structure
- `.grd/STATE.md` -- current position and accumulated context
...
- /grd:remove-phase {target} -- remove it entirely
- /grd:plan-phase {target} -- plan it differently from scratch
...
Use search_files to search `.grd/phases/` for references to the target phase number, its results, or its key outputs.
...
Write `.grd/phases/{target_dir}/SUPERSEDED.md`:
...
ls -d .grd/phases/${target_integer}.* 2>/dev/null
```
...
```bash
RESULT=$(grd phase insert "${target_phase}" "${phase_name_with_revised_prefix}")
if [ $? -ne 0 ]; then
...
Write `.grd/phases/{replacement_dir}/CONTEXT.md`:
...
- Original phase: `.grd/phases/{target_dir}/`
- Original summary: `.grd/phases/{target_dir}/{target}-01-SUMMARY.md` (etc.)
- Supersession record: `.grd/phases/{target_dir}/SUPERSEDED.md`
```
...
Use search_files to search `.grd/phases/` and `.grd/ROADMAP.md` for:
...
Consider: /grd:revise-phase {X} "{cascading_reason}"
```

</step>

<step name="update_state">
Update project state with the revision event using grd commands (ensures STATE.md + state.json stay in sync):

1. Record the supersession decision:

```bash
grd state add-decision --phase "${target}" --summary "Superseded Phase ${target} (${name}) with Phase ${replacement_number}" --rationale "${reason}"
```

2. Add a blocker noting the results are under revision:

```bash
grd state add-blocker --text "Phase ${target} results under revision -- superseded by Phase ${replacement_number}: ${reason}"
```
...
Do NOT edit STATE.md directly — always use grd state commands to maintain state.json sync.
</step>
...
```bash
PRE_CHECK=$(grd pre-commit-check --files .grd/ROADMAP.md .grd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs(phase-${target_padded}): supersede and replace — ${reason}" --files .grd/
```
</step>
...
`/grd:plan-phase {replacement_number}`

<sub>`/clear` first -> fresh context window</sub>

**Review affected downstream phases first?**
{List /grd:show-phase commands for affected phases}
...
- Don't mark future/unstarted phases as superseded -- use /grd:remove-phase instead
...
- Don't use `python` for grd invocations -- always use `grd`
...
- [ ] Blocker recorded via `grd state add-blocker` for results under revision
