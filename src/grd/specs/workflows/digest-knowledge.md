...
- stop and route approval, promotion, or stable-target mutation requests to `grd:review-knowledge`

This workflow does not claim downstream runtime ingestion or planner/verifier trust propagation. Review approval and stable promotion are handled by `grd:review-knowledge`, and downstream runtime consumption remains deferred to later phases.

Called from `grd:digest-knowledge`.
</purpose>
...
If the input does not clearly map to a single knowledge-doc target, the workflow must stop and ask. If the target already exists as stable or superseded, the workflow must not silently repurpose it as a draft authoring target. Route that request to `grd:review-knowledge` instead.
</core_principle>
...
- `knowledge_path` means an explicit path under `GRD/knowledge/` pointing to a `.md` file
...
- The canonical knowledge directory is `GRD/knowledge/`
- The canonical file name is `GRD/knowledge/{knowledge_id}.md`
...
In those cases, stop and route the user to `grd:review-knowledge` instead of silently mutating the document into a different state.
</step>
...
```bash
grd frontmatter validate GRD/knowledge/{knowledge_id}.md --schema knowledge
```
