...
\`\`\`bash
/grd:arxiv-submission {paper_dir}
\`\`\`
```
...
```bash
PRE_CHECK=$(grd pre-commit-check --files "${SUBMISSION_DIR}/SUBMISSION-MANIFEST.md" "${SUBMISSION_DIR}/00README.XXX" .gitignore 2>&1) || true
echo "$PRE_CHECK"
