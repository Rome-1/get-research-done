...
```bash
for f in .grd/STATE.md .grd/PROJECT.md .grd/ROADMAP.md; do
...
**Delegate ROADMAP.md and STATE.md updates to grd CLI:**

```bash
TRANSITION=$(grd phase complete "${CURRENT_PHASE}")
if [ $? -ne 0 ]; then
...
**Append decisions from this phase to .grd/DECISIONS.md.**
...
```bash
EXISTING=$(grep -c "| ${CURRENT_PHASE} |" .grd/DECISIONS.md 2>/dev/null || echo 0)
```
...
```bash
LAST_ID=$(grep -c '^| DEC-' .grd/DECISIONS.md 2>/dev/null || echo 0)
```
...
If `.grd/DECISIONS.md` doesn't exist, create it with the header from the template:
...
- [ ] New entries appended to .grd/DECISIONS.md (or file created)
...
- [ ] All modified `.grd/` files committed
...
```bash
AUTO_COMPACT=$(grd state compact 2>&1)
```
...
if echo "$AUTO_COMPACT" | grep -q '"compacted": true'; then
  PRE_CHECK=$(grd pre-commit-check --files .grd/STATE.md .grd/STATE-ARCHIVE.md .grd/state.json 2>&1) || true
  echo "$PRE_CHECK"

  grd commit "chore: auto-compact state after phase-${CURRENT_PHASE} transition" --files .grd/STATE.md .grd/STATE-ARCHIVE.md .grd/state.json
fi
```
...
This ensures derivation state is captured even without explicit `/grd:pause-work`. After phase completion, scan the phase's SUMMARY.md files for equations, conventions, and key results, then append them to `.grd/DERIVATION-STATE.md`.
...
- `## Key Results` body section -> key equations and results (use `grd summary-extract --field key_results` to extract)
...
grd commit "docs(phase-${CURRENT_PHASE}): autofill derivation state" --files .grd/DERIVATION-STATE.md
```
...
```bash
if [ -f .grd/config.json ]; then cat .grd/config.json; else echo "WARNING: config.json not found — parallelization defaults apply"; fi
```
...
- **2+ ready phases AND `parallelization: true`:**
  ...
  Launch parallel `/grd:execute-phase` for each ready phase simultaneously.
  ...
  Options:
  1. Execute all in parallel (launch simultaneous /grd:execute-phase for each)
  2. Execute sequentially starting with Phase [lowest] (standard path)
  ```

  Wait for user decision.

  - **If parallel (option 1):** Launch parallel `/grd:execute-phase` for each
    ready phase.
...
After parallel `/grd:execute-phase` invocations all complete, run the
...
- **Convention conflicts:** Did they adopt incompatible conventions (e.g.,
  ...
  ```bash
  cat .grd/phases/XX-phase-a/*-SUMMARY.md .grd/phases/YY-phase-b/*-SUMMARY.md
  ```
...
**Use the transition result from `grd phase complete`:**
...
Exit skill and invoke slash_command("/grd:plan-phase [X+1]")
...
- `/grd:research-phase [X+1]` — investigate unknowns in literature
- Review roadmap
...
Exit skill and invoke slash_command("/grd:complete-milestone {version}")
...
- `/grd:verify-work` — systematic validation before completing milestone
