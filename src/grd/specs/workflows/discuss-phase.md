...
```bash
ROADMAP_INFO=$(grd roadmap get-phase "${PHASE}")
if [ "$(echo "$ROADMAP_INFO" | grd json get .found --default false)" != "true" ]; then
  echo "Phase ${PHASE} not found in ROADMAP.md."
  echo ""
  echo "Use /grd:progress to see available phases."
  exit 1
fi

phase_name=$(echo "$ROADMAP_INFO" | grd json get .phase_name --default "")
...
```
Created: .grd/phases/${PADDED_PHASE}-${SLUG}/${PADDED_PHASE}-CONTEXT.md
...
**Also available:**
- `/grd:plan-phase ${PHASE} --skip-research` -- plan without literature review
- `/grd:list-phase-assumptions ${PHASE}` -- see what the AI assumes before planning
...
- options:
  - "Plan now" -- proceed to `grd:plan-phase ${PHASE}`
  - "Review context first" -- show CONTEXT.md, then offer to plan
  - "Done for now" -- exit

If "Plan now": Tell the user to run `grd:plan-phase ${PHASE}` (the AI cannot chain commands directly, but this explicit suggestion enables one-click continuation in the web UI).
...
```bash
PRE_CHECK=$(grd pre-commit-check --files "${phase_dir}/${padded_phase}-CONTEXT.md" 2>&1) || true
echo "$PRE_CHECK"

grd commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
