...
Template for `.grd/PROJECT.md` — the living physics research project context document.
...
**Requirements:** Tracked in `.grd/REQUIREMENTS.md` (single source of truth). Do not duplicate requirements content in PROJECT.md.
...
- Populated during project initialization into `.grd/CONVENTIONS.md` and `.grd/NOTATION_GLOSSARY.md`
...
- Inline summary table for quick access; full log in `.grd/DECISIONS.md`
...
5. Decisions to log? — Add row to Key Decisions table and to `.grd/DECISIONS.md`
...
1. **Map existing work first** via `/grd:map-research`
