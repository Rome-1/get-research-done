...
| `current_plan` | `string \| integer \| null` | `grd state advance-plan` | Executor, orchestrators |
...
```json
{
  ...
  "resume_file": ".grd/phases/03/.continue-here"
}
```
...
|-------|-------|----------------------|
| **grd-executor** | `convention_lock`, `position`, `intermediate_results` | `state advance-plan`, `state update`, `result add`, `convention set` |
