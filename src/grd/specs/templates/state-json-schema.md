---
template_version: 1
purpose: Canonical schema for .grd/state.json — the machine-readable research state sidecar
---

# state.json Schema

Canonical schema for `.grd/state.json`. This file is the authoritative machine-readable state. STATE.md is a human-readable view generated from it.

Source of truth: `default_state_dict()` in `grd.core.state`.
...
| `core_research_question` | `string \| null` | `/grd:new-project` |
| `current_focus` | `string \| null` | Phase transitions, `grd state update` |
...
```json
{
  ...
  "context_intake": {
    ...
    "must_include_prior_outputs": [".grd/phases/01-setup/01-01-SUMMARY.md"],
...
|-------|------|-----------|---------|
| `current_phase` | `string \| null` | `grd phase complete`, `state update` | All agents (via init) |
| `current_phase_name` | `string \| null` | `grd phase complete`, `state update` | All agents (via init) |
| `total_phases` | `integer \| null` | `grd phase add/remove` | Progress display |
| `current_plan` | `string \| integer \| null` | `grd state advance` | Executor, orchestrators |
...
| `progress_percent` | `integer` | `grd state update-progress` | Progress display |
| `paused_at` | `string \| null` | `/grd:pause-work`, `/grd:resume-work` | Resume workflow |
...
**Written by:** `grd convention set <key> <value>`
**Read by:** grd-executor (load_conventions), grd-planner, grd-consistency-checker, grd-notation-coordinator, grd-paper-writer
...
| `verified` | `boolean` | No | Whether verified by grd-verifier |
| `verification_records` | `VerificationEvidence[]` | No | Structured provenance attached by `grd result verify` |

**Written by:** `grd result add`
**Read by:** grd-executor (downstream phases), grd-verifier, grd-paper-writer
...
**Written by:** `grd state add-decision`
...
**Written by:** `grd approximation add`
...
**Written by:** `grd uncertainty add`
...
**Written by:** `grd state record-session`, `/grd:pause-work`

`session` stores the last session timestamp, advisory machine identity, stop location, and handoff resume file. Keep `resume_file` project-relative when it points inside the repository; `grd state record-session` normalizes project-local absolute paths back to that form before persisting them. Omitting `--resume-file` preserves the current handoff pointer, while explicit placeholders such as `—`, `None`, or `null` clear it. `grd init resume` surfaces `session.resume_file` as `execution_resume_file`, may rank it as a non-resumable `session_resume_file` candidate, and compares `hostname`/`platform` with the current machine to emit a non-blocking `machine_change_notice`.

---

## Validation Rules

Run via `grd state validate`. Current checks:
...
| Agent | Reads | Writes (via grd CLI) |
|-------|-------|----------------------|
| **grd-executor** | `convention_lock`, `position`, `intermediate_results` | `state advance`, `state update`, `result add`, `convention set` |
| **grd-planner** | `convention_lock`, `position`, `decisions`, `blockers` | (reads only — orchestrator writes) |
| **grd-verifier** | `convention_lock`, `position` | (reads only) |
| **grd-debugger** | full state | `state add-blocker` |
| **grd-consistency-checker** | `convention_lock`, `intermediate_results` | (reads only) |
| **grd-notation-coordinator** | `convention_lock` | `convention set` |
| **grd-paper-writer** | `convention_lock`, `intermediate_results`, `decisions` | (reads only) |
