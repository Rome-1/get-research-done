...
Template for `~/.grd/learned-patterns/patterns-by-domain/{domain}/{category}-{slug}.md` -- a reusable error pattern captured from debugging or verification across GRD projects.
...
For how patterns are managed, read, and written, see `@{GRD_INSTALL_DIR}/references/shared/cross-project-patterns.md`.
...
Concrete, actionable checks that the verifier can perform. Reference specific verification patterns from `@get-research-done/domains/physics/verification/core/verification-core.md` (or the relevant domain file) where applicable.
