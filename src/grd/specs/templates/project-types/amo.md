...
See `domains/physics/verification/core/verification-core.md` for universal checks and `domains/physics/verification/domains/verification-domain-amo.md` for AMO-specific verification (selection rule compliance, TRK sum rule, dipole approximation validity, RWA bounds, Rabi normalization, AC Stark shift sign, decoherence rate positivity, optical theorem, Franck-Condon overlaps).
