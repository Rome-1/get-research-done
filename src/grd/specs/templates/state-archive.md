...
<!-- Used by: grd state compact command (state_compact in grd.core.state). -->

# State Archive Template

Template for `.grd/STATE-ARCHIVE.md` - historical state entries archived from STATE.md during compaction.

**Purpose:** When STATE.md exceeds the line budget (1500 lines), `grd state compact` moves older entries here. This preserves the full decision and metric history while keeping STATE.md focused on the current and recent phases.

**Created by:** `grd state compact` (automatic). Do not create manually.
...
- Compaction triggers when STATE.md exceeds 1500 lines (STATE_LINES_BUDGET in grd.core.constants)
