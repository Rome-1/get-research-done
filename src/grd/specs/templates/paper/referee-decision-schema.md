...
Canonical source of truth for `.grd/review/REFEREE-DECISION{round_suffix}.json`.

This JSON is the machine-readable adjudication summary consumed by `grd validate referee-decision`. It must stay semantically aligned with `.grd/REFEREE-REPORT{round_suffix}.md` and `.grd/review/REVIEW-LEDGER{round_suffix}.json`.
