...
Canonical source of truth for `.grd/review/REFEREE-DECISION.json` (or `.grd/review/REFEREE-DECISION{round_suffix}.json` in revision rounds).

This JSON is the machine-readable adjudication summary consumed by `grd validate referee-decision`. It must stay semantically aligned with `.grd/REFEREE-REPORT.md` and `.grd/review/REVIEW-LEDGER{round_suffix}.json`.
