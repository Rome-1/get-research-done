...
Template for `.grd/paper/REFEREE_RESPONSE{round_suffix}.md` — tracks referee comments, responses, and manuscript changes for peer review.
...
**Decision artifacts loaded:** [REFEREE-DECISION{round_suffix}.json, REVIEW-LEDGER{round_suffix}.json, or "none"]
...
[If `.grd/review/REVIEW-LEDGER{round_suffix}.json` or `.grd/review/REFEREE-DECISION{round_suffix}.json` exists, list every blocking issue here. Keep the `REF-*` IDs identical to the referee report.]

| Issue ID | Source Artifact | Blocking Reason | Resolution Plan | Status |
| -------- | --------------- | --------------- | --------------- | ------ |
| REF-001  | REVIEW-LEDGER{round_suffix}.json | [Unsupported central claim / unresolved math issue / etc.] | [Narrow claim, revise text, or add evidence] | [Open / In progress / Cleared] |
...
- One file per round of review (create `.grd/paper/REFEREE_RESPONSE-R2.md` for second round)
...
**Integration with GRD workflow:**

- New calculations requested by referees become new phases in ROADMAP.md
- Use /grd:add-phase or /grd:insert-phase to add referee-requested work
- Verification of new calculations follows standard GRD verification workflow
- Track new calculations in the "New Calculations Summary" table
- Use `.grd/review/REVIEW-LEDGER{round_suffix}.json` and `.grd/review/REFEREE-DECISION{round_suffix}.json` to identify recommendation floors and blocking items, but do not invent new `REF-*` IDs from those JSON files
