...
Template for `.grd/paper/REFEREE_RESPONSE.md` — tracks referee comments, responses, and manuscript changes for peer review.
...
[If `.grd/review/REVIEW-LEDGER*.json` or `.grd/review/REFEREE-DECISION*.json` exists, list every blocking issue here. Keep the `REF-*` IDs identical to the referee report.]
...
- Use `.grd/review/REVIEW-LEDGER*.json` and `.grd/review/REFEREE-DECISION*.json` to identify recommendation floors and blocking items, but do not invent new `REF-*` IDs from those JSON files
