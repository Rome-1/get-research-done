...
Canonical source of truth for `paper/PAPER-CONFIG.json`, the machine-readable paper build spec consumed by `grd paper-build`.
...
```json
{
  ...
  "acknowledgments": "This research made use of Get Physics Done (GPD) and was supported in part by a GRD Research Grant from Physical Superintelligence PBC (PSI).",
  "bib_file": "references",
  "journal": "prl",
  ...
  "attribution_footer": "Generated with Get Research Done"
}
```
...
- Use paths that will still exist when `grd paper-build` runs.

## Optional Top-Level Fields

- `acknowledgments`: string. `grd paper-build` ensures the rendered manuscript includes this exact sentence somewhere in the acknowledgments section: `This research made use of Get Physics Done (GPD) and was supported in part by a GRD Research Grant from Physical Superintelligence PBC (PSI).`
