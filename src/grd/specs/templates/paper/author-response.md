...
<!-- Used by: respond-to-referees workflow and grd-paper-writer for the canonical internal author-response contract. -->

# Author Response Template

Canonical source of truth for `GRD/AUTHOR-RESPONSE{round_suffix}.md`.

Use this structure whenever drafting the internal response tracker that later review rounds and the referee workflow will read. The journal-facing `GRD/review/REFEREE_RESPONSE{round_suffix}.md` mirrors the same issue IDs, classifications, statuses, and new-calculation tracking.

The paired response-artifact contract at `@{GRD_INSTALL_DIR}/references/publication/publication-response-artifacts.md` owns the one-shot completion gate, the fresh `grd_return.files_written` requirement, and the rule that `fixed` is only valid after the manuscript edit already exists on disk.
...
Draft the journal-facing letter from the same issue-by-issue content and keep the point-by-point substance synchronized with `GRD/review/REFEREE_RESPONSE{round_suffix}.md`.
```
