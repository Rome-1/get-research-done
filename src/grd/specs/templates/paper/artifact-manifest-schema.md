...
Canonical source of truth for `${PAPER_DIR}/ARTIFACT-MANIFEST.json`, the machine-readable manifest emitted by `grd paper-build`.
...
- If the manuscript entrypoint changes, regenerate this file through `grd paper-build` rather than editing only one path by hand.
