...
Template for spawning a fresh grd-executor agent to continue plan execution after a checkpoint pause. Uses a fresh agent with explicit state instead of resume to avoid serialization issues with parallel tool calls.
...
5. Orchestrator spawns fresh grd-executor with filled template
