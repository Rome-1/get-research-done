...
## Persistent Derivation State (appended to .grd/DERIVATION-STATE.md on pause)
...
workflow extracts it and appends it to `.grd/DERIVATION-STATE.md` before
...
- The `<persistent_state>` section is the exception: its content is appended to `.grd/DERIVATION-STATE.md` BEFORE this file is deleted, so equations/conventions/results accumulate permanently across all sessions
