...
Template for spawning grd-debugger agent. The agent contains all physics debugging expertise - this template provides problem context only.
...
```python
task(prompt=template, subagent_type="grd-debugger", description="Debug VAL-001")
# model parameter from profile tier — omit on single-model platforms
```
...
The grd-debugger agent applies a systematic approach to physics calculation errors:
...
<prior_state>
Read the file at .grd/debug/{slug}.md
</prior_state>
