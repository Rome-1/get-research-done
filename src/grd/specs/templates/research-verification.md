...
Template for `.grd/phases/XX-name/{phase}-VERIFICATION.md` -- persistent research verification session tracking.
...
Use `@{GRD_INSTALL_DIR}/templates/verification-report.md` for the canonical verification frontmatter contract. This template adds the researcher-session body scaffold (`Current Check`, conversational logs, and diagnosis flow) on top of that same verification ledger.
...
plan_contract_ref: .grd/phases/XX-name/{phase}-{plan}-PLAN.md#/contract
contract_results:
  claims:
    claim-main:
      ...
      proof_audit:
        ...
        audit_artifact_path: GRD/phases/01-benchmark/01-PROOF-REDTEAM.md
      ...
      evidence:
        - verifier: grd-verifier
          ...
          evidence_path: ".grd/phases/XX-name/{phase}-VERIFICATION.md"
  deliverables:
    deliverable-main:
      status: not_attempted
...
suggested_contract_checks:
  ...
  - check: "missing decisive reference comparison"
    ...
    evidence_path: "GRD/phases/01-benchmark/reference-comparison.csv"
source:
...
- Use `gap_subject_kind` for the body scaffold that feeds `/grd:plan-phase --gaps`; reserve bare `subject_kind` for canonical frontmatter ledgers such as `comparison_verdicts`
...
4. Diagnosis details are recorded in the session narrative or linked debug artifact, while the `grd:plan-phase --gaps` export stays on the canonical gap keys
...
**If you also keep a richer local diagnosis note, keep it explicitly separate from the `grd:plan-phase --gaps` export:**
...
- gap_subject_kind: "claim"
  ...
  debug_session: ".grd/debug/wrong-critical-temp.md"
```
...
**Creation:** When /grd:verify-work starts new verification session
...
- gap_subject_kind: "claim"
  ...
  - "Increase equilibration to 10^5 sweeps minimum, add autocorrelation analysis"
    debug_session: ".grd/debug/mc-equilibration.md"
