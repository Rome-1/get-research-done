...
`grd:digest-knowledge`, `grd:review-knowledge`, or direct frontmatter validation. Do not invent ad-hoc
...
- The canonical filename is `{knowledge_id}.md` under `GRD/knowledge/`.
...
review:
  ...
  review_round: 1
  reviewer_kind: human
  reviewer_id: grd-knowledge-reviewer
  decision: approved
  summary: "The document is reviewed and ready for downstream use."
  approval_artifact_path: "GRD/knowledge/reviews/K-renormalization-group-fixed-points-R1-REVIEW.md"
...
- `approval_artifact_path` must point at the canonical review artifact under `GRD/knowledge/reviews/`, and `approval_artifact_sha256` must match that artifact.
...
6. project-wide mandatory `GRD/knowledge/` creation
7. path-only discovery across `GRD/`
