...
Template for `.grd/STATE.md` — the research project's living memory.
...
See: .grd/PROJECT.md (updated [date])

**Machine-readable scoping contract:** `.grd/state.json` field `project_contract`
...
[From .grd/todos/pending/ — ideas captured during sessions]
...
**Decisions:** Full log: `.grd/DECISIONS.md` (single source of truth). Recent high-impact decisions shown in Accumulated Context for quick access.

**Active Approximations:** Tracks all approximations currently in use, their validity ranges, and the controlling parameters. Convention lock is an 18-field snapshot of active conventions — full convention catalog lives in `.grd/CONVENTIONS.md`.

**Pending Todos:** Ideas captured via /grd:add-todo

- Count of pending todos
- Reference to .grd/todos/pending/
