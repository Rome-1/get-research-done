...
If NO_GIT: Run `git init` silently. GRD research projects always get their own repo.
</git_check>
...
```bash
grd commit "docs: initialize [project-name] ([N] phases)" --files .grd/
```
...
```bash
grd commit "docs({phase}-{plan}): complete [plan-name] plan" --files ${phase_dir}/{phase}-{plan}-PLAN.md ${phase_dir}/{phase}-{plan}-SUMMARY.md .grd/STATE.md .grd/ROADMAP.md
```
...
```bash
grd commit "wip: [phase-name] paused at task [X]/[Y]" --files .grd/
```
