<overview>
GitHub lifecycle management for GRD research projects -- push timing, branch strategy, PR workflows, deliverable handling, repository hygiene, and tagging conventions.
</overview>
...
| SUMMARY.md, STATE.md, ROADMAP.md | YES | GRD metadata |
...
| GRD checkpoint (internal) | `grd-checkpoint/{description}` | `grd-checkpoint/phase-03-plan-02-1709312400` |
...
git tag grd-checkpoint/phase-03-plan-02-$(date +%s)
```
...
5. **GRD checkpoint tags are ephemeral.** Clean up `grd-checkpoint/*` tags after successful plan completion (see `execute-plan-checkpoints.md`).
