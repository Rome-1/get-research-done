...
**Subagent advantage:** Each subagent (grd-executor, grd-planner, etc.) gets a fresh context window. The orchestrator's context stays lean. This is why `../../workflows/execute-plan.md` routes non-interactive work to subagents.
</context_consumption>
...
- Save state NOW via `/grd:pause-work`
...
3. If mid-plan: create `.continue-here.md` via `/grd:pause-work`
4. Ensure intermediate results are saved to files (not just in conversation memory)

**After /clear:**

1. `/grd:resume-work` restores full context from STATE.md and .continue-here.md
...
When creating plans (via /grd:plan-phase), consider context budget:
...
4. Recommend `/clear` + `/grd:resume-work`
...
Different GRD agents have different context profiles:

| Agent                | Typical Usage | Why                                           |
| -------------------- | ------------- | --------------------------------------------- |
| grd-executor         | 40-70%        | Executes tasks, reads files, runs derivations |
| grd-planner          | 30-50%        | Reads research + state, produces plans        |
| grd-phase-researcher | 40-60%        | Web searches, literature, synthesis           |
| grd-plan-checker     | 20-30%        | Reads plans, checks against goals             |
| grd-verifier         | 30-50%        | Reads results, runs validation checks         |
| grd-debugger         | 50-80%        | Iterative investigation, heavy reads          |
| grd-paper-writer     | 50-70%        | Large reference reads + iterative drafting    |
