...
This document defines the standardized pattern for spawning GRD agents across runtimes.
...
AGENT_MODEL=$(grd resolve-model grd-{agent})

# Spawn agent — readonly=false is REQUIRED for file-producing agents
task(
  subagent_type="grd-{agent}",
  model="{AGENT_MODEL}",    # Omit if AGENT_MODEL is empty
  readonly=false,
  prompt="First, read {GRD_AGENTS_DIR}/grd-{agent}.md for your role and instructions.\n\n{task_prompt}",
...
|--------|-------------------|
| **Subagent spawning** | `task(subagent_type="grd-{agent}", model="{model}", readonly=false, prompt="...")` or equivalent; omit `model` when it resolves empty |
| **Projected command surface** | Invoke the runtime's installed GRD command or agent action surface. For example, some runtimes expose `/grd:{agent}` slash commands. |
...
1. **Always resolve model first:** `grd resolve-model grd-{agent}`
2. **If model is null or empty:** Omit the `model` parameter from task(). The runtime will use its default model.
3. **Agent instructions path:** `{GRD_AGENTS_DIR}/grd-{agent}.md` (resolved by installer per runtime)
4. **grd CLI surface:** author plain `grd ...` in source prompts. The installer rewrites shell calls to the runtime-managed GRD CLI bridge during install; source prompts must stay runtime-agnostic.
5. **Never hardcode runtime-specific paths** — use `{GRD_INSTALL_DIR}` for specs assets and `{GRD_AGENTS_DIR}` for agent prompts, and let the installer project shell `grd` calls onto the correct runtime bridge.
...
2. **Model semantics:** The `model` parameter is omitted when `grd resolve-model` returns empty.
...
For GRD-owned runtime surfaces, use the effective installed runtime rather than a merely active but uninstalled higher-priority runtime. This applies to `grd resolve-model`, runtime-native command rendering, and other installer-backed workflow surfaces.
