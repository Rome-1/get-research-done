...
4. **`/clear` explanation** -- always include, and pair it with the next command instead of leaving it as a dead-end. If project rediscovery is still required, say so explicitly and point to `grd resume` or `grd resume --recent` before reopening the runtime instead of implying that `/clear` performs recovery.
...
<sub>`/clear` first, then run `grd:execute-phase 2`</sub>

---

**Also available:**
- Review plan before executing
- `/grd:list-phase-assumptions 2` -- check physical assumptions
...
<sub>`/clear` first, then run `grd:plan-phase 2`</sub>
...
<sub>`/clear` first, then run `grd:plan-phase 3`</sub>
