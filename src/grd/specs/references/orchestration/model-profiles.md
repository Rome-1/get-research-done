...
`grd resolve-tier` exposes the abstract tier assignment for a given agent.
`grd resolve-model` resolves that tier to a runtime-specific override only when `.grd/config.json` defines `model_overrides.<runtime>.<tier>`.
If no override is configured for the active runtime, `grd resolve-model` returns empty output and the task call should omit `model` so the platform uses its default model.

## Profile Definitions

| Agent                    | `deep-theory` | `numerical` | `exploratory` | `review` | `paper-writing` |
| ------------------------ | ------------- | ----------- | ------------- | -------- | --------------- |
| grd-planner              | tier-1        | tier-1      | tier-1        | tier-1   | tier-1          |
| grd-roadmapper           | tier-1        | tier-1      | tier-2        | tier-1   | tier-2          |
| grd-executor             | tier-1        | tier-2      | tier-2        | tier-2   | tier-1          |
| grd-phase-researcher     | tier-1        | tier-1      | tier-1        | tier-2   | tier-2          |
| grd-project-researcher   | tier-1        | tier-2      | tier-1        | tier-2   | tier-3          |
| grd-research-synthesizer | tier-1        | tier-2      | tier-2        | tier-2   | tier-1          |
| grd-debugger             | tier-1        | tier-1      | tier-2        | tier-1   | tier-2          |
| grd-research-mapper      | tier-2        | tier-3      | tier-3        | tier-3   | tier-3          |
| grd-verifier             | tier-1        | tier-1      | tier-2        | tier-1   | tier-2          |
| grd-plan-checker         | tier-2        | tier-2      | tier-2        | tier-1   | tier-2          |
| grd-consistency-checker  | tier-1        | tier-2      | tier-2        | tier-1   | tier-2          |
| grd-paper-writer         | tier-1        | tier-2      | tier-2        | tier-2   | tier-1          |
| grd-literature-reviewer  | tier-1        | tier-2      | tier-1        | tier-2   | tier-2          |
| grd-bibliographer        | tier-2        | tier-3      | tier-3        | tier-2   | tier-1          |
| grd-explainer            | tier-1        | tier-2      | tier-1        | tier-1   | tier-1          |
...
| grd-experiment-designer  | tier-2        | tier-1      | tier-2        | tier-2   | tier-3          |
| grd-notation-coordinator | tier-2        | tier-3      | tier-3        | tier-2   | tier-2          |
...
```
1. Read .grd/config.json
2. Get model_profile (default: "review")
3. Look up agent in table above
4. Resolve tier via `grd resolve-tier`
5. Resolve runtime override via `grd resolve-model`
6. Omit `model` when `grd resolve-model` returns empty
```
...
Per-project default: Set in `.grd/config.json`:
...
**Why tier-1 for grd-planner across all profiles?**
...
**Why tier-1 for grd-executor in deep-theory?**
...
**Why tier-1 for grd-verifier in numerical and review?**
...
**Why tier-1 for grd-research-synthesizer in paper-writing?**
...
**Why tier-3 for grd-research-mapper?**
...
**Why tier-1 for grd-paper-writer in deep-theory and paper-writing?**
...
**Why tier-1 for grd-literature-reviewer in exploratory?**
