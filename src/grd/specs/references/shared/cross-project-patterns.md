...
How the global pattern library works -- a persistent knowledge base of physics error patterns, convention pitfalls, and recurring issues that accumulates across all GRD projects.
...
The library lives outside any single project at the resolved global pattern-library root: `GRD_PATTERNS_ROOT` -> `GRD_DATA_DIR/learned-patterns` -> `~/.grd/learned-patterns`.
</core_principle>
...
When the debugger (grd-debugger) resolves a debug session and identifies a root cause:

1. The `Resolution.lessons_learned` field in the debug file captures the project-specific lesson
2. The debugger writes the same lesson to the project's `.grd/INSIGHTS.md`
...
When the verifier (grd-verifier) finds that a particular class of error recurs across multiple phases or plans within a project:
...
1. Read `index.json`
2. Filter by domain tags matching the current project's domain (from `.grd/PROJECT.md`)
...
> **Status:** Agent definitions reference the global pattern library. grd-planner reads top 5 patterns by severity during `consult_learned_patterns`. grd-verifier checks domain-matching patterns during verification setup. grd-executor reads critical/high patterns before starting work. grd-debugger reads existing patterns before investigating and writes/updates patterns after confirming root causes. Agents also use project-local `.grd/INSIGHTS.md` and `.grd/ERROR-PATTERNS.md` for within-project pattern learning.
...
| ------------ | ------------------------------- | ------------------------------ | ------------------------------------------------------------ |
| grd-planner  | `consult_learned_patterns` step | Top 5 by relevance             | Adds prevention steps to plans, contract links, and acceptance-test guards |
| grd-executor | Before starting task execution  | All critical/high for domain   | Watches for trigger conditions during derivation             |
| grd-verifier | During verification setup       | All patterns matching domain   | Adds pattern-specific checks to verification report          |
| grd-debugger | After confirming root cause     | Existing patterns for matching | Writes new pattern or updates existing one                   |
...
The global pattern library and project-local `.grd/INSIGHTS.md` serve complementary roles:

| Aspect     | INSIGHTS.md                           | Global Pattern Library                            |
| ---------- | ------------------------------------- | ------------------------------------------------- |
| Scope      | Single project                        | All projects                                      |
| Location   | `.grd/INSIGHTS.md`               | `~/.grd/learned-patterns/` |
...
When the pattern library does not yet exist (first GRD session on a machine):

1. Agents that attempt to read `index.json` and find it missing should create the directory structure:
   ```
   mkdir -p ~/.grd/learned-patterns/patterns-by-domain/{qft,condensed-matter,stat-mech,gr,amo,nuclear,classical,fluid,plasma,astro,mathematical,soft-matter,quantum-info}
   ```
