...
Prefer the canonical hyphenated metric values reported by `grd --raw convention list` (`mostly-plus`, `mostly-minus`); underscore aliases are normalized but are not the canonical lock spelling.
