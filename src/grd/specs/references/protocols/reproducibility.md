...
When you need a machine-readable manifest for submission or review preflight, use `@{GRD_INSTALL_DIR}/templates/paper/reproducibility-manifest.md` as the canonical schema/example. In that manifest, stochastic `random_seeds[].computation` values must exactly match `execution_steps[].name`, and `environment.lock_file` should name the real lock file used to pin dependencies (for example `uv.lock`).
...
```json
{
  ...
  "mcp_servers": {
    "grd-verification": "1.1.0",
    "grd-conventions": "1.1.0",
    "grd-arxiv": "0.3.2"
  },
  "platform": {
    "os": "Linux 6.1.0",
...
If you are recording GRD-managed MCP services, use the public server keys from the runtime descriptors (`grd-conventions`, `grd-errors`, `grd-patterns`, `grd-protocols`, `grd-skills`, `grd-state`, `grd-verification`, `grd-arxiv`).
