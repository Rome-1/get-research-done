...
Configuration options for `.grd/` directory behavior in physics research projects.
...
| `git.phase_branch_template`     | `"grd/phase-{phase}-{slug}"` | Branch template for the `per-phase` strategy                                                   |
| `git.milestone_branch_template` | `"grd/{milestone}-{slug}"`   | Branch template for the `per-milestone` strategy                                               |
...
- `.grd/CONVENTIONS.md` — human-readable convention reference
- `.grd/state.json` (`convention_lock`) — machine-readable convention state

Manage those values with:

- `grd convention set <key> <value>`
- `/grd:validate-conventions`
...
3. **Existing tracked files:** If `.grd/` was previously tracked:
   ```bash
   git rm -r --cached .grd/
...
- Branch name from `phase_branch_template` (e.g., `grd/phase-03-hamiltonian-construction`)
...
- Branch name from `milestone_branch_template` (e.g., `grd/v1.0-ground-state-calculation`)
