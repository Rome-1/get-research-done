...
GRD adapts its research strategy along an explore↔exploit spectrum. The research mode controls how broadly the system searches for approaches vs how deeply it executes a known methodology.
...
1. Log the transition: `grd state add-decision --phase N --summary "Adaptive mode narrowed toward exploit behavior" --rationale "Approach lock established in phase N: [approach description]"`
...
The user can override at any time: `/grd:settings` or `grd config set research_mode explore`
...
`research_mode` is the only persisted adaptive-mode knob in `.grd/config.json`. There is no separate `adaptive_transition` block; transition readiness is inferred from project state, anchors, and verification outcomes.

## Command Interface

```bash
# Set research mode
grd config set research_mode explore
grd config set research_mode balanced
grd config set research_mode exploit
grd config set research_mode adaptive

# Check current mode
grd --raw config get research_mode
