...
1. **Unitarity bounds.** In d dimensions: scalar Δ ≥ (d-2)/2 (free field saturation). Spin-ℓ current: Δ ≥ d + ℓ - 2 (for ℓ ≥ 1). Stress tensor: Δ = d exactly. Any violation indicates an error (`domains/physics/verification/errors/llm-physics-errors.md` #40, scaling dimension errors).
