...
1. **Verify the periodicity condition.** Bosonic fields are periodic, fermionic fields are antiperiodic. Getting this wrong changes the Matsubara frequencies (`domains/physics/verification/errors/llm-physics-errors.md` #9). The KMS condition ⟨A(τ)B(0)⟩ = ⟨B(0)A(τ + β)⟩ (bosonic) or = -⟨B(0)A(τ + β)⟩ (fermionic) is the fundamental statement.
...
|---|---|---|
| Wrong Matsubara frequency (bosonic 2nπT vs fermionic (2n+1)πT) | Check periodicity: bosons periodic, fermions antiperiodic | `domains/physics/verification/errors/llm-physics-errors.md` #9 |
| Wrong analytic continuation iωₙ → ω + iε instead of ω + iη | Check causality: G^R analytic in upper half-plane | `domains/physics/verification/errors/llm-physics-errors.md` #21 |
...
| Using T=0 Feynman rules at finite T | Verify propagators include thermal occupation factors | `domains/physics/verification/errors/llm-physics-errors.md` #20 |
