...
<purpose>
This reference is loaded by GRD agents when research involves a specific physics subfield. It provides:

1. **For the planner (grd-planner):** Which methods and tools are standard for the subfield, what validation checks to include in plans
2. **For the executor (grd-executor):** Correct conventions, standard software, typical numerical parameters
3. **For the verifier (grd-verifier):** Subfield-specific validation strategies, known exact results, standard benchmarks
4. **For the researcher (grd-phase-researcher):** What tools and libraries to investigate, what communities and databases to consult
</purpose>
...
GRD is designed for physics research broadly, with particular strength in problems that involve symbolic manipulation, numerical computation, or both. Load the relevant subfield reference for your project's domain.
