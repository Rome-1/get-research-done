...
Set via: `grd config set autonomy balanced` and `grd config set research_mode balanced`.

Read via: `grd init` includes both fields in the init JSON output.
...
4. Update config via `grd config set research_mode exploit`
