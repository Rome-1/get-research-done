...
Quantitative readiness score (0-100) for physics manuscripts. Used by `/grd:write-paper` to determine if the paper is ready for submission packaging. Each category is scored independently; the total is a weighted sum.
...
| Check | Points | How to verify |
|-------|--------|---------------|
| Decisive figures/tables labeled with units | 3 | Read `.grd/paper/FIGURE_TRACKER.md` `figure_registry` and verify `has_units: true` for decisive artifacts |
...
| Check | Points | How to verify |
|-------|--------|---------------|
| Convention lock complete (all relevant fields set) | 5 | `grd convention check` returns complete: true |
| ASSERT_CONVENTION in all derivation files | 5 | Search derivation files for `ASSERT_CONVENTION:` and compare against the convention lock; `grd pre-commit-check` does not currently validate these assertions |
...
| Decisive outputs have explicit comparison verdicts and anchors | 3 | `comparison_verdicts` exist for decisive results and cite the right anchors; decisive figures should link back to `.grd/comparisons/*-COMPARISON.md` when relevant |
...
When invoked during `/grd:write-paper` (step: quality_assessment), prefer the artifact-driven path:

```bash
grd --raw validate paper-quality --from-project .
```
...
If you need to provide a manual JSON instead, use `@{GRD_INSTALL_DIR}/templates/paper/paper-quality-input-schema.md` as the schema source of truth.
...
- **`/grd:write-paper`**: Runs quality scoring after all sections drafted, before generating submission package
- **`/grd:arxiv-submission`**: Requires score ≥ 80 to proceed (override with `--force`)
- **`/grd:respond-to-referees`**: Re-scores after revision to track improvement
...
- The scorer still emits a per-journal `minimum_submission_score`, and `grd:arxiv-submission` uses that surfaced minimum instead of a universal 80-point threshold.
