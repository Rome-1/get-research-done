...
1. **After draft completion.** The `/grd:write-paper` workflow produces a manuscript with all sections, equations, figures, and bibliography in place. Peer review does not run on incomplete drafts or outlines.
2. **Before final PDF.** Peer review must complete and its findings must be addressed before the manuscript is packaged for submission (e.g., via `/grd:arxiv-submission`).
3. **Explicit invocation.** Peer review runs when the user invokes `/grd:peer-review` or when the write-paper workflow reaches its internal review gate. It is not triggered automatically by file saves or partial edits.
...
| Artifacts | `.grd/review/` JSON stage reports, `.grd/REFEREE-REPORT{round_suffix}.md` / `.tex` | Journal referee reports |
...
3. **Verification coverage.** At least one verification report exists under `.grd/phases/`.
4. **Preflight pass.** `grd validate review-preflight peer-review --strict` exits zero.
...
2. **Referee decision is valid.** `.grd/review/REFEREE-DECISION{round_suffix}.json` passes `grd validate referee-decision ... --strict --ledger ...`, including non-empty `manuscript_path` alignment with the review ledger and stage artifacts.
3. **Review ledger is valid.** `.grd/review/REVIEW-LEDGER{round_suffix}.json` passes `grd validate review-ledger ...`, including a non-empty `manuscript_path`.
...
When strict submission preflight sees `.grd/review/REVIEW-LEDGER*.json` and `.grd/review/REFEREE-DECISION*.json`, it treats the latest round-specific pair as authoritative and blocks packaging unless that condition is satisfied for the active manuscript.
...
- The built-in validators accept the matching artifact type:
  ...
  - `grd validate review-stage-report .grd/review/STAGE-<stage_id>{round_suffix}.json`
...
1. **Review summary** (`.grd/REFEREE-REPORT{round_suffix}.md` / `.tex`): Human-readable narrative of the panel findings, recommendation, and rationale.
2. **Review ledger** (`.grd/review/REVIEW-LEDGER{round_suffix}.json`): Machine-readable list of all issues with severity, blocking status, affected claims, and required actions.
3. **Referee decision** (`.grd/review/REFEREE-DECISION{round_suffix}.json`): Final recommendation, confidence, blocking issue IDs, and stage artifact references.
...
3. **Write author response.** Document how each finding was addressed in `.grd/AUTHOR-RESPONSE{round_suffix}.md` (or `-R2.md` / `-R3.md` for subsequent rounds).
4. **Re-enter review.** After revisions, re-run `/grd:peer-review` for the next round. The pipeline detects prior reports and author responses to increment the round number automatically.
