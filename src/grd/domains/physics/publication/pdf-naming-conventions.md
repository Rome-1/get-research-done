...
- `grd.mcp.paper.models.derive_output_filename` -- Pure function implementing the rules above.
- `grd.mcp.paper.compiler.build_paper` -- Calls `derive_output_filename` to name the primary emitted `.tex` file; the compiler then produces a `.pdf` with a matching stem.
