...
GRD verification is outcome-focused. A derivation that deviates from the plan but produces correct physics passes. A derivation that follows the plan exactly but has a sign error fails.
...
| STATE.md                | .grd/STATE.md                         | Project conventions, active approximations, unit system            |
| config.json             | .grd/config.json                      | Project configuration                                              |
| INSIGHTS.md             | .grd/INSIGHTS.md (if exists)          | Known problem patterns for extra scrutiny                          |
| ERROR-PATTERNS.md       | .grd/ERROR-PATTERNS.md (if exists)    | Previous error patterns to check against                           |
...
There are two fundamentally different approaches to verification. GRD requires the first and forbids the second.
...
GRD's verification separation follows the same principle, combined with the requirement for computational verification:

| Good peer review                   | GRD computational verification            |
| ---------------------------------- | ----------------------------------------- |
...
| Bad peer review                        | GRD pattern matching (FORBIDDEN)      |
| -------------------------------------- | ------------------------------------- |
