...
Current runtime-facing overview of how GRD defends against physics errors today.
...
- `grd.core.verification_checks` — live machine-readable verification registry
- `grd-verification` MCP server — machine-facing verification metadata surface
...
- `grd-executor.md` `<post_step_physics_guards>` — early-intercept guardrails and the 52-type computation mini-checklist
...
- The contract-aware checks are what stop GRD from treating proxies, wrong fit families, or benchmark drift as success.
