...
_DEFAULT_REVIEW_CONTRACTS: dict[str, dict[str, object]] = {
    "grd:write-paper": {
        ...
        "required_outputs": ["paper/main.tex", ".grd/REFEREE-REPORT.md", ".grd/REFEREE-REPORT.tex"],
        "required_evidence": [
            "existing manuscript",
    ...
    "grd:respond-to-referees": {
        "review_mode": "publication",
        "required_outputs": [".grd/paper/REFEREE_RESPONSE.md", ".grd/AUTHOR-RESPONSE.md"],
        "required_evidence": [
            "existing manuscript",
    ...
    "grd:verify-work": {
        "review_mode": "review",
        "required_outputs": [".grd/phases/XX-name/{phase}-VERIFICATION.md"],
...
def get_command(name: str) -> CommandDef:
    ...
    if normalized.startswith("grd:"):
        candidates.append(normalized.replace("grd:", "", 1))
...
def get_skill(name: str) -> SkillDef:
    ...
    for candidate in (
        normalized,
        normalized.replace("grd:", "grd-", 1) if normalized.startswith("grd:") else None,
        normalized.replace("grd-", "grd:", 1) if normalized.startswith("grd-") else None,
        f"grd-{normalized}" if normalized and not normalized.startswith(("grd-", "grd:")) else None,
    ):
