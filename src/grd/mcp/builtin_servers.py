"""Built-in MCP server definitions for GRD install, session launch, and release descriptors.

Provides the canonical registry of GRD's built-in MCP servers. Used by:
- Adapter install flow (_configure_runtime)
- Session launch (build_mcp_config_file)
- Public infra descriptor generation (infra/grd-*.json)
"""
...
_BUILTIN_SERVERS: dict[str, _ServerDef] = {
    "grd-conventions": {
        "command": "python",
        "args": ["-m", "grd.mcp.servers.conventions_server"],
    ...
    "grd-patterns": {
        "command": "python",
        "args": ["-m", "grd.mcp.servers.patterns_server"],
        "env": {"LOG_LEVEL": "${LOG_LEVEL:-WARNING}"},
    },
    "grd-protocols": {
        "command": "python",
        "args": ["-m", "grd.mcp.servers.protocols_server"],
        "env": {"LOG_LEVEL": "${LOG_LEVEL:-WARNING}"},
    },
    "grd-skills": {
        "command": "python",
        "args": ["-m", "grd.mcp.servers.skills_server"],
        "env": {"LOG_LEVEL": "${LOG_LEVEL:-WARNING}"},
    },
    "grd-state": {
        "command": "python",
    ...
    "grd-verification": {
        "command": "python",
        "args": ["-m", "grd.mcp.servers.verification_server"],
        "env": {"LOG_LEVEL": "${LOG_LEVEL:-WARNING}"},
    },
    "grd-arxiv": {
        "command": "python",
...
_PUBLIC_DESCRIPTOR_METADATA: dict[str, dict[str, object]] = {
    ...
    "grd-skills": {
        "description": (
            "GRD skill discovery and routing. Tools for listing, retrieving, auto-routing, "
            "and indexing GRD workflow skills for agent prompt injection."
        ),
        "capabilities": [
            "list_skills",
            "get_skill",
        ...
        "health_check": {
            "tool": "list_skills",
            "input": {},
            "expect": "contains grd-execute-phase",
        },
    },
    "grd-state": {
        "description": (
            "GRD project state management. Tools for querying state, phase info, progress, "
...
# Keys that are GRD-managed and should be removed on uninstall.
GRD_MCP_SERVER_KEYS = frozenset(_BUILTIN_SERVERS.keys())
...
def merge_managed_mcp_servers(
    ...
    *,
    ...
    """Merge managed GRD MCP entries into a runtime config mapping."""
