"""MCP server for GRD physics verification.
...
Usage:
    python -m grd.mcp.servers.verification_server
    # or via entry point:
    grd-mcp-verification
"""
...
from grd.core.protocol_bundles import ResolvedProtocolBundle, get_protocol_bundle, render_protocol_bundle_context
from grd.core.verification_checks import (
...
logger = logging.getLogger("grd-verification")

mcp = FastMCP("grd-verification")
...
_CONTRACT_APPROACH_POLICY_INPUT_SCHEMA: dict[str, object] = _object_schema(
    {
        ...
        "allowed_estimator_families": _string_or_string_list_schema(),
        "forbidden_estimator_families": _string_or_string_list_schema(),
        "allowed_fit_families": _string_or_string_list_schema(),
        "forbidden_fit_families": _string_or_string_list_schema(),
        "stop_and_rethink_conditions": _string_or_string_list_schema(),
    },
    additional_properties=False,
)
...
_CONTRACT_REFERENCE_INPUT_SCHEMA: dict[str, object] = _object_schema(
    {
        ...
        "required_actions": _enum_string_or_string_list_schema(_CONTRACT_REFERENCE_ACTION_VALUES),
    },
...
def _contract_check_request_hint(check_key: str, *, contract: ResearchContract | None = None) -> dict[str, object]:
    ...
    if check_key == "contract.benchmark_reproduction":
        ...
        if len(regime_candidates) == 1 and len(limit_tests) == 1:
            ...
            if limit_test is not None and limit_test.pass_condition:
                enriched_hint["required_request_fields"] = [
                    field for field in enriched_hint["required_request_fields"] if field != "metadata.expected_behavior"
                ]
...
def _validate_binding_payload(binding: dict[str, object], *, allowed_targets: Iterable[str]) -> str | None:
    ...
    allowed_keys = {key for target in allowed_targets for key in (f"{target}_id", f"{target}_ids")}
...
def run_check(check_id: str, domain: str, artifact_content: str) -> dict:
    ...
    """
    with grd_span("mcp.verification.run_check", check_type=check_id, domain=domain):
        try:
            ...
            if check_meta.check_id == "5.1":
                ...
                # Formal statement: look for Lean-specific markers that don't
                # collide with English prose ("theorem" by itself is ambiguous;
                # a fenced ```lean block or ``.lean`` file reference is not).
                lean_markers = [".lean", "```lean", ":= by", ":= sorry", "#check", ":=\n"]
                if not any(marker in artifact_content for marker in lean_markers):
                    issues.append(
                        "No Lean statement or .lean reference found; run `grd lean check` against the formal statement."
                    )

            elif check_meta.check_id == "5.21":
                # Formal proof: require both the presence of formal content AND
                # the absence of placeholders that admit proof gaps.
                lean_markers = [".lean", "```lean", ":= by", "#check"]
                gap_markers = ["sorry", "admit", "axiom "]
                has_formal = any(marker in artifact_content for marker in lean_markers)
                has_gap = any(marker in artifact_content for marker in gap_markers)
                if not has_formal:
                    issues.append(
                        "No Lean proof content or .lean reference found; a complete machine-checked proof is required."
                    )
                if has_gap:
                    issues.append(
                        "Proof content contains `sorry`, `admit`, or unattested `axiom` — not a complete proof."
                    )
...
_SUPPORTED_BINDING_KEYS: dict[str, str] = {
    key: target for target in _BINDING_TARGETS for key in (f"{target}_id", f"{target}_ids")
}
...
def run_contract_check(request: RunContractCheckPayload) -> dict:
    ...
    with grd_span("mcp.verification.run_contract_check"):
        try:
            ...
            if has_check_key and has_check_id:
                if check_key_meta is None or check_id_meta is None or check_key_meta.check_id != check_id_meta.check_id:
                    return _error_result(
                        "check_key and check_id must identify the same contract check when both are provided"
                    )
...
def suggest_contract_checks(contract: SuggestContractPayload, active_checks: StringListPayload = None) -> dict:
    ...
    ``contract`` must be an object with the normal GRD contract structure.
    ...
    with grd_span("mcp.verification.suggest_contract_checks"):
        try:
...
def get_checklist(domain: str) -> dict:
    ...
    including which live verifier-registry checks (currently 5.1-5.21) each maps to.
    """
    with grd_span("mcp.verification.checklist", domain=domain):
        try:
...
def get_bundle_checklist(bundle_ids: list[str]) -> dict:
    ...
    with grd_span("mcp.verification.bundle_checklist", bundle_count=len(bundle_ids)):
        try:
...
def dimensional_check(expressions: list[str]) -> dict:
    ...
    """
    with grd_span("mcp.verification.dimensional_check"):
...
def limiting_case_check(expression: str, limits: dict[str, str]) -> dict:
    ...
    """
    with grd_span("mcp.verification.limiting_case"):
...
def symmetry_check(expression: str, symmetries: list[str]) -> dict:
    ...
    """
    with grd_span("mcp.verification.symmetry_check"):
        validated_expression, error = _validate_string(expression, field_name="expression")
        if error is not None:
            return error
...
def main() -> None:
    """Run the grd-verification MCP server."""
    from grd.mcp.servers import run_mcp_server

    run_mcp_server(mcp, "GRD Verification MCP Server")
