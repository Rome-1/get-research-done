"""MCP server for GRD cross-project pattern library.

Thin MCP wrapper around grd.core.patterns. Exposes pattern CRUD
and search as MCP tools for solver agents.

Usage:
    python -m grd.mcp.servers.patterns_server
    # or via entry point:
    grd-mcp-patterns
"""
...
logger = logging.getLogger("grd-patterns")

mcp = FastMCP("grd-patterns")

# Default patterns library root — used when GRD_PATTERNS_ROOT / GRD_DATA_DIR
# env vars are not set. Falls back to the global ~/.grd data directory.
...
def lookup_pattern(
    ...
    """Search the GRD pattern library for physics error patterns.
    ...
    """
    with grd_span("mcp.patterns.lookup", domain=domain or "", category=category or ""):
        try:
            if keywords:
...
def add_pattern(
    ...
    """
    with grd_span("mcp.patterns.add", domain=domain, category=category):
        try:
            result = pattern_add(
                domain=domain,
                title=title,
...
def promote_pattern(pattern_id: str) -> dict:
    ...
    """
    with grd_span("mcp.patterns.promote", pattern_id=pattern_id):
        try:
...
def seed_patterns() -> dict:
    ...
    """
    with grd_span("mcp.patterns.seed"):
        try:
...
def list_domains() -> dict:
    ...
    """
    with grd_span("mcp.patterns.list_domains"):
        try:
            return stable_mcp_response(
                {
...
def main() -> None:
    """Run the grd-patterns MCP server."""
