...
def get_error_class(error_id: int) -> dict[str, object]:
    ...
    """
    with grd_span("mcp.errors.get", error_class_id=error_id):
        try:
            store = _get_store()
...
def check_error_classes(computation_desc: str) -> dict[str, object]:
    ...
    """
    with grd_span("mcp.errors.check"):
        try:
            store = _get_store()
...
def get_detection_strategy(error_id: int) -> dict[str, object]:
    ...
    Args:
        error_id: Numeric error class ID (1-104).
    """
    with grd_span("mcp.errors.detection_strategy", error_class_id=error_id):
        try:
            store = _get_store()
            error = store.get(error_id)
            if error is None:
...
def get_traceability(error_id: int) -> dict[str, object]:
    ...
    Args:
        error_id: Numeric error class ID (1-104).
    """
    with grd_span("mcp.errors.traceability", error_class_id=error_id):
        try:
            store = _get_store()
            error = store.get(error_id)
            if error is None:
...
def list_error_classes(domain: str | None = None) -> dict[str, object]:
    ...
    """
    with grd_span("mcp.errors.list", domain=domain or "all"):
        try:
            store = _get_store()
...
def main() -> None:
    """Run the grd-errors MCP server."""
    run_mcp_server(mcp, "GRD Errors MCP Server")
