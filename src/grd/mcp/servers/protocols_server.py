...
class ProtocolStore:
    ...
    def _load_all(self, protocols_dir: Path) -> None:
        with grd_span("protocols.load_all", protocols_dir=str(protocols_dir)):
...
def main() -> None:
    """Run the grd-protocols MCP server."""
    run_mcp_server(mcp, "GRD Protocols MCP Server")
