"""GRD-owned bridge for the optional arxiv_mcp_server integration."""
...
from grd.core.arxiv_source_download import (
    ARXIV_DEFAULT_STORAGE_PATH,
    download_arxiv_source_archive,
)
from grd.version import __version__ as GRD_VERSION
...
def build_server(config: ArxivBridgeConfig) -> tuple[Server, ArxivBridge]:
    ...
    server = Server("grd-arxiv", version=GRD_VERSION, lifespan=lifespan)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="GRD arXiv MCP bridge")
    ...
    async with stdio_server() as (read_stream, write_stream):
        await server.run(
            ...
            InitializationOptions(
                server_name="grd-arxiv",
                server_version=GRD_VERSION,
...
def main() -> None:
    """Console entry point for the GRD arXiv MCP bridge."""
