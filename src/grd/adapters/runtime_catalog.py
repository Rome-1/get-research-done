...
def _runtime_managed_install_surface_policy(descriptor: RuntimeDescriptor) -> ManagedInstallSurfacePolicy:
    ...
    return ManagedInstallSurfacePolicy(
        grd_content_globs=(f"{install_root}/**/*",),
        nested_command_globs=() if flat_command_runtime or external_command_runtime else ("commands/grd/**/*",),
        flat_command_globs=("command/grd-*.md",) if flat_command_runtime else (),
        managed_agent_globs=("agents/grd-*.md", "agents/grd-*.toml"),
    )
...
def get_managed_install_surface_policy(runtime: str | None = None) -> ManagedInstallSurfacePolicy:
    ...
    return ManagedInstallSurfacePolicy(
        grd_content_globs=_merge_unique(policy.grd_content_globs for policy in policies),
