...
def configure_opencode_permissions(config_dir: Path) -> bool:
    ...
    if config["permission"]["read"].get(grd_path) != "allow":
        config["permission"]["read"][grd_path] = "allow"
        modified = True
    ...
    if config["permission"]["external_directory"].get(grd_path) != "allow":
        config["permission"]["external_directory"][grd_path] = "allow"
        modified = True
...
def uninstall_opencode(target_dir: Path, *, config_dir: Path) -> dict[str, int]:
    ...
    if oc_config_path_mcp.exists():
        ...
        if isinstance(oc_mcp, dict) and isinstance(oc_mcp.get("mcp"), dict):
            ...
            grd_keys = [k for k in oc_mcp["mcp"] if k in GRD_MCP_SERVER_KEYS]
            for k in grd_keys:
                del oc_mcp["mcp"][k]
            if grd_keys:
                if not oc_mcp["mcp"]:
...
class OpenCodeAdapter(RuntimeAdapter):
    ...
    def _install_content(self, grd_root: Path, target_dir: Path, path_prefix: str, failures: list[str]) -> None:
        super()._install_content(grd_root, target_dir, path_prefix, failures)
    ...
    def _configure_runtime(self, target_dir: Path, is_global: bool) -> dict[str, object]:
        ...
        return {
            ...
            "grd_files": getattr(self, "_gpd_files_count", 0),
