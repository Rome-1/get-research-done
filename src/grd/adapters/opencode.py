...
- Flat command structure: commands/grd/help.md → command/grd-help.md
- Frontmatter conversion: allowed-tools → tools map, color name→hex, strip name field
- Tool name mapping: AskUserQuestion→question, /grd:→/grd-
...
def convert_tool_name(tool_name: str) -> str:
    """Convert a canonical GRD tool name or runtime alias to OpenCode format.
...
def _managed_mcp_server_keys() -> frozenset[str]:
    """Return GRD-managed OpenCode MCP server keys, including optional integrations."""
...
def convert_claude_to_opencode_frontmatter(content: str, path_prefix: str | None = None) -> str:
    """Convert canonical GRD frontmatter to OpenCode format.

    Transformations:
    - Replace tool name references in content
    - Replace /grd: with /grd- (flat command structure)
    ...
    converted = converted.replace("/grd:", "/grd-")
...
def _rewrite_grd_shell_line(line: str, bridge_command: str) -> str:
    ...
    *,
    ...
    for entry in sorted(src_dir.iterdir()):
        if entry.is_dir():
            ...
            if dest_name == "grd-help.md":
                content = _rewrite_opencode_help_wording(content)

            dest_path.write_text(content, encoding="utf-8")
            if managed_command_files is not None and dest_name.startswith("grd-"):
...
def _opencode_managed_permission_keys(config_dir: Path) -> tuple[str, ...]:
    ...
    return (f"{actual_config_dir.as_posix()}/get-research-done/*",)
...
def _load_manifest_opencode_generated_command_files(target_dir: Path) -> tuple[str, ...]:
    ...
    for entry in command_files:
        if isinstance(entry, str) and entry.startswith("grd-") and entry.endswith(".md"):
...
def _load_manifest_opencode_command_files(target_dir: Path) -> tuple[str, ...]:
    ...
    for rel_path in manifest_files:
        ...
        if name.startswith("grd-") and name.endswith(".md"):
            tracked.append(name)
    return tuple(dict.fromkeys(tracked))
...
def configure_opencode_permissions(config_dir: Path) -> bool:
    ...
    grants for the GRD path. Returns True if config was modified.
    """
    ...
    if not isinstance(read_permissions, dict):
        ...
        modified = True
    if read_permissions.get(grd_path) != "allow":
        read_permissions[grd_path] = "allow"
        modified = True
    ...
    if not isinstance(external_permissions, dict):
        ...
        modified = True
    if external_permissions.get(grd_path) != "allow":
        external_permissions[grd_path] = "allow"
        modified = True
...
# ---------------------------------------------------------------------------
# Manifest (OpenCode-specific: uses command/ not commands/grd/)
# ---------------------------------------------------------------------------
...
def write_manifest(
    ...
    *,
    ...
    OpenCode-specific: scans ``command/grd-*.md`` (flat) instead of
    ``commands/grd/`` (nested).
    """
    from datetime import UTC, datetime

    grd_dir = config_dir / "get-research-done"
        ...
        manifest[_MANIFEST_OPENCODE_GENERATED_COMMAND_FILES_KEY] = sorted(
            {
                ...
                if isinstance(name, str) and name.startswith("grd-") and name.endswith(".md")
            }
        )

    # get-research-done/ files
    grd_hashes = generate_manifest(grd_dir)
    for rel, h in grd_hashes.items():
        manifest["files"]["get-research-done/" + rel] = h

    # command/grd-*.md files (OpenCode flat structure)
    if command_dir.exists():
        for f in sorted(command_dir.iterdir()):
            if f.name.startswith("grd-") and f.name.endswith(".md"):
...
def uninstall_opencode(target_dir: Path, *, config_dir: Path) -> dict[str, int]:
    """Uninstall GRD from an OpenCode config directory.

    Removes GRD-specific files/directories, preserves user content.
    ...
    if manifest_file.exists():
        ...
        if isinstance(manifest_payload, dict):
            state = manifest_payload.get("grd_runtime_permissions")
    ...
    # 3. Remove GRD agent files (grd-*.md only)
    ...
    # 4b. Remove GRD update cache files.
    ...
    # 5. Remove GRD MCP servers from opencode.json (uses "mcp" key, not "mcpServers")
    ...
    if oc_config_path_mcp.exists():
        ...
        if isinstance(oc_mcp, dict) and isinstance(oc_mcp.get("mcp"), dict):
            ...
            grd_keys = [k for k in oc_mcp["mcp"] if k in GPD_MCP_SERVER_KEYS]
            for k in grd_keys:
                del oc_mcp["mcp"][k]
            if grd_keys:
                if not oc_mcp["mcp"]:
...
def _write_mcp_servers_opencode(config_dir: Path, servers: dict[str, dict[str, object]]) -> int:
    ...
    from grd.mcp.builtin_servers import merge_managed_mcp_entry
...
class OpenCodeAdapter(RuntimeAdapter):
    ...
    def missing_install_artifacts(self, target_dir: Path) -> tuple[str, ...]:
        ...
        if not command_dir.is_dir():
            missing_command_files.append("command/grd-*.md")

        if missing_command_files and "command/grd-*.md" not in missing_command_files:
            missing_command_files.append("command/grd-*.md")
    ...
    def _install_content(self, grd_root: Path, target_dir: Path, path_prefix: str, failures: list[str]) -> None:
        ...
        skill_dest = target_dir / "get-research-done"
        self._grd_files_count = sum(1 for _ in skill_dest.rglob("*") if _.is_file())
    ...
    def _install_hooks(self, grd_root: Path, target_dir: Path, failures: list[str]) -> None:
        del grd_root, target_dir, failures
    ...
    def _configure_runtime(self, target_dir: Path, is_global: bool) -> dict[str, object]:
        ...
        return {
            ...
            "grd_files": getattr(self, "_grd_files_count", 0),
            "mcpServers": mcp_count,
        }

    def runtime_permissions_status(self, target_dir: Path, *, autonomy: str) -> dict[str, object]:
        """Report whether OpenCode permissions are aligned with GRD autonomy."""
        ...
        if not config_valid:
            ...
            message = "OpenCode opencode.json is malformed; GRD will not treat it as a defaulted permission state."
            ...
            if managed_by_grd:
                message = "OpenCode is still pinned to a GRD-managed `permission = allow` setting from an earlier yolo sync."
            elif configured_mode == "yolo":
                message = (
                    "OpenCode is still configured for `permission = allow`, but GRD left it untouched because "
        ...
        if config_parse_error is not None:
            ...
            return {
                **status,
                ...
                "warning": "OpenCode opencode.json is malformed; GRD will not overwrite it.",
            }
        config = config or {}
        changed = False
    ...
    def uninstall(self, target_dir: Path) -> dict[str, object]:
        """Remove GRD from an OpenCode config directory.

        OpenCode-specific cleanup:
        - command/grd-*.md (flat structure, not commands/grd/)
        - opencode.json permission entries
        - Standard GRD dirs (get-research-done/, agents/, hooks/)
        """
        ...
        with grd_span("adapter.uninstall", runtime=self.runtime_name, target=str(target_dir)) as span:
            ...
            removed: list[str] = []
            if result["commands"]:
                removed.append(f"{result['commands']} GRD commands")
            if result["dirs"]:
                removed.append("get-research-done/")
            if result["agents"]:
                removed.append(f"{result['agents']} GRD agents")
            if result["hooks"]:
                removed.append(f"{result['hooks']} GRD hooks")
