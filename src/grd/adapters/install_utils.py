...
PATCHES_DIR_NAME = "grd-local-patches"
MANIFEST_NAME = "grd-file-manifest.json"
MAX_INCLUDE_EXPANSION_DEPTH = 10
COMMANDS_DIR_NAME = "commands"
FLAT_COMMANDS_DIR_NAME = "command"
AGENTS_DIR_NAME = "agents"
HOOKS_DIR_NAME = "hooks"
GRD_INSTALL_DIR_NAME = "get-research-done"
CACHE_DIR_NAME = "cache"
UPDATE_CACHE_FILENAME = "grd-update-check.json"

# Subdirectories of specs/ that make up the installed get-research-done/ content.
# Shared by all adapters.
GRD_CONTENT_DIRS = ("references", "templates", "workflows")
...
def build_runtime_cli_bridge_command(
    ...
    *,
    ...
    """Return the shell-safe runtime-agnostic GRD bridge command.

    Installed prompts author plain ``grd`` in source form. During install, the
...
def should_preserve_public_local_cli_command(command: str) -> bool:
    ...
    Installed model-facing content should keep these canonical `grd ...`
    commands visible exactly as documented instead of rewriting them to the
    runtime bridge.
    """

    normalized = command.strip()
    if not normalized.startswith("grd "):
        return False
...
def _replace_runtime_placeholders(
    ...
    if scope_flag:
        content = content.replace("{GRD_INSTALL_SCOPE_FLAG}", scope_flag)
    ...
    content = content.replace("{GRD_CONFIG_DIR}", config_dir)
    content = content.replace("{GRD_GLOBAL_CONFIG_DIR}", global_config_dir)
    content = content.replace("{GRD_RUNTIME_FLAG}", install_flag)
    return content
...
def replace_placeholders(
    ...
    """Replace GRD path placeholders in file content.

    Replaces ``{GRD_INSTALL_DIR}``, ``{GRD_AGENTS_DIR}``, and runtime
    placeholders with *path_prefix*.

    Source prompt/spec content should use canonical placeholders such as
    ``{GRD_CONFIG_DIR}`` so the adapter layer can rewrite them to the concrete
...
def protect_runtime_agent_prompt(content: str, runtime: str) -> str:
    ...
    template inputs. GRD agent prompts use those forms as instructional shell
...
def _inject_command_visibility_sections_from_frontmatter(content: str) -> str:
    ...
    if not command_name.startswith("grd:") and not has_agent_only_frontmatter and not has_command_only_frontmatter:
        return content
    eol = _preferred_markdown_eol(preamble, frontmatter, separator, body)
    section = ""
    section_heading = ""
    if command_name.startswith("grd:") or has_command_only_frontmatter:
...
def expand_at_includes(
    ...
    *,
    ...
    Args:
        ...
        src_root: Source root directory (repo's ``get-research-done/`` dir).
    ...
    Examples::
        ...
        'no includes here'
        >>> expand_at_includes("@.grd/notes.md", "/src", "/runtime/")
        '@.grd/notes.md'
    """
    ...
    for line in lines:
        ...
        # .grd/ relative paths — project-specific, skip
        if include_path.startswith(".grd/"):
            result.append(line)
            continue
...
def write_manifest(
    ...
    *,
    ...
    commands_dir = config_dir / "commands" / "grd"
    ...
    # get-research-done/
    for rel, h in generate_manifest(grd_dir).items():
        files["get-research-done/" + rel] = h

    # commands/grd/
    if commands_dir.exists():
        for rel, h in generate_manifest(commands_dir).items():
            files["commands/grd/" + rel] = h

    # agents/grd-*.(md|toml)
    if agents_dir.exists():
        for f in sorted(agents_dir.iterdir()):
            if f.name.startswith("grd-") and f.suffix in {".md", ".toml"}:
...
def managed_hook_paths(config_dir: Path) -> set[str]:
    """Return bundled hook paths that are safe to treat as GRD-managed.

    Only manifest-tracked and exact hash-matched bundled hooks are treated as
    GRD-managed. Unknown hook files must be preserved even if they import
    ``grd`` or reuse a reserved bundled hook filename.
    """
...
def _managed_install_paths(
    ...
    *,
    ...
    grd_dir = config_dir / "get-research-done"
    for rel in generate_manifest(grd_dir).keys():
        managed_paths.append(f"get-research-done/{rel}")

    commands_dir = config_dir / "commands" / "grd"
    for rel in generate_manifest(commands_dir).keys():
        managed_paths.append(f"commands/grd/{rel}")

    command_dir = config_dir / "command"
    if command_dir.exists():
        for entry in sorted(command_dir.iterdir()):
            if entry.is_file() and entry.name.startswith("grd-") and entry.suffix == ".md":
                managed_paths.append(f"command/{entry.name}")

    agents_dir = config_dir / "agents"
    if agents_dir.exists():
        for entry in sorted(agents_dir.iterdir()):
            if entry.is_file() and entry.name.startswith("grd-") and entry.suffix in {".md", ".toml"}:
...
def save_local_patches(
    ...
    *,
    ...
    """Detect user-modified GRD files and back them up before overwriting.

    Compares current files against the install manifest.  Modified files are
    copied to ``grd-local-patches/`` with backup metadata.
...
def remove_stale_agents(agents_dest: Path, new_agent_names: set[str]) -> None:
    """Remove stale grd-* agent files not in *new_agent_names*.
...
def ensure_update_hook(
    ...
    *,
    ...
    """Ensure SessionStart has one up-to-date GRD update-check hook.
...
def hook_python_interpreter() -> str:
    """Return the interpreter that should run installed GRD hook scripts.

    Hook scripts import ``grd.*`` modules, so they need the same interpreter
    ...
    installs prefer the shared ``~/.grd/venv`` interpreter when available so
    hooks and MCP servers do not inherit an unrelated ambient Python.
    """
    override = expand_tilde(os.environ.get("GRD_PYTHON", "").strip())
    if override:
        return override

    try:
        from grd.version import checkout_root, resolve_checkout_python
