"""Runtime-agnostic helpers for canonical GRD tool translation."""
...
"""Canonical GRD tool names supported across runtimes."""
...
def _load_default_policies() -> dict[str, RuntimeToolPolicy]:
    ...
    for descriptor in iter_runtime_descriptors():
        module = import_module(f"grd.adapters.{descriptor.runtime_name.replace('-', '_')}")
...
def canonical(name: str, alias_map: Mapping[str, str] | None = None) -> str:
    """Normalize a tool name to its canonical GRD form."""
...
def translate(
    ...
    *,
    ...
    """Translate a canonical GRD tool name using one runtime's tool map."""
