...
class ClaudeCodeAdapter(RuntimeAdapter):
    ...
    def runtime_permissions_status(self, target_dir: Path, *, autonomy: str) -> dict[str, object]:
        ...
        if desired_mode == "yolo":
            ...
            message = "Claude Code is still pinned to a GRD-managed bypassPermissions default from an earlier yolo sync."
        return {
    ...
    def sync_runtime_permissions(self, target_dir: Path, *, autonomy: str) -> dict[str, object]:
        ...
        if changed:
            ...
            result["message"] = (
                "Claude Code is still configured for bypassPermissions, but GRD left it untouched because "
...
def _rewrite_grd_cli_invocations(content: str, command: str) -> str:
    ...
    for line in content.splitlines(keepends=True):
        ...
        if in_shell_fence:
            rewritten.append(_rewrite_grd_shell_line(line, command))
            continue

        rewritten.append(_rewrite_inline_grd_command_spans(line, command))

    return "".join(rewritten)


def _rewrite_inline_grd_command_spans(content: str, command: str) -> str:
    """Rewrite inline markdown code spans that execute ``grd`` commands."""
    return _INLINE_GRD_COMMAND_RE.sub(lambda match: f"`{command}{match.group('command')[3:]}`", content)
