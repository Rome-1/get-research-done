...
The active model profile (from `.grd/config.json`) controls not just which model tier is used, but how deeply and in what style you investigate.
...
<references>
- `@{GRD_INSTALL_DIR}/references/verification/core/verification-core.md` -- Verification checks that may have failed, universal patterns to understand what went wrong
...
- `{GRD_INSTALL_DIR}/references/shared/cross-project-patterns.md` -- Cross-project pattern library design: how patterns are stored, indexed, and evolved across projects (see `<cross_project_pattern_awareness>` section for runtime integration)
</references>
...
See `@{GRD_INSTALL_DIR}/references/shared/shared-protocols.md` -> Common Physics Error Taxonomy for the full 16-category error table. Always consider these categories when forming hypotheses.
...
if ls .grd/phases/*-*/*-SUMMARY.md 1>/dev/null 2>&1; then
  grep -E "provides:|consumes:" .grd/phases/*-*/*-SUMMARY.md
else
...
with open(".grd/debug/diagnostic_output.json", "a") as f:
...
### Key Files
- `.grd/debug/sign-error-cross-section.md` (this file)
...
### Next Agent Should
1. Read `.grd/debug/{slug}.md` — full investigation state is there
...
```
TROUBLESHOOT_DIR=.grd/debug
TROUBLESHOOT_RESOLVED_DIR=.grd/debug/resolved
```

### Debug File Format

Each debug session file (`.grd/debug/{slug}.md`) should include YAML frontmatter for reliable parsing on resume:
...
At the start of any debugging session, check if `.grd/INSIGHTS.md` exists. If it does, read it to:
...
```bash
if [ -f .grd/config.json ]; then
  cat .grd/config.json
else
  echo "WARNING: .grd/config.json not found — defaulting to deep-theory profile"
fi
```
...
1. `find_files(".grd/phases/*/VERIFICATION.md")` -- find all verification reports
2. Read the most recently modified file
3. `find_files(".grd/phases/*/REVIEW.md")` -- find all review reports
...
Use `find_files(".grd/debug/*.md")` and filter out files containing "resolved" in their names.
...
```bash
mkdir -p .grd/debug/resolved
mv .grd/debug/{slug}.md .grd/debug/resolved/
```

**Commit the correction:**

Commit corrected research files via grd (includes pre-commit validation):

```bash
grd commit "fix: {brief description} -- root cause: {root_cause}" --files path/to/corrected-file.py path/to/other-file.py
```

Then commit the resolved debugging session docs:

```bash
grd commit "docs: resolve debugging session {slug}" --files .grd/debug/resolved/{slug}.md
```
...
**Troubleshooting Session:** .grd/debug/resolved/{slug}.md
...
**How to record:** Follow the `record-insight` workflow from `@{GRD_INSTALL_DIR}/workflows/record-insight.md`.
...
After "ROOT CAUSE FOUND" and verification complete, record the confirmed root cause to `.grd/ERROR-PATTERNS.md` so that verifiers and planners can proactively check for recurrence.
...
```bash
grd commit "docs: record error pattern - {brief description}" --files .grd/ERROR-PATTERNS.md
```
