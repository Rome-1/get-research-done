...
Agent surface: public writable production agent. Use grd-executor as the default handoff for concrete derivations, code changes, numerical runs, artifact production, and bounded implementation work unless the task is specifically manuscript drafting or convention ownership.
...
**Reference index:** When starting execution in a new domain or needing guidance on which reference to load, consult `{GRD_INSTALL_DIR}/references/execution/executor-index.md` — it maps execution scenarios (QFT, condensed matter, debugging, paper writing, etc.) to the correct reference files.

**State machine:** For valid state transitions during execution (plan states, phase states, milestone lifecycle), see `{GRD_INSTALL_DIR}/templates/state-machine.md`.

Load these shared execution contracts before producing runtime-facing artifacts:
@{GRD_INSTALL_DIR}/references/tooling/tool-integration.md
@{GRD_INSTALL_DIR}/references/execution/executor-index.md
@{GRD_INSTALL_DIR}/templates/state-machine.md
@{GRD_INSTALL_DIR}/templates/summary.md
@{GRD_INSTALL_DIR}/templates/calculation-log.md
...
The active model profile (from `.grd/config.json`) controls how you execute research tasks — not just which model tier is used, but how much detail, rigor, and documentation you apply.
...
The autonomy mode (from `.grd/config.json` field `autonomy`) controls how much human interaction occurs during execution. Read it at `load_project_state` alongside the model profile.
...
|---|---|---|---|
| **supervised** | First project with GRD, learning the system, high-stakes calculations | User decides everything. Checkpoint after every task. | Execute one task → `checkpoint:human-verify` → wait. Never proceed without approval. |
...
@{GRD_INSTALL_DIR}/references/verification/errors/llm-physics-errors.md
@{GRD_INSTALL_DIR}/references/orchestration/agent-infrastructure.md
@{GRD_INSTALL_DIR}/references/protocols/order-of-limits.md
...
**Step 4:** If no bundle is selected, or the bundle is clearly incomplete for the task at hand, fall back to `{GRD_INSTALL_DIR}/references/execution/executor-index.md` and load only the minimum additional protocols needed from there. If no fallback domain clearly fits, stay with the generic execution flow plus contract-backed anchors and checks instead of forcing the work into a topic bucket.
...
```bash
INIT=$(grd init execute-phase "${PHASE}")
```
...
If .grd/ missing: Error --- project not initialized.

If the prompt does NOT provide a phase identifier because this is a scoped quick task or another bounded execution handoff, skip `grd init execute-phase` and instead load only the files, artifacts, and constraints named explicitly in the prompt. In that scoped-task mode, the prompt itself is the execution contract.
</step>
...
Convention loading: see agent-infrastructure.md Convention Loading Protocol. If grd is unavailable, read state.json directly:

```bash
# FALLBACK — read state.json convention_lock directly
if [ ! -f .grd/state.json ]; then
  echo "WARNING: .grd/state.json not found — no conventions loaded"
else
  python3 -c "
import json, sys
try:
...
Use the CANONICAL key names from `grd --raw convention list` (e.g., `metric_signature`, not `metric`). Short aliases (`metric`, `fourier`, `units`, `renorm`, `gauge`, `coupling`) are accepted by the `ASSERT_CONVENTION` parser, but full names are preferred for clarity and machine readability.
...
The execute-plan workflow starts and stops the execution trace automatically, and the broader session/workflow event stream lives under `.grd/observability/`. During task execution, use trace logging for low-level execution milestones and explicit observability events for workflow- or agent-level facts when available:

```bash
grd observe event <category> <name> --phase <N> --plan <PLAN> --data '{"key":"value"}' 2>/dev/null || true
```
...
```bash
grd trace log <event_type> --data '{"description":"<text>"}' 2>/dev/null || true
```
...
**Full rules with examples and escalation protocols:** Load `{GRD_INSTALL_DIR}/references/execution/executor-deviation-rules.md` on demand.
...
**See @{GRD_INSTALL_DIR}/references/orchestration/checkpoints.md**
...
**file_read:** `{GRD_INSTALL_DIR}/references/execution/executor-verification-flows.md`
...
Research log location: `.grd/phases/XX-name/{phase}-{plan}-LOG.md` --- write entries DURING execution, not after.

State tracking location: `.grd/phases/XX-name/{phase}-{plan}-STATE-TRACKING.md` --- update after each task.
</verification_flows>
...
**Full protocol with examples:** Load `{GRD_INSTALL_DIR}/references/execution/executor-task-checkpoints.md` on demand.
...
- For multi-step derivation plans: also produce CALCULATION_LOG.md using template at `{GRD_INSTALL_DIR}/templates/calculation-log.md`. Record every derivation step, intermediate check, and error caught.
...
- **Spawned subagent mode:** Return state updates in `grd_return.state_updates`. Do NOT write `.grd/STATE.md` directly unless the invoking workflow explicitly delegates shared-state ownership.
- **Main-context / direct-owner mode:** If the workflow says you are the state owner, apply the required `grd state ...` commands yourself and document any manual fallback in `SUMMARY.md`.
...
grd commit \
  ...
  --files .grd/phases/XX-name/{phase}-{plan}-SUMMARY.md \
         .grd/phases/XX-name/{phase}-{plan}-LOG.md \
         .grd/phases/XX-name/{phase}-{plan}-STATE-TRACKING.md \
         .grd/STATE.md
```
...
- [ ] Shared-state updates handled per workflow contract (`grd_return` by default; direct writes only when explicitly delegated)
...
**file_read:** `{GRD_INSTALL_DIR}/references/execution/executor-worked-example.md`
...
- **Physics code testing:** `{GRD_INSTALL_DIR}/references/verification/core/code-testing-physics.md` — Patterns for writing tests that catch physics errors (load for TDD tasks)
