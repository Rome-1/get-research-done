...
def test_run_contract_check_tool_description_surfaces_request_requirements() -> None:
    ...
    from grd.mcp.verification_contract_policy import verification_contract_surface_summary_text
...
def test_suggested_claim_alignment_template_is_runnable_without_clause_audit_preset() -> None:
    from grd.mcp.servers.verification_server import run_contract_check, suggest_contract_checks
...
def test_patterns_tools_expose_domain_category_and_severity_enums() -> None:
    from grd.core.patterns import VALID_CATEGORIES, VALID_DOMAINS, VALID_SEVERITIES
    from grd.mcp.servers.patterns_server import mcp
...
def test_public_descriptors_surface_contract_and_optional_dependency_visibility() -> None:
    ...
    verification = descriptors["grd-verification"]
    ...
    for field in (
        ...
        assert "grd-state_emit_phase_event" not in text
        assert "Phase Lifecycle Events" not in text

    conventions = descriptors["grd-conventions"]
...
def test_public_protocols_infra_descriptor_matches_live_catalog_surface() -> None:
    descriptor = json.loads((Path(__file__).resolve().parents[2] / "infra" / "grd-protocols.json").read_text(encoding="utf-8"))
...
def test_public_verification_infra_descriptor_surfaces_semantic_contract_rules() -> None:
    ...
    descriptor = json.loads(
        (Path(__file__).resolve().parents[2] / "infra" / "grd-verification.json").read_text(encoding="utf-8")
    )

    description = descriptor["description"]
    assert description.startswith("GRD physics verification checks.")
    assert verification_contract_surface_summary_text() in description
