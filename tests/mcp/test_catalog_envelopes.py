...
def test_protocols_error_envelope() -> None:
    ...
    with patch("grd.mcp.servers.protocols_server._get_store", side_effect=OSError("protocol catalog offline")):
        result = get_protocol("perturbation-theory")
...
def test_patterns_success_envelope() -> None:
    ...
    with patch("grd.mcp.servers.patterns_server.pattern_list", return_value=listing):
...
def test_patterns_error_envelope() -> None:
    ...
    with patch("grd.mcp.servers.patterns_server.pattern_list", side_effect=OSError("pattern store offline")):
        result = lookup_pattern(domain="qft")
...
def test_errors_success_envelope() -> None:
    ...
    with patch("grd.mcp.servers.errors_mcp._get_store", return_value=store):
...
def test_errors_error_envelope() -> None:
    ...
    with patch("grd.mcp.servers.errors_mcp._get_store", side_effect=OSError("error catalog offline")):
        result = get_error_class(1)
...
def test_conventions_success_envelope() -> None:
    from grd.mcp.servers.conventions_server import KNOWN_CONVENTIONS, SUBFIELD_DEFAULTS, subfield_defaults
