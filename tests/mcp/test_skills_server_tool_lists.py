...
import grd.registry as registry_module
from grd.core.workflow_staging import WorkflowStage, WorkflowStageConditionalAuthority, WorkflowStageManifest
...
def test_get_skill_command_surfaces_agent_metadata() -> None:
    ...
    assert result["agent"] == "grd-planner"
    assert result["allowed_tools_surface"] == "command.allowed-tools"
    assert result["structured_metadata_authority"]["agent"] == "mirrored"
    assert "agent: grd-planner" in result["content"]
...
def test_list_skills_filters_consistency_checker_by_verification_category() -> None:
    ...
    consistency_checker = SkillDef(
        ...
        path="/tmp/grd-consistency-checker.md",
        source_kind="agent",
        registry_name="grd-consistency-checker",
    )
    help_skill = SkillDef(
        name="grd-help",
        description="Help.",
    ...
    with patch("grd.mcp.servers.skills_server._load_skill_index", return_value=[consistency_checker, help_skill]):
...
def test_list_skills_filters_beginner_help_entrypoints_by_help_category() -> None:
    ...
    help_skill = registry_module.get_skill("grd-help")
    start_skill = registry_module.get_skill("grd-start")
    tour_skill = registry_module.get_skill("grd-tour")
...
def test_get_skill_command_surfaces_spawn_contract_sidecar_without_content_injection() -> None:
    ...
    spawn_contracts = (
        {
            "agent": "grd-notation-coordinator",
            "shared_state_policy": "return_only",
            "write_scope": {"mode": "scoped_write", "paths": ["GRD/CONVENTIONS.md"]},
            "expected_artifacts": ["GRD/CONVENTIONS.md"],
        },
    )
    command = CommandDef(
        name="grd:new-project",
    ...
    result["spawn_contracts"][0]["expected_artifacts"].append("GRD/EXTRA.md")
    assert command.spawn_contracts[0]["expected_artifacts"] == ["GRD/CONVENTIONS.md"]
...
def test_get_skill_index_surfaces_spawn_contract_presence_sidecar() -> None:
    ...
    with (
        patch("grd.mcp.servers.skills_server._load_skill_index", return_value=[skill]),
        patch("grd.mcp.servers.skills_server.content_registry.get_command", return_value=command),
    ):
        result = get_skill_index()

    assert result["total_skills"] == 1
    assert result["command_envelopes"]["grd-new-project"]["has_spawn_contracts"] is True
    assert result["command_envelopes"]["grd-new-project"]["has_review_contract"] is False
...
def test_get_skill_verify_work_surfaces_staged_loading_sidecar() -> None:
    ...
    with (
        patch(
            "grd.registry.resolve_workflow_stage_manifest_path",
        ...
        registry_module.invalidate_cache()
        result = get_skill("grd-verify-work")
    ...
    assert result["staged_loading"]["stages"][3]["writes_allowed"] == ["GRD/phases/XX-name/XX-VERIFICATION.md"]
    ...
    assert result["staged_loading"]["stages"][4]["writes_allowed"] == ["GRD/phases/XX-name/XX-VERIFICATION.md"]
...
def test_get_skill_plan_checker_agent_surfaces_direct_schema_dependency_and_least_privilege(
    ...
    copytree(Path(__file__).resolve().parents[2] / "src" / "grd" / "agents", agents_dir)
...
def test_skills_server_import_is_resilient_to_registry_index_failure(monkeypatch) -> None:
    ...
    sys.modules.pop("grd.mcp.servers.skills_server", None)

    module = importlib.import_module("grd.mcp.servers.skills_server")
