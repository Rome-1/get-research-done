...
def test_error_store_rejects_missing_authoritative_catalog(tmp_path: Path) -> None:
    ...
    with patch("grd.mcp.servers.errors_mcp.ERROR_CATALOG_FILES", ["verification/errors/missing.md"]):
...
def test_error_store_rejects_duplicate_error_ids(tmp_path: Path) -> None:
    ...
    with patch(
        "grd.mcp.servers.errors_mcp.ERROR_CATALOG_FILES",
        ["verification/errors/catalog-a.md", "verification/errors/catalog-b.md"],
    ), patch("grd.mcp.servers.errors_mcp.TRACEABILITY_FILE", "verification/errors/traceability.md"):
...
def test_error_store_rejects_duplicate_traceability_rows(tmp_path: Path) -> None:
    ...
    with patch("grd.mcp.servers.errors_mcp.ERROR_CATALOG_FILES", ["verification/errors/catalog.md"]), patch(
        "grd.mcp.servers.errors_mcp.TRACEABILITY_FILE",
...
def test_conventions_server_rejects_relative_project_dirs(tool_fn: str, kwargs: dict[str, object]) -> None:
    from grd.mcp.servers import conventions_server
...
@pytest.mark.parametrize(
    ...
    [
        ("grd.mcp.servers.conventions_server", "convention_check", {"lock": {}, "unexpected": True}),
        ("grd.mcp.servers.errors_mcp", "list_error_classes", {"unexpected": True}),
        ("grd.mcp.servers.patterns_server", "seed_patterns", {"unexpected": True}),
        ("grd.mcp.servers.protocols_server", "list_protocols", {"unexpected": True}),
        ("grd.mcp.servers.skills_server", "get_skill_index", {"unexpected": True}),
    ],
)
...
@pytest.mark.parametrize(
    "module_name",
    [
        "grd.mcp.servers.conventions_server",
        "grd.mcp.servers.errors_mcp",
        "grd.mcp.servers.patterns_server",
        "grd.mcp.servers.protocols_server",
        "grd.mcp.servers.skills_server",
        "grd.mcp.servers.state_server",
        "grd.mcp.servers.verification_server",
    ],
)
...
def test_configure_mcp_logging_forces_stderr_and_disables_root_propagation() -> None:
    from grd.mcp.servers import configure_mcp_logging

    logger = logging.getLogger("grd-test-configure-mcp-logging")
    configured = logger
    ...
    try:
        configured = configure_mcp_logging("grd-test-configure-mcp-logging")
...
def test_state_mcp_tools_reject_unknown_arguments_and_preserve_absolute_path_enforcement(tmp_path: Path) -> None:
    ...
    from grd.mcp.servers.state_server import mcp
...
def test_verification_mcp_tools_reject_unknown_arguments(tool_name: str, arguments: dict[str, object]) -> None:
    from grd.mcp.servers.verification_server import mcp
...
def test_protocol_store_defaults_invalid_tier_for_sorting(tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("grd.mcp.servers.protocols_server.PROTOCOL_DOMAINS_MANIFEST", domain_manifest)
...
def test_protocol_store_rejects_invalid_context_cost_frontmatter(
    ...
    from grd.mcp.servers.protocols_server import ProtocolStore, _load_protocol_domain_manifest
...
def test_protocol_store_rejects_boolean_manifest_schema_version(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from grd.mcp.servers.protocols_server import _load_protocol_domain_manifest
