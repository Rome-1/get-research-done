"""Tests for all 7 GRD MCP servers.
...
class TestConventionsServer:
    """Tests for grd.mcp.servers.conventions_server tool functions."""
...
class TestErrorsMcp:
    """Tests for grd.mcp.servers.errors_mcp tool functions."""
    ...
    def test_check_error_classes(self):
        from grd.mcp.servers.errors_mcp import check_error_classes

        result = check_error_classes("angular momentum coupling calculation")
        assert result["match_count"] >= 1

    def test_get_detection_strategy(self):
        from grd.mcp.servers.errors_mcp import get_detection_strategy
    ...
    def test_get_traceability(self):
        from grd.mcp.servers.errors_mcp import get_traceability
...
class TestPatternsServer:
    """Tests for grd.mcp.servers.patterns_server tool functions."""
    ...
    def test_lookup_pattern_by_domain(self):
        ...
        with patch("grd.mcp.servers.patterns_server.pattern_list", return_value=mock_result):
    ...
    def test_add_pattern(self):
        ...
        with patch("grd.mcp.servers.patterns_server.pattern_add", return_value=mock_result):
            result = add_pattern(
                domain="qft",
    ...
    def test_promote_pattern(self):
        from grd.mcp.servers.patterns_server import promote_pattern
        ...
        with patch("grd.mcp.servers.patterns_server.pattern_promote", return_value=mock_result):
            result = promote_pattern("p1")
        assert result["promoted"] is True

    def test_seed_patterns(self):
        from grd.mcp.servers.patterns_server import seed_patterns

        mock_result = MagicMock()
        mock_result.model_dump.return_value = {"seeded": True, "added": 8, "skipped": 0, "total": 8}

        with patch("grd.mcp.servers.patterns_server.pattern_seed", return_value=mock_result):
...
class TestProtocolsServer:
    """Tests for grd.mcp.servers.protocols_server tool functions."""
    ...
    def _mock_store(self):
        ...
        self.store = mock
        with patch("grd.mcp.servers.protocols_server._get_store", return_value=mock):
            yield
    ...
    def test_get_protocol_checkpoints_found(self):
        from grd.mcp.servers.protocols_server import get_protocol_checkpoints
...
class TestSkillsServer:
    """Tests for grd.mcp.servers.skills_server tool functions."""
    ...
    def _mock_skill_registry(self, tmp_path):
        ...
        (commands_dir / "plan-phase.md").write_text(
            ...
            "Read @{GRD_INSTALL_DIR}/workflows/plan-phase.md and {GRD_AGENTS_DIR}/grd-planner.md.\n",
            encoding="utf-8",
        )
        ...
        (commands_dir / "slides.md").write_text(
            ...
            "Read @{GRD_INSTALL_DIR}/workflows/slides.md.\n",
            encoding="utf-8",
        )
        (commands_dir / "peer-review.md").write_text(
            ...
            "Use @{GRD_INSTALL_DIR}/templates/paper/review-ledger-schema.md and "
        ...
        with (
            patch("grd.registry.COMMANDS_DIR", commands_dir),
            patch("grd.registry.AGENTS_DIR", agents_dir),
        ):
    ...
    def test_get_skill_not_found(self):
        ...
        result = get_skill("grd-nonexistent")
        assert "error" in result
    ...
    def test_route_skill_plan(self):
        ...
        assert result["suggestion"] == "grd-plan-phase"
    ...
    def test_route_skill_slides(self):
        ...
        assert result["suggestion"] == "grd-slides"
    ...
    def test_route_skill_suggest_next_from_direct_question(self):
        ...
        assert result["suggestion"] == "grd-suggest-next"
    ...
    def test_route_skill_no_match_without_help_falls_back_to_first_skill(self):
        ...
        assert result["suggestion"] == "grd-debugger"
        assert result["confidence"] <= 0.1
    ...
    def test_route_skill_matches_multiword_resume_phrase(self):
        ...
        assert result["suggestion"] == "grd-resume-work"
...
class TestStateServer:
    ...
    def test_get_state_no_state(self):
        ...
        with patch("grd.mcp.servers.state_server.load_state_json", return_value=None):
            result = get_state("/fake/project")
        assert "error" in result

    def test_get_state_grd_error(self):
    ...
    def test_get_phase_info_not_found(self):
        ...
        with patch("grd.core.phases.find_phase", return_value=None):
    ...
    def test_advance_plan(self):
        ...
        with patch("grd.mcp.servers.state_server.state_advance_plan", return_value=mock_result):
    ...
    def test_validate_state(self):
        ...
        with patch("grd.mcp.servers.state_server.state_validate", return_value=mock_result):
    ...
    def test_run_health_check(self):
        ...
        with patch("grd.mcp.servers.state_server.run_health", return_value=mock_report):
            result = run_health_check("/fake/project")
    ...
    def test_run_health_check_with_fix(self):
        ...
        with patch("grd.mcp.servers.state_server.run_health", return_value=mock_report) as mock_fn:
    ...
    def test_get_config(self):
        ...
        with patch("grd.mcp.servers.state_server.load_config", return_value=mock_config):
            result = get_config("/fake/project")
...
class TestVerificationServer:
    """Tests for grd.mcp.servers.verification_server tool functions."""
    ...
    def test_get_checklist_all_domains(self):
        from grd.mcp.servers.verification_server import DOMAIN_CHECKLISTS, get_checklist
    ...
    def test_verification_coverage_recommendation(self):
        ...
        result = get_verification_coverage(
            ...
            active_checks=list(
                __import__("grd.core.verification_checks", fromlist=["VERIFICATION_CHECKS"]).VERIFICATION_CHECKS.keys()
            ),
        )
    ...
    def test_parse_dimensions(self):
        from grd.mcp.servers.verification_server import _parse_dimensions
...
"""
    ...
    def test_strip_bold(self):
        from grd.mcp.servers.errors_mcp import _strip_bold

        assert _strip_bold("**Bold text**") == "Bold text"
        assert _strip_bold("No bold") == "No bold"

    def test_infer_domain_from_id(self):
        from grd.mcp.servers.errors_mcp import _infer_domain_from_id
...
class TestProtocolStoreParsing:
    ...
    def test_extract_sections(self):
        from grd.mcp.servers.protocols_server import _extract_sections
...
"""
    ...
    def test_extract_steps(self):
        from grd.mcp.servers.protocols_server import _extract_steps_and_checkpoints
...
class TestServerHelpers:
    """Test shared helpers from grd.mcp.servers.__init__."""

    def test_parse_frontmatter_safe_valid(self):
        from grd.mcp.servers import parse_frontmatter_safe
