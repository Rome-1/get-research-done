...
def test_resolve_api_key_prefers_canonical_env_over_alias() -> None:
    from grd.mcp.integrations.wolfram_bridge import (
...
def test_resolve_api_key_rejects_compatibility_alias() -> None:
    from grd.mcp.integrations.wolfram_bridge import resolve_api_key
...
def test_resolve_endpoint_and_api_key_use_the_managed_descriptor_path(monkeypatch: pytest.MonkeyPatch) -> None:
    from grd.mcp.integrations import wolfram_bridge as module
...
def test_load_settings_requires_a_nonempty_api_key() -> None:
    from grd.mcp.integrations.wolfram_bridge import load_settings
...
def test_load_settings_uses_process_environment_when_no_mapping_is_passed(
    ...
    from grd.mcp.integrations.wolfram_bridge import GPD_WOLFRAM_MCP_API_KEY_ENV, load_settings
...
async def test_bridge_open_uses_bearer_token_and_initializes_session(monkeypatch) -> None:
    from grd.mcp.integrations import wolfram_bridge as module
    from grd.mcp.integrations.wolfram_bridge import WolframBridge, WolframBridgeConfig
...
def test_build_server_registers_expected_server_name() -> None:
    from grd.mcp.integrations.wolfram_bridge import WolframBridgeConfig, build_server
    ...
    assert server.name == "grd-wolfram"
...
def test_pyproject_exposes_the_wolfram_console_script() -> None:
    ...
    assert '"grd-mcp-wolfram" = "grd.mcp.integrations.wolfram_bridge:main"' in text
