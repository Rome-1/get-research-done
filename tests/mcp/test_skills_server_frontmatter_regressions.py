...
def test_get_skill_surfaces_malformed_reference_frontmatter(tmp_path: Path) -> None:
    ...
    command = CommandDef(
        ...
        path=str(tmp_path / "grd-peer-review.md"),
        source="commands",
    )
    skill = SkillDef(
        name="grd-peer-review",
        description="Review.",
        content="See @{GRD_INSTALL_DIR}/references/publication/peer-review-panel.md\n",
        category="review",
        path=str(tmp_path / "grd-peer-review.md"),
        source_kind="command",
    ...
    def _patched_portable_reference_path(raw_path: str, *, base_path: Path | None = None):
        if raw_path == "@{GRD_INSTALL_DIR}/references/publication/peer-review-panel.md":
    ...
    try:
        with (
            ...
            patch("grd.mcp.servers.skills_server._portable_reference_path", side_effect=_patched_portable_reference_path),
        ):
            result = skills_server.get_skill("grd-peer-review")
    finally:
        skills_server._reference_document_metadata.cache_clear()
