...
def test_load_settings_uses_default_storage_root() -> None:
    from grd.core.arxiv_source_download import ARXIV_DEFAULT_STORAGE_PATH
    from grd.mcp.servers.arxiv_bridge import load_settings
...
async def test_bridge_open_spawns_upstream_server_with_storage_path(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    from grd.mcp.servers import arxiv_bridge as module
    from grd.mcp.servers.arxiv_bridge import ArxivBridge, ArxivBridgeConfig
...
def test_build_server_registers_expected_server_name() -> None:
    from grd.mcp.servers.arxiv_bridge import ArxivBridgeConfig, build_server

    server, bridge = build_server(ArxivBridgeConfig())

    assert server.name == "grd-arxiv"
...
def test_module_entrypoint_runs_main(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
            message=r"'grd\.mcp\.servers\.arxiv_bridge' found in sys\.modules .*",
            category=RuntimeWarning,
        )
        runpy.run_module("grd.mcp.servers.arxiv_bridge", run_name="__main__")
