...
def test_protocol_catalog_tools_reject_blank_inputs_up_front() -> None:
    from grd.mcp.servers.protocols_server import get_protocol, get_protocol_checkpoints, list_protocols
...
def test_skill_catalog_tools_reject_blank_and_unknown_filters_up_front() -> None:
    ...
    from grd.mcp.servers.skills_server import get_skill, list_skills
...
def test_verification_catalog_tools_reject_blank_inputs_up_front() -> None:
    from grd.mcp.servers.verification_server import get_checklist, run_check
...
def test_catalog_tool_schemas_publish_non_blank_contracts() -> None:
    specs = [
        ("grd.mcp.servers.protocols_server", "get_protocol", "name"),
        ("grd.mcp.servers.protocols_server", "list_protocols", "domain"),
        ("grd.mcp.servers.protocols_server", "get_protocol_checkpoints", "name"),
        ("grd.mcp.servers.skills_server", "list_skills", "category"),
        ("grd.mcp.servers.skills_server", "get_skill", "name"),
        ("grd.mcp.servers.verification_server", "run_check", "check_id"),
        ("grd.mcp.servers.verification_server", "run_check", "domain"),
        ("grd.mcp.servers.verification_server", "run_check", "artifact_content"),
        ("grd.mcp.servers.verification_server", "get_checklist", "domain"),
    ]
...
def test_catalog_filter_schemas_publish_authoritative_enum_values() -> None:
    ...
    protocol_schema = _tool_schema("grd.mcp.servers.protocols_server", "list_protocols")
    skill_schema = _tool_schema("grd.mcp.servers.skills_server", "list_skills")
    manifest = json.loads(
        (Path(__file__).resolve().parents[2] / "src" / "grd" / "specs" / "references" / "protocols" / "protocol-domains.json").read_text(encoding="utf-8")
    )
...
def test_run_check_schema_publishes_live_identifier_enum() -> None:
    from grd.mcp.servers import verification_server

    schema = _tool_schema("grd.mcp.servers.verification_server", "run_check")
...
def test_get_bundle_checklist_schema_publishes_unique_non_blank_bundle_ids() -> None:
    schema = _tool_schema("grd.mcp.servers.verification_server", "get_bundle_checklist")
...
def test_protocol_catalog_schema_refreshes_from_live_manifest_values(monkeypatch: pytest.MonkeyPatch) -> None:
    from grd.mcp.servers import protocols_server
...
def test_skill_catalog_schema_refreshes_from_live_registry_categories(monkeypatch: pytest.MonkeyPatch) -> None:
    from grd import registry as content_registry
    from grd.mcp.servers import skills_server
