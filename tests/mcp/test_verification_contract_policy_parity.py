...
def test_verification_contract_policy_text_stays_aligned_across_public_surfaces() -> None:
    ...
    verification_descriptor = descriptors["grd-verification"]
    tools = {tool.name: tool for tool in anyio.run(mcp.list_tools)}
    infra_descriptor = json.loads((Path(__file__).resolve().parents[2] / "infra" / "grd-verification.json").read_text())
    repo_root = Path(__file__).resolve().parents[2]
    plan_schema = (repo_root / "src/grd/specs/templates/plan-contract-schema.md").read_text(encoding="utf-8")
    state_schema = (repo_root / "src/grd/specs/templates/state-json-schema.md").read_text(encoding="utf-8")
