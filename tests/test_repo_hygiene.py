...
def _is_hygiene_artifact(rel_path: Path) -> bool:
    ...
    if parts[:1] == ("GRD",) and rel_path.name in _GENERATED_GPD_FILES:
        return True
...
def test_grd_utils_package_exposes_only_live_utility_modules() -> None:
    utils_dir = REPO_ROOT / "src" / "grd" / "utils"
