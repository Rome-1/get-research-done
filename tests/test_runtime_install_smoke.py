...
from grd.runtime_cli import _build_repair_command, _resolve_local_config_dir, main
...
def test_runtime_cli_rejects_manifestless_ancestor_install(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys) -> None:
    ...
    seed_complete_runtime_install(ancestor, runtime=_PRIMARY_RUNTIME)
    (ancestor / "grd-file-manifest.json").unlink()
    ...
    assert f"GRD runtime bridge rejected missing install manifest at `{ancestor.resolve()}`." in captured.err
...
def test_runtime_cli_rejects_wrong_runtime_manifest_for_explicit_target(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert f"GRD runtime bridge mismatch for {primary_adapter.display_name} at `{target_dir}`." in captured.err
