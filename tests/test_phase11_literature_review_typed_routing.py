...
def test_literature_review_workflow_routes_on_typed_status_and_artifact_gate() -> None:
    ...
    assert "Verify `GRD/literature/{slug}-REVIEW.md` exists on disk" in workflow
    assert "Verify `GRD/literature/{slug}-CITATION-SOURCES.json` exists on disk and remains aligned with the review's Full Reference List" in workflow
    assert "Return `grd_return.status: completed` only when the review is named in `grd_return.files_written` and the sidecar is present, readable, and aligned on disk" in workflow
...
def test_literature_reviewer_shows_base_return_fields_and_one_shot_checkpointing() -> None:
    agent = _read(AGENTS_DIR / "grd-literature-reviewer.md")
    ...
    assert "When reaching a checkpoint, return a typed `grd_return` checkpoint and stop." in agent
    assert "orchestrator presents it to the user and spawns a fresh continuation run after the response" in agent
    assert "Use `grd_return.status: completed` for a finished review." in agent

    completed_block = agent.split("Use `grd_return.status: completed` for a finished review.", 1)[1]
    status_idx = completed_block.index("  status: completed | checkpoint | blocked | failed")
    files_idx = completed_block.index("  files_written: [GRD/literature/{slug}-REVIEW.md]")
