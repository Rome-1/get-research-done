...
class TestCodexLifecycle:
    ...
    def test_uninstall_preserves_user_owned_grd_skill_dir(self, tmp_path: Path, grd_root: Path) -> None:
        ...
        preserved_skill = skills_dir / "grd-user-keep"
...
class TestManifestConsistency:
    ...
    def test_target_dir_matching_global_dir_infers_global_scope(self, tmp_path: Path) -> None:
        ...
        manifest = json.loads((target_dir / "grd-file-manifest.json").read_text(encoding="utf-8"))
...
def test_clean_local_uninstall_removes_grd_owned_runtime_artifacts(tmp_path: Path, grd_root: Path, runtime: str) -> None:
    ...
    assert not (target / "cache" / "grd-update-check.json").exists()
    assert not (target / "cache" / "grd-update-check.json.inflight").exists()
