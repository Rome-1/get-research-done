...
def test_project_researcher_uses_staged_mode_and_one_shot_checkpoint_language() -> None:
    ...
    assert "typed `grd_return.status: checkpoint`" in source
...
def test_new_project_synthesizer_return_stays_typed_and_file_backed() -> None:
    ...
    assert "If `completed`, verify `GRD/literature/SUMMARY.md` exists and is named in the fresh return." in workflow
