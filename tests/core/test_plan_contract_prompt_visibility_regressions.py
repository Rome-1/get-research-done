...
def test_planner_prompt_surfaces_default_salvage_and_specific_semantics() -> None:
    ...
    assert "`tool_requirements` pass `grd validate plan-preflight <PLAN.md>`" in planner_prompt
...
def test_planner_and_checker_examples_surface_concrete_contract_anchors() -> None:
    planner_prompt = (REPO_ROOT / "src/grd/agents/grd-planner.md").read_text(encoding="utf-8")
    checker_prompt = (REPO_ROOT / "src/grd/agents/grd-plan-checker.md").read_text(encoding="utf-8")
    ...
    assert "GRD/phases/01-vacuum-polarization/01-01-SUMMARY.md" in planner_prompt
    assert "GRD/phases/00-baseline/00-01-SUMMARY.md#gauge-and-tensor-convention" in planner_prompt
    ...
    assert "GRD/phases/00-baseline/00-01-SUMMARY.md" in checker_prompt
    assert "GRD/phases/00-baseline/00-01-SUMMARY.md#gauge-unit-and-notation-conventions" in checker_prompt


def test_plan_checker_prompt_surfaces_direct_schema_visibility_and_read_only_authority() -> None:
    checker_prompt = (AGENTS_DIR / "grd-plan-checker.md").read_text(encoding="utf-8")

    assert checker_prompt.count("@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md") >= 2
    assert "{GRD_INSTALL_DIR}/references/shared/shared-protocols.md" in checker_prompt
    assert "@{GRD_INSTALL_DIR}/references/shared/shared-protocols.md" not in checker_prompt
    ...
    assert "GRD/phases/00-baseline/00-01-SUMMARY.md" in checker_prompt
    assert "GRD/phases/00-baseline/00-01-SUMMARY.md#gauge-unit-and-notation-conventions" in checker_prompt
    assert "GRD/phases/00-baseline/00-01-SUMMARY.md#gauge-and-tensor-convention" in checker_prompt
    assert "GRD/phases/01-vacuum-polarization/01-01-SUMMARY.md" in checker_prompt
...
def test_phase_prompt_surfaces_default_salvage_and_hard_plan_requirements() -> None:
    ...
    assert phase_prompt.count("@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md") == 1
    for token in (
        "tool_requirements",
        "researcher_setup",
        "type: execute",
...
def test_planner_prompt_stays_compact_while_preserving_canonical_contract_wiring() -> None:
    ...
    assert "GRD/phases/00-baseline/00-01-SUMMARY.md#gauge-and-tensor-convention" in planner_prompt
    assert "GRD/phases/01-vacuum-polarization/01-01-SUMMARY.md" in planner_prompt
    assert "@{GRD_INSTALL_DIR}/workflows/execute-plan.md" not in planner_role
    assert "@{GRD_INSTALL_DIR}/templates/summary.md" not in planner_role
    assert "@{GRD_INSTALL_DIR}/references/protocols/order-of-limits.md" not in planner_role
...
def test_planner_gap_closure_example_keeps_execute_type_and_required_contract_block() -> None:
    ...
    assert "must_include_prior_outputs: [\"GRD/phases/XX-name/XX-NN-SUMMARY.md\"]" in planner_prompt
