...
def _read_state_json(project: Path) -> dict[str, object]:
    return json.loads((project / "GRD" / "state.json").read_text(encoding="utf-8"))
...
def _observability_sync_helper():
    from grd.core.observability import sync_execution_visibility_from_canonical_continuation
...
def test_ensure_session_resolves_nested_workspace_to_project_root(tmp_path: Path, monkeypatch) -> None:
    ...
    assert session is not None
    assert (project / "GRD" / "observability" / "current-session.json").exists()
    assert not (nested / "GRD" / "observability").exists()
...
def test_bounded_segment_helper_uses_the_canonical_continuation_symbol_only(tmp_path: Path, monkeypatch) -> None:
    project = _bootstrap_project(tmp_path)
    resume_file = project / "GRD" / "phases" / "01-test" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("", encoding="utf-8")

    import grd.core.continuation as continuation
    ...
    def canonical_helper(project_root: Path, execution_snapshot: dict[str, object]) -> dict[str, object]:
        ...
        assert execution_snapshot["resume_file"] == "GRD/phases/01-test/.continue-here.md"
        return {
            "resume_file": "GRD/phases/01-test/.continue-here.md",
    ...
    monkeypatch.setattr(
        ...
        lambda snapshot: {"resume_file": "GRD/phases/01-test/.continue-here.md"},
    )
    ...
    assert derived.resume_file == "GRD/phases/01-test/.continue-here.md"
...
def test_execution_events_dual_write_durable_bounded_segment_from_resumable_snapshot(
    ...
    resume_file = project / "GRD" / "phases" / "05-test" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("", encoding="utf-8")
    ...
    observe_event(
        ...
        data={
            "execution": {
                ...
                "resume_file": "GRD/phases/05-test/.continue-here.md",
            }
        },
    )

    current_execution = json.loads((project / "GRD" / "observability" / "current-execution.json").read_text(encoding="utf-8"))
    assert current_execution["segment_status"] == "awaiting_user"
    assert current_execution["resume_file"] == "GRD/phases/05-test/.continue-here.md"

    state = _read_state_json(project)
    bounded_segment = state["continuation"]["bounded_segment"]
    assert bounded_segment["resume_file"] == "GRD/phases/05-test/.continue-here.md"
...
def test_execution_events_clear_durable_bounded_segment_when_execution_becomes_non_resumable(
    ...
    (project / "GRD" / "state.json").write_text(
        json.dumps(
            {
                "continuation": {
                    "bounded_segment": {
...
def test_sync_execution_visibility_from_canonical_continuation_updates_live_caches_and_preserves_provenance(
    ...
    resume_file = project / "GRD" / "phases" / "03-analysis" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("", encoding="utf-8")
    ...
    _write_json(project / "GRD" / "state.json", state)
    ...
    from grd.core.execution_lineage import project_execution_lineage_head, write_execution_lineage_head
    ...
    reloaded_head = json.loads((project / "GRD" / "lineage" / "execution-head.json").read_text(encoding="utf-8"))
    snapshot = get_current_execution(project)
...
def test_sync_execution_visibility_from_canonical_continuation_noops_without_live_execution(
    ...
    assert not (project / "GRD" / "lineage" / "execution-head.json").exists()
...
def test_sync_execution_visibility_from_canonical_continuation_noops_on_conflicting_lane_identity(
    ...
    before_head = json.loads((project / "GRD" / "lineage" / "execution-head.json").read_text(encoding="utf-8"))

    _observability_sync_helper()(project)

    after_current = json.loads((observability_dir / "current-execution.json").read_text(encoding="utf-8"))
    after_head = json.loads((project / "GRD" / "lineage" / "execution-head.json").read_text(encoding="utf-8"))
...
def test_derive_execution_visibility_waiting_state_is_not_marked_possibly_stalled(tmp_path: Path, monkeypatch) -> None:
    ...
    from grd.core.observability import derive_execution_visibility
...
def test_derive_execution_visibility_marks_only_active_segments_possibly_stalled_after_30_minutes(
    ...
    assert visibility.suggested_next_commands[0].command == "grd observe show --session sess-active --last 20"
    ...
    assert any("grd observe show --session sess-active --last 20" in step for step in visibility.suggested_next_steps)
...
def test_derive_execution_visibility_uses_valid_progress_command_in_help_paths(
    ...
    visibility = derive_execution_visibility(project)
    assert visibility is not None
    assert any(suggestion.command == "grd progress bar" for suggestion in visibility.suggested_next_commands)
...
def test_derive_execution_visibility_marks_snapshot_only_visibility_when_lineage_head_is_malformed(
    ...
    (project / "GRD" / "lineage").mkdir(parents=True, exist_ok=True)
    (project / "GRD" / "lineage" / "execution-head.json").write_text("{not valid json}", encoding="utf-8")
    ...
    assert all(suggestion.command != "grd observe show --session sess-snapshot-only --last 20" for suggestion in visibility.suggested_next_commands)
    assert visibility.suggested_next_commands[0].command == "grd observe sessions --last 5"
...
def test_derive_execution_visibility_marks_trace_only_visibility_when_current_snapshot_is_malformed(
    ...
    (project / "GRD" / "observability" / "current-execution.json").write_text("{not valid json}", encoding="utf-8")
...
def test_derive_execution_visibility_marks_degraded_visibility_for_malformed_live_files(
    ...
    (project / "GRD" / "lineage").mkdir(parents=True, exist_ok=True)
    (project / "GRD" / "lineage" / "execution-head.json").write_text("{also not valid json}", encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "plan": "01",
                "segment_status": "paused",
                "resume_file": "GRD/phases/03-test-phase/.continue-here.md",
    ...
    observe_event(
        ...
        data={
            "execution": {
                ...
                "resume_file": "GRD/phases/04-test/.continue-here.md",
            }
        },
    )
    observe_event(
...
def test_segment_pause_forces_paused_status_and_keeps_resume_semantics(tmp_path: Path, monkeypatch) -> None:
    ...
    observe_event(
        ...
        plan="01",
        session_id=session.session_id,
        data={"execution": {"segment_status": "awaiting_user", "resume_file": "GRD/phases/05-test/.continue-here.md"}},
    )
    ...
    assert snapshot.first_result_gate_pending is False
    assert snapshot.downstream_locked is False
    assert snapshot.resume_file == "GRD/phases/05-test/.continue-here.md"
        ...
        json.dumps(
            {
                ...
                "plan": "01",
                "segment_status": "awaiting_user",
                "resume_file": "GRD/phases/03-test/.continue-here.md",
                "updated_at": _iso_minutes_ago(45),
            }
        ),
        encoding="utf-8",
...
def test_export_logs_writes_filtered_json_exports(tmp_path: Path) -> None:
    ...
    from grd.core.observability import export_logs
