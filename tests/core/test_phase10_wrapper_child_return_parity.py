...
def test_wrapper_child_return_contract_is_typed_and_thin(
    ...
    assert "Follow `@{GRD_INSTALL_DIR}/workflows/literature-review.md` exactly." in text
    ...
    assert "grd_return.status: completed" not in text
    assert "grd_return.status: checkpoint" not in text
