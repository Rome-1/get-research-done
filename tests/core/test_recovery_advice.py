...
from grd.core.recent_projects import list_recent_projects, record_recent_project
from grd.core.recovery_advice import (
...
def _project(tmp_path: Path, name: str = "project") -> Path:
    root = tmp_path / name
    (root / "GRD" / "observability").mkdir(parents=True, exist_ok=True)
    return root
...
def test_build_recovery_advice_prefers_current_workspace_recovery_state(tmp_path: Path) -> None:
    ...
    advice = build_recovery_advice(
        ...
        continue_command="/grd:resume-work",
        fast_next_command="/grd:suggest-next",
    )
    ...
    assert advice.primary_command == "grd resume"
...
def test_build_recovery_advice_treats_canonical_bounded_segment_as_authoritative_without_live_overlay(
    ...
    assert advice.active_resume_kind == "bounded_segment"
    assert advice.active_resume_origin == "continuation.bounded_segment"
    assert advice.active_resume_pointer == "GRD/phases/06/.continue-here.md"
...
def test_build_recovery_advice_passes_data_root_to_init_resume(
    ...
    monkeypatch.setattr("grd.core.recovery_advice.init_resume", _fake_init_resume)
...
def test_serialize_recovery_orientation_is_canonical_first_and_omits_legacy_resume_aliases(
    ...
    assert orientation["active_resume_pointer"] == "GRD/phases/06/.continue-here.md"
...
def test_serialize_recovery_advice_is_canonical_first_and_omits_legacy_resume_aliases(
    ...
    assert public["active_resume_pointer"] == "GRD/phases/06/.continue-here.md"
    assert public["actions"][0]["kind"] == "primary"
    assert public["actions"][0]["command"] == "grd resume"
...
def test_build_recovery_advice_marks_auto_selected_recent_project_recovery(
    ...
    resume_file = selected_project / "GRD" / "phases" / "02" / ".continue-here.md"
    ...
    assert advice.primary_command == "grd resume --recent"
    assert advice.project_reentry_reason == "GRD found the only recoverable recent project on this machine and selected it automatically."
    assert advice.active_resume_kind == "bounded_segment"
    assert advice.active_resume_origin == "continuation.bounded_segment"
    assert advice.current_workspace_has_recovery is True
...
def test_build_recovery_advice_marks_auto_selected_recent_project_recovery_without_recent_rows(
    ...
    assert advice.active_resume_kind == "bounded_segment"
    assert advice.active_resume_origin == "continuation.bounded_segment"
    assert advice.active_resume_pointer == "GRD/phases/02/.continue-here.md"
    assert advice.current_workspace_has_resume_file is True
    assert advice.current_workspace_resumable is True
...
def test_build_recovery_advice_marks_ambiguous_recent_projects_as_explicit_selection(
    ...
    assert advice.primary_command == "grd resume --recent"
    assert advice.project_reentry_reason == "GRD found 2 recoverable recent projects on this machine, so you need to choose one."
            ...
            "resume_candidates": [
                {
                    ...
                    "origin": "continuation.bounded_segment",
                    "status": "paused",
                    "resume_file": "GRD/phases/04/04-01-EXECUTE.md",
                }
            ],
        },
    )
...
def test_build_recovery_advice_prefers_continuity_handoff_over_advisory_live_execution(tmp_path: Path) -> None:
    project = _project(tmp_path)
    handoff = project / "GRD" / "phases" / "07" / ".continue-here.md"
    ...
    advice = build_recovery_advice(
        ...
        resume_payload={
            ...
            "active_resume_kind": "continuity_handoff",
            "active_resume_origin": "continuation.handoff",
            "active_resume_pointer": "GRD/phases/07/.continue-here.md",
            "continuity_handoff_file": "GRD/phases/07/.continue-here.md",
            "recorded_continuity_handoff_file": "GRD/phases/07/.continue-here.md",
        },
    )
...
def test_build_recovery_advice_prefers_canonical_continuity_fields_over_conflicting_legacy_execution_flags(
    ...
    assert advice.active_resume_kind == "continuity_handoff"
    assert advice.active_resume_origin == "continuation.handoff"
    assert advice.active_resume_pointer == "GRD/phases/09/.continue-here.md"
    assert advice.continuity_handoff_file == "GRD/phases/09/.continue-here.md"
    assert advice.has_continuity_handoff is True
    assert advice.current_workspace_has_resume_file is True
    assert advice.has_local_recovery_target is True
...
def test_build_recovery_advice_prefers_canonical_resume_fields_over_legacy_top_level_aliases(
    ...
    assert advice.active_resume_kind == "continuity_handoff"
    assert advice.active_resume_origin == "continuation.handoff"
    assert advice.active_resume_pointer == "GRD/phases/10/.continue-here.md"
    assert advice.continuity_handoff_file == "GRD/phases/10/.continue-here.md"
    assert advice.execution_resumable is False
    assert advice.current_workspace_has_resume_file is True
    assert advice.has_continuity_handoff is True
...
def test_build_recovery_advice_ignores_nested_resume_surface_compat_wrapper(
    ...
    project = _project(tmp_path)
    handoff = project / "GRD" / "phases" / "11" / ".continue-here.md"
    handoff.parent.mkdir(parents=True, exist_ok=True)
    handoff.write_text("resume\n", encoding="utf-8")
    ...
    advice = build_recovery_advice(
        ...
        resume_payload={
            "resume_surface": {
                "execution_resume_file": "GRD/phases/11/.continue-here.md",
                ...
                "segment_candidates": [
                    {
                        ...
                        "status": "handoff",
                        "resume_file": "GRD/phases/11/.continue-here.md",
                    }
                ],
            },
            "has_live_execution": False,
...
def test_build_recovery_advice_ignores_arbitrary_nested_resume_surface_wrapper(
    ...
    project = _project(tmp_path)
    handoff = project / "GRD" / "phases" / "12" / ".continue-here.md"
    handoff.parent.mkdir(parents=True, exist_ok=True)
    handoff.write_text("resume\n", encoding="utf-8")
    ...
    advice = build_recovery_advice(
        ...
        resume_payload={
            "recovery": {
                "resume_surface": {
                    "execution_resume_file": "GRD/phases/12/.continue-here.md",
                    "execution_resume_file_source": "session_resume_file",
                    "segment_candidates": [
                        {
                            "source": "session_resume_file",
                            "status": "handoff",
                            "resume_file": "GRD/phases/12/.continue-here.md",
                        }
                    ],
                }
            },
    ...
    advice = build_recovery_advice(
        ...
        resume_payload={
            "resume_surface": {
                ...
                "session_resume_file": "GRD/phases/05/.continue-here.md",
                "recorded_session_resume_file": "GRD/phases/05/.continue-here.md",
                "execution_resume_file": "GRD/phases/05/.continue-here.md",
            },
            "has_live_execution": False,
        },
    )
...
def test_build_recovery_advice_ignores_top_level_legacy_missing_handoff_alias_without_canonical_support(
    ...
    advice = build_recovery_advice(
        ...
        recent_rows=[],
        resume_payload={
            "missing_session_resume_file": "GRD/phases/06/.continue-here.md",
        },
    )
...
def test_build_recovery_advice_keeps_missing_handoff_without_false_resume_file(tmp_path: Path) -> None:
    ...
    advice = build_recovery_advice(
        ...
        recent_rows=[],
        resume_payload={
            "missing_continuity_handoff_file": "GRD/phases/06/.continue-here.md",
        },
    )
...
def test_build_recovery_advice_prefers_missing_handoff_over_advisory_live_execution(tmp_path: Path) -> None:
    ...
    advice = build_recovery_advice(
        ...
        resume_payload={
            "has_live_execution": True,
            "missing_continuity_handoff_file": "GRD/phases/08/.continue-here.md",
        },
    )
...
def test_build_recovery_advice_keeps_machine_change_notice_in_current_workspace_priority(tmp_path: Path) -> None:
    ...
    assert advice.primary_reason == "GRD found recent projects on this machine, but none are selected automatically."
...
def test_build_recovery_advice_describes_recent_projects_that_are_not_auto_selectable(
    ...
    assert advice.decision_source == "recent-projects"
    assert advice.primary_command == "grd resume --recent"
    assert advice.project_reentry_reason == "GRD found recent projects on this machine, but none are ready to reopen automatically."
