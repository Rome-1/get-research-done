...
def _expand(name: str) -> str:
    return expand_at_includes(_read(name), REPO_ROOT / "src/grd", "/runtime/")
...
def test_planner_workflows_expand_the_shared_planner_template_once_per_route() -> None:
    ...
    planner_agent_raw = (AGENTS_DIR / "grd-planner.md").read_text(encoding="utf-8")
    ...
    assert planner_agent_raw.count("@{GRD_INSTALL_DIR}/templates/phase-prompt.md") == 1
    assert planner_agent_raw.count("@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md") == 1
    ...
    assert planner_template.count("@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md") == 1
    ...
    assert "{GRD_INSTALL_DIR}/templates/phase-prompt.md" not in plan_phase_prompt
    assert "{GRD_INSTALL_DIR}/templates/plan-contract-schema.md" not in plan_phase_prompt
