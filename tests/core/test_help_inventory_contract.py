...
def _help_command_inventory(*contents: str) -> set[str]:
    ...
    pattern = re.compile(r"(?m)(?<![A-Za-z0-9-])(?:grd:|/grd:|grd\s+)([a-z0-9-]+)\b")
...
def test_help_inventory_covers_registry_command_inventory() -> None:
    ...
    help_inventory = _help_command_inventory(
        _read("src/grd/commands/help.md"),
        _read("src/grd/specs/workflows/help.md"),
    )
...
def test_help_inventory_uses_runtime_neutral_framing_in_shared_source() -> None:
    ...
    assert all("canonical in-runtime slash-command names in `/grd:*` form" not in content for content in help_sources)
    assert all("/grd:*" not in content for content in help_sources)
...
def test_help_workflow_paper_toolchain_doctor_row_is_single_sourced() -> None:
    ...
    assert len(re.findall(r"(?m)^\s*grd doctor --runtime <runtime> --local\|--global\b.*$", help_workflow)) == 1
    assert len(re.findall(r"(?m)^\s*grd doctor --runtime <runtime> --global\b.*$", help_workflow)) == 0
...
def test_help_workflow_keeps_concise_local_cli_surface_note() -> None:
    ...
    assert "Use `grd --help` to inspect the executable local install/readiness/permissions/diagnostics surface directly." in help_workflow
    assert "The bootstrap installer owns Node.js / Python / `venv` prerequisites; use `grd --help`" not in help_workflow
...
def test_help_workflow_files_and_structure_and_knowledge_lifecycle_coverages() -> None:
    ...
    assert "The literature survey lives under `GRD/literature/`, and reviewed knowledge docs live under `GRD/knowledge/` with review artifacts in `GRD/knowledge/reviews/`." in help_workflow
    assert "Drafts stay `draft` until reviewed, and they move into `in_review` while a review round is open" in help_workflow
    assert "If the target is `stable` or `superseded`, route the user to `grd:review-knowledge`" in help_workflow
    ...
    assert "Migration/backfill for older or provisional docs remains deferred; use canonical `GRD/knowledge/{knowledge_id}.md` targets for now." in help_workflow
    ...
    assert "Example topic: `grd:digest-knowledge \"renormalization group fixed points\"`" in help_workflow
    assert "Example modern arXiv: `grd:digest-knowledge 2401.12345v2`" in help_workflow
    assert "Example legacy arXiv: `grd:digest-knowledge hep-th/9901001`" in help_workflow
    assert "Example source file: `grd:digest-knowledge ./notes/rg-notes.md`" in help_workflow
    assert "Example explicit knowledge path: `grd:digest-knowledge GRD/knowledge/K-renormalization-group-fixed-points.md`" in help_workflow
