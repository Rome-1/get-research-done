...
from grd.core.recent_projects import (
    RecentProjectsError,
...
class TestRecentProjectsRootResolution:
    ...
    def test_defaults_to_home_grd_dir(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestRecentProjectsIndexPersistence:
    def test_save_and_load_round_trip(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert loaded.rows[0].resume_file == "GRD/phases/03/.continue-here.md"
    ...
    def test_load_round_trip_keeps_canonical_rows_only(
        ...
        store_root = tmp_path / "cache"
        project_root = tmp_path / "project"
        handoff_file = project_root / "GRD" / "phases" / "03" / ".continue-here.md"
        project_root.mkdir()
    ...
    def test_record_preserves_explicit_recovery_classification_fields(self, tmp_path: Path) -> None:
        ...
        project_root = tmp_path / "project"
        project_root.mkdir()
        resume_file = project_root / "GRD" / "phases" / "03" / ".continue-here.md"
    ...
    def test_record_recent_project_reads_index_after_lock_acquisition(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("grd.core.recent_projects.file_lock", _inject_existing_row)
...
class TestRecentProjectsListing:
    ...
    def test_list_keeps_missing_handoff_rows_but_marks_them_non_resumable(self, tmp_path: Path) -> None:
        ...
        assert len(rows) == 1
        assert rows[0].available is True
        assert rows[0].resume_file == "GRD/phases/03/.continue-here.md"
