...
def test_plan_and_execute_phase_require_proof_redteam_gates() -> None:
    ...
    assert "`grd-check-proof` is the canonical owner" in execute_phase
    assert 'task(\n     subagent_type="grd-check-proof"' in execute_phase
...
def test_verification_workflows_fail_closed_on_missing_proof_coverage() -> None:
    ...
    assert "spawn `grd-check-proof` once to repair that gap" in verify_phase
    ...
    assert "CHECK_PROOF_MODEL=$(grd resolve-model grd-check-proof)" in verify_work
    assert 'task(\n  subagent_type="grd-check-proof"' in verify_work
    ...
    assert "grd-check-proof" in derive_equation
    assert "CHECK_PROOF_MODEL=$(grd resolve-model grd-check-proof)" in derive_equation
    assert 'task(\n  subagent_type="grd-check-proof"' in derive_equation
...
def test_quick_publication_and_settings_surfaces_block_proof_bypass() -> None:
    ...
    assert "GRD/review/PROOF-REDTEAM{round_suffix}.md" in write_paper
    ...
    assert "expect a sibling `GRD/review/PROOF-REDTEAM{round_suffix}.md` artifact" not in peer_review
