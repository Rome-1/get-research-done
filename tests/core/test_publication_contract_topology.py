...
REFERENCES_DIR = REPO_ROOT / "src" / "grd" / "specs" / "references" / "publication"
TEMPLATES_DIR = REPO_ROOT / "src" / "grd" / "specs" / "templates" / "paper"
...
def test_publication_contract_files_use_canonical_names_without_compatibility_shims() -> None:
    ...
    assert "GRD/REFEREE-REPORT{round_suffix}.md" in round_contract
    assert "GRD/AUTHOR-RESPONSE{round_suffix}.md" in round_contract
    ...
    assert "grd_return.files_written" in response_contract
    ...
    assert "grd paper-build" in manuscript_preflight
...
def test_publication_workflows_and_agents_reference_only_the_canonical_publication_contracts() -> None:
    for path in (
        AGENTS_DIR / "grd-paper-writer.md",
        AGENTS_DIR / "grd-referee.md",
    ...
    paper_writer = (AGENTS_DIR / "grd-paper-writer.md").read_text(encoding="utf-8")
...
def test_publication_workflow_prompt_surfaces_surface_the_shared_manuscript_root_contract_before_round_or_response_policy() -> None:
    ...
    bootstrap_include = "@{GRD_INSTALL_DIR}/references/publication/publication-bootstrap-preflight.md"
    handoff_include = "@{GRD_INSTALL_DIR}/references/publication/publication-response-writer-handoff.md"
