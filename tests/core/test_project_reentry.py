...
def test_resolve_project_reentry_prefers_current_workspace_recovery(tmp_path: Path) -> None:
    workspace = _make_grd_workspace(tmp_path / "workspace", project=True)
...
def test_resolve_project_reentry_walks_up_to_ancestor_project_root(tmp_path: Path) -> None:
    project = _make_grd_workspace(tmp_path / "project", project=True)
...
def test_resolve_project_reentry_surfaces_partial_recoverable_workspace(tmp_path: Path) -> None:
    workspace = _make_grd_workspace(tmp_path / "workspace", roadmap=True, state=True)
    ...
    assert resolution.recoverable_candidates_count == 1
    assert len(resolution.candidates) == 1
    assert resolution.candidates[0].reason == "workspace carries partial recoverable GRD state"
...
def test_resolve_project_reentry_prefers_unique_strong_recent_project_over_partial_recoverable_current_workspace(
    ...
    workspace = _make_grd_workspace(tmp_path / "workspace", roadmap=True, state=True)
    recent = _make_grd_workspace(tmp_path / "recent-project", project=True)
...
def test_resolve_project_reentry_does_not_prefer_unrecoverable_state_file_over_recent_project(tmp_path: Path) -> None:
    workspace = _make_grd_workspace(tmp_path / "workspace")
    (workspace / "GRD" / "state.json").write_text("[]\n", encoding="utf-8")
    recent = _make_grd_workspace(tmp_path / "recent-project", project=True)
...
def test_resolve_project_reentry_rejects_string_booleans_on_recent_project_rows(tmp_path: Path) -> None:
    workspace = _make_grd_workspace(tmp_path / "workspace")
    recent = _make_grd_workspace(tmp_path / "recent-project", project=True, roadmap=True, state=True)
...
def test_resolve_project_reentry_treats_backup_only_state_as_recoverable_current_workspace(tmp_path: Path) -> None:
    ...
    (workspace / "GRD" / "state.json.bak").write_text(json.dumps(backup_state, indent=2) + "\n", encoding="utf-8")
...
def test_resolve_project_reentry_skips_recent_project_scan_for_recoverable_current_workspace(
    ...
    monkeypatch.setattr("grd.core.project_reentry.list_recent_projects", _unexpected_scan)
...
def test_resolve_project_reentry_ignores_empty_grd_workspace_when_unique_recent_project_is_recoverable(
    tmp_path: Path,
) -> None:
    workspace = _make_grd_workspace(tmp_path / "workspace")
    project = _make_grd_workspace(tmp_path / "recent-project", project=True)
...
def test_resolve_project_reentry_exposes_selected_candidate_metadata_for_bounded_segment_recent_project(
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    project = _make_grd_workspace(tmp_path / "recent-bounded", project=True)
...
def test_resolve_project_reentry_leaves_selected_candidate_empty_for_missing_handoff_only_recent_project(
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    project = _make_grd_workspace(tmp_path / "recent-missing-handoff", project=True)
...
def test_resolve_project_reentry_prefers_unique_strong_recent_candidate_over_weak_recent_candidate(
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    strong = _make_grd_workspace(tmp_path / "recent-strong", project=True)
    weak = _make_grd_workspace(tmp_path / "recent-weak", project=True)
...
def test_resolve_project_reentry_requires_user_selection_for_ambiguous_recent_projects(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    first = _make_grd_workspace(tmp_path / "recent-a", project=True)
    second = _make_grd_workspace(tmp_path / "recent-b", project=True)
...
def test_resolve_project_reentry_orders_recent_projects_by_canonical_recorded_at(
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    older_session_newer_recorded = _make_grd_workspace(tmp_path / "recent-newer-recorded", project=True)
    newer_session_older_recorded = _make_grd_workspace(tmp_path / "recent-older-recorded", project=True)
...
def test_resolve_project_reentry_orders_bounded_recent_target_ahead_of_handoff_without_auto_selecting(
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    bounded = _make_grd_workspace(tmp_path / "recent-bounded", project=True)
    handoff = _make_grd_workspace(tmp_path / "recent-handoff", project=True)
...
def test_resolve_project_reentry_prefers_current_workspace_over_recent_project(tmp_path: Path) -> None:
    workspace = _make_grd_workspace(tmp_path / "workspace", project=True)
    strong_recent = _make_grd_workspace(tmp_path / "recent-strong", project=True)
