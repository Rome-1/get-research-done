...
def test_write_paper_bibliographer_step_routes_on_typed_return_contract() -> None:
    ...
    assert "Return a typed `grd_return` envelope." in workflow
    ...
    assert "**If the bibliographer completed with issues recorded in the audit report or `GRD/references-status.json`:**" in workflow
