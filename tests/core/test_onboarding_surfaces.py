...
from grd.adapters import iter_runtime_descriptors
from grd.command_labels import validated_public_command_prefix
from grd.core import onboarding_surfaces as onboarding_surfaces_module
from grd.core import public_surface_contract as public_surface_contract_module
from grd.core.onboarding_surfaces import (
...
def test_beginner_onboarding_surface_contract_exposes_hub_and_ladder() -> None:
    ...
    assert beginner_onboarding_caveats() == (
        "GRD is not a standalone app.",
        "GRD does not install your runtime for you.",
        "GRD does not include model access, billing, or API credits.",
        "This hub is the beginner path, not the full reference.",
        "If evidence, references, or artifacts are missing, say so explicitly; GRD should not invent them.",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("install_local_example", "grd install <runtime> --local"),
        ("doctor_local_command", "grd doctor --runtime <runtime> --local"),
        ("doctor_global_command", "grd doctor --runtime <runtime> --global"),
        ("validate_command_context_command", "grd validate command-context grd:<name>"),
    ],
)
...
def test_public_surface_contract_loader_rejects_additive_keys(monkeypatch, tmp_path: Path) -> None:
    canonical_path = Path(__file__).resolve().parents[2] / "src" / "grd" / "core" / "public_surface_contract.json"
...
def test_public_surface_contract_loader_requires_authoritative_local_cli_bridge_commands(
    ...
    invalid_payload["local_cli_bridge"]["commands"] = [
        command for command in invalid_payload["local_cli_bridge"]["commands"] if command != "grd doctor"
    ]

    _load_public_surface_contract_with_payload(monkeypatch, tmp_path, invalid_payload)
    with pytest.raises(ValueError, match=r"local_cli_bridge\.commands must include 'grd doctor'"):
        load_public_surface_contract()
...
def test_public_surface_contract_loader_requires_recovery_ladder_commands_to_stay_public(
    ...
    invalid_payload = copy.deepcopy(canonical_payload)
    invalid_payload["local_cli_bridge"]["commands"] = [
        command for command in invalid_payload["local_cli_bridge"]["commands"] if command != "grd resume --recent"
    ]

    _load_public_surface_contract_with_payload(monkeypatch, tmp_path, invalid_payload)
    with pytest.raises(ValueError, match=r"local_cli_bridge\.commands must include 'grd resume --recent'"):
        load_public_surface_contract()
...
def test_public_surface_contract_bridge_note_surfaces_runtime_readiness_and_plan_validation() -> None:
    ...
    assert "grd doctor --runtime <runtime> --local" not in note
    assert "grd doctor --runtime <runtime> --global" not in note
    assert "grd validate plan-preflight <PLAN.md>" in note
    assert public_surface_contract_module.local_cli_doctor_local_command() == "grd doctor --runtime <runtime> --local"
    assert public_surface_contract_module.local_cli_doctor_global_command() == "grd doctor --runtime <runtime> --global"
    assert "grd validate plan-preflight <PLAN.md>" in public_surface_contract_module.local_cli_plan_preflight_command()
    assert (
...
def test_doc_surface_contract_helpers_read_runtime_normalized_contract(
    ...
    contract = public_surface_contract_module.PublicSurfaceContract(
        ...
        local_cli_bridge=public_surface_contract_module.LocalCliBridgeContract(
            commands=(
                "grd --help",
                "grd doctor",
                "grd validate unattended-readiness --runtime <runtime> --autonomy balanced",
                "grd permissions status --runtime <runtime> --autonomy balanced",
                "grd permissions sync --runtime <runtime> --autonomy balanced",
                "grd resume",
                "grd resume --recent",
                "grd observe execution",
                "grd cost",
                "grd presets list",
                "grd validate plan-preflight <PLAN.md>",
                "grd integrations status wolfram",
            ),
            named_commands=public_surface_contract_module.LocalCliNamedCommandsContract(
                help="grd --help",
                doctor="grd doctor",
                unattended_readiness="grd validate unattended-readiness --runtime <runtime> --autonomy balanced",
                permissions_status="grd permissions status --runtime <runtime> --autonomy balanced",
                permissions_sync="grd permissions sync --runtime <runtime> --autonomy balanced",
                resume="grd resume",
                resume_recent="grd resume --recent",
                observe_execution="grd observe execution",
                cost="grd cost",
                presets_list="grd presets list",
                plan_preflight="grd validate plan-preflight <PLAN.md>",
                integrations_status_wolfram="grd integrations status wolfram",
            ),
        ...
        recovery_ladder=public_surface_contract_module.RecoveryLadderContract(
            ...
            local_snapshot_command="grd resume",
    ...
    assert local_cli_help_command() == "grd --help"
    assert local_cli_doctor_command() == "grd doctor"
    ...
    assert local_cli_permissions_sync_command() == "grd permissions sync --runtime <runtime> --autonomy balanced"
    assert public_surface_contract_module.local_cli_plan_preflight_command() == "grd validate plan-preflight <PLAN.md>"
    assert recovery_local_snapshot_command() == "grd resume"
    ...
    assert bridge_note.startswith("Use `grd --help`, `grd doctor`")
    ...
    assert public_surface_contract_module.local_cli_install_local_example_command() == "grd install <runtime> --local"
    assert (
        public_surface_contract_module.local_cli_validate_command_context_command()
        == "grd validate command-context grd:<name>"
    )
    ...
    contract = public_surface_contract_module.PublicSurfaceContract(
        ...
        local_cli_bridge=public_surface_contract_module.LocalCliBridgeContract(
            commands=(
                "grd help dynamic",
                "grd doctor dynamic",
                "grd validate dynamic-unattended --runtime <runtime> --autonomy balanced",
                "grd permissions status dynamic --runtime <runtime> --autonomy balanced",
                "grd permissions sync dynamic --runtime <runtime> --autonomy balanced",
                "grd resume dynamic",
                "grd resume dynamic --recent",
                "grd observe dynamic",
                "grd cost dynamic",
                "grd presets dynamic",
                "grd validate dynamic-plan <PLAN.md>",
                "grd integrations status dynamic-wolfram",
            ),
            named_commands=public_surface_contract_module.LocalCliNamedCommandsContract(
                help="grd help dynamic",
                doctor="grd doctor dynamic",
                unattended_readiness="grd validate dynamic-unattended --runtime <runtime> --autonomy balanced",
                permissions_status="grd permissions status dynamic --runtime <runtime> --autonomy balanced",
                permissions_sync="grd permissions sync dynamic --runtime <runtime> --autonomy balanced",
                resume="grd resume dynamic",
                resume_recent="grd resume dynamic --recent",
                observe_execution="grd observe dynamic",
                cost="grd cost dynamic",
                presets_list="grd presets dynamic",
                plan_preflight="grd validate dynamic-plan <PLAN.md>",
                integrations_status_wolfram="grd integrations status dynamic-wolfram",
            ),
            terminal_phrase="from your normal terminal",
            purpose_phrase="dynamic diagnostics",
            install_local_example="grd install dynamic --local",
            doctor_local_command="grd doctor dynamic --runtime <runtime> --local",
            doctor_global_command="grd doctor dynamic --runtime <runtime> --global",
            validate_command_context_command="grd validate dynamic-context grd:<name>",
        ),
        post_start_settings=public_surface_contract_module.PostStartSettingsContract(
        ...
        recovery_ladder=public_surface_contract_module.RecoveryLadderContract(
            ...
            local_snapshot_command="grd resume dynamic",
            local_snapshot_phrase="inspect the current workspace",
            cross_workspace_command="grd resume dynamic --recent",
