...
def test_plan_phase_requires_plan_artifacts_before_accepting_success() -> None:
    ...
    assert "extract the fresh plan artifact list from `grd_return.files_written`" in plan_phase
    assert "verify that every named file exists, is readable, and ends in `-PLAN.md`" in plan_phase
    assert "If the planner says complete but no plan files are present, treat the handoff as incomplete until a fresh continuation names them in `grd_return.files_written`" in plan_phase
...
def test_validate_conventions_requires_artifact_and_lock_before_success() -> None:
    ...
    assert "grd_return.status: completed" in validate_conventions
    assert "grd_return.files_written" in validate_conventions
    assert "expected artifact exists on disk" in validate_conventions
    assert "Verify that `GRD/CONVENTIONS.md` exists and that `grd convention list` reflects the resolved fields before accepting the update." in validate_conventions
