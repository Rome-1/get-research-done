...
def test_verify_work_verifier_handoff_stays_one_shot_and_routes_on_typed_status() -> None:
    ...
    assert "Spawn `grd-verifier` once and let it own the physics policy." in workflow
    assert 'subagent_type="grd-verifier"' in workflow
    assert "<spawn_contract>" in workflow
    assert "Route only on the canonical verification frontmatter and `grd_return.status`; do not route on headings or marker strings." in workflow
    assert "If the canonical verification artifact is missing, unreadable, absent from `grd_return.files_written`, or fails contract validation, treat the handoff as incomplete and request a fresh verifier continuation. Never trust the return text alone." in workflow
    assert "Human-readable headings in the verifier output are presentation only; route on the canonical verification frontmatter and `grd_return.status`, not on headings or marker strings." in workflow
...
def test_verify_work_verifier_sync_requires_artifact_gate_before_downstream_routing() -> None:
    ...
    assert "the same path appears in `grd_return.files_written`" in workflow
    assert 'grd validate verification-contract "${phase_dir}/${phase_number}-VERIFICATION.md"' in workflow
    assert "Do not recompute canonical verification status in this workflow." in workflow
    ...
    assert workflow.count("Changed verification files fail `grd pre-commit-check` when this header is missing or mismatched against the active lock.") == 1
...
def test_verify_work_gap_plan_checker_routes_on_canonical_grd_return_status() -> None:
    workflow = _read(WORKFLOWS_DIR / "verify-work.md")
    checker = _read(AGENTS_DIR / "grd-plan-checker.md")

    assert 'grd --raw init verify-work "${PHASE_ARG}" --stage gap_repair' in workflow
    assert "staged payload as the source of truth for planner and checker routing" in workflow
    assert "If the checker returns a structured `grd_return`, route on `grd_return.status` and the structured plan lists, not on presentation text:" in workflow
    ...
    assert "Headings such as `## VERIFICATION PASSED`, `## ISSUES FOUND`, and `## PLAN_BLOCKED — Escalation to User` are presentation only. Route on `grd_return.status`." in checker
    assert "Headings above are presentation only. Route on `grd_return.status`, the approved/blocked plan lists, and `issues`." in checker
...
def test_verify_work_gap_plan_success_reconciles_files_written_and_disk_artifacts() -> None:
    ...
    assert "Before treating the handoff as complete, verify that the expected `PLAN.md` files exist in the phase directory and are listed in `grd_return.files_written` from the fresh planner run." in workflow
    assert (
    ...
    assert "Before accepting the handoff as complete, confirm the expected `PLAN.md` files are present, readable, and listed in `grd_return.files_written` from the planner turn." in workflow
    ...
    assert "Planner runs must return a structured `grd_return` envelope." in planner_prompt
    assert "Do not route on them; route on `grd_return.status` and the artifact gate below." in planner_prompt
    assert "- `grd_return.status: completed` means the planner wrote the expected PLAN.md artifacts and they passed the on-disk artifact check." in planner_prompt
    assert "Always verify `grd_return.files_written` against the expected plan artifacts before accepting completion." in planner_prompt
...
def test_verify_work_proof_check_handoff_uses_structured_freshness_and_fail_closed_artifact_gates() -> None:
    ...
    assert "If `grd-check-proof` still cannot produce a passed audit, keep the verification status fail-closed." in workflow
