"""Prompt budget regressions for the `grd-literature-reviewer` agent surface."""
...
def test_grd_literature_reviewer_prompt_stays_within_expected_budget_and_keeps_the_contract_shell_tight() -> None:
    path = AGENTS_DIR / "grd-literature-reviewer.md"
    ...
    assert "GRD/literature/{slug}-REVIEW.md" in source
    assert "GRD/literature/{slug}-CITATION-SOURCES.json" in source
