...
def test_digest_knowledge_is_same_stem_wired_and_help_indexed() -> None:
    ...
    assert "name: grd:digest-knowledge" in command
    assert "context_mode: project-aware" in command
    assert "@{GRD_INSTALL_DIR}/workflows/digest-knowledge.md" in command
    ...
    assert "grd:digest-knowledge" in command_index
    assert "grd:digest-knowledge" in detailed_reference
    assert "grd:review-knowledge" in command_index
    assert "grd:review-knowledge" in detailed_reference
...
def test_digest_knowledge_workflow_remains_draft_only_and_routes_review_requests() -> None:
    ...
    assert _contains_any(workflow, "route approval", "route the user to `grd:review-knowledge`", "review-knowledge")
...
def test_digest_knowledge_workflow_documents_deterministic_target_resolution_and_input_validation() -> None:
    ...
    assert "GRD/knowledge/{knowledge_id}.md" in workflow
