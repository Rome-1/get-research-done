...
NEW_PROJECT = REPO_ROOT / "src" / "grd" / "specs" / "workflows" / "new-project.md"
PROJECT_CONTRACT_SCHEMA = REPO_ROOT / "src" / "grd" / "specs" / "templates" / "project-contract-schema.md"
STATE_SCHEMA = REPO_ROOT / "src" / "grd" / "specs" / "templates" / "state-json-schema.md"
QUESTIONING = REPO_ROOT / "src" / "grd" / "specs" / "references" / "research" / "questioning.md"


def _expanded(path: Path) -> str:
    return expand_at_includes(path.read_text(encoding="utf-8"), REPO_ROOT / "src/grd/specs", "/runtime/")
...
def test_new_project_prompt_surfaces_the_canonical_contract_schema_for_project_contract_grounding() -> None:
    ...
    assert "POST_SCOPE_INIT=$(grd --raw init new-project --stage post_scope)" in new_project_text
...
def test_new_project_contract_rule_block_is_not_duplicated() -> None:
    ...
    step4_block = new_project_text.split(
        ...
        1,
    )[1].split("Then synthesize all context into `GRD/PROJECT.md`", 1)[0]
...
def test_project_contract_schema_slice_keeps_contract_critical_rules_visible() -> None:
    ...
    assert "Canonical schema for the `project_contract` object inside `GRD/state.json`." in contract_schema_text
...
def test_state_schema_surfaces_the_exact_approved_mode_grounding_rule() -> None:
    ...
    assert "`.grd/phases/.../*-SUMMARY.md` or `paper/main.tex`" in state_schema_text
    assert "`.grd/phases/.../SUMMARY.md`" not in state_schema_text
    assert (
    ...
    assert '"must_include_prior_outputs": ["GRD/phases/00-baseline/00-01-SUMMARY.md"]' in state_schema_text
    assert "`GRD/phases/.../*-SUMMARY.md` or `paper/main.tex`" not in state_schema_text
    assert "`GRD/phases/.../SUMMARY.md`" not in state_schema_text
    assert (
    ...
    assert "grd --raw validate project-contract - --mode approved" in state_schema_text
...
def test_new_project_and_questioning_gate_do_not_treat_missing_anchor_notes_as_approval_ready_grounding() -> None:
    ...
    assert "grd --raw validate project-contract - --mode approved" in new_project_text
