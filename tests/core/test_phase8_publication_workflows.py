...
def test_respond_to_referees_balanced_mode_does_not_force_parse_confirmation() -> None:
    ...
    assert "RESEARCH_MODE=$(echo \"$INIT\" | grd json get .research_mode --default balanced)" in workflow
    assert (
    ...
    assert "<autonomy_mode>{AUTONOMY}</autonomy_mode>" in workflow
    assert "<research_mode>{RESEARCH_MODE}</research_mode>" in workflow
    assert "Treat `GRD/AUTHOR-RESPONSE{round_suffix}.md` and `GRD/review/REFEREE_RESPONSE{round_suffix}.md` as the response success gate." in workflow
    assert "fresh `grd_return.files_written`" in workflow
...
def test_peer_review_stage_six_requires_report_artifacts_and_threads_mode_context() -> None:
    ...
    assert "confirm `GRD/REFEREE-REPORT{round_suffix}.md` and `GRD/REFEREE-REPORT{round_suffix}.tex` exist before treating the final recommendation as complete." in workflow
