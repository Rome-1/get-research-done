...
def test_load_recent_projects_rows_ignores_additive_fields(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("grd.core.recent_projects.list_recent_projects", lambda store_root=None, last=None: [row])
...
def test_render_recent_resume_summary_keeps_runtime_specific_commands_generic(
    ...
    assert "/grd:resume-work" not in output
    assert "$grd-resume-work" not in output
    assert "/grd:suggest-next" not in output
    assert "$grd-suggest-next" not in output
...
def test_resume_recent_requests_only_the_bounded_recent_picker_window(monkeypatch) -> None:
    ...
    monkeypatch.setattr("grd.core.recent_projects.list_recent_projects", _fake_list_recent_projects)
