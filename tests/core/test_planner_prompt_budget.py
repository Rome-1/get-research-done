"""Prompt-budget regressions for `grd-planner` bootstrap loading."""
...
PLANNER_PATH = REPO_ROOT / "src" / "grd" / "agents" / "grd-planner.md"
...
def test_planner_bootstrap_does_not_eagerly_load_execution_or_completion_only_materials() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/templates/phase-prompt.md" in role
    assert "@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md" in role
    assert "@{GRD_INSTALL_DIR}/workflows/execute-plan.md" not in role
    assert "@{GRD_INSTALL_DIR}/templates/summary.md" not in role
    assert "@{GRD_INSTALL_DIR}/references/protocols/order-of-limits.md" not in role
    assert role.index("@{GRD_INSTALL_DIR}/templates/phase-prompt.md") < role.index(
        "before any `PLAN.md` emission."
    )
    assert role.index("@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md") < role.index(
        "before any `PLAN.md` emission."
    )
