...
def test_new_project_project_researcher_scouts_route_on_typed_return_and_reject_stale_results() -> None:
    ...
    assert workflow.count('subagent_type="grd-project-researcher"') == 4
    assert "Route on `grd_return.status` and `grd_return.files_written`." in workflow
    ...
    assert "If `completed`, verify the expected artifact exists on disk and is named in the fresh `grd_return.files_written`." in workflow
    ...
    assert "If a scout reports success but its `expected_artifacts` entry (`GRD/literature/{FILE}`) is missing, treat that scout as incomplete." in workflow
...
def test_new_milestone_project_researcher_scouts_require_fresh_continuations_and_stale_file_rejection() -> None:
    ...
    assert "Route on `grd_return.status` and `grd_return.files_written`, not on the human-readable handoff text." in workflow
    ...
    assert "Before trusting the scout handoff, route on `grd_return.status` and `grd_return.files_written`, then re-read the expected output files from disk and count only artifacts that actually exist. Do not trust the runtime handoff status by itself." in workflow
