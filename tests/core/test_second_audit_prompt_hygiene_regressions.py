...
COMMANDS_DIR = Path("src/grd/commands")
AGENTS_DIR = Path("src/grd/agents")
REFERENCES_DIR = Path("src/grd/specs/references")
TEMPLATES_DIR = Path("src/grd/specs/templates")
...
def test_help_resume_boundary_note_is_concise_and_contract_aligned() -> None:
    ...
    assert help_workflow.count("@{grd_install_dir}/references/orchestration/resume-vocabulary.md") == 1
...
def test_quick_command_and_workflow_keep_the_project_gate_and_drop_the_custom_state_table() -> None:
    ...
    assert "Records completion through structured `grd state` commands" in quick_command
    ...
    assert "They still require an initialized project workspace with `GRD/PROJECT.md` and the `GRD/` directory." in quick_workflow
    assert "They only need `GRD/` to exist for directory structure." not in quick_workflow
...
def test_branch_hypothesis_and_transition_workflows_keep_state_updates_structured() -> None:
    ...
    assert "grd state add-decision" in branch_hypothesis
    assert "save_state_markdown" not in transition_workflow
    assert "STATE.md directly" not in transition_workflow
    assert "grd state update-progress" in transition_workflow
    assert "grd state update" in transition_workflow
    assert "grd state patch" in transition_workflow
...
def test_write_paper_command_defers_the_route_list_to_the_workflow() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/workflows/write-paper.md" in write_paper
    ...
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-response-writer-handoff.md" in write_paper_workflow
...
def test_debug_workflow_path_note_is_not_self_contradictory() -> None:
    ...
    assert "Debug files use the `GRD/debug/` path." in debug_workflow
...
def test_debugger_session_paths_keep_the_active_and_resolved_lifecycles_separate() -> None:
    ...
    debug_agent = (AGENTS_DIR / "grd-debugger.md").read_text(encoding="utf-8")
    debug_workflow = (WORKFLOWS_DIR / "debug.md").read_text(encoding="utf-8")

    assert "Create: GRD/debug/{slug}.md" in debug_command
    assert "Debug file path: GRD/debug/{slug}.md" in debug_command
    assert "files_written: [GRD/debug/{slug}.md, ...]" in debug_agent
    assert "session_file: GRD/debug/{slug}.md" in debug_agent
    assert "**Troubleshooting Session:** GRD/debug/resolved/{slug}.md" in debug_agent
    ...
    assert "typed `grd_return` envelope and the session file instead" in debug_workflow
...
def test_settings_workflow_reuses_one_terminal_follow_up_list() -> None:
    ...
    assert settings_workflow.count("grd validate unattended-readiness --runtime <runtime> --autonomy <mode>") == 1
...
def test_sync_state_workflow_keeps_optional_commit_outside_core_reconcile_path() -> None:
    ...
    assert "No state files found. Run grd:new-project" in sync_state
    ...
    assert sync_state.index("grd --raw state validate") < sync_state.index("<step name=\"optional_commit\">")
