...
PUBLICATION_SHARED_PREFLIGHT_INCLUDE = "@{GRD_INSTALL_DIR}/templates/paper/publication-manuscript-root-preflight.md"
...
PUBLICATION_ROUND_ARTIFACTS_PATH = "{GRD_INSTALL_DIR}/references/publication/publication-review-round-artifacts.md"
...
def test_shared_protocols_require_permission_before_dependency_installs() -> None:
    ...
    assert "## GRD CLI Commit Protocol" not in verifier_raw
    assert "@{GRD_INSTALL_DIR}/references/orchestration/agent-infrastructure.md" not in verifier_raw
...
def test_bibliographer_uses_lightweight_path_mentions_for_metadata_only_reference_packs() -> None:
    bibliographer_text = (AGENTS_DIR / "grd-bibliographer.md").read_text(encoding="utf-8")
...
def test_consistency_checker_prompt_keeps_the_canonical_contract_and_stays_least_privileged() -> None:
    source = (AGENTS_DIR / "grd-consistency-checker.md").read_text(encoding="utf-8")

    assert "one-shot handoff" in source
    assert "status: completed | checkpoint | blocked | failed" in source
    assert "files_written: [GRD/phases/{scope}/CONSISTENCY-CHECK.md]" in source
    assert "GRD/CONSISTENCY-CHECK.md" in source
    assert "@{GRD_INSTALL_DIR}" not in source
    ...
    assert "grd pattern add" not in source
...
def test_representative_commands_expose_expected_context_modes() -> None:
    ...
    assert "core GRD commands" in tour_description
...
def test_respond_to_referees_references_staged_review_artifacts() -> None:
    ...
    assert "GRD/review/REVIEW-LEDGER{round_suffix}.json" in writer_text
    assert "GRD/review/REFEREE-DECISION{round_suffix}.json" in writer_text
...
def test_review_workflows_keep_round_suffix_artifacts_visible_and_anchor_response_outputs() -> None:
    ...
    write_paper_expanded = expand_at_includes(write_paper, REPO_ROOT / "src" / "grd", "/runtime/")
...
def test_publication_commands_accept_documented_manuscript_layouts() -> None:
    ...
    assert "GRD/review/PROOF-REDTEAM{round_suffix}.md" in peer_review
    assert "grd-check-proof" in peer_review
...
def test_proof_contract_prompts_surface_explicit_theorem_fields_and_review_bindings() -> None:
    ...
    peer_review = (WORKFLOWS_DIR / "peer-review.md").read_text(encoding="utf-8")
    check_proof = (AGENTS_DIR / "grd-check-proof.md").read_text(encoding="utf-8")
    ...
    assert (
        "the `grd-check-proof` task must carry the active `manuscript_path`, `manuscript_sha256`, `round`, theorem-bearing `claim_ids`, and `proof_artifact_paths`"
        in peer_review
    )
    assert "copy exactly from `GRD/review/CLAIMS{round_suffix}.json`" in peer_review
    ...
    assert "{GRD_INSTALL_DIR}/templates/proof-redteam-schema.md" in check_proof
    assert "{GRD_INSTALL_DIR}/references/verification/core/proof-redteam-protocol.md" in check_proof
    assert "@{GRD_INSTALL_DIR}/references/publication/peer-review-panel.md" not in check_proof
...
def test_write_paper_and_arxiv_submission_keep_the_build_boundary_explicit() -> None:
    ...
    assert 'grd paper-build "${PAPER_DIR}/PAPER-CONFIG.json" --output-dir "${PAPER_DIR}"' in write_paper
    assert (
    ...
    assert (
        ...
        "pdflatex, and `grd paper-build` remains the canonical manuscript scaffold contract."
    ) in write_paper
    assert 'grd paper-build "${PAPER_DIR}/PAPER-CONFIG.json" --output-dir "${PAPER_DIR}"' in arxiv
...
def test_new_project_command_avoids_stale_workflow_line_counts() -> None:
    ...
    assert "Read {GRD_INSTALL_DIR}/workflows/new-project.md first and follow it exactly." in command_text
...
def test_progress_workflow_surfaces_contract_load_and_validation_state() -> None:
    ...
    assert (
        "Read `{GRD_INSTALL_DIR}/workflows/progress.md` with the file-read tool and follow it exactly." in command_text
    )
    assert "INIT=$(grd --raw init progress --include state,roadmap,project,config)" not in command_text
    assert 'CONTEXT=$(grd --raw validate command-context progress "$ARGUMENTS")' not in command_text
...
def test_planning_prompts_keep_contract_gate_in_light_mode_and_all_modes() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md" in planner_prompt
    assert (
        "Use `@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md` as the canonical contract source." in planner_prompt
    )
    ...
    assert "grd_return.status: completed" in planner_prompt
    ...
    assert "grd_return.status: completed" in workflow_text
...
def test_plan_checker_requires_contract_gate_and_reference_artifacts() -> None:
    ...
    assert "{GRD_INSTALL_DIR}/templates/plan-contract-schema.md" in checker_agent
...
def test_roadmap_template_and_workflows_surface_phase_contract_coverage() -> None:
    ...
    new_project_roadmapper = _find_single_task(WORKFLOWS_DIR / "new-project.md", "grd-roadmapper").text
    new_milestone_roadmapper = _find_single_task(WORKFLOWS_DIR / "new-milestone.md", "grd-roadmapper").text
    ...
    assert "grd_return:" in roadmapper_agent
    ...
    assert "grd_return.files_written" in new_project_roadmapper
    assert "GRD/REQUIREMENTS.md" in new_project_roadmapper
    assert "do not rely on runtime completion text alone." in new_project_roadmapper
    assert "grd_return.files_written" in new_milestone_roadmapper
    assert "treat existing files as stale unless the same paths appear in `grd_return.files_written`" in (
...
def test_research_prompt_surfaces_use_canonical_literature_outputs() -> None:
    project_researcher = (AGENTS_DIR / "grd-project-researcher.md").read_text(encoding="utf-8")
    research_synthesizer = (AGENTS_DIR / "grd-research-synthesizer.md").read_text(encoding="utf-8")
    ...
    for content in (project_researcher, research_synthesizer, phase_researcher, roadmapper_agent):
        assert "GRD/research/" not in content

    assert "GRD/literature/" in project_researcher
    assert "GRD/literature/SUMMARY.md" in research_synthesizer
    assert "GRD/literature/SUMMARY.md" in phase_researcher
...
def test_reference_workflows_require_anchor_registry_propagation() -> None:
    ...
    bibliographer_agent = (AGENTS_DIR / "grd-bibliographer.md").read_text(encoding="utf-8")
    ...
    assert "Follow `@{GRD_INSTALL_DIR}/workflows/literature-review.md` exactly." in literature_command
    ...
    assert "GRD/literature/{slug}-CITATION-SOURCES.json" in literature_agent
    assert "compatible with the `CitationSource` shape" in literature_agent
    assert "grd paper-build --citation-sources" in literature_agent
    ...
    assert "Follow the workflow at `@{GRD_INSTALL_DIR}/workflows/map-research.md`." in map_command
...
def test_file_producing_command_surfaces_use_canonical_spawn_contract() -> None:
    ...
    assert "grd --raw validate command-context literature-review" in literature
    assert "Follow `@{GRD_INSTALL_DIR}/workflows/literature-review.md` exactly." in literature
    assert "First, read {GRD_AGENTS_DIR}/grd-literature-reviewer.md for your role and instructions" not in literature
    assert "Write to: GRD/literature/{slug}-REVIEW.md" not in literature
...
def test_research_phase_command_delegates_file_path_and_return_routing_to_the_workflow() -> None:
    ...
    assert "Follow the workflow at `@{GRD_INSTALL_DIR}/workflows/research-phase.md`." in command
    assert "grd --raw init phase-op --include state,config" not in command
    assert "Research depth follows the workflow-owned `research_mode`." in command
    assert "grd_return.status: completed" not in command
    assert "grd_return.files_written" not in command
    ...
    assert 'grd --raw init research-phase "${phase_arg}" --stage "${stage_name}"' in workflow
    assert "Write to: {phase_dir}/{phase_number}-RESEARCH.md" in workflow
    assert "grd_return.files_written" in workflow
    assert 'RESEARCH_MODE=$(echo "$BOOTSTRAP_INIT" | grd json get .research_mode --default balanced)' in workflow
...
def test_audit_milestone_uses_canonical_phase_helpers_instead_of_raw_glob_discovery() -> None:
    ...
    assert "grd phase list" in audit
    assert "grd show-phase <phase-number>" in audit
    assert "`find_files` `GRD/phases/*/*-VERIFICATION.md` by hand" in audit
    assert "cat GRD/phases/01-*/*-VERIFICATION.md" not in audit
    assert "cat GRD/phases/02-*/*-VERIFICATION.md" not in audit
...
def test_discover_command_does_not_emit_phase_only_commit_placeholders_for_standalone_mode() -> None:
    ...
    assert 'grd commit "discover(${phase_number})' not in discover
    assert "GRD/phases/${padded_phase}-${phase_slug}/RESEARCH.md" not in discover
    assert "DISCOVERY.md" not in discover


def test_workflows_use_raw_json_when_shell_snippets_pipe_cli_output_into_grd_json_get() -> None:
    ...
    assert 'PHASE_INFO=$(grd --raw roadmap get-phase "${phase_number}")' in research_workflow
    assert 'grd --raw state snapshot | grd json get .decisions --default "[]"' in research_workflow
    ...
    assert 'RESEARCH_MODE=$(echo "$BOOTSTRAP_INIT" | grd json get .research_mode --default balanced)' in research_workflow
    assert 'grd --raw config get research_mode' not in research_workflow
    assert 'grd --raw init phase-op --include state,config "${PHASE}"' not in research_command
    ...
    assert 'grd --raw init map-research --stage "${stage_name}"' in map_workflow
    assert 'RESEARCH_MODE=$(echo "$BOOTSTRAP_INIT" | grd json get .research_mode --default balanced)' in map_workflow
    assert 'grd --raw config get research_mode' not in map_workflow
    assert 'grd --raw init map-research' not in map_command
    assert "ROADMAP=$(grd --raw roadmap analyze)" in progress_workflow
    assert "ROADMAP=$(grd --raw roadmap analyze)" not in progress_command
    assert (
        "Read `{GRD_INSTALL_DIR}/workflows/progress.md` with the file-read tool and follow it exactly."
        in progress_command
    )
    assert (
        'grd --raw summary-extract <path> --field one_liner | grd json get .one_liner --default ""' in progress_workflow
    )
    assert "PHASES=$(grd --raw phase list)" in gaps_workflow
    assert (
        'PHASE_GOAL=$(grd --raw roadmap get-phase "${phase_number}" | grd json get .goal --default "")'
        in execute_workflow
    )
    assert (
        'grd --raw summary-extract "$summary" --field one_liner | grd json get .one_liner --default ""'
        in execute_workflow
    )
    assert "ROADMAP=$(grd --raw roadmap analyze)" in milestone_workflow
    assert (
        'grd --raw summary-extract "$summary" --field one_liner | grd json get .one_liner --default ""'
        in milestone_workflow
    )
    assert "ROADMAP=$(grd --raw roadmap analyze)" in graph_workflow
    assert "ROADMAP=$(grd --raw roadmap analyze)" in validate_conventions
    assert transition_workflow.count("ROADMAP=$(grd --raw roadmap analyze)") == 2
    assert "ROADMAP=$(grd --raw roadmap analyze)" in export_workflow
    assert 'PHASE_INFO=$(grd --raw roadmap get-phase "${phase_number}")' in show_phase
    assert "ROADMAP=$(grd --raw roadmap analyze)" in show_phase
    assert 'grd --raw roadmap get-phase "${phase_number}"' in verify_phase
    assert 'grd --raw roadmap get-phase "${phase_number}"' in verify_work
...
def test_workflow_and_command_docs_use_raw_output_for_machine_parsed_cli_json() -> None:
    ...
    for path in prompt_paths:
        ...
        for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            if re.search(r"\bgpd init\b", line) and "grd --raw init" not in line:
                offenders.append(f"{path.relative_to(REPO_ROOT)}:{line_number}: {line.strip()}")
            if re.search(r"\bgpd summary-extract\b", line) and "grd --raw summary-extract" not in line:
                offenders.append(f"{path.relative_to(REPO_ROOT)}:{line_number}: {line.strip()}")
            if re.search(r"\bgpd state compact\b", line) and "grd --raw state compact" not in line:
                offenders.append(f"{path.relative_to(REPO_ROOT)}:{line_number}: {line.strip()}")
...
def test_planner_subagent_prompt_uses_raw_init_placeholder_source() -> None:
    ...
    assert "| `{phase_number}` | `grd --raw init plan-phase` |" in planner_subagent_prompt
...
def test_research_phase_uses_resolved_phase_dir_for_artifact_paths_and_context_lookups() -> None:
    ...
    assert "Follow the workflow at `@{GRD_INSTALL_DIR}/workflows/research-phase.md`." in research_command
    ...
    assert "GRD/phases/${PHASE}-{slug}/${PHASE}-RESEARCH.md" not in research_workflow
    assert "GRD/phases/${PHASE}-{slug}/${PHASE}-RESEARCH.md" not in research_command
...
def test_audit_milestone_command_does_not_preload_raw_verification_globs() -> None:
    ...
    assert "find_files: GRD/phases/*/*SUMMARY.md" in audit_command
    assert "grd phase list" in audit_command
    assert "grd show-phase <phase-number>" in audit_command
    assert "find_files: GRD/phases/*/*-VERIFICATION.md" not in audit_command
...
def test_sensitivity_analysis_workflow_uses_canonical_cli_commands() -> None:
    ...
    assert "grd --raw init progress --include state,config" in workflow
    assert "grd --raw init phase-op" in workflow
    assert "grd uncertainty add" in workflow
    assert "grd commit" in workflow
    assert "grd CLI init progress" not in workflow
    assert "grd CLI init phase-op" not in workflow
    assert "grd CLI uncertainty add" not in workflow
    assert "grd CLI commit" not in workflow
...
def test_phase_research_and_verification_surfaces_keep_anchor_checks_mandatory() -> None:
    ...
    planner_agent = (AGENTS_DIR / "grd-planner.md").read_text(encoding="utf-8")
    verify_workflow = (WORKFLOWS_DIR / "verify-work.md").read_text(encoding="utf-8")
    verify_workflow_expanded = expand_at_includes(verify_workflow, REPO_ROOT / "src/grd", "/runtime/")
...
def test_phase_researcher_prompt_keeps_the_one_shot_handoff_and_return_contract_visible() -> None:
    ...
    assert "grd_return:" in phase_researcher
    ...
    assert "grd_return.files_written" in research_workflow
    assert "Follow the workflow at `@{GRD_INSTALL_DIR}/workflows/research-phase.md`." in research_command
    assert 'grd --raw init research-phase "${phase_arg}" --stage "${stage_name}"' in research_workflow
...
def test_verify_phase_and_gap_reverify_prompts_surface_contract_context_before_contract_checks() -> None:
    ...
    assert "{GRD_INSTALL_DIR}/workflows/verify-phase.md" in execute_phase
    assert "{GRD_INSTALL_DIR}/templates/verification-report.md" in execute_phase
    assert "{GRD_INSTALL_DIR}/templates/contract-results-schema.md" in execute_phase
    assert "grd --raw init phase-op {PHASE_NUMBER}" in execute_phase
...
def test_stage4_templates_and_workflows_surface_contract_results_and_verdict_ledgers() -> None:
    ...
    assert verify_workflow.count("First, read {GRD_AGENTS_DIR}/grd-planner.md for your role and instructions.") == 2
    for token in (
        "tool_requirements",
        "gap_closure",
    ):
...
def test_execute_phase_and_execute_plan_use_staged_execution_bootstrap_instead_of_monolithic_init() -> None:
    ...
    assert "grd --raw init execute-phase \"${phase}\" --include state,config" not in execute_plan
    assert 'grd --raw init execute-phase "${phase}" --stage phase_bootstrap' in execute_plan
    assert 'grd --raw init execute-phase "${phase}" --stage phase_classification' in execute_plan
    assert 'grd --raw init execute-phase "${phase}" --stage wave_planning' in execute_plan
    assert 'grd --raw init execute-phase "${phase}" --stage aggregate_and_verify' in execute_plan
...
def test_verification_prompts_keep_suggested_contract_check_bindings_schema_tight() -> None:
    ...
    assert (
        "Reload `@{GRD_INSTALL_DIR}/templates/contract-results-schema.md` immediately before writing"
...
def test_manuscript_documentation_uses_current_manuscript_root_paths_only() -> None:
    ...
    assert "GRD/paper/" not in explain
    assert "GRD/paper/" not in manuscript_outline
    assert "paper/EXPERIMENTAL_COMPARISON.md" in execute_phase
    assert "${PAPER_DIR}/EXPERIMENTAL_COMPARISON.md" not in execute_phase
    assert "GRD/paper/EXPERIMENTAL_COMPARISON.md" not in execute_phase
...
def test_publication_prompts_surface_strict_semantic_manuscript_gates() -> None:
    ...
    for content in (respond, arxiv):
        ...
        assert "@{GRD_INSTALL_DIR}/references/shared/canonical-schema-discipline.md" not in content
    ...
    assert (
        "Treat `grd paper-build` as the authoritative step that regenerates the resolved manuscript-root "
...
def test_publication_command_contexts_surface_schema_docs_before_generation() -> None:
    ...
    respond_workflow_expanded = _expand_prompt_surface(WORKFLOWS_DIR / "respond-to-referees.md")
    arxiv_workflow_expanded = _expand_prompt_surface(WORKFLOWS_DIR / "arxiv-submission.md")
    shared_preflight_include = "@{GRD_INSTALL_DIR}/templates/paper/publication-manuscript-root-preflight.md"
    bootstrap_preflight_include = "@{GRD_INSTALL_DIR}/references/publication/publication-bootstrap-preflight.md"
    response_handoff_include = "@{GRD_INSTALL_DIR}/references/publication/publication-response-writer-handoff.md"
    round_artifacts_include = "@{GRD_INSTALL_DIR}/references/publication/publication-review-round-artifacts.md"
...
def test_skill_surface_exposes_contract_references_for_paper_and_review_workflows() -> None:
    ...
    arxiv_submission = get_skill("grd-arxiv-submission")
    respond_to_referees = get_skill("grd-respond-to-referees")
...
def test_bibliographer_skill_surface_stays_direct_only() -> None:
    ...
    bibliographer = get_skill("grd-bibliographer")
...
def test_review_and_execution_prompts_expand_required_schema_sources() -> None:
    ...
    review_reader_raw = (AGENTS_DIR / "grd-review-reader.md").read_text(encoding="utf-8")
    referee_raw = (AGENTS_DIR / "grd-referee.md").read_text(encoding="utf-8")
    ...
    assert "{GRD_INSTALL_DIR}/references/publication/peer-review-panel.md" in review_reader_raw
    assert "{GRD_INSTALL_DIR}/references/publication/peer-review-panel.md" in referee_raw
    assert "{GRD_INSTALL_DIR}/templates/paper/review-ledger-schema.md" in referee_raw
    assert "{GRD_INSTALL_DIR}/templates/paper/referee-decision-schema.md" in referee_raw
...
def test_planner_and_summary_prompt_surfaces_expand_contract_schema_bodies() -> None:
    ...
    assert 'must_include_prior_outputs: ["GRD/phases/00-baseline/00-01-SUMMARY.md"]' in phase_prompt
    assert (
        "user_asserted_anchors: "
        '["GRD/phases/00-baseline/00-01-SUMMARY.md#vacuum-polarization-normalization"]' in phase_prompt
    )
    ...
    assert planner_prompt.count("@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md") == 1
    for token in (
...
def test_plan_contract_schema_surfaces_downstream_contract_fields_and_normalization_rules() -> None:
    ...
    assert 'must_include_prior_outputs: ["GRD/phases/00-baseline/00-01-SUMMARY.md"]' in plan_schema
    assert 'user_asserted_anchors: ["GRD/phases/00-baseline/00-01-SUMMARY.md#lattice-normalization"]' in plan_schema
    assert 'known_good_baselines: ["GRD/phases/00-baseline/00-01-SUMMARY.md#published-large-n-curve"]' in plan_schema
...
def test_stage5_execution_surfaces_use_bounded_review_cadence_and_first_result_gates() -> None:
    ...
    assert '# task(subagent_type="grd-notation-coordinator"' not in execute_phase
    assert '# task(subagent_type="grd-experiment-designer"' not in execute_phase
    ...
    assert "grd init resume" not in resume_work
...
def test_debug_command_and_workflow_wire_directly_to_grd_debugger() -> None:
...
def test_resume_command_keeps_internal_resume_backend_details_out_of_public_prompt_surface() -> None:
    ...
    assert "grd init resume" not in resume_command
...
def test_execution_observability_and_resume_workflow_surfaces_stay_conservative_about_stalls() -> None:
    ...
    assert "Display GRD help by delegating to the workflow-owned help surface." in help_command
    assert "@{GRD_INSTALL_DIR}/workflows/help.md" in help_command
...
def test_pause_resume_and_help_wiring_keep_runtime_handoff_and_local_snapshot_boundary() -> None:
    ...
    assert "grd:resume-work" in resume_work
    assert "grd resume" in resume_work
    assert "grd resume --recent" in resume_work
    assert "grd init resume" not in resume_work
    ...
    assert "grd:resume-work" in pause_work
    assert "grd resume" in pause_work
    assert "grd resume --recent" in pause_work
    ...
    assert_recovery_ladder_contract(
        help_workflow,
        resume_work_fragments=("grd:resume-work",),
        suggest_next_fragments=("grd:suggest-next",),
        pause_work_fragments=("grd:pause-work",),
    )
...
def test_state_portability_reference_keeps_resume_public_vocabulary_note_compact() -> None:
    ...
    assert "grd observe execution" in help_workflow
...
def test_pause_resume_and_derivation_templates_preserve_result_id_continuity() -> None:
    ...
    assert "grd state record-session \\" in pause_work
...
def test_stage7_runtime_parity_docs_use_canonical_model_resolution_and_generic_handoff_rules() -> None:
    ...
    assert "First, read {GRD_AGENTS_DIR}/grd-planner.md for your role and instructions." in quick
    ...
    assert "Contract intake: {contract_intake}" in quick
    assert "Project contract gate: {project_contract_gate}" in quick
    assert "grd validate plan-preflight" in quick
...
def test_stage8_surfaces_decisive_comparisons_paper_quality_artifacts_and_profile_invariants() -> None:
    ...
    assert "GRD/paper/FIGURE_TRACKER.md" not in execute_phase
...
def test_publication_workflows_refresh_bibliography_audit_after_bibliography_changes() -> None:
    ...
    assert "`grd paper-build` is the authoritative step that regenerates `${PAPER_DIR}/BIBLIOGRAPHY-AUDIT.json`" in write_paper
    assert "the derived `reference_id -> bibtex_key` bridge" in write_paper
    assert (
        "Prefer the `reference_id -> bibtex_key` mapping surfaced by `grd paper-build` over reconstructing manuscript keys manually from prose or source ordering"
        in write_paper
    )
...
def test_settings_command_keeps_wrapper_thin_and_delegates_manual_to_workflow() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/workflows/settings.md" in settings_command
