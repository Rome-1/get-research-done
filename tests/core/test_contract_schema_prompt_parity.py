...
SPECS_DIR = REPO_ROOT / "src/grd/specs"
...
def test_expanded_verifier_and_executor_prompts_keep_canonical_result_ledger_fields_visible() -> None:
    verifier_prompt = _expanded(AGENTS_DIR / "grd-verifier.md")
    executor_prompt = _expanded(AGENTS_DIR / "grd-executor.md")

    tokens = _field_tokens(*RESULT_MODELS)
    phrases = _choice_phrases(*RESULT_MODELS)

    _assert_tokens_visible(verifier_prompt, tokens, label="expanded grd-verifier.md")
    _assert_tokens_visible(executor_prompt, tokens, label="expanded grd-executor.md")
    _assert_phrases_visible(verifier_prompt, phrases, label="expanded grd-verifier.md")
    _assert_phrases_visible(executor_prompt, phrases, label="expanded grd-executor.md")
