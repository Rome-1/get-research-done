...
from grd.core.public_surface_contract import resume_authority_fields
...
def test_peer_review_reliability_reference_matches_peer_review_workflow_invocation() -> None:
    ...
    reliability = (
        REPO_ROOT / "src/grd/specs/references/publication/peer-review-reliability.md"
    ).read_text(encoding="utf-8")

    expected = 'grd validate review-preflight peer-review "$ARGUMENTS" --strict'

    assert expected in workflow
    assert expected in reliability
    assert "grd validate review-preflight peer-review --strict" not in reliability
...
def test_reapply_patches_keeps_manifest_regeneration_contract_honest() -> None:
    ...
    assert "The managed file manifest is rebuilt by the next `grd:update`" in workflow
...
def test_help_update_describes_bootstrap_update_surface_not_repo_pull() -> None:
    ...
    assert "Pulls latest GRD files from the repository" not in workflow
...
def test_help_resume_surface_stays_user_facing() -> None:
    workflow = expand_at_includes(_workflow_text("help.md"), REPO_ROOT / "src/grd", "/runtime/").lower()
...
def test_resume_work_keeps_public_resume_vocabulary_and_nested_compatibility_intake_separate() -> None:
    ...
    resume_work_workflow = expand_at_includes(_workflow_text("resume-work.md"), REPO_ROOT / "src/grd", "/runtime/")
...
def test_sync_state_keeps_state_json_authority_before_markdown_repair() -> None:
    ...
    sync_state_workflow = expand_at_includes(raw_sync_state_workflow, REPO_ROOT / "src/grd", "/runtime/")

    assert "@{GRD_INSTALL_DIR}/workflows/sync-state.md" in raw_sync_state_command
    assert "@{GRD_INSTALL_DIR}/templates/state-json-schema.md" not in raw_sync_state_command
    assert "@{GRD_INSTALL_DIR}/templates/state-json-schema.md" in raw_sync_state_workflow
...
def test_resume_workflow_routes_new_projects_before_state_reconstruction() -> None:
    ...
    new_project_line = "**If `planning_exists` is false:** This is a new project - route to grd:new-project and do not attempt STATE.md reconstruction."
...
def test_paper_quality_scoring_reference_tracks_per_journal_gate_and_generic_fallback() -> None:
    scoring = (
        REPO_ROOT / "src/grd/specs/references/publication/paper-quality-scoring.md"
    ).read_text(encoding="utf-8")
