...
from grd.core.runtime_command_surfaces import format_active_runtime_command
from grd.core.runtime_hints import (
    _hydrate_resume_context_from_recent_project,
    build_runtime_hint_payload,
    workflow_preset_surface_note,
)
from grd.core.surface_phrases import (
    cost_inspect_action,
...
def _bootstrap_project(tmp_path: Path) -> Path:
    ...
    (project / "GRD" / "observability").mkdir(parents=True, exist_ok=True)
    return project


def _bootstrap_recoverable_project(tmp_path: Path) -> Path:
    project = _bootstrap_project(tmp_path)
    (project / "GRD" / "STATE.md").write_text("# Research State\n", encoding="utf-8")
    (project / "GRD" / "ROADMAP.md").write_text("# Roadmap\n", encoding="utf-8")
    (project / "GRD" / "PROJECT.md").write_text("# Project\n", encoding="utf-8")
    return project
...
def _write_current_session(project: Path, *, session_id: str) -> None:
    current_session_path = project / "GRD" / "observability" / "current-session.json"
...
def _write_current_execution(project: Path, *, session_id: str, extra_execution: dict[str, object] | None = None) -> None:
    execution_path = project / "GRD" / "observability" / "current-execution.json"
    payload = {
        "session_id": session_id,
...
def _write_canonical_continuation(
    ...
    *,
    ...
    state_path = project / "GRD" / "state.json"
...
def test_build_runtime_hint_payload_merges_source_sections_and_actions(tmp_path: Path) -> None:
    ...
    session_id = "sess-001"
    resume_file = project / "GRD" / "phases" / "03" / ".continue-here.md"
    ...
    assert payload.orientation["primary_command"] == "grd resume"
    ...
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/03/.continue-here.md"
    ...
    assert any(action.startswith("Run `grd resume`") for action in payload.next_actions)
    assert not any("grd cost" in action for action in payload.next_actions)
...
def test_build_runtime_hint_payload_does_not_recover_intent_during_read_only_discovery(tmp_path: Path) -> None:
    ...
    before_json_tmp = (layout.grd / ".state-json-tmp").read_text(encoding="utf-8")
    before_md_tmp = (layout.grd / ".state-md-tmp").read_text(encoding="utf-8")
    ...
    assert (layout.grd / ".state-json-tmp").read_text(encoding="utf-8") == before_json_tmp
    assert (layout.grd / ".state-md-tmp").read_text(encoding="utf-8") == before_md_tmp
...
def test_build_runtime_hint_payload_handles_absent_execution_snapshot(tmp_path: Path) -> None:
    ...
    assert payload.recovery["project_reentry_summary"] == "GRD found recent projects on this machine, but none are ready to reopen automatically."
    assert payload.orientation["mode"] == "recent-projects"
    assert payload.orientation["primary_command"] == "grd resume --recent"
...
def test_build_runtime_hint_payload_auto_selects_unique_recoverable_recent_project(tmp_path: Path) -> None:
    ...
    data_root = tmp_path / "data"
    resume_file = project / "GRD" / "phases" / "01" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("resume\n", encoding="utf-8")
    record_recent_project(
        project,
    ...
    assert (
        ...
        == "GRD auto-selected the only recoverable recent project on this machine. last seen 2026-03-27T11:55:00+00:00; stopped at Phase 01; resume file ready."
    )
    assert "resume file ready" in payload.recovery["current_project_summary"]
    assert payload.orientation["resume_surface_schema_version"] == 1
    assert payload.orientation["mode"] == "recent-projects"
    ...
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/01/.continue-here.md"
    assert payload.orientation["continuity_handoff_file"] == "GRD/phases/01/.continue-here.md"
    assert payload.orientation["recorded_continuity_handoff_file"] == "GRD/phases/01/.continue-here.md"
...
def test_build_runtime_hint_payload_auto_selected_bounded_segment_recent_project_stays_bounded_segment(
    ...
    project = _bootstrap_recoverable_project(tmp_path / "project-root")
    data_root = tmp_path / "data"
    resume_file = project / "GRD" / "phases" / "02" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("resume\n", encoding="utf-8")
    record_recent_project(
        project,
    ...
    monkeypatch.setattr(
        "grd.core.runtime_hints._resume_context",
        lambda _cwd, data_root=None: {
            ...
            "active_resume_pointer": "GRD/phases/02/.continue-here.md",
            "resume_candidates": [
                {
                    ...
                    "status": "paused",
                    "resume_file": "GRD/phases/02/.continue-here.md",
                    "resume_pointer": "GRD/phases/02/.continue-here.md",
                }
            ],
    ...
    assert payload.orientation["active_resume_kind"] == "bounded_segment"
    assert payload.orientation["active_resume_origin"] == "continuation.bounded_segment"
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/02/.continue-here.md"
...
def test_hydrate_resume_context_from_recent_project_rejects_string_booleans() -> None:
    ...
    assert hydrated["missing_continuity_handoff_file"] == "GRD/phases/05/.continue-here.md"
...
def test_build_runtime_hint_payload_prefers_selected_project_resume_state_for_auto_selected_recent_project(
    ...
    assert (
        payload.recovery["project_reentry_summary"]
        == "GRD auto-selected the only recoverable recent project on this machine. last seen 2026-03-27T12:05:00+00:00; stopped at Phase 03; resume file ready."
    )
    assert payload.recovery["project_reentry"]["candidates"][0]["resume_target_kind"] == "handoff"
    assert payload.recovery["project_reentry"]["candidates"][0]["reason"] == "recent project cache entry with projected continuity handoff"
    assert payload.orientation["mode"] == "recent-projects"
...
def test_build_runtime_hint_payload_promotes_auto_selected_recent_bounded_segment_over_same_pointer_handoff(
    ...
    project = _bootstrap_recoverable_project(tmp_path / "selected-project")
    data_root = tmp_path / "data"
    resume_file = project / "GRD" / "phases" / "04" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("resume\n", encoding="utf-8")
    _write_canonical_handoff(project, resume_file="GRD/phases/04/.continue-here.md", phase="04")
    record_recent_project(
        project,
        session_data={
    ...
    assert payload.orientation["active_resume_kind"] == "bounded_segment"
    assert payload.orientation["active_resume_origin"] == "continuation.bounded_segment"
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/04/.continue-here.md"
    assert payload.orientation["continuity_handoff_file"] == "GRD/phases/04/.continue-here.md"
    assert payload.orientation["has_continuity_handoff"] is True
    assert payload.orientation["current_workspace_resumable"] is False
    assert payload.orientation["has_local_recovery_target"] is True
    assert payload.orientation["resume_candidates_count"] == 1
...
def test_build_runtime_hint_payload_uses_selected_project_reentry_candidate_for_summary_text(
    ...
    monkeypatch.setattr(
        "grd.core.runtime_hints.resolve_project_reentry",
    ...
    monkeypatch.setattr("grd.core.runtime_hints._resume_context", lambda _cwd, data_root=None: {})
    monkeypatch.setattr("grd.core.runtime_hints.list_recent_projects", lambda store_root=None, last=None: [])
    ...
    assert payload.recovery["project_reentry_summary"] == (
        "GRD auto-selected the only recoverable recent project on this machine. "
...
def test_build_runtime_hint_payload_preserves_existing_local_target_over_recent_project_hydration(
    ...
    monkeypatch.setattr(
        ...
        lambda _cwd, data_root=None: {
            ...
            "active_resume_kind": "bounded_segment",
            "active_resume_origin": "continuation.bounded_segment",
            "active_resume_pointer": "GRD/phases/08/.continue-here.md",
            "execution_resumable": True,
            "resume_candidates": [
                {
                    "kind": "bounded_segment",
                    "origin": "continuation.bounded_segment",
                    "status": "paused",
                    "resume_file": "GRD/phases/08/.continue-here.md",
                    "resume_pointer": "GRD/phases/08/.continue-here.md",
                }
            ],
            "has_live_execution": True,
        },
    ...
    assert payload.orientation["active_resume_kind"] == "bounded_segment"
    assert payload.orientation["active_resume_origin"] == "continuation.bounded_segment"
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/08/.continue-here.md"
    assert payload.orientation["continuity_handoff_file"] is None
    assert payload.orientation["has_local_recovery_target"] is True
    assert payload.orientation["current_workspace_resumable"] is True
...
def test_build_runtime_hint_payload_does_not_hydrate_over_existing_canonical_continuity_handoff_pointer(
    ...
    project = _bootstrap_recoverable_project(tmp_path / "project-root")
    data_root = tmp_path / "data"
    resume_file = project / "GRD" / "phases" / "99" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("resume\n", encoding="utf-8")
    record_recent_project(
        project,
        session_data={
            "last_date": "2026-03-27T11:55:00+00:00",
            "stopped_at": "Phase 99",
            "resume_file": "GRD/phases/99/.continue-here.md",
        },
        store_root=data_root,
    )
    monkeypatch.setattr(
...
def test_build_runtime_hint_payload_does_not_treat_missing_handoff_only_state_as_local_target(
    ...
    assert payload.orientation["continuity_handoff_file"] is None
    assert payload.orientation["recorded_continuity_handoff_file"] is None
    assert payload.orientation["missing_continuity_handoff_file"] == "GRD/phases/04/.continue-here.md"
    assert payload.orientation["has_continuity_handoff"] is False
...
def test_build_runtime_hint_payload_keeps_ambiguous_recent_projects_explicit(tmp_path: Path) -> None:
    ...
    data_root = tmp_path / "data"
    first_resume = first_project / "GRD" / "phases" / "01" / ".continue-here.md"
    second_resume = second_project / "GRD" / "phases" / "02" / ".continue-here.md"
    ...
    assert payload.recovery["project_reentry_summary"] == "GRD found multiple recoverable recent projects on this machine, so you need to choose one."
...
def test_build_runtime_hint_payload_keeps_cost_guidance_quiet_when_best_effort_telemetry_has_no_guardrail(
    ...
    monkeypatch.setattr("grd.core.runtime_hints.build_cost_summary", lambda *args, **kwargs: fake_summary)
...
def test_build_runtime_hint_payload_skips_cost_advisory_when_runtime_exposes_no_threshold(
    ...
    fake_summary = _fake_cost_summary(
        ...
        guidance=[
            f"{_TEST_RUNTIME} does not currently expose a GRD-managed usage telemetry collection path, so `grd cost` may remain empty even when work runs."
        ],
    )
    monkeypatch.setattr("grd.core.runtime_hints.build_cost_summary", lambda *args, **kwargs: fake_summary)
...
def test_build_runtime_hint_payload_uses_shared_resume_contract_without_recent_project_row(
    ...
    project = _bootstrap_project(tmp_path)
    data_root = tmp_path / "data"
    resume_file = project / "GRD" / "phases" / "05" / ".continue-here.md"
    resume_file.parent.mkdir(parents=True, exist_ok=True)
    resume_file.write_text("resume\n", encoding="utf-8")
    ...
    assert payload.orientation["active_resume_kind"] == "bounded_segment"
    assert payload.orientation["active_resume_origin"] == "continuation.bounded_segment"
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/05/.continue-here.md"
...
def test_build_runtime_hint_payload_prefers_canonical_continuity_fields_over_conflicting_legacy_execution_flags(
    ...
    project = _bootstrap_project(tmp_path)
    data_root = tmp_path / "data"
    handoff = project / "GRD" / "phases" / "09" / ".continue-here.md"
            ...
            "resume_candidates": [
                {
                    ...
                    "origin": "continuation.handoff",
                    "status": "handoff",
                    "resume_file": "GRD/phases/09/.continue-here.md",
                }
            ],
            "active_resume_kind": "continuity_handoff",
            "active_resume_origin": "continuation.handoff",
            "active_resume_pointer": "GRD/phases/09/.continue-here.md",
            "continuity_handoff_file": "GRD/phases/09/.continue-here.md",
            "recorded_continuity_handoff_file": "GRD/phases/09/.continue-here.md",
            "execution_resumable": True,
            "execution_resume_file": "GRD/phases/09/legacy-live.md",
    ...
    assert payload.orientation["active_resume_kind"] == "continuity_handoff"
    assert payload.orientation["active_resume_origin"] == "continuation.handoff"
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/09/.continue-here.md"
    assert payload.orientation["continuity_handoff_file"] == "GRD/phases/09/.continue-here.md"
    assert payload.orientation["execution_resumable"] is False
    assert payload.orientation["has_continuity_handoff"] is True
    assert "actions" not in payload.orientation
    _assert_no_resume_compat_aliases(payload.orientation)
...
def test_build_runtime_hint_payload_prefers_canonical_resume_fields_over_legacy_top_level_aliases(
    ...
    project = _bootstrap_project(tmp_path)
    data_root = tmp_path / "data"
    handoff = project / "GRD" / "phases" / "10" / ".continue-here.md"
    handoff.parent.mkdir(parents=True, exist_ok=True)
    handoff.write_text("resume\n", encoding="utf-8")
    monkeypatch.setattr(
        "grd.core.runtime_hints._resume_context",
        lambda _cwd, data_root=None: {
            ...
            "active_resume_kind": "continuity_handoff",
            "active_resume_origin": "continuation.handoff",
            "active_resume_pointer": "GRD/phases/10/.continue-here.md",
            "continuity_handoff_file": "GRD/phases/10/.continue-here.md",
            "recorded_continuity_handoff_file": "GRD/phases/10/.continue-here.md",
            "resume_mode": "bounded_segment",
            "execution_resumable": True,
            "execution_resume_file": "GRD/phases/10/legacy-live.md",
            "execution_resume_file_source": "current_execution",
            "has_live_execution": True,
        },
    )
    ...
    assert payload.orientation["active_resume_kind"] == "continuity_handoff"
    assert payload.orientation["active_resume_origin"] == "continuation.handoff"
    assert payload.orientation["active_resume_pointer"] == "GRD/phases/10/.continue-here.md"
    assert payload.orientation["continuity_handoff_file"] == "GRD/phases/10/.continue-here.md"
    assert payload.orientation["execution_resumable"] is False
    assert payload.orientation["has_continuity_handoff"] is True
    assert "actions" not in payload.orientation
    _assert_no_resume_compat_aliases(payload.orientation)
...
def test_build_runtime_hint_payload_prefers_canonical_bounded_segment_over_conflicting_live_overlay(
    ...
    project = _bootstrap_project(tmp_path)
    data_root = tmp_path / "data"
    canonical_resume_file = "GRD/phases/07/.continue-here.md"
    overlay_resume_file = "GRD/phases/07/overlay.md"
...
def test_build_runtime_hint_payload_rediscovery_branch_handles_non_resumable_current_project(
    ...
    data_root = tmp_path / "data"
    missing_handoff = current_project / "GRD" / "phases" / "04" / ".continue-here.md"
    ...
    assert payload.recovery["current_project"] is not None
    assert payload.recovery["current_project"]["resume_file"] == "GRD/phases/04/.continue-here.md"
    assert payload.recovery["current_project"]["resumable"] is False
...
def test_build_runtime_hint_payload_surfaces_recent_project_missing_handoff_provenance(
    ...
    missing_resume_file = "GRD/phases/04/.continue-here.md"
    current_project = {
        "source": "recent_project",
    ...
    monkeypatch.setattr("grd.core.runtime_hints.resolve_project_reentry", lambda *args, **kwargs: fake_reentry)
    monkeypatch.setattr(
        "grd.core.runtime_hints._selected_reentry_candidate",
...
def test_build_runtime_hint_payload_formats_generic_runtime_follow_up_when_runtime_detection_fails(
    ...
    monkeypatch.setattr("grd.core.runtime_hints._runtime_command", lambda *args, **kwargs: None)
...
def test_runtime_hints_runtime_command_delegates_to_shared_helper(tmp_path: Path) -> None:
    from grd.core import runtime_hints

    with patch("grd.core.runtime_hints.format_active_runtime_command", return_value="shared command") as mock_format:
...
def test_format_active_runtime_command_uses_descriptor_public_surface_without_adapter_lookup() -> None:
    ...
    with patch(
        "grd.core.runtime_command_surfaces.resolve_active_runtime_descriptor",
...
def test_build_runtime_hint_payload_machine_change_only_keeps_local_resume_without_in_runtime_followups(
    ...
    assert not any(action.startswith("Run `grd resume`") for action in payload.next_actions)
    assert not any("resume-work" in action for action in payload.next_actions)
    assert not any("suggest-next" in action for action in payload.next_actions)
...
def test_build_runtime_hint_payload_include_recovery_false_ignores_recent_project_state(
    ...
    monkeypatch.setattr(
        "grd.core.runtime_hints.list_recent_projects",
...
def test_build_runtime_hint_payload_suppresses_duplicate_resume_recovery_nudge(tmp_path: Path) -> None:
    ...
    resume_actions = [action for action in payload.next_actions if "`grd resume`" in action]
...
def test_workflow_preset_surface_note_is_command_oriented_and_preview_first() -> None:
    ...
    assert "grd presets list" in note
    assert "grd presets show <preset>" in note
    assert "grd presets apply <preset> --dry-run" in note
