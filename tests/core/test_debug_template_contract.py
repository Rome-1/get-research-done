...
SESSION_PATH = "GRD/debug/{slug}.md"
...
def test_debug_session_template_and_prompt_share_the_same_goal_status_and_resume_vocabulary() -> None:
    ...
    assert "Read the file at GRD/debug/{slug}.md first, then continue from next_action." in debug_template
    assert "Read the file at GRD/debug/{slug}.md first, then continue from next_action." in prompt
