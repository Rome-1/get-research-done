...
AGENTS_DIR = Path(__file__).resolve().parents[2] / "src" / "grd" / "agents"
...
def test_verifier_prompt_keeps_the_canonical_return_contract_visible() -> None:
    ...
    assert "Return changed paths in `grd_return.files_written`." in verifier
    assert "`grd_return.files_written` is fail-closed:" in verifier
