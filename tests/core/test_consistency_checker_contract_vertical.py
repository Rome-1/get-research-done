...
def test_validate_conventions_seam_is_one_shot_and_artifact_gated_before_notation_resolution() -> None:
    ...
    assert "Thin wrapper around `grd-consistency-checker` for convention validation." in workflow
    assert "Spawn `grd-consistency-checker` once and let it own convention policy." in workflow
    assert "Runtime delegation rule: this is a one-shot handoff." in workflow
    assert workflow.count('subagent_type="grd-consistency-checker"') == 1
    assert workflow.count('subagent_type="grd-notation-coordinator"') == 0
    assert "grd-notation-coordinator" in workflow
    assert "Route only on the canonical `grd_return.status`:" in workflow
    assert "Do not route on checker-local text markers or headings." in workflow
    assert "If the checker's `next_actions` call for notation repair, spawn `grd-notation-coordinator` with the checker report and the same scope." in workflow
    assert "Keep that handoff thin: the coordinator owns the repair policy, not this workflow." in workflow
    assert "If the checker returns `grd_return.status: completed`, accept success only after verifying that:" in workflow
...
def test_consistency_checker_and_notation_coordinator_keep_ownership_boundaries_separate() -> None:
    checker = _read_agent("grd-consistency-checker")
    notation = _read_agent("grd-notation-coordinator")

    assert "Scope boundary: `grd-verifier` owns within-phase correctness. You own between-phase consistency only." in checker
    ...
    assert "the grd-consistency-checker DETECTS convention violations but delegates resolution to this agent" in notation
...
def test_audit_milestone_consumes_checker_reports_without_spawning_notation_resolution() -> None:
    ...
    assert workflow.count('subagent_type="grd-consistency-checker"') == 1
    assert "grd-notation-coordinator" not in workflow
    ...
    assert "status: completed | checkpoint | blocked | failed" in checker
    assert "Human-readable headings in the report are presentation only; route on `grd_return.status`." in checker
