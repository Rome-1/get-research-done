...
from grd.core.continuation import (
...
def test_normalize_continuation_reference_normalizes_project_local_absolute_path(tmp_path: Path) -> None:
    resume_path = _write_resume(tmp_path, "GRD/phases/03-analysis/.continue-here.md")

    normalized = normalize_continuation_reference(tmp_path, str(resume_path))

    assert normalized == "GRD/phases/03-analysis/.continue-here.md"
...
def test_normalize_continuation_normalizes_canonical_references(tmp_path: Path) -> None:
    ...
    assert continuation.handoff.resume_file == "GRD/phases/03-analysis/.continue-here.md"
    assert continuation.bounded_segment is not None
    assert continuation.bounded_segment.resume_file == "GRD/phases/03-analysis/.continue-here.md"
...
def test_canonical_bounded_segment_field_inventory_matches_documented_payload() -> None:
    checkpoints = (REPO_ROOT / "src/grd/specs/references/orchestration/checkpoints.md").read_text(encoding="utf-8")
    continuation_prompt = (REPO_ROOT / "src/grd/specs/templates/continuation-prompt.md").read_text(encoding="utf-8")
...
def test_canonical_bounded_segment_from_execution_snapshot_normalizes_lineage_fields(
    ...
    assert segment.resume_file == "GRD/phases/03-analysis/.continue-here.md"
...
def test_resolve_continuation_prefers_canonical_state_over_legacy_inputs(tmp_path: Path) -> None:
    _write_resume(tmp_path, "GRD/phases/03-analysis/.continue-here.md")
    _write_resume(tmp_path, "GRD/phases/03-analysis/handoff.md")
    _write_resume(tmp_path, "GRD/phases/03-analysis/legacy.md")
    ...
    projection = resolve_continuation(
        ...
        state={
            "continuation": {
                "schema_version": 1,
                "handoff": {
                    "resume_file": "GRD/phases/03-analysis/handoff.md",
            ...
            "session": {
                "resume_file": "GRD/phases/03-analysis/legacy.md",
    ...
    assert projection.active_resume_file == "GRD/phases/03-analysis/.continue-here.md"
    assert projection.handoff_resume_file == "GRD/phases/03-analysis/handoff.md"
...
def test_resolve_continuation_keeps_canonical_handoff_when_live_bounded_segment_exists(
    ...
    assert projection.active_resume_file == "GRD/phases/03-analysis/handoff.md"
    assert projection.handoff_resume_file == "GRD/phases/03-analysis/handoff.md"
...
def test_resolve_continuation_projects_live_execution_when_canonical_pointer_is_invalid(
    ...
    external_resume.write_text("resume\n", encoding="utf-8")
    _write_resume(tmp_path, "GRD/phases/03-analysis/legacy.md")
    _write_resume(tmp_path, "GRD/phases/03-analysis/live.md")
    ...
    projection = resolve_continuation(
        ...
        current_execution={
            "resume_file": "GRD/phases/03-analysis/live.md",
            "segment_status": "paused",
            "segment_id": "live-seg",
        },
    )
    ...
    assert projection.continuation.bounded_segment is not None
    assert projection.continuation.bounded_segment.resume_file == "GRD/phases/03-analysis/live.md"
    assert projection.recorded_handoff_resume_file is None
    assert projection.handoff_resume_file is None
    assert projection.active_resume_file == "GRD/phases/03-analysis/live.md"
    assert projection.active_resume_source == ContinuationResumeSource.BOUNDED_SEGMENT
    assert projection.resumable is True
...
def test_resolve_continuation_falls_back_to_handoff_when_canonical_bounded_segment_pointer_is_missing(
    ...
    projection = resolve_continuation(
        ...
        state={
            "continuation": {
                "handoff": {"resume_file": "GRD/phases/03-analysis/handoff.md"},
                "bounded_segment": {
                    "resume_file": "GRD/phases/03-analysis/.continue-here.md",
                    "segment_status": "paused",
                    "segment_id": "seg-4",
    ...
    assert projection.source == ContinuationSource.CANONICAL
    assert projection.recorded_handoff_resume_file == "GRD/phases/03-analysis/handoff.md"
    assert projection.missing_handoff_resume_file == "GRD/phases/03-analysis/handoff.md"
...
def test_resolve_continuation_projects_portable_current_execution_when_canonical_continuation_is_missing(
    ...
    assert projection.continuation.bounded_segment.resume_file == "GRD/phases/03-analysis/.continue-here.md"
    assert projection.continuation.bounded_segment.phase == "03"
...
def test_resolve_continuation_ignores_nonportable_or_missing_live_snapshot_without_promoting_session_handoff(
    ...
    projection = resolve_continuation(
        tmp_path,
        state={"session": {"resume_file": "GRD/phases/03-analysis/handoff.md"}},
        current_execution={
            "segment_status": "paused",
            "segment_id": "seg-4",
            "resume_file": str(external_resume),
...
def test_resolve_continuation_preserves_partial_canonical_state_without_falling_back_to_session(
    ...
    _write_resume(tmp_path, "GRD/phases/03-analysis/canonical-handoff.md")
    _write_resume(tmp_path, "GRD/phases/03-analysis/legacy-session.md")
    ...
    projection = resolve_continuation(
        ...
        state={
            "continuation": {
                "schema_version": 1,
                "handoff": {
                    "resume_file": "GRD/phases/03-analysis/canonical-handoff.md",
            ...
            "session": {
                "resume_file": "GRD/phases/03-analysis/legacy-session.md",
    ...
    assert projection.source == ContinuationSource.CANONICAL
    assert projection.continuation.handoff.resume_file == "GRD/phases/03-analysis/canonical-handoff.md"
    assert projection.continuation.handoff.stopped_at == "Canonical stop"
    assert projection.continuation.bounded_segment is None
    assert projection.active_resume_file == "GRD/phases/03-analysis/canonical-handoff.md"
    assert projection.handoff_resume_file == "GRD/phases/03-analysis/canonical-handoff.md"
    assert projection.recorded_handoff_resume_file == "GRD/phases/03-analysis/canonical-handoff.md"
...
def test_normalize_continuation_salvages_partial_child_objects(tmp_path: Path) -> None:
    ...
    assert continuation.handoff.resume_file == "GRD/phases/03-analysis/canonical-handoff.md"
    ...
    assert continuation.bounded_segment is not None
    assert continuation.bounded_segment.resume_file == "GRD/phases/03-analysis/canonical-handoff.md"
