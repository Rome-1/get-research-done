...
COMMAND_DOC = REPO_ROOT / "src/grd/commands/derive-equation.md"
WORKFLOW_DOC = REPO_ROOT / "src/grd/specs/workflows/derive-equation.md"


def test_derive_equation_command_doc_promises_registry_writeback() -> None:
    text = COMMAND_DOC.read_text(encoding="utf-8")

    assert "canonical result lookup via `grd result search`" in text
    assert 'direct stored-result inspection via `grd result show "{result_id}"`' in text
    assert "Record the derived equation in the project's `intermediate_results` registry through the executable `grd result persist-derived` bridge" in text
...
def test_derive_equation_workflow_reuses_prior_results_and_persists_final_equation() -> None:
    ...
    assert "Use `grd result search` to locate the canonical result first" in text
    ...
    assert "Persist the final derived equation through the executable `grd result persist-derived` bridge when project state is available." in text
    assert "grd result persist-derived --id \"{result_id}\" --derivation-slug \"{derivation_slug}\"" in text
    assert "grd result persist-derived --derivation-slug \"{derivation_slug}\" --equation \"{final_equation}\"" in text
    ...
    assert "If `grd result persist-derived` reports multiple matches for the same equation or description" in text
    ...
    assert "Final derived equation persisted through the executable `grd result persist-derived` bridge in project mode" in text
