...
from grd.core.constants import PLANNING_DIR_NAME, PROJECT_FILENAME, ROADMAP_FILENAME, STATE_JSON_FILENAME
from grd.core.context import init_resume
from grd.core.project_reentry import recoverable_project_context
...
def _make_recoverable_project(root: Path) -> Path:
    grd_dir = root / PLANNING_DIR_NAME
    grd_dir.mkdir(parents=True)
    (grd_dir / STATE_JSON_FILENAME).write_text(json.dumps(default_state_dict()), encoding="utf-8")
    (grd_dir / ROADMAP_FILENAME).write_text("# Roadmap\n", encoding="utf-8")
    (grd_dir / PROJECT_FILENAME).write_text("# Project\n", encoding="utf-8")
    return root
...
def test_recoverable_project_context_treats_unreadable_state_as_non_recoverable(
    ...
    grd_dir.mkdir(parents=True)
    (grd_dir / STATE_JSON_FILENAME).write_text('{"position": {}}', encoding="utf-8")

    def _raise_unreadable(*args, **kwargs):
        raise PermissionError("sandbox denied lockfile creation")

    monkeypatch.setattr("grd.core.state.peek_state_json", _raise_unreadable)
...
def test_init_resume_ignores_empty_nested_grd_stub_when_workspace_has_real_ancestor_project(
    tmp_path: Path,
) -> None:
