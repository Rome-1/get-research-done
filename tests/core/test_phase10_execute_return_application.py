...
EXECUTE_PHASE = ROOT / "src" / "grd" / "specs" / "workflows" / "execute-phase.md"
EXECUTE_PLAN = ROOT / "src" / "grd" / "specs" / "workflows" / "execute-plan.md"
...
def test_execute_phase_keeps_artifact_gates_explicit() -> None:
    ...
    assert "grd-check-proof" in text
...
def test_execute_plan_update_current_position_uses_canonical_applicator() -> None:
    ...
    assert "grd apply-return-updates" in section
    assert "grd state advance" not in section
    assert "grd state record-metric" not in section
...
def test_execute_plan_decision_and_continuation_handoffs_use_canonical_applicator() -> None:
    ...
    assert "grd apply-return-updates" in decisions
    assert "grd state add-decision" not in decisions
    assert "grd state add-blocker" not in decisions

    assert "grd apply-return-updates" in continuation
    assert "grd state record-session" not in continuation


def test_execute_plan_routes_state_application_through_canonical_applicator() -> None:
    text = _read(EXECUTE_PLAN)

    assert "grd apply-return-updates" in text
