"""Tests for grd.core.context — context assembly for AI agent commands."""
...
from grd.core.errors import ConfigError, ValidationError
from grd.core.frontmatter import compute_knowledge_reviewed_content_sha256
from grd.core.recent_projects import record_recent_project
from grd.core.reproducibility import compute_sha256
...
def _create_config(tmp_path: Path, config: dict) -> Path:
    ...
    config_path = tmp_path / "GRD" / "config.json"
...
def _install_fake_plan_phase_manifest(monkeypatch: pytest.MonkeyPatch) -> _FakePlanPhaseManifest:
    ...
    monkeypatch.setattr("grd.core.workflow_staging.load_workflow_stage_manifest", fake_load_workflow_stage_manifest)
    return manifest
...
def _write_project_contract_state(tmp_path: Path) -> None:
    ...
    (tmp_path / "GRD" / "state.json").write_text(json.dumps(state), encoding="utf-8")
...
def _write_structured_state_payload(tmp_path: Path) -> None:
    ...
    state_path = tmp_path / "GRD" / "state.json"
...
def _write_stat_mech_project(tmp_path: Path) -> None:
    project = tmp_path / "GRD" / "PROJECT.md"
    project.write_text(
        """# Test Project
...
def _write_research_map_anchor_files(tmp_path: Path) -> None:
    map_dir = tmp_path / "GRD" / "research-map"
...
| ------ | ---- | ---------------- | ------------------ | --------------- | ---------------- |
| prior-baseline | prior artifact | `.grd/phases/01-test-phase/01-SUMMARY.md` | Baseline summary for calibration and later comparisons | use | planning/execution |
...
- Universal crossing window
  ...
  - Compared in: `.grd/phases/01-test-phase/01-SUMMARY.md`
  - Status: pending

## Prior Artifacts and Baselines

- `.grd/phases/01-test-phase/01-SUMMARY.md`: Prior baseline summary that later phases must keep visible
...
- Result from Author et al., Journal, 2024: agreement still needs confirmation
  - Comparison in: `.grd/phases/01-test-phase/01-SUMMARY.md`
""",
        encoding="utf-8",
    )
...
class TestInitExecutePhase:
    ...
    def test_json_only_state_counts_as_existing(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "GRD" / "state.json").write_text(json.dumps(default_state_dict()), encoding="utf-8")
    ...
    def test_state_exists_uses_recoverable_backup_without_persisting_repair(
        ...
        _setup_project(tmp_path)
        (tmp_path / "GRD" / "state.json.bak").write_text(
...
class TestInitPlanPhase:
    ...
    def test_surfaces_active_reference_context_and_reference_artifacts(self, tmp_path: Path) -> None:
        ...
        assert "[ref-benchmark]" in ctx["active_reference_context"]
        assert ".grd/literature/benchmark-REVIEW.md" in ctx["literature_review_files"]
        assert ".grd/research-map/REFERENCES.md" in ctx["research_map_reference_files"]
        assert ".grd/research-map/VALIDATION.md" in ctx["reference_artifact_files"]
    ...
    def test_surfaces_stable_knowledge_docs_in_runtime_reference_payload(self, tmp_path: Path) -> None:
        ...
        assert "GRD/knowledge/K-renormalization-group-fixed-points.md" in ctx["knowledge_doc_files"]
        assert ctx["knowledge_doc_count"] == 2
        assert ctx["stable_knowledge_doc_files"] == ["GRD/knowledge/K-renormalization-group-fixed-points.md"]
        ...
        assert "GRD/knowledge/K-renormalization-group-fixed-points.md" in ctx["reference_artifact_files"]
    ...
    def test_prefers_literature_review_files_over_legacy_research_when_both_exist(
        ...
        assert ctx["literature_review_files"] == ["GRD/literature/canonical-REVIEW.md"]
        assert ctx["citation_source_files"] == ["GRD/literature/canonical-CITATION-SOURCES.json"]
        assert "Canonical details." in ctx["reference_artifacts_content"]
        assert "Legacy details." not in ctx["reference_artifacts_content"]
        assert "GRD/research/legacy-REVIEW.md" not in ctx["reference_artifact_files"]
    ...
    def test_falls_back_to_legacy_research_review_files_when_literature_is_missing(
        ...
        assert ctx["literature_review_files"] == ["GRD/research/legacy-REVIEW.md"]
        assert ctx["citation_source_files"] == ["GRD/research/legacy-CITATION-SOURCES.json"]
        assert "Legacy details." in ctx["reference_artifacts_content"]
        assert "GRD/research/legacy-REVIEW.md" in ctx["reference_artifact_files"]
    ...
    def test_surfaces_derived_citation_sources_without_changing_reference_artifact_fields(self, tmp_path: Path) -> None:
        ...
        assert "GRD/literature/benchmark-CITATION-SOURCES.json" in ctx["citation_source_files"]
        ...
        assert "GRD/literature/benchmark-CITATION-SOURCES.json" not in ctx["reference_artifact_files"]
    ...
    def test_non_authoritative_project_contract_does_not_select_protocol_bundles(
        ...
        monkeypatch.setattr(
            "grd.core.context._load_project_contract",
            lambda cwd: (
                contract,
                {
                    "status": "loaded",
                    "source_path": "GRD/state.json",
                    "provenance": "fallback",
        ...
        derived_references = [
            {
                ...
                "source_artifacts": ["GRD/research-map/REFERENCES.md"],
    ...
    def test_does_not_persist_canonical_reference_merges(self, tmp_path: Path) -> None:
        ...
        assert before == after
        assert not (tmp_path / "GRD" / "STATE.md").exists()
    ...
    def test_reports_canonical_project_contract_merge_validation_failures(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("grd.core.context._merge_contract_reference_payload", _invalid_merge)
...
class TestInitNewProject:
    ...
    def test_new_project_bootstrap_skips_reference_artifact_ingestion(
        ...
        monkeypatch.setattr("grd.core.context.ingest_reference_artifacts", _boom)
    ...
    def test_stage_scope_approval_returns_only_contract_fields(self, tmp_path: Path) -> None:
        ...
        assert ctx["staged_loading"]["writes_allowed"] == ["GRD/state.json"]
    ...
    def test_write_paper_bootstrap_stays_small_without_stage(self, tmp_path: Path) -> None:
        _setup_project(tmp_path)
        (tmp_path / "GRD" / "PROJECT.md").write_text("# Project\n\nPaper target.\n", encoding="utf-8")
        _write_project_contract_state(tmp_path)
        _write_manuscript_proof_review_artifacts(tmp_path)
...
class TestInitNewMilestone:
    ...
    def test_new_milestone_stage_roadmap_authoring_filters_payload(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "GRD" / "PROJECT.md").write_text("# Project\n\nMilestone context.\n", encoding="utf-8")
        (tmp_path / "GRD" / "STATE.md").write_text("# State\n\nReady.\n", encoding="utf-8")
        (tmp_path / "GRD" / "REQUIREMENTS.md").write_text("# Requirements\n\n- Confirm objective.\n", encoding="utf-8")
        (tmp_path / "GRD" / "ROADMAP.md").write_text("# Roadmap\n\n## Milestone v1.0\n", encoding="utf-8")
        _write_project_contract_state(tmp_path)
        _write_literature_review_anchor_file(tmp_path)
        _write_research_map_anchor_files(tmp_path)
...
class TestInitResume:
    ...
    def test_with_interrupted_agent(self, tmp_path: Path) -> None:
        _setup_project(tmp_path)
        (tmp_path / "GRD" / "current-agent-id.txt").write_text("agent-123\n")
    ...
    def test_resume_prefers_explicit_grd_workspace_over_recent_project(self, tmp_path: Path) -> None:
        ...
        (workspace / "GRD" / "current-agent-id.txt").write_text("agent-local\n", encoding="utf-8")

        _setup_project(recent_project)
        from grd.core.state import default_state_dict

        (recent_project / "GRD" / "state.json").write_text(
            json.dumps(default_state_dict()),
            encoding="utf-8",
        )
        (recent_project / "GRD" / "ROADMAP.md").write_text("# Roadmap\n", encoding="utf-8")
        (recent_project / "GRD" / "PROJECT.md").write_text("# Project\n", encoding="utf-8")
        resume_path = recent_project / "GRD" / "phases" / "01-analysis" / ".continue-here.md"
        resume_path.parent.mkdir(parents=True, exist_ok=True)
        resume_path.write_text("resume\n", encoding="utf-8")
        record_recent_project(
            ...
            session_data={
                ...
                "resume_file": "GRD/phases/01-analysis/.continue-here.md",
            },
    ...
    def test_exposes_bounded_segment_resume_candidate(self, tmp_path: Path) -> None:
        ...
        _write_current_execution(
            tmp_path,
            {
                ...
                "resume_file": ".grd/phases/03-analysis/.continue-here.md",
        ...
        assert ctx["active_resume_pointer"] == "GRD/phases/03-analysis/.continue-here.md"
        ...
        assert ctx["resume_candidates"][0]["resume_pointer"] == "GRD/phases/03-analysis/.continue-here.md"
        assert "source" not in ctx["resume_candidates"][0]
    ...
    def test_canonical_bounded_segment_carries_last_result_id_and_hydrates_result(
        ...
        (tmp_path / "GRD" / "state.json").write_text(json.dumps(state), encoding="utf-8")
        resume_path = tmp_path / "GRD" / "phases" / "03-analysis" / ".continue-here.md"
        resume_path.parent.mkdir(parents=True, exist_ok=True)
        resume_path.write_text("resume\n", encoding="utf-8")
    ...
    def test_session_resume_file_no_longer_hydrates_resume_authority(self, tmp_path: Path) -> None:
        ...
        state = default_state_dict()
        state["session"]["resume_file"] = "GRD/phases/03-analysis/.continue-here.md"
    ...
    def test_state_md_fallback_no_longer_hydrates_resume_authority_from_legacy_session(self, tmp_path: Path) -> None:
        ...
        resume_path.parent.mkdir(parents=True, exist_ok=True)
        resume_path.write_text("resume\n", encoding="utf-8")
        (tmp_path / "GRD" / "STATE.md").write_text(generate_state_markdown(state), encoding="utf-8")
    ...
    def test_init_resume_propagates_unexpected_continuation_errors(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("grd.core.context.resolve_continuation", _boom)
...
class TestInitVerifyWork:
    ...
    def test_verify_work_surfaces_derived_stable_knowledge_docs(self, tmp_path: Path) -> None:
        ...
        assert ctx["knowledge_doc_warnings"] == []
        assert "GRD/knowledge/K-renormalization-group-fixed-points.md" in ctx["reference_artifact_files"]
        assert "GRD/knowledge/K-work-in-progress.md" not in ctx["reference_artifact_files"]
        assert "Draft knowledge body." not in ctx["reference_artifacts_content"]
        assert "non-stable knowledge doc(s) remain inventory-visible only" in ctx["active_reference_context"]
    ...
    def test_bootstraps_phase_proof_review_manifest(self, tmp_path: Path) -> None:
        ...
        assert (tmp_path / "GRD" / "phases" / "01-setup" / "01-PROOF-REVIEW-MANIFEST.json").exists()
    ...
    def test_exposes_manuscript_proof_review_status(self, tmp_path: Path) -> None:
        ...
        assert status["anchor_artifact"] == "GRD/review/PROOF-REDTEAM.md"
...
class TestInitMilestoneOp:
    ...
    def test_archive_inventory_counts_top_level_files(self, tmp_path: Path) -> None:
        _setup_project(tmp_path)
        archive_dir = tmp_path / "GRD" / "milestones"
...
class TestInitProgress:
    ...
    def test_progress_prefers_explicit_grd_workspace_config_over_recent_project(self, tmp_path: Path) -> None:
        workspace = tmp_path / "workspace"
        recent_project = tmp_path / "recent-project"
        data_root = tmp_path / "data"
    ...
    def test_progress_prefers_live_execution_pause_state(self, tmp_path: Path) -> None:
        ...
        _write_current_execution(
            tmp_path,
            {
                ...
                "phase": "02",
                "segment_status": "paused",
                "resume_file": ".grd/phases/02-analysis/.continue-here.md",
    ...
    def test_progress_normalizes_absolute_live_execution_resume_file(self, tmp_path: Path) -> None:
        _setup_project(tmp_path)
        resume_path = tmp_path / "GRD" / "phases" / "02-analysis" / ".continue-here.md"
        resume_path.parent.mkdir(parents=True, exist_ok=True)
        resume_path.write_text("resume\n", encoding="utf-8")
        _write_current_execution(
            tmp_path,
        ...
        assert ctx["current_execution"]["resume_file"] == "GRD/phases/02-analysis/.continue-here.md"
        assert ctx["current_execution_resume_file"] == "GRD/phases/02-analysis/.continue-here.md"
        assert ctx["execution_resume_file"] == "GRD/phases/02-analysis/.continue-here.md"
    ...
    def test_includes_project(self, tmp_path: Path) -> None:
        _setup_project(tmp_path)
        (tmp_path / "GRD" / "PROJECT.md").write_text("# My Project")
    ...
    def test_load_project_contract_fallback_salvages_nested_metadata_must_surface(
        ...
        from grd.core.state import ensure_state_schema

        normalized_state = ensure_state_schema({"project_contract": contract})
        monkeypatch.setattr("grd.core.context._peek_state_json", lambda cwd: (normalized_state, [], "state.json"))
        monkeypatch.setattr("grd.core.context._load_raw_project_contract_payload", lambda cwd: None)
...
class TestExtractFrontmatterField:
    ...
    def test_empty_value_does_not_bleed_into_next_line(self, tmp_path: Path) -> None:
        ...
        from grd.core.context import _extract_frontmatter_field
