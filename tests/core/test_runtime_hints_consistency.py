...
def _write_current_session(project: Path, *, session_id: str) -> None:
    (project / "GRD" / "observability" / "current-session.json").write_text(
        json.dumps(
            {
...
def _write_current_execution(project: Path, *, session_id: str) -> None:
    (project / "GRD" / "observability" / "current-execution.json").write_text(
        json.dumps(
            {
                "session_id": session_id,
...
def test_build_runtime_hint_payload_reuses_one_resolved_runtime_for_commands_and_source_meta(
    ...
    with patch("grd.hooks.runtime_detect.detect_runtime_for_grd_use", unstable_detector), patch(
...
def test_build_runtime_hint_payload_falls_back_to_cost_summary_runtime_when_no_install_runtime_exists(
    ...
    with patch("grd.core.runtime_hints._installed_runtime_for_surface", return_value=None), patch(
        "grd.core.runtime_hints.build_cost_summary",
