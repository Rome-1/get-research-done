...
def _transient_root_artifacts():
    sentinel_root = "__grd_repo_graph_test__"
    transient_roots = (
        ".npm-cache",
        ".pytest_cache",
        ".ruff_cache",
...
def test_graph_matches_explicit_peer_review_directory_resolution_contract() -> None:
    ...
    assert "`src/grd/cli.py -> peer-review manuscript candidate family {target/main.tex, target/main.md}`" in graph
...
def test_graph_captures_hook_runtime_wiring_edges() -> None:
    ...
    unexpected_edges = [
        "`src/grd/hooks/notify.py -> src/grd/adapters/__init__.py`",
    ]
...
def test_graph_captures_checkpoint_feature_edges() -> None:
    ...
    unexpected_edges = [
        "`src/grd/core/state.py -> src/grd/core/checkpoints.py::sync_phase_checkpoints`",
    ]
...
def test_graph_claude_artifact_language_matches_tree() -> None:
    ...
    if not (REPO_ROOT / ".claude").exists():
        assert "- `.claude/commands/grd/*.md`" not in graph
        assert "- `.claude/agents/*.md`" not in graph
        assert "- `.claude/get-research-done/workflows/**/*.md`" not in graph
        assert "- `.claude/get-research-done/templates/**/*.md`" not in graph
...
def test_graph_sync_repairs_stale_marked_blocks() -> None:
    ...
    stale = replace_marked_block(
        ...
        "\n".join(
            (
                SAME_STEM_COMMAND_WORKFLOW_START,
                "- `src/grd/commands/old.md -> src/grd/specs/workflows/old.md`",
                SAME_STEM_COMMAND_WORKFLOW_END,
            )
        ),
    )
