...
from grd.core.workflow_staging import resolve_workflow_stage_manifest_path, validate_workflow_stage_manifest_payload
...
def test_arxiv_submission_command_declares_manuscript_root_gates_without_first_match_discovery() -> None:
    ...
    assert "Follow `@{GRD_INSTALL_DIR}/workflows/arxiv-submission.md` exactly." in command
    ...
    assert "@{GRD_INSTALL_DIR}/templates/paper/publication-manuscript-root-preflight.md" not in command
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-review-round-artifacts.md" not in command
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-response-artifacts.md" not in command
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-bootstrap-preflight.md" not in command
...
def test_arxiv_submission_workflow_resolves_manifest_based_manuscript_root_without_globbing() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-bootstrap-preflight.md" in workflow
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-review-round-artifacts.md" in workflow
    assert "@{GRD_INSTALL_DIR}/references/publication/peer-review-reliability.md" in workflow
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-response-artifacts.md" not in workflow
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-response-writer-handoff.md" not in workflow
    ...
    assert "grd paper-build" in workflow
