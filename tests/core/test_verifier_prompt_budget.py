"""Prompt budget regressions for the `grd-verifier` agent surface."""
...
from grd.adapters.install_utils import project_markdown_for_runtime
...
def _projected_verifier_prompt(runtime: str) -> str:
    return project_markdown_for_runtime(
        (AGENTS_DIR / "grd-verifier.md").read_text(encoding="utf-8"),
        runtime=runtime,
        path_prefix=PATH_PREFIX,
        surface_kind="agent",
        src_root=SOURCE_ROOT,
        protect_agent_prompt_body=True,
        command_name="grd-verifier",
    )


def test_grd_verifier_prompt_surface_stays_within_expected_budget() -> None:
    metrics = measure_prompt_surface(
        AGENTS_DIR / "grd-verifier.md",
        src_root=SOURCE_ROOT,
        path_prefix=PATH_PREFIX,
    )
...
def test_projected_grd_verifier_prompt_surface_stays_within_runtime_budget(runtime: str) -> None:
