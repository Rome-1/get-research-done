...
def test_resume_docs_use_canonical_paths_and_no_legacy_resume_command() -> None:
    resume_doc = (ROOT / "src/grd/specs/workflows/resume-work.md").read_text(encoding="utf-8")
    portability_doc = (ROOT / "src/grd/specs/references/orchestration/state-portability.md").read_text(encoding="utf-8")
    schema_doc = (ROOT / "src/grd/specs/templates/state-json-schema.md").read_text(encoding="utf-8")
    state_doc = (ROOT / "src/grd/specs/templates/state.md").read_text(encoding="utf-8")
    new_project_doc = (ROOT / "src/grd/specs/workflows/new-project.md").read_text(encoding="utf-8")
    transition_doc = (ROOT / "src/grd/specs/workflows/transition.md").read_text(encoding="utf-8")
    execute_plan_doc = (ROOT / "src/grd/specs/workflows/execute-plan.md").read_text(encoding="utf-8")

    for doc in (resume_doc, portability_doc):
        assert ".grd/" not in doc
        assert re.search(r"/grd:resume(?!-work)\b", doc) is None
        assert "auto_checkpoint" not in doc

    assert "/grd:resume-work" in portability_doc
    ...
    assert "usable state from `.grd/state.json`, `.grd/state.json.bak`, or `.grd/STATE.md`" in resume_doc
    ...
    assert "/grd:sync-state" in portability_doc
    assert "grd state sync" not in portability_doc
    ...
    assert ".grd/state.json.session" in new_project_doc
    assert (
        "**Last session:** [current ISO timestamp]\n"
    ...
    assert "Update the same values under `.grd/state.json.session`" in transition_doc
    assert "save_state_markdown" in transition_doc
    assert "grd --raw state snapshot" not in transition_doc
    ...
    assert ".grd/state.json .grd/PROJECT.md" in transition_doc
...
def test_generate_state_markdown_surfaces_machine_readable_contract_line() -> None:
    ...
    assert "**Machine-readable scoping contract:** `.grd/state.json` field `project_contract`" in markdown
