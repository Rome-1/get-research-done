...
from grd.core.planner_artifacts import resolve_planner_artifact_freshness


def test_resolve_planner_artifact_freshness_accepts_exact_match_and_normalizes_reported_paths(
    tmp_path: Path,
) -> None:
    artifact = tmp_path / "GRD" / "phases" / "01-foundations" / "01-PLAN.md"
    artifact.parent.mkdir(parents=True, exist_ok=True)
    artifact.write_text("# Plan\n", encoding="utf-8")

    result = resolve_planner_artifact_freshness(
        tmp_path,
        ["GRD/phases/01-foundations/01-PLAN.md"],
        ["GRD\\phases\\01-foundations\\01-PLAN.md"],
    )
...
def test_resolve_planner_artifact_freshness_fails_closed_when_artifact_is_missing_from_return(
    ...
    assert "grd_return.files_written" in result.detail


def test_resolve_planner_artifact_freshness_fails_closed_when_return_names_unexpected_artifacts(
    tmp_path: Path,
) -> None:
    expected = tmp_path / "GRD" / "phases" / "01-foundations" / "01-PLAN.md"
    unexpected = tmp_path / "GRD" / "phases" / "01-foundations" / "01-SUMMARY.md"
    ...
    result = resolve_planner_artifact_freshness(
        tmp_path,
        [expected],
        ["GRD/phases/01-foundations/01-PLAN.md", "GRD/phases/01-foundations/01-SUMMARY.md"],
    )
