...
def test_research_phase_command_prompt_budget_keeps_delegation_authorities_out_of_the_wrapper() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/workflows/research-phase.md" in command_text
    assert "@{GRD_INSTALL_DIR}/references/orchestration/model-profile-resolution.md" in command_text
    assert "@{GRD_INSTALL_DIR}/references/orchestration/runtime-delegation-note.md" not in command_text
