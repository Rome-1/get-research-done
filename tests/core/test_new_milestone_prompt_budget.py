...
def test_new_milestone_command_stays_thin_and_only_eagerly_loads_the_workflow() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/references/research/questioning.md" not in command_text
    assert "@{GRD_INSTALL_DIR}/references/ui/ui-brand.md" not in command_text
    assert "@{GRD_INSTALL_DIR}/templates/project.md" not in command_text
    assert "@{GRD_INSTALL_DIR}/templates/requirements.md" not in command_text
    assert "The workflow handles the full milestone initialization flow:" not in command_text
    assert "Read {GRD_INSTALL_DIR}/references/research/questioning.md only when you need guided milestone questioning." in command_text
    assert "Read {GRD_INSTALL_DIR}/templates/project.md only when updating `GRD/PROJECT.md`." in command_text
    assert "Read {GRD_INSTALL_DIR}/templates/requirements.md only when writing `GRD/REQUIREMENTS.md`." in command_text
    assert "Read {GRD_INSTALL_DIR}/references/ui/ui-brand.md only when rendering branded completion or status blocks." in command_text
