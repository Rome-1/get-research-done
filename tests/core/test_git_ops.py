"""Tests for grd.core.git_ops — commit and pre-commit check logic."""
...
class TestPreCommitCheck:
    ...
    @patch("grd.core.git_ops._exec_git")
    ...
    def test_scratch_commit_target_fails_storage_validation(self, tmp_path: Path) -> None:
        ...
        result = cmd_pre_commit_check(tmp_path, [".grd/tmp/final.csv"])
    ...
    def test_internal_artifact_commit_target_fails_storage_validation(self, tmp_path: Path) -> None:
        ...
        result = cmd_pre_commit_check(tmp_path, [".grd/paper/main.tex"])
    ...
    def test_derivation_markdown_with_matching_assertion_passes(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/derivation-01.md",
            "<!-- ASSERT_CONVENTION: metric_signature=mostly-minus -->\n\n# Derivation\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/derivation-01.md"])
    ...
    def test_derivation_markdown_missing_assertion_fails_when_lock_exists(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/derivation-02.md",
            "# Derivation\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/derivation-02.md"])
    ...
    def test_derivation_markdown_mismatch_fails_when_lock_exists(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/derivation-03.md",
            "<!-- ASSERT_CONVENTION: metric_signature=mostly-plus -->\n\n# Derivation\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/derivation-03.md"])
    ...
    def test_derivation_markdown_missing_required_key_fails_when_lock_exists(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/derivation-04.md",
            "<!-- ASSERT_CONVENTION: metric_signature=mostly-minus -->\n\n# Derivation\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/derivation-04.md"])
    ...
    def test_derivation_python_with_matching_assertion_passes(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/derivation-05.py",
            "# ASSERT_CONVENTION: metric_signature=mostly-minus\n\nvalue = 1\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/derivation-05.py"])
    ...
    def test_derivation_python_missing_assertion_fails_when_lock_exists(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/derivation-06.py",
            "print('derivation helper')\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/derivation-06.py"])
    ...
    def test_non_derivation_markdown_remains_unaffected(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/summary.md",
            "# Notes\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/summary.md"])
    ...
    def test_phase_verification_markdown_with_matching_assertion_passes(self, tmp_path: Path) -> None:
        ...
        self._write_markdown(
            tmp_path,
            "GRD/phases/02-derivation/02-VERIFICATION.md",
            "<!-- ASSERT_CONVENTION: metric_signature=mostly-minus -->\n\n# Verification\n",
        )

        result = cmd_pre_commit_check(tmp_path, ["GRD/phases/02-derivation/02-VERIFICATION.md"])
...
class TestCommit:
    ...
    def test_successful_commit(self, tmp_path: Path) -> None:
        ...
        mock_precheck.assert_called_once_with(tmp_path, [".grd/STATE.md"])
    ...
    def test_default_files_stages_planning(self, tmp_path: Path) -> None:
        with (
            ...
            # Verify the git add was called with .grd/
            add_call = mock_git.call_args_list[0]
            assert ".grd/" in add_call[0][1]
            mock_precheck.assert_called_once_with(tmp_path, [".grd/"])
    ...
    def test_commit_blocks_when_pre_commit_check_fails(self, tmp_path: Path) -> None:
        ...
        with (
            patch("grd.core.config.load_config", return_value=MagicMock(commit_docs=True)),
            patch("grd.core.git_ops.cmd_pre_commit_check", return_value=pre_commit),
            patch("grd.core.git_ops._exec_git") as mock_git,
        ):
    ...
    def test_commit_skips_when_commit_docs_disabled(self, tmp_path: Path) -> None:
        with (
            patch("grd.core.config.load_config", return_value=MagicMock(commit_docs=False)),
            patch("grd.core.git_ops.cmd_pre_commit_check") as mock_precheck,
            patch("grd.core.git_ops._exec_git") as mock_git,
        ):
            result = cmd_commit(tmp_path, "test: skipped", files=[".grd/STATE.md"])
...
class TestCommitCLI:
    ...
    @patch("grd.core.git_ops.cmd_commit")
    def test_commit_cli(self, mock_commit: MagicMock) -> None:
        ...
        result = runner.invoke(app, ["commit", "test: message", "--files", ".grd/STATE.md"])
    ...
    def test_commit_cli_accepts_multiple_paths_after_single_files_flag(self, mock_commit: MagicMock) -> None:
        mock_commit.return_value = CommitResult(
            committed=True,
            message="test: message",
            files=[".grd/PROJECT.md", ".grd/state.json"],
            sha="abc1234",
        )
        result = runner.invoke(
            app,
            ["commit", "test: message", "--files", ".grd/PROJECT.md", ".grd/state.json"],
        )
        assert result.exit_code == 0
    ...
    def test_pre_commit_check_cli_accepts_multiple_paths_after_single_files_flag(self, mock_check: MagicMock) -> None:
        ...
        result = runner.invoke(
            app,
            ["pre-commit-check", "--files", ".grd/PROJECT.md", ".grd/state.json"],
        )
        assert result.exit_code == 0
        mock_check.assert_called_once_with(
            ANY,
            [".grd/PROJECT.md", ".grd/state.json"],
        )
