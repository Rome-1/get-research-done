...
import grd.mcp.managed_integrations as managed_integrations
from grd.mcp.managed_integrations import (
...
def test_managed_optional_mcp_helpers_project_from_registry(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert servers == {"grd-alpha": {"command": "alpha-bridge", "args": []}}
    assert managed_integrations.managed_optional_mcp_server_keys() == frozenset({"grd-alpha", "grd-beta"})
    assert alpha.calls == [
