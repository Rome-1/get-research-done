...
def test_execute_phase_reverification_routes_on_typed_status_not_legacy_verifier_text() -> None:
    ...
    assert "After the artifact gate passes, use the canonical verifier verdict from `grd_return.verification_status` or the written report frontmatter:" in workflow
    ...
    assert "If the verifier output is malformed or omits `grd_return.status`" in workflow
...
def test_execute_phase_reverification_requires_files_written_and_disk_artifact_gate() -> None:
    ...
    assert "Do not accept `grd_return.status: completed` until" in workflow
