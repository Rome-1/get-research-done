...
def test_state_record_session_normalizes_project_local_absolute_resume_file(
    ...
    cwd = state_project_factory(tmp_path)
    resume_path = cwd / "GRD" / "phases" / "03-analysis" / ".continue-here.md"
    resume_path.parent.mkdir(parents=True, exist_ok=True)
    resume_path.write_text("resume\n", encoding="utf-8")
    monkeypatch.setattr(
        state_module,
    ...
    assert result.recorded is True
    assert stored["session"]["resume_file"] == ".grd/phases/03-analysis/.continue-here.md"
    assert "**Resume file:** .grd/phases/03-analysis/.continue-here.md" in markdown
...
def test_state_record_session_rejects_nonportable_resume_file(
    ...
    state_path = cwd / "GRD" / "state.json"
    markdown_path = cwd / "GRD" / "STATE.md"
...
def test_init_resume_surfaces_machine_change_and_session_resume_candidate(
    ...
    assert ctx["execution_resume_file"] == ".grd/phases/03-analysis/.continue-here.md"
    ...
    assert ctx["segment_candidates"] == [
        {
            ...
            "resume_pointer": "GRD/phases/03-analysis/.continue-here.md",
        }
    ]
...
def test_init_resume_uses_canonical_continuation_when_legacy_session_conflicts(
    ...
    cwd = state_project_factory(tmp_path)
    canonical_resume = cwd / "GRD" / "phases" / "03-analysis" / ".continue-here.md"
    canonical_resume.parent.mkdir(parents=True, exist_ok=True)
    canonical_resume.write_text("resume\n", encoding="utf-8")
    legacy_resume = cwd / "GRD" / "phases" / "03-analysis" / "legacy.md"
    ...
    assert ctx["continuity_handoff_file"] == "GRD/phases/03-analysis/.continue-here.md"
    assert ctx["recorded_continuity_handoff_file"] == "GRD/phases/03-analysis/.continue-here.md"
    ...
    assert ctx["resume_candidates"][0]["resume_file"] == "GRD/phases/03-analysis/.continue-here.md"
...
def test_init_resume_auto_selects_unique_recoverable_recent_project(tmp_path: Path, state_project_factory, monkeypatch) -> None:
    ...
    resume_path = project_root / "GRD" / "phases" / "03-analysis" / ".continue-here.md"
    resume_path.parent.mkdir(parents=True, exist_ok=True)
    resume_path.write_text("resume\n", encoding="utf-8")
...
def test_init_resume_promotes_auto_selected_recent_bounded_segment_over_same_pointer_handoff(
    ...
    state_path = project_root / "GRD" / "state.json"
    state = json.loads(state_path.read_text(encoding="utf-8"))
    state["intermediate_results"] = [
        {
            "id": "result-recent-03",
...
def test_init_resume_prefers_canonical_handoff_over_live_execution_and_keeps_execution_advisory(
    ...
    _update_state_session(
        ...
        hostname="builder-01",
        platform="Linux 6.1 x86_64",
        resume_file=".grd/phases/03-analysis/alternate-resume.md",
    )
    ...
    assert ctx["segment_candidates"][1]["resume_file"] == ".grd/phases/03-analysis/alternate-resume.md"
...
def test_init_resume_normalizes_project_local_absolute_current_execution_resume_file(
    ...
    cwd = state_project_factory(tmp_path)
    absolute_resume_path = cwd / "GRD" / "phases" / "03-analysis" / ".continue-here.md"
    ...
    assert ctx["current_execution_resume_file"] == ".grd/phases/03-analysis/.continue-here.md"
    assert ctx["execution_resume_file"] == ".grd/phases/03-analysis/.continue-here.md"
    assert ctx["segment_candidates"][0]["resume_file"] == ".grd/phases/03-analysis/.continue-here.md"
...
def test_init_resume_ignores_nonportable_current_execution_resume_file_and_uses_session_handoff(
    ...
    assert ctx["execution_resume_file_source"] == "session_resume_file"
    assert ctx["execution_resume_file"] == ".grd/phases/03-analysis/alternate-resume.md"
    assert ctx["resume_mode"] is None
    ...
    assert ctx["segment_candidates"] == [
        {
            ...
            "kind": "continuity_handoff",
            "origin": "continuation.handoff",
            "resume_pointer": "GRD/phases/03-analysis/alternate-resume.md",
        }
    ]
    assert "compat_resume_surface" not in ctx
...
def test_init_resume_surfaces_missing_session_handoff_as_advisory_candidate(
    ...
    missing_handoff = cwd / "GRD" / "phases" / "03-analysis" / "alternate-resume.md"
    ...
    assert ctx["recorded_continuity_handoff_file"] == "GRD/phases/03-analysis/alternate-resume.md"
    assert ctx["missing_continuity_handoff_file"] == "GRD/phases/03-analysis/alternate-resume.md"
    assert ctx["has_continuity_handoff"] is True
...
def test_init_resume_leaves_missing_result_id_unhydrated(
    ...
    state_path.write_text(json.dumps(state), encoding="utf-8")
    handoff = cwd / "GRD" / "phases" / "03-analysis" / "alternate-resume.md"
    ...
    assert ctx["active_resume_kind"] == "continuity_handoff"
    assert ctx["active_resume_origin"] == "continuation.handoff"
    assert ctx["active_resume_pointer"] == "GRD/phases/03-analysis/alternate-resume.md"
...
def test_init_resume_leaves_selected_candidate_absent_for_ambiguous_recent_projects(
    ...
    for project, stamp, phase in (
        ...
        resume_path = project / "GRD" / "phases" / f"{phase}-analysis" / ".continue-here.md"
        resume_path.parent.mkdir(parents=True, exist_ok=True)
        resume_path.write_text("resume\n", encoding="utf-8")
        record_recent_project(
            project,
            session_data={
                ...
                "resume_file": f"GRD/phases/{phase}-analysis/.continue-here.md",
            },
            store_root=data_root,
        )
...
def test_init_resume_nested_workspace_probe_does_not_create_fake_grd_dir(
    tmp_path: Path,
