...
def test_new_project_and_new_milestone_route_roadmaps_on_typed_status() -> None:
    ...
    for workflow in (new_project, new_milestone):
        ...
        assert "GRD/REQUIREMENTS.md" in workflow
        assert "Do not trust the runtime handoff status by itself." in workflow

    assert "If the roadmapper reports `grd_return.status: completed`, verify that `GRD/ROADMAP.md`, `GRD/STATE.md`, and `GRD/REQUIREMENTS.md` are readable and named in `grd_return.files_written`." in new_project
    ...
    assert "If the roadmapper reports `grd_return.status: completed`, verify that `GRD/ROADMAP.md`, `GRD/STATE.md`, and `GRD/REQUIREMENTS.md` are readable and named in `grd_return.files_written`." in new_milestone
    ...
    assert "treat existing files as stale unless the same paths appear in `grd_return.files_written`" in new_milestone
    assert "If any expected artifact is missing from disk or from `grd_return.files_written`, treat the handoff as incomplete and request a fresh continuation." in new_milestone
...
def test_audit_milestone_consumes_a_typed_consistency_checker_return_without_routing_convention_ownership() -> None:
    ...
    checker = (AGENTS_DIR / "grd-consistency-checker.md").read_text(encoding="utf-8")
