"""Round-trip integration tests that exercise multiple grd/core modules together.
...
def _setup_project(tmp_path: Path) -> Path:
    """Create a minimal GRD project structure."""
...
class TestPhaseLifecycle:
    ...
    def test_completing_a_phase_updates_roadmap_and_advances_state(self, tmp_path: Path) -> None:
        ...
        assert (tmp_path / "GRD" / "phase-checkpoints" / "01-setup.md").exists()
...
class TestPhaseRemoveRenumber:
    ...
    def test_removes_phase_and_renumbers_subsequent(self, tmp_path: Path) -> None:
        ...
        files = list((tmp_path / "GRD" / "phases" / renamed_dir).iterdir())
    ...
    def test_refuses_remove_with_summaries_without_force(self, tmp_path: Path) -> None:
        self._create_fixture(tmp_path)
        d = tmp_path / "GRD" / "phases" / "02-derivation"
    ...
    def test_phase_remove_resyncs_checkpoint_shelf(self, tmp_path: Path) -> None:
        self._create_fixture(tmp_path)
        derivation_dir = tmp_path / "GRD" / "phases" / "02-derivation"
        (derivation_dir / "02-01-SUMMARY.md").write_text(
            '---\nphase: "02"\nplan: "01"\ndepth: full\nprovides: []\ncompleted: "2026-02-23"\none-liner: "Derivation summary"\n---\n\n# Summary\n',
            encoding="utf-8",
        )
        validation_dir = tmp_path / "GRD" / "phases" / "03-validation"
        ...
        assert (tmp_path / "GRD" / "phase-checkpoints" / "02-derivation.md").exists()
        assert (tmp_path / "GRD" / "phase-checkpoints" / "03-validation.md").exists()

        result = phase_remove(tmp_path, "2", force=True)

        assert result.removed == "2"
        assert not (tmp_path / "GRD" / "phase-checkpoints" / "02-derivation.md").exists()
        assert (tmp_path / "GRD" / "phase-checkpoints" / "02-validation.md").exists()
        checkpoints_index = (tmp_path / "GRD" / "CHECKPOINTS.md").read_text(encoding="utf-8")
...
class TestMilestoneLifecycle:
    ...
    def test_full_milestone_lifecycle(self, tmp_path: Path) -> None:
        ...
        milestones = (tmp_path / "GRD" / "MILESTONES.md").read_text()
        assert "v1.0 Core Framework" in milestones

        # Archive should have ROADMAP copy
        archive = tmp_path / "GRD" / "milestones" / "v1.0-ROADMAP.md"
