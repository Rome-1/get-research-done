...
def test_init_context_uses_runtime_detect_directory_fallback(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with monkeypatch.context() as runtime_env:
        ...
        with (
            patch("grd.core.context.Path.home", return_value=tmp_path),
            patch("grd.hooks.runtime_detect.detect_active_runtime", return_value="unknown"),
            patch("grd.hooks.runtime_detect.detect_runtime_for_grd_use", return_value="unknown"),
        ):
            module = importlib.reload(context_module)
            ctx = module.init_new_project(tmp_path)
...
def test_detect_platform_delegates_install_fallback_to_runtime_detector(
    ...
    with monkeypatch.context() as runtime_env:
        ...
        with (
            ...
            patch("grd.hooks.runtime_detect.detect_active_runtime", return_value="unknown"),
            patch("grd.hooks.runtime_detect.detect_runtime_for_grd_use", return_value="unknown"),
            patch("grd.hooks.runtime_detect.detect_runtime_install_target", side_effect=_fake_detect_runtime_install_target),
        ):
            assert context_module._detect_platform(tmp_path) == installed_runtime
...
def test_init_context_prefers_explicit_grd_runtime_override(
    ...
    with monkeypatch.context() as runtime_env:
        _clear_runtime_env(runtime_env)
        runtime_env.setenv(ENV_GRD_ACTIVE_RUNTIME, preferred_runtime)
        (tmp_path / secondary_adapter.local_config_dir_name / GRD_INSTALL_DIR_NAME).mkdir(parents=True)
        (tmp_path / preferred_adapter.local_config_dir_name / GRD_INSTALL_DIR_NAME).mkdir(parents=True)
...
def test_resolve_model_delegates_runtime_specific_lookup_to_config_helper(tmp_path: Path) -> None:
    ...
    assert calls == {"project_dir": tmp_path, "agent_name": "grd-planner", "runtime": runtime}
...
def test_resolve_model_falls_back_to_platform_detection_when_runtime_detector_returns_unknown(tmp_path: Path) -> None:
    ...
    with (
        ...
        result = context_module._resolve_model(tmp_path, "grd-executor")

    assert result == "fallback-model"
    assert calls == {"project_dir": tmp_path, "agent_name": "grd-executor", "runtime": runtime}
...
def test_resolve_model_uses_runtime_unknown_constant_not_literal(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("grd.hooks.runtime_detect.detect_runtime_for_grd_use", return_value=runtime_unknown),
    ...
    assert calls == {"project_dir": tmp_path, "agent_name": "grd-executor", "runtime": None}
...
def test_detect_platform_uses_runtime_unknown_constant_not_literal(
    ...
    with monkeypatch.context() as runtime_env:
        ...
        with (
            patch("grd.hooks.runtime_detect.RUNTIME_UNKNOWN", runtime_unknown),
            patch("grd.core.context.Path.home", return_value=tmp_path),
            patch("grd.hooks.runtime_detect.detect_active_runtime", return_value=runtime_unknown),
            patch("grd.hooks.runtime_detect.detect_runtime_for_grd_use", return_value=runtime_unknown),
            patch("grd.hooks.runtime_detect.detect_runtime_install_target", side_effect=_fake_detect_runtime_install_target),
        ):
            assert context_module._detect_platform(tmp_path) == installed_runtime
...
def test_detect_platform_degrades_cleanly_when_runtime_detect_import_fails(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    runtime_detect_module = sys.modules.get("grd.hooks.runtime_detect")
    monkeypatch.setitem(sys.modules, "grd.hooks.runtime_detect", None)
    try:
        assert context_module._detect_platform(tmp_path) == "unknown"
    finally:
        if runtime_detect_module is None:
            sys.modules.pop("grd.hooks.runtime_detect", None)
        else:
            monkeypatch.setitem(sys.modules, "grd.hooks.runtime_detect", runtime_detect_module)
