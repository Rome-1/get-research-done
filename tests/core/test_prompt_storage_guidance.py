...
def test_compare_branches_uses_in_memory_branch_summary_extraction() -> None:
    ...
    assert "do not write it to `.grd/tmp/` just to run a path-based extractor." in workflow_text
...
def test_parameter_sweep_keeps_internal_docs_in_grd_and_durable_data_in_artifacts() -> None:
    ...
    assert "Do not put point-result JSON under `.grd/phases/**`." in workflow_text
...
def test_compare_experiment_uses_durable_workspace_roots_for_inputs_and_outputs() -> None:
    command_text = (REPO_ROOT / "src" / "grd" / "commands" / "compare-experiment.md").read_text(encoding="utf-8")
    ...
    assert "Treat `.grd/**` as internal provenance only." in command_text
    assert "ls .grd/phases/*/results/" not in command_text
    assert "artifacts/comparisons/{slug}/" in workflow_text
    assert "Do not place final comparison figures, tables, or scripts under `.grd/`." in workflow_text
...
def test_write_paper_uses_durable_figure_and_literature_roots() -> None:
    ...
    assert "ls .grd/literature/*-REVIEW.md 2>/dev/null" in workflow_text
    assert ".grd/phases/*/figures/" not in workflow_text
    assert ".grd/phases/*/LITERATURE-REVIEW.md" not in workflow_text
