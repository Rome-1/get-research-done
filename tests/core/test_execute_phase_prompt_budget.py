...
def test_execute_phase_command_stays_thin_and_only_eagerly_loads_the_workflow() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/workflows/execute-phase.md" in command_text
    assert "@{GRD_INSTALL_DIR}/references/ui/ui-brand.md" not in command_text
    assert "@{GRD_INSTALL_DIR}/templates/summary.md" not in command_text
    assert "@{GRD_INSTALL_DIR}/templates/contract-results-schema.md" not in command_text
    assert "Read {GRD_INSTALL_DIR}/workflows/execute-phase.md first and follow it exactly." in command_text
...
def test_execute_phase_workflow_refreshes_stage_context_in_order() -> None:
    ...
    assert 'grd --raw init execute-phase "${PHASE_ARG}" --include state,config' not in workflow_text
    assert "execute-plan.md owns plan-local execution semantics" in workflow_text
    assert "# task(subagent_type=\"grd-notation-coordinator\"" not in workflow_text
    assert "# task(subagent_type=\"grd-experiment-designer\"" not in workflow_text
