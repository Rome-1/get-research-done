...
def test_referee_routes_on_status_and_shows_base_return_fields_first() -> None:
    source = _read_agent("grd-referee.md")

    assert (
        "The markdown headings `## REVIEW COMPLETE`, `## REVIEW INCOMPLETE`, and `## CHECKPOINT REACHED` are human-readable labels only."
        in source
    )
    assert "Route on `grd_return.status` and the written review artifacts, not on heading text." in source
...
def test_project_researcher_uses_presentation_only_heading_mapping_and_base_fields_first() -> None:
    source = _read_agent("grd-project-researcher.md")

    assert "grd_return:" in source
    assert "status: completed | checkpoint | blocked | failed" in source
    assert "files_written: [GRD/literature/SUMMARY.md, GRD/literature/METHODS.md, ...]" in source
    ...
    assert "Headings above are presentation only; route on grd_return.status." in source

    status_idx = source.index("  status: completed | checkpoint | blocked | failed")
    files_idx = source.index("  files_written: [GRD/literature/SUMMARY.md, GRD/literature/METHODS.md, ...]")
