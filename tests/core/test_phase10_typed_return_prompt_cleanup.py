...
ROADMAPPER = REPO_ROOT / "src/grd/agents/grd-roadmapper.md"
PLANNER = REPO_ROOT / "src/grd/agents/grd-planner.md"
PHASE_RESEARCHER = REPO_ROOT / "src/grd/agents/grd-phase-researcher.md"
EXECUTOR_COMPLETION = REPO_ROOT / "src/grd/specs/references/execution/executor-completion.md"
...
def test_roadmapper_prompt_example_includes_required_base_return_fields() -> None:
    ...
    assert envelope.startswith("grd_return:\n")
...
def test_new_project_roadmapper_task_block_requires_requirements_freshness_and_named_files_written() -> None:
    roadmapper_task = _find_single_task(WORKFLOWS_DIR / "new-project.md", "grd-roadmapper")

    assert "grd_return.files_written" in roadmapper_task.text
    assert "GRD/REQUIREMENTS.md" in roadmapper_task.text
...
def test_planner_tangent_guidance_routes_on_typed_checkpoint_status() -> None:
    ...
    assert "return `grd_return.status: checkpoint` with the four options above instead of silently branching." in planner
    assert (
        "create the recommended main-line plan only and set `grd_return.status: checkpoint` when multiple live alternatives still matter."
        in planner
    )
...
def test_phase_researcher_machine_readable_return_is_typed_first() -> None:
    ...
    assert "grd_return:" in researcher
    ...
    assert "Headings above are presentation only; route on grd_return.status." in researcher
...
def test_executor_completion_spawned_handoff_example_keeps_base_fields_and_extensions() -> None:
    ...
    assert 'files_written: ["GRD/phases/XX-name/{phase}-{plan}-SUMMARY.md"]' in completion
