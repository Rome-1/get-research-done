...
PLANNER_SUBAGENT_PROMPT = REPO_ROOT / "src" / "grd" / "specs" / "templates" / "planner-subagent-prompt.md"


def test_planner_subagent_prompt_stays_thin_and_fail_closed() -> None:
    prompt = PLANNER_SUBAGENT_PROMPT.read_text(encoding="utf-8")

    assert prompt.count("@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md") == 1
    assert "Use `@{GRD_INSTALL_DIR}/templates/plan-contract-schema.md` as the canonical contract source." in prompt
    ...
    assert prompt.count("grd_return.status: checkpoint") >= 3
...
def test_planner_subagent_prompt_keeps_scope_selection_and_revision_glue_only() -> None:
    ...
    assert (
        ...
        "`grd_return.status: checkpoint` rather than guessing."
    ) in prompt
    assert (
        "If the approved project contract is missing or no longer sufficient to identify the right phase slice, return "
        "`grd_return.status: checkpoint` instead of patching around guessed scope."
    ) in prompt
