...
from grd.core.paper_quality import score_paper_quality
...
""".strip()
    ...
    _write(
        tmp_path / "GRD" / "paper" / "FIGURE_TRACKER.md",
        """---
figure_registry:
  - id: fig-benchmark
    label: "Fig. 1"
...
""",
    ...
    _write(
        tmp_path / "GRD" / "comparisons" / "benchmark-COMPARISON.md",
        """---
...
""",
    ...
    _write(
        tmp_path / "GRD" / "phases" / "01-benchmark" / "01-01-PLAN.md",
        (STAGE0_FIXTURES_DIR / "plan_with_contract.md").read_text(encoding="utf-8"),
    )
    _write(
        tmp_path / "GRD" / "phases" / "01-benchmark" / "01-SUMMARY.md",
        (FIXTURES_DIR / "summary_with_contract_results.md").read_text(encoding="utf-8"),
    )
    _write(
        tmp_path / "GRD" / "phases" / "01-benchmark" / "01-VERIFICATION.md",
...
""".strip()
    ...
    _write(tmp_path / "GRD" / "phases" / "00-baseline" / "00-01-SUMMARY.md", "baseline summary\n")
    _write(
        tmp_path / "GRD" / "phases" / "01-benchmark" / "01-01-PLAN.md",
...
def test_build_paper_quality_input_normalizes_empty_contract_results_reference_lists(tmp_path: Path) -> None:
    plan_dir = tmp_path / "GRD" / "phases" / "01-benchmark"
    _write(
...
def test_build_paper_quality_input_does_not_fall_back_to_paper_root_when_manuscript_resolution_is_ambiguous(
    ...
    with pytest.raises(GRDError, match="paper-quality artifact resolution requires an unambiguous manuscript root"):
...
def test_build_paper_quality_input_counts_only_matching_derivation_assertions(
    ...
    _write(
        tmp_path / "GRD" / "analysis" / "derivation-valid.md",
        "<!-- ASSERT_CONVENTION: metric_signature=mostly-plus, fourier_convention=physics -->\n\n# Derivation\n",
    )
    _write(
        tmp_path / "GRD" / "analysis" / "derivation-mismatch.md",
        "<!-- ASSERT_CONVENTION: metric_signature=mostly-minus, fourier_convention=physics -->\n\n# Derivation\n",
    )
    _write(
        tmp_path / "GRD" / "phases" / "01-benchmark" / "derivation-missing.md",
        "# Derivation\n",
    )
...
def test_build_paper_quality_input_counts_python_and_tex_derivation_artifacts(
    ...
    _write(
        tmp_path / "GRD" / "analysis" / "derivation-shell.py",
        "# ASSERT_CONVENTION: metric_signature=mostly-plus, fourier_convention=physics\n\nprint('ok')\n",
    )
    _write(
        tmp_path / "GRD" / "analysis" / "derivation-notes.tex",
        "% ASSERT_CONVENTION: metric_signature=mostly-plus, fourier_convention=physics\n\n\\section{Derivation}\n",
    )
    _write(
        tmp_path / "GRD" / "analysis" / "derivation-outline.md",
        "<!-- ASSERT_CONVENTION: metric_signature=mostly-plus, fourier_convention=physics -->\n\n# Derivation\n",
    )
...
provides: [benchmark comparison]
completed: 2026-03-13
plan_contract_ref: .grd/phases/01-benchmark/01-99-PLAN.md#/contract
contract_results:
  claims:
    claim-benchmark:
      status: passed
...
def test_publication_review_surfaces_keep_protocol_bundle_guidance_additive() -> None:
    ...
    experimental_template = (repo_root / "src/grd/specs/templates/paper/experimental-comparison.md").read_text(
        encoding="utf-8"
    )

    assert "protocol_bundle_context" in write_paper
    assert "additive specialized-publication guidance" in write_paper
    assert ".grd/comparisons/*-COMPARISON.md" in write_paper
    ...
    assert ".grd/paper/FIGURE_TRACKER.md" in peer_review
    assert ".grd/comparisons/*-COMPARISON.md" in peer_review
