...
from grd.core.workflow_staging import validate_workflow_stage_manifest_payload
...
def test_write_paper_command_stays_thin_and_only_eagerly_loads_the_workflow() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/workflows/write-paper.md" in command_text
    assert "@{GRD_INSTALL_DIR}/references/publication/publication-pipeline-modes.md" not in command_text
    assert "@{GRD_INSTALL_DIR}/references/publication/peer-review-panel.md" not in command_text
    ...
    assert "GRD/review/CLAIMS{round_suffix}.json" not in command_text
    assert "GRD/review/STAGE-reader{round_suffix}.json" not in command_text
