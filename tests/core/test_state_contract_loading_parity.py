...
from grd.core.context import _load_project_contract, init_progress
from grd.core.state import default_state_dict, generate_state_markdown, load_state_json, save_state_json, state_load
...
def test_load_state_json_uses_backup_when_primary_root_is_not_an_object(tmp_path: Path) -> None:
    ...
    (layout.grd / STATE_JSON_BACKUP_FILENAME).write_text(json.dumps(recovered, indent=2) + "\n", encoding="utf-8")
...
def test_state_and_context_keep_primary_state_when_primary_root_is_dict_but_schema_corrupt(
    ...
    assert ctx["project_contract_load_info"]["source_path"].endswith(".grd/state.json")
...
def test_state_and_context_restore_backup_project_contract_when_primary_state_is_missing_or_unreadable(
    ...
    with caplog.at_level(logging.WARNING, logger="grd.core.context"):
        ctx = init_progress(tmp_path)
