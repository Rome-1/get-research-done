...
import grd.core.costs as costs
...
def test_record_usage_uses_contract_declared_payload_attribution_keys(
    ...
    assert record.agent_name == "grd-executor"
...
def test_record_usage_falls_back_to_workspace_state_when_payload_keys_are_undeclared(
    ...
    (project / "GRD" / "current-agent-id.txt").write_text("agent-fallback\n", encoding="utf-8")
    ...
    record = record_usage_from_runtime_payload(
        _payload(
            ...
            output_tokens=30,
            cost_usd=0.02,
            extra={"subagent_id": "agent-explicit", "subagent_type": "grd-executor"},
        ),
        runtime=_TELEMETRY_RUNTIME,
        cwd=project,
        data_root=data_root,
...
def test_record_usage_does_not_claim_workspace_agent_id_without_matching_active_session(
    ...
    (project / "GRD" / "current-agent-id.txt").write_text("agent-stale\n", encoding="utf-8")
    _write_current_execution(
        project,
        {
...
def test_record_usage_uses_project_root_for_workspace_state_attribution_and_summary_from_nested_workspace(
    ...
    nested = project / "src"
    nested.mkdir()
    (project / "GRD" / "current-agent-id.txt").write_text("agent-parent\n", encoding="utf-8")
    _write_current_execution(
        project,
        {
...
def test_build_cost_summary_surfaces_advisory_budget_thresholds(
    ...
    data_root = tmp_path / "data"
    project = _bootstrap_project(tmp_path)
    (project / "GRD" / "config.json").write_text(
        json.dumps(
            {
                "execution": {
...
def test_build_cost_summary_surfaces_active_runtime_capabilities_without_records(
    ...
    monkeypatch.setattr("grd.core.config.load_config", lambda _cwd: _Config())
    monkeypatch.setattr("grd.hooks.runtime_detect.detect_runtime_for_grd_use", lambda cwd=None: no_telemetry_runtime)
...
def test_build_cost_summary_surfaces_best_effort_runtime_capabilities_without_records(
    ...
    monkeypatch.setattr("grd.core.config.load_config", lambda _cwd: _Config())
    monkeypatch.setattr("grd.hooks.runtime_detect.detect_runtime_for_grd_use", lambda cwd=None: _TELEMETRY_RUNTIME)
