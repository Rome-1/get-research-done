...
def test_debug_workflow_uses_typed_child_return_and_skips_artifact_inventory() -> None:
    ...
    assert "typed `grd_return` envelope" in workflow
    assert "grd_return.session_file" in workflow
    ...
    assert "mkdir -p GRD/debug" not in workflow
...
def test_execute_phase_debugger_bypass_uses_explicit_debug_contract() -> None:
    ...
    assert "Create: GRD/debug/{FAILED_PLAN}.md" in workflow
