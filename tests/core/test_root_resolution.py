...
from grd.core.root_resolution import (
    RootResolutionBasis,
...
def test_resolve_project_roots_ignores_a_file_named_gpd(tmp_path: Path) -> None:
    ...
    workspace.mkdir(parents=True)
    (project / "GRD").write_text("not a layout directory", encoding="utf-8")
...
def test_resolve_project_roots_prefers_stronger_ancestor_over_nested_empty_grd_stub(
    tmp_path: Path,
) -> None:
    project = tmp_path / "project"
    workspace = project / "workspace" / "notes"
    nested_stub = workspace / "GRD"
    _make_project_root(project)
...
def test_resolve_project_roots_falls_back_to_nearest_bare_grd_when_no_stronger_ancestor_exists(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "workspace"
