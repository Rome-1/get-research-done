...
EXECUTION_REFERENCE_DIR = REPO_ROOT / "src/grd/specs/references/execution"


def _read_executor_prompt() -> str:
    return (AGENTS_DIR / "grd-executor.md").read_text(encoding="utf-8")
...
def test_executor_summary_creation_requires_loading_contract_schema_before_frontmatter() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/templates/contract-results-schema.md" in summary_creation
...
def test_executor_completion_reference_exposes_required_summary_depth_and_completion_fields() -> None:
    ...
    assert "plan_contract_ref: \".grd/phases/XX-name/{phase}-{plan}-PLAN.md#/contract\"" in completion
