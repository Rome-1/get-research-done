"""Focused regressions for the canonical ``grd_return`` contract."""
...
def test_accepts_nested_state_and_continuation_payloads() -> None:
    ...
    result = validate_grd_return_markdown(content)
...
def test_accepts_synthesizer_style_completed_return_with_summary_only_file_list() -> None:
    content = _wrap_return_block(
        "  status: completed\n"
        "  files_written: [GRD/literature/SUMMARY.md]\n"
        "  issues: []\n"
        "  next_actions: []\n"
    )
    ...
    assert result.fields["files_written"] == ["GRD/literature/SUMMARY.md"]
