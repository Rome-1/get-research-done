"""Tests for grd.core.phases — phase lifecycle and roadmap management."""
...
import grd.core.phases as phases_module
...
def _seed_state_pair(
    ...
    *,
    ...
    (tmp_path / "GRD" / "state.json").write_text(json.dumps(state, indent=2), encoding="utf-8")
    return state
...
def test_phase_add(tmp_path: Path) -> None:
    ...
    roadmap = (tmp_path / "GRD" / "ROADMAP.md").read_text()
...
def test_phase_add_updates_total_phases_when_roadmap_contains_decimal_phases(tmp_path: Path) -> None:
    ...
    state = (tmp_path / "GRD" / "STATE.md").read_text(encoding="utf-8")
    state_json = json.loads((tmp_path / "GRD" / "state.json").read_text(encoding="utf-8"))
...
def test_phase_add_leaves_state_files_unchanged_when_atomic_state_save_fails(
    ...
    before_md = (tmp_path / "GRD" / "STATE.md").read_text(encoding="utf-8")
    before_json = (tmp_path / "GRD" / "state.json").read_text(encoding="utf-8")

    def _boom(_cwd: Path, _state_content: str) -> None:
        raise RuntimeError("sync exploded")

    monkeypatch.setattr("grd.core.phases._save_state_markdown", _boom)

    with pytest.raises(RuntimeError, match="sync exploded"):
        phase_add(tmp_path, "New Feature")

    assert (tmp_path / "GRD" / "STATE.md").read_text(encoding="utf-8") == before_md
    assert (tmp_path / "GRD" / "state.json").read_text(encoding="utf-8") == before_json
    assert (tmp_path / "GRD" / "ROADMAP.md").read_text(encoding="utf-8") == before_roadmap
    assert sorted(d.name for d in (tmp_path / "GRD" / "phases").iterdir() if d.is_dir()) == []
...
def test_phase_insert_rolls_back_roadmap_and_directory_when_atomic_state_save_fails(
    ...
    before_roadmap = (tmp_path / "GRD" / "ROADMAP.md").read_text(encoding="utf-8")

    def _boom(_cwd: Path, _state_content: str) -> None:
        raise RuntimeError("sync exploded")

    monkeypatch.setattr("grd.core.state.save_state_markdown_locked", _boom)
...
def _assert_canonical_state_lock_and_locked_writer(
    ...
    monkeypatch.setattr("grd.core.phases.file_lock", _recording_file_lock(lock_calls))
    monkeypatch.setattr(
        "grd.core.state.save_state_markdown",
...
def test_phase_remove_rolls_back_roadmap_and_phase_tree_when_atomic_state_save_fails(
    ...
    before_md = (tmp_path / "GRD" / "STATE.md").read_text(encoding="utf-8")
    before_json = (tmp_path / "GRD" / "state.json").read_text(encoding="utf-8")
    before_dirs = sorted(d.name for d in (tmp_path / "GRD" / "phases").iterdir() if d.is_dir())
    ...
    assert (tmp_path / "GRD" / "STATE.md").read_text(encoding="utf-8") == before_md
    assert (tmp_path / "GRD" / "state.json").read_text(encoding="utf-8") == before_json
    assert sorted(d.name for d in (tmp_path / "GRD" / "phases").iterdir() if d.is_dir()) == before_dirs
...
def test_milestone_complete_rolls_back_archives_when_atomic_state_save_fails(
    ...
    audit_file = tmp_path / "GRD" / "v1.0-MILESTONE-AUDIT.md"
    ...
    assert not (tmp_path / "GRD" / "MILESTONES.md").exists()
...
def test_milestone_complete_rolls_back_partial_archives_when_audit_move_fails(
    ...
    monkeypatch.setattr("grd.core.phases.shutil.move", _boom)
...
def test_phase_remove_integer_removes_descendant_subtree_and_clears_removed_current_plan(tmp_path: Path) -> None:
    ...
    roadmap = (tmp_path / "GRD" / "ROADMAP.md").read_text(encoding="utf-8")
...
def test_phase_complete_uses_roadmap_for_unscaffolded_next_phase(tmp_path: Path) -> None:
    ...
    state = (tmp_path / "GRD" / "STATE.md").read_text()
    assert "**Current Phase:** 02" in state
...
def test_phase_remove_remaps_current_phase_state_after_renumbering(tmp_path: Path) -> None:
    ...
    state_json = json.loads((tmp_path / "GRD" / "state.json").read_text())
