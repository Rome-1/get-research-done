...
def test_plan_phase_separates_planner_checkpoint_handling_from_checker_revision() -> None:
    ...
    assert "spawn a fresh `grd-planner` continuation handoff" in source
...
def test_plan_phase_routes_checker_statuses_through_structured_fields() -> None:
    ...
    assert "`grd_return.status: completed`" in source
    assert "`grd_return.status: checkpoint`" in source
    assert "`grd_return.status: blocked`" in source
    assert "`grd_return.status: failed`" in source
...
def test_plan_phase_reloads_each_stage_and_validates_only_fresh_plan_files() -> None:
    ...
    assert 'grd --raw init plan-phase "$PHASE" --stage research_routing' in source
    assert 'grd --raw init plan-phase "$PHASE" --stage planner_authoring' in source
    assert 'grd --raw init plan-phase "$PHASE" --stage checker_revision' in source
    assert 'FRESH_PLAN_FILES=$(echo "$PLANNER_RETURN" | grd json list .grd_return.files_written --default "")' in source
...
def test_plan_phase_researcher_checkpoint_path_is_a_fresh_continuation_handoff() -> None:
    ...
    assert "After the continuation returns, rerun the same `grd_return.files_written` and on-disk artifact gate above before advancing." in source
