...
def test_verify_work_command_only_eagerly_loads_the_workflow() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/references/verification/core/verification-core.md" not in command_text
    assert "@{GRD_INSTALL_DIR}/templates/verification-report.md" not in command_text
...
def test_verify_work_workflow_defers_heavy_authorities_until_later_steps() -> None:
    ...
    report_owner_marker = "Keep the current check display, summary, and session overlay in sync with the verifier output. The canonical verifier report content remains owned by `grd-verifier`."
