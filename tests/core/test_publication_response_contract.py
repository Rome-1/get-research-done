...
def test_publication_response_writer_handoff_defines_one_shot_child_returns() -> None:
    ...
    assert "grd_return.files_written" in source
    assert "GRD/AUTHOR-RESPONSE{round_suffix}.md" in source
    assert "GRD/review/REFEREE_RESPONSE{round_suffix}.md" in source
    assert "publication-artifact-gates.md" not in source
...
def test_publication_review_round_artifacts_define_canonical_round_family() -> None:
    ...
    assert "GRD/REFEREE-REPORT{round_suffix}.md" in source
    assert "GRD/review/REVIEW-LEDGER{round_suffix}.json" in source
    assert "GRD/review/REFEREE-DECISION{round_suffix}.json" in source
    assert "GRD/AUTHOR-RESPONSE{round_suffix}.md" in source
    assert "GRD/review/REFEREE_RESPONSE{round_suffix}.md" in source
    assert "GRD/review/PROOF-REDTEAM{round_suffix}.md" in source
...
def test_paper_writer_and_referee_load_the_canonical_publication_response_contracts() -> None:
    ...
    assert "grd_return.files_written" in write_paper
    ...
    assert "fresh child `grd_return.files_written`" in respond
