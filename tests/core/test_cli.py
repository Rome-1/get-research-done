...
from grd.core.frontmatter import FrontmatterParseError
...
from grd.core.project_reentry import resolve_project_reentry
...
from grd.core.state import default_state_dict, generate_state_markdown, save_state_json, save_state_markdown
...
def test_help_surfaces_core_and_auxiliary_commands() -> None:
    ...
    assert "Check GRD installation and environment health" in result.output
    assert "inspect runtime readiness" in result.output
    assert "install" in result.output
    assert "Install GRD skills, agents, and hooks into runtime" in result.output
    assert "uninstall" in result.output
    assert "Remove GRD skills, agents, and hooks from runtime" in result.output
    ...
    assert "grd doctor" in normalized_output
    ...
    assert "grd observe execution" in normalized_output
...
def test_install_help_uses_public_surface_examples() -> None:
    ...
    normalized_output = _normalize_cli_output(result.output)
    assert local_cli_install_local_example_command() in normalized_output
    assert "grd install <runtime> # single runtime, local" not in normalized_output
...
def test_integrations_enable_and_disable_wolfram_persist_project_local_config(tmp_path: Path) -> None:
    ...
    config_path = project_root / "GRD" / "integrations.json"
...
def test_integrations_commands_fail_outside_real_project(tmp_path: Path, command: str) -> None:
    ...
    payload = json.loads(result.output)
    assert "real GRD project root" in payload["error"]
    assert not (tmp_path / "GRD").exists()
...
def test_integrations_commands_use_project_root_config_from_nested_workspace(tmp_path: Path) -> None:
    ...
    saved = json.loads(config_path.read_text(encoding="utf-8"))
    assert saved["wolfram"]["enabled"] is True
    assert not (nested_workspace / "GRD").exists()


def test_integrations_status_rejects_legacy_api_key_env_field(tmp_path: Path) -> None:
    config_path = tmp_path / "GRD" / "integrations.json"
    config_path.parent.mkdir(parents=True)
...
def _sample_cost_summary(workspace: Path) -> CostSummary:
    ...
    return CostSummary(
        ...
        guidance=[
            f"Current model posture: profile `review` with {_COST_TEST_RUNTIME} runtime defaults. Use `grd:set-tier-models` to pin explicit tier-1, tier-2, and tier-3 model IDs.",
        ],
    )
...
def test_cost_raw_outputs_summary_payload(tmp_path: Path) -> None:
    ...
    with patch("grd.core.costs.build_cost_summary", return_value=summary) as mock_build:
    ...
    assert payload["advisory"]["next_action"] == "Run `grd cost` for the local usage/cost summary and any USD budget warnings."
...
def test_cost_human_output_stays_read_only_and_advisory(tmp_path: Path) -> None:
    ...
    with patch("grd.core.costs.build_cost_summary", return_value=summary):
...
def test_permissions_status_raw_includes_runtime_capabilities(tmp_path: Path) -> None:
    ...
    with (
        patch("grd.cli._resolve_permissions_runtime_name", return_value=runtime),
        patch("grd.cli._resolve_permissions_target_dir", return_value=target_dir),
        patch("grd.cli._resolve_permissions_autonomy", return_value="yolo"),
        patch(
            "grd.adapters.get_adapter",
...
def test_doctor_help_surfaces_runtime_readiness_mode() -> None:
    ...
    normalized_output = _normalize_cli_output(result.output)
    assert result.exit_code == 0
    assert "Check GRD installation and environment health" in normalized_output
...
def test_active_runtime_settings_command_falls_back_to_runtime_neutral_reference(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("grd.hooks.runtime_detect.detect_runtime_for_grd_use", lambda cwd=None: None)
...
def test_permissions_runtime_resolution_prefers_installed_runtime_selector_when_requested(
    ...
    monkeypatch.setattr(
        "grd.hooks.runtime_detect.detect_runtime_for_grd_use",
...
def test_permissions_status_surfaces_runtime_capabilities_and_config_scope() -> None:
    ...
    with (
        patch("grd.cli._resolve_permissions_runtime_name", return_value=runtime),
        patch("grd.cli._resolve_permissions_target_dir", return_value=target_dir),
        patch("grd.cli._resolve_permissions_autonomy", return_value="balanced"),
        patch("grd.adapters.get_adapter", return_value=_DummyAdapter()),
    ):
        ...
        def runtime_permissions_status(self, target_dir: Path, *, autonomy: str) -> dict[str, object]:
            return {
                ...
                "message": (
                    ...
                    "but GRD left it untouched because that setting was not created by a prior GRD yolo sync."
                ),
            }
    ...
    class _DummyAdapter:
        def sync_runtime_permissions(self, target_dir: Path, *, autonomy: str) -> dict[str, object]:
            return {
                ...
                "requires_relaunch": True,
                "managed_by_gpd": True,
                "launch_command": str(target_dir / "get-physics-done" / "bin" / f"{runtime}-grd-yolo"),
                "message": f"{runtime_display_name(runtime)} only supports yolo at launch time. The GRD launcher is ready for the next session.",
            }
...
def test_permissions_status_reports_incomplete_owned_install_instead_of_generic_missing(
    ...
    adapter = _PermissionsTargetAdapter(
        ...
        missing_install_artifacts=("agents/grd-help/SKILL.md",),
    )

    with (
        patch("grd.cli._resolve_permissions_runtime_name", return_value=PRIMARY_RUNTIME),
        patch("grd.cli._resolve_permissions_autonomy", return_value="balanced"),
        patch("grd.hooks.runtime_detect.detect_runtime_install_target", return_value=None),
        patch("grd.hooks.runtime_detect.detect_install_scope", return_value=None),
        patch("grd.adapters.get_adapter", return_value=adapter),
    ):
        result = runner.invoke(app, ["--raw", "permissions", "status", "--runtime", PRIMARY_RUNTIME, "--autonomy", "balanced"])

    assert result.exit_code == 1
    parsed = json.loads(result.output)
    assert "incomplete GRD install" in parsed["error"]
    assert "Missing artifacts:" in parsed["error"]
    assert "No GRD install found" not in parsed["error"]
...
def test_resume_recovery_advice_uses_resolved_runtime_commands(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        lambda cwd=None: ("/grd:resume-work", "/grd:suggest-next"),
    )
    ...
    advice = cli_module._resume_recovery_advice(
        resume_payload={
            ...
            "active_resume_pointer": "GRD/phases/01/.continue-here.md",
            "continuity_handoff_file": "GRD/phases/01/.continue-here.md",
        },
        recent_rows=[],
        cwd=Path("/tmp/runtime-advice"),
    )

    assert advice.continue_command == "/grd:resume-work"
    assert advice.fast_next_command == "/grd:suggest-next"
...
def test_resume_runtime_commands_logs_runtime_resolution_failures(
    ...
    monkeypatch.setattr("grd.hooks.runtime_detect.detect_runtime_for_grd_use", _boom)

    with caplog.at_level(logging.WARNING, logger="grd.cli"):
...
def test_resume_recent_raw_surfaces_machine_local_recent_projects(
    ...
    home = tmp_path / "home"
    recent_index_dir = home / ".grd" / "recent-projects"
    ...
    (resumable_root / "GRD" / "phases" / "04").mkdir(parents=True, exist_ok=True)
    (resumable_root / "GRD" / "phases" / "04" / ".continue-here.md").write_text("resume\n", encoding="utf-8")
...
def test_resume_recent_human_output_surfaces_command_and_missing_projects(
    ...
    monkeypatch.setattr(
        "grd.core.recent_projects.list_recent_projects",
    ...
    assert "Resume:" in result.output
    assert "grd --cwd" in result.output
...
def test_resume_plain_output_hints_recent_when_workspace_is_missing(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert "No GRD planning directory" in result.output
...
def test_resume_plain_output_surfaces_auto_selected_recent_project(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _cwd: {
            ...
            "resume_candidates": [
                {
                    ...
                    "plan": "01",
                    "segment_id": "seg-3",
                    "resume_file": "GRD/phases/03/.continue-here.md",
                    "resumable": True,
                    "origin": "canonical_continuation",
                }
            ],
            ...
            "active_resume_origin": "canonical_continuation",
            "active_resume_pointer": "GRD/phases/03/.continue-here.md",
...
def test_load_recent_projects_rows_returns_canonical_display_rows(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("grd.core.recent_projects.list_recent_projects", lambda store_root=None, last=None: [canonical_row])
...
def test_load_recent_projects_rows_rejects_malformed_helper_rows(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(cli_module.GRDError, match="unexpected field"):
...
def test_load_recent_projects_rows_prefers_stronger_recovery_over_newer_weaker_target(
    ...
    stronger_root.mkdir()
    (stronger_root / "GRD" / "phases" / "02").mkdir(parents=True)
    (stronger_root / "GRD" / "phases" / "02" / ".continue-here.md").write_text("resume", encoding="utf-8")
...
def test_load_recent_projects_rows_matches_canonical_project_reentry_sorting(
    ...
    for project in (first_project, second_project):
        handoff = project / "GRD" / "phases" / "01" / ".continue-here.md"
...
def test_resume_recent_surfaces_recovery_error_annotation_when_introspection_fails(
    ...
    handoff = project_root / "GRD" / "phases" / "01" / ".continue-here.md"
    handoff.parent.mkdir(parents=True, exist_ok=True)
    handoff.write_text("resume", encoding="utf-8")
    ...
    monkeypatch.setattr("grd.core.context.init_resume", lambda _cwd: (_ for _ in ()).throw(RuntimeError("boom")))
...
def test_resume_candidate_origin_uses_canonical_user_facing_labels() -> None:
    origin, label = cli_module._resume_candidate_origin(
        ...
        active_execution={"resume_file": "GRD/phases/02/.continue-here.md"},
        current_execution={"resume_file": "GRD/phases/02/alternate.md"},
    )
...
def test_resume_plain_output_surfaces_canonical_bounded_segment_without_live_snapshot(
    ...
    monkeypatch.setattr(
        ...
        lambda _cwd: {
            ...
            "resume_candidates": [
                {
                    ...
                    "plan": "01",
                    "segment_id": "seg-6",
                    "resume_file": "GRD/phases/06/.continue-here.md",
                    "resumable": True,
                    "origin": "canonical_continuation",
                }
            ],
            ...
            "active_resume_kind": "bounded_segment",
            "active_resume_origin": "canonical_continuation",
            "active_resume_pointer": "GRD/phases/06/.continue-here.md",
            "has_live_execution": False,
            "execution_resumable": True,
            "execution_paused_at": None,
            "autonomy": None,
...
def test_resume_plain_output_keeps_machine_change_notice_when_session_handoff_is_primary(
    ...
    monkeypatch.setattr(
        ...
        lambda _cwd: {
            ...
            "continuity_handoff_file": "GRD/phases/04/.continue-here.md",
            "active_resume_kind": "continuity_handoff",
            "active_resume_origin": "canonical_continuation",
            "active_resume_pointer": "GRD/phases/04/.continue-here.md",
            "has_live_execution": False,
            "execution_paused_at": None,
            "autonomy": None,
            "research_mode": None,
...
def test_resume_plain_output_surfaces_missing_handoff_status(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _cwd: {
            ...
            "missing_continuity_handoff_file": "GRD/phases/04/.continue-here.md",
            "has_live_execution": False,
            "execution_paused_at": None,
            "autonomy": None,
            "research_mode": None,
...
def test_resume_raw_adds_canonical_recovery_projection_fields(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["primary_recovery_target"]["target"] == "./GRD/phases/01/.continue-here.md"
...
def test_resume_raw_drops_malformed_resume_candidates_from_public_output(
    ...
    assert payload["recovery_candidates"][0]["target"] == "./GRD/phases/01/.continue-here.md"
...
def test_command_supports_project_reentry_prefers_explicit_metadata() -> None:
    assert (
        cli_module._command_supports_project_reentry(
            SimpleNamespace(name="grd:custom", context_mode="project-required", project_reentry_capable=True)
        )
        is True
    )
    assert (
        cli_module._command_supports_project_reentry(
            SimpleNamespace(name="grd:progress", context_mode="project-required", project_reentry_capable=False)
        )
        is False
    )
    assert (
        cli_module._command_supports_project_reentry(
            SimpleNamespace(name="grd:progress", context_mode="project-required")
        )
        is False
    )
...
def test_command_supports_project_reentry_requires_explicit_positive_metadata() -> None:
    assert (
        cli_module._command_supports_project_reentry(
            SimpleNamespace(name="grd:plan-phase", context_mode="project-required")
        )
        is False
    )
...
def test_validate_command_context_help_surfaces_registry_argument_name() -> None:
    ...
    assert "Command registry key or grd:name" in result.output


@patch("grd.core.contract_validation.validate_project_contract")
def test_validate_project_contract_uses_ancestor_project_root_from_nested_cwd(
    ...
    (project_root / "GRD").mkdir(parents=True, exist_ok=True)
...
def test_validate_plan_preflight_blocks_on_missing_required_wolfram(
    ...
    monkeypatch.setattr("grd.core.tool_preflight.shutil.which", lambda _binary: None)
...
def test_state_update_uses_workspace_cwd_when_no_project_root_is_verified(mock_update, tmp_path: Path) -> None:
    ...
    (other_project / "GRD").mkdir(parents=True)
    ...
    with (
        patch("grd.cli.resolve_project_root", return_value=None),
        patch("grd.cli._status_command_cwd", return_value=other_project.resolve(strict=False)),
    ):
...
@patch("grd.core.state.state_set_project_contract")
@patch("grd.core.contract_validation.validate_project_contract")
...
@patch("grd.core.state.state_record_session")
def test_state_record_session_forwards_last_result_id(mock_record_session):
    ...
    result = runner.invoke(
        ...
        [
            ...
            "Paused at task 2/5",
            "--resume-file",
            "GRD/phases/01-test-phase/.continue-here.md",
            "--last-result-id",
            "R-bridge-01",
        ],
    )
    ...
    assert kwargs["resume_file"] == "GRD/phases/01-test-phase/.continue-here.md"
...
@patch("grd.core.phases.list_phase_files")
...
def test_validate_phase_artifacts_reports_expected_frontmatter_errors(tmp_path: Path) -> None:
    ...
    with patch("grd.core.frontmatter.validate_frontmatter", side_effect=FrontmatterParseError("bad yaml")):
        failures = cli_module._validate_phase_artifacts(tmp_path / "GRD" / "phases", "summary")
...
def test_validate_phase_artifacts_does_not_swallow_programmer_errors(tmp_path: Path) -> None:
    ...
    with patch("grd.core.frontmatter.validate_frontmatter", side_effect=RuntimeError("boom")):
        with pytest.raises(RuntimeError, match="boom"):
            cli_module._validate_phase_artifacts(tmp_path / "GRD" / "phases", "summary")
...
def test_progress_uses_workspace_cwd_when_no_project_root_is_verified(
    ...
    with (
        patch("grd.cli.resolve_project_root", return_value=None),
        patch("grd.cli._status_command_cwd", side_effect=AssertionError("unexpected reentry lookup")),
    ):
...
@patch("grd.core.results.result_search", create=True)
...
@patch("grd.core.results.result_upsert", create=True)
...
@patch("grd.core.state.save_state_json_locked")
@patch("grd.core.observability.sync_execution_visibility_from_canonical_continuation", create=True)
@patch("grd.core.results.result_upsert", create=True)
...
@patch("grd.core.observability.sync_execution_visibility_from_canonical_continuation", side_effect=RuntimeError("boom"))
@patch("grd.core.results.result_upsert", create=True)
...
@patch("grd.core.results.result_verify", create=True)
...
@patch("grd.cli._resolve_derived_result_id")
@patch("grd.core.state.state_carry_forward_continuation_last_result_id")
@patch("grd.core.observability.sync_execution_visibility_from_canonical_continuation", create=True)
@patch("grd.core.results.result_upsert_derived", create=True)
...
@patch("grd.core.results.result_update", create=True)
...
@patch("grd.core.results.result_add", create=True)
...
@pytest.mark.parametrize(
    ("patch_target", "argv"),
    [
        ("grd.core.extras.approximation_add", ["approximation", "add", "adiabatic"]),
        ("grd.core.extras.uncertainty_add", ["uncertainty", "add", "mass", "--phase", "10"]),
        ("grd.core.extras.question_add", ["question", "add", "Why", "does", "it", "drift?"]),
        ("grd.core.extras.question_resolve", ["question", "resolve", "Why", "does", "it", "drift?"]),
        ("grd.core.extras.calculation_add", ["calculation", "add", "Evaluate", "kernel"]),
        ("grd.core.extras.calculation_complete", ["calculation", "complete", "Evaluate", "kernel"]),
    ],
)
...
def test_convention_set_fails_closed_for_malformed_primary_state(
    ...
    monkeypatch.setattr("grd.core.conventions.convention_set", _fake_convention_set)
...
def test_result_persist_derived_raw_skips_cleanly_without_project_state(tmp_path: Path) -> None:
    ...
    assert not (tmp_path / "GRD" / "state.json").exists()
...
def test_doctor_live_executable_probes_pass_through(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("grd.core.health.run_doctor", fake_run_doctor)
...
def test_doctor_target_dir_infers_install_scope(mock_doctor, tmp_path: Path) -> None:
    ...
    mock_doctor.return_value = mock_result
    runtime_name = list_runtimes()[0]
    target_dir = tmp_path / ".grd-target"

    with patch("grd.cli._target_dir_matches_global", return_value=True) as mock_matches_global:
        result = runner.invoke(
            app,
...
def test_doctor_target_dir_stays_local_when_target_is_not_global(mock_doctor, tmp_path: Path) -> None:
    ...
    with patch("grd.cli._target_dir_matches_global", return_value=False) as mock_matches_global:
        result = runner.invoke(
            app,
            ["--cwd", str(tmp_path), "--raw", "doctor", "--runtime", runtime_name, "--target-dir", str(target_dir)],
        )
...
def test_doctor_runtime_mode_defaults_to_local_target_when_scope_is_unspecified(
    ...
    with patch(
        "grd.hooks.runtime_detect.detect_runtime_install_target",
...
def test_doctor_rejects_target_dir_without_runtime(tmp_path: Path) -> None:
    result = runner.invoke(app, ["doctor", "--target-dir", str(tmp_path / ".grd-target")])
...
def test_validate_unattended_readiness_wires_local_runtime_scope_through_health_builder(
    ...
    monkeypatch.setattr("grd.core.health.run_doctor", fake_run_doctor)
    monkeypatch.setattr("grd.core.health.build_unattended_readiness_result", fake_build_unattended_readiness_result)
...
def test_runtime_surface_helpers_track_the_active_runtime_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cli_module, "detect_runtime_for_grd_use", lambda cwd=None: runtime_name)
    monkeypatch.setattr("grd.hooks.runtime_detect.detect_runtime_for_grd_use", lambda cwd=None: runtime_name)
    ...
    assert cli_module._runtime_surface_dispatch_note(cwd=workspace) == (
        ...
        "It does not guarantee a same-name local `grd` subcommand exists."
    )
...
def test_runtime_surface_helpers_fall_back_when_runtime_resolution_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cli_module, "detect_runtime_for_grd_use", lambda cwd=None: None)
    monkeypatch.setattr("grd.hooks.runtime_detect.detect_runtime_for_grd_use", lambda cwd=None: None)
...
def test_runtime_surface_helpers_fall_back_on_unexpected_detection_failures(
    ...
    monkeypatch.setattr(cli_module, "detect_runtime_for_grd_use", _raise_runtime_error)
    assert cli_module._active_runtime_command_prefix(cwd=workspace) is None
...
def test_observe_execution_raw_reads_local_visibility_snapshot(tmp_path: Path) -> None:
    observability = tmp_path / "GRD" / "observability"
    ...
    assert payload["next_check_command"] == "grd observe show --session cli-session-1 --last 20"
    assert "execution event trail" in payload["next_check_reason"]
    assert payload["suggested_next_steps"]
    assert any("grd observe show --session cli-session-1 --last 20" in step for step in payload["suggested_next_steps"])
...
def test_observe_export_resolves_output_dir_relative_to_cwd(tmp_path: Path) -> None:
    ...
    with (
        patch("grd.core.observability.export_logs", side_effect=fake_export_logs),
        patch("grd.cli._output"),
    ):
        result = runner.invoke(
            app,
            [
...
@patch("grd.core.context.init_new_project", side_effect=ValueError("Unknown new-project stage 'bogus'."))
...
def test_init_verify_work_preserves_plain_call_shape(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("grd.core.context.init_verify_work", fake_init)
...
def test_init_plan_phase_preserves_plain_call_shape(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("grd.core.context.init_plan_phase", fake_init)
...
def test_init_plan_phase_rejects_stage_and_include_mix(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_init(
        ...
        raise ValueError(
            "grd init plan-phase does not allow --include together with --stage; "
...
def test_init_resume_help_surfaces_recovery_snapshot_entrypoint() -> None:
    ...
    assert result.exit_code == 0
    normalized_output = _normalize_cli_output(result.output)
    assert "Usage: grd init resume" in normalized_output
    assert "Assemble context for resuming previous work." in normalized_output


@patch("grd.core.context.init_resume")
...
def test_validate_paper_quality_from_project_rejects_ambiguous_manuscript_roots(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 1
    assert isinstance(result.exception, cli_module.GRDError)
...
def test_paper_build_without_bibliography_does_not_import_pybtex(tmp_path: Path, monkeypatch) -> None:
    import grd.mcp.paper.compiler  # noqa: F401
...
def test_paper_build_auto_discovers_single_literature_citation_sources_sidecar(tmp_path: Path) -> None:
    ...
    with (
        patch("grd.mcp.paper.compiler.detect_latex_toolchain", return_value=_toolchain_capability()),
        patch("grd.mcp.paper.compiler.build_paper", new=AsyncMock(return_value=result_payload)) as mock_build,
    ):
    ...
    payload = json.loads(result.output)
    assert payload["bibliography_source"] == ""
    assert payload["citation_sources_path"] == "../GRD/literature/topic-CITATION-SOURCES.json"
...
def test_paper_build_auto_discovers_legacy_research_citation_sources_sidecar_when_literature_is_missing(
    ...
    assert result.exit_code == 0
    payload = json.loads(result.output)
    assert payload["citation_sources_path"] == "../GRD/research/topic-CITATION-SOURCES.json"
    assert any("temporary directory" in warning for warning in payload["warnings"])
    assert mock_build.await_args.kwargs["citation_sources"] is not None
...
def test_paper_build_surfaces_toolchain_failure_details(tmp_path: Path) -> None:
    ...
    with (
        patch("grd.mcp.paper.compiler.detect_latex_toolchain", return_value=mock_toolchain),
        patch("grd.mcp.paper.compiler.build_paper", new=AsyncMock(return_value=result_payload)),
    ):
        result = runner.invoke(app, ["--raw", "--cwd", str(tmp_path), "paper-build"], catch_exceptions=False)
...
def test_paper_build_toolchain_payload_surfaces_missing_bibtex_as_hard_failure_risk() -> None:
    with patch(
        "grd.mcp.paper.compiler.detect_latex_toolchain",
