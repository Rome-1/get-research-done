...
def test_resume_work_command_stays_thin_and_only_eagerly_loads_the_workflow() -> None:
    ...
    assert "@{GRD_INSTALL_DIR}/workflows/resume-work.md" in command_text
    assert "Resume research from the selected project's canonical state." in command_text
    assert "Follow the workflow at `@{GRD_INSTALL_DIR}/workflows/resume-work.md`." in command_text
    assert "requires:" in command_text
    assert 'files: ["GRD/ROADMAP.md"]' in command_text
    assert "GRD/STATE.md" not in command_text
