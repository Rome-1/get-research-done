...
from grd.core.public_surface_contract import recovery_ladder_note as public_recovery_ladder_note
...
def test_cost_surface_phrases_stay_conservative_and_advisory() -> None:
    ...
    assert "grd cost" in cost_after_runs_guidance()
...
def test_recovery_surface_phrases_cover_current_and_cross_project_paths() -> None:
    assert "grd resume" in recovery_resume_action()
    assert "grd resume --recent" in recovery_recent_action()
    assert (
        recovery_primary_reason(
    ...
    assert (
        recovery_fast_next_action(fast_next_command="/grd:suggest-next")
    ...
    ladder_note = recovery_ladder_note(
        resume_work_phrase="`/grd:resume-work`",
        suggest_next_phrase="`/grd:suggest-next`",
        pause_work_phrase="`/grd:pause-work`",
    )
...
def test_recovery_next_actions_keep_recent_projects_follow_ups() -> None:
    assert recovery_next_actions(
        primary_command="grd resume --recent",
        mode="recent-projects",
        continue_command="runtime `resume-work`",
        fast_next_command="runtime `suggest-next`",
    ) == [
...
def test_recovery_action_lines_render_structured_actions_with_availability_filter() -> None:
    actions = [
        {"kind": "primary", "command": "grd resume --recent", "availability": "now"},
...
def test_observe_surface_phrases_stay_read_only_and_route_follow_ups_explicitly() -> None:
    assert command_follow_up_action(command="grd observe show --last 20", reason="inspect the recent execution trail") == (
        "Run `grd observe show --last 20` to inspect the recent execution trail."
    )
    assert observe_execution_action() == (
        "Run `grd observe execution` for read-only long-run visibility from your normal terminal."
    )
    ...
    assert (
        observe_tangent_routing_note(
            tangent_phrase="/grd:tangent",
            branch_phrase="/grd:branch-hypothesis",
        )
        == "If `grd observe execution` surfaces an alternative-path follow-up or `branch later` recommendation, route it through `/grd:tangent` first; use `/grd:branch-hypothesis` only after that explicit choice."
    )
    assert (
        observe_tangent_routing_note(
        ...
        == "If `grd observe execution` surfaces an alternative-path follow-up or `branch later` recommendation, route it through runtime `tangent` first; use runtime `branch-hypothesis` only after that explicit choice."
    )
...
def test_preset_and_local_bridge_phrases_remain_command_oriented() -> None:
    ...
    for token in (
        "grd presets list",
        "grd presets show <preset>",
        "grd presets apply <preset> --dry-run",
        "core-research",
        "theory",
        "numerics",
