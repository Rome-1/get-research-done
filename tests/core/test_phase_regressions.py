...
class TestRoadmapCheckboxMatching:
    def _make_project(self, tmp_path: Path, roadmap_content: str) -> Path:
        ...
        grd_dir.mkdir()
        (grd_dir / "phases").mkdir()
        (grd_dir / "ROADMAP.md").write_text(roadmap_content, encoding="utf-8")
        return tmp_path
...
def test_phase_add_does_not_emit_none_slug(tmp_path: Path) -> None:
    ...
    with patch("grd.core.phases.generate_slug", return_value=None):
        result = phase_add(tmp_path, "!!!")

    assert "None" not in result.directory
    created_dirs = [entry.name for entry in (tmp_path / "GRD" / "phases").iterdir() if entry.is_dir()]
...
class TestProgressPercentCapping:
    def _setup_phases(self, tmp_path: Path, plan_counts: dict[str, int], summary_counts: dict[str, int]) -> None:
        grd_dir = tmp_path / "GRD"
        phases_dir = grd_dir / "phases"
        grd_dir.mkdir(parents=True, exist_ok=True)
        (grd_dir / "ROADMAP.md").write_text("## v1.0: Test\n\n", encoding="utf-8")
...
def test_phase_complete_uses_utc_date_near_midnight(tmp_path: Path) -> None:
    ...
    with patch("grd.core.phases.datetime") as mock_datetime:
