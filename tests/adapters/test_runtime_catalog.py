...
import grd.adapters.runtime_catalog as runtime_catalog
...
_RUNTIME_CATALOG_PATH = Path(__file__).resolve().parents[2] / "src" / "grd" / "adapters" / "runtime_catalog.json"
_RUNTIME_CATALOG_SCHEMA_PATH = Path(__file__).resolve().parents[2] / "src" / "grd" / "adapters" / "runtime_catalog_schema.json"
...
def test_runtime_catalog_records_native_include_support() -> None:
    ...
    assert get_runtime_descriptor("claude-code").public_command_surface_prefix == "/grd:"
    assert get_runtime_descriptor("gemini").public_command_surface_prefix == "/grd:"
    assert get_runtime_descriptor("codex").public_command_surface_prefix == "$grd-"
    assert get_runtime_descriptor("opencode").public_command_surface_prefix == "/grd-"
...
def test_shared_install_metadata_is_centralized_in_runtime_catalog() -> None:
    ...
    assert metadata.manifest_name == "grd-file-manifest.json"
    assert metadata.patches_dir_name == "grd-local-patches"
...
def test_managed_install_surface_policy_is_derived_from_runtime_metadata() -> None:
    ...
    assert claude_policy.grd_content_globs == ("get-physics-done/**/*",)
    assert claude_policy.nested_command_globs == ("commands/grd/**/*",)
    assert claude_policy.flat_command_globs == ()
    assert claude_policy.managed_agent_globs == ("agents/grd-*.md", "agents/grd-*.toml")

    assert opencode_policy.nested_command_globs == ()
    assert opencode_policy.flat_command_globs == ("command/grd-*.md",)
    assert codex_policy.nested_command_globs == ()
    assert codex_policy.flat_command_globs == ()
    assert merged_policy.nested_command_globs == ("commands/grd/**/*",)
    assert merged_policy.flat_command_globs == ("command/grd-*.md",)
