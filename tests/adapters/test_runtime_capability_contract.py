...
from grd.core.costs import build_cost_summary, record_usage_from_runtime_payload
from grd.core.surface_phrases import cost_summary_surface_note
...
def test_build_cost_summary_empty_workspace_guidance_tracks_runtime_capabilities(
    ...
    monkeypatch.setattr(runtime_detect, "detect_runtime_for_grd_use", lambda cwd=None: runtime_name)
    ...
    if capabilities.telemetry_completeness == "none":
        assert any(
            f"{runtime_name} does not currently expose a GRD-managed usage telemetry collection path" in item
            for item in summary.guidance
        )
        assert not any(
...
def test_public_runtime_surfaces_stay_conservative_when_capabilities_differ() -> None:
    ...
    settings_workflow = (REPO_ROOT / "src/grd/specs/workflows/settings.md").read_text(encoding="utf-8")
    ...
    if any(descriptor.capabilities.permissions_surface != "unsupported" for descriptor in descriptors):
        assert "grd validate unattended-readiness --runtime <runtime> --autonomy balanced" in readme
