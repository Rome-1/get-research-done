...
import grd.adapters.runtime_catalog as runtime_catalog_module
import grd.core.public_surface_contract as public_surface_contract_module
from grd.adapters import get_adapter, iter_runtime_descriptors
from grd.core.public_surface_contract import beginner_onboarding_hub_url
from grd.core.surface_phrases import recovery_ladder_note
...
def _assert_single_runtime_next_steps(output: str, runtime: str) -> None:
    ...
    ordered_patterns = (
        ...
        re.escape("7. Use grd --help for local diagnostics and later setup."),
    )
    cursor = 0
...
def doctor_check_runtime_launcher(runtime: str) -> dict[str, object]:
    ...
    warnings = [] if launcher_path else [f"Install or expose {{RUNTIME_LABELS[runtime]}} before running GRD there."]
    return {{
...
if args[:4] == ["-m", "grd.cli", "--raw", "doctor"]:
...
if args[:3] == ["-m", "grd.cli", "install"]:
    ...
    if len(runtimes) == 1:
        ...
        print("7. Use grd --help for local diagnostics and later setup.")
    else:
        for runtime in runtimes:
            print(
        ...
        print("Use grd --help for local diagnostics and later setup.")
    record()
    raise SystemExit(0)
    ...
    *,
    ...
    if schema_payload is None:
        schema_payload = json.loads(
            (REPO_ROOT / "src" / "grd" / "core" / "public_surface_contract_schema.json").read_text(encoding="utf-8")
        )
...
const payload = require("./src/grd/core/public_surface_contract.json");
...
missingDoctorPayload.local_cli_bridge.commands = missingDoctorPayload.local_cli_bridge.commands.filter(
  (command) => command !== "grd doctor"
);
assert.throws(
  () => validateSharedPublicSurfaceContract(missingDoctorPayload),
  /local_cli_bridge\.commands must include "grd doctor"/
);
"""
    )
...
def test_bootstrap_public_surface_contract_validator_stays_in_parity_with_python_loader(
    ...
    canonical_payload = json.loads((REPO_ROOT / "src" / "grd" / "core" / "public_surface_contract.json").read_text(encoding="utf-8"))
...
"""
    ...
    additive_payload = json.loads((REPO_ROOT / "src" / "grd" / "core" / "public_surface_contract.json").read_text(encoding="utf-8"))
...
"""
    ...
    missing_payload = json.loads((REPO_ROOT / "src" / "grd" / "core" / "public_surface_contract.json").read_text(encoding="utf-8"))
...
def test_bootstrap_public_surface_contract_schema_validator_stays_in_parity_with_python_loader(
    ...
    canonical_payload = json.loads(
        (REPO_ROOT / "src" / "grd" / "core" / "public_surface_contract.json").read_text(encoding="utf-8")
    )
...
const catalog = require("./src/grd/adapters/runtime_catalog.json");
const runtimeCatalogSchema = require("./src/grd/adapters/runtime_catalog_schema.json");
...
def test_bootstrap_runtime_catalog_validator_stays_in_parity_with_python_loader(
    ...
    canonical_payload = json.loads((REPO_ROOT / "src" / "grd" / "adapters" / "runtime_catalog.json").read_text(encoding="utf-8"))
...
"""
    ...
    additive_payload = json.loads((REPO_ROOT / "src" / "grd" / "adapters" / "runtime_catalog.json").read_text(encoding="utf-8"))
...
"""
    ...
    duplicate_payload = json.loads((REPO_ROOT / "src" / "grd" / "adapters" / "runtime_catalog.json").read_text(encoding="utf-8"))
...
def test_bootstrap_uses_managed_virtualenv_and_skips_host_pip(tmp_path: Path) -> None:
    ...
    managed_runtime_doctor = [
        entry
        for entry in entries
        if entry["managed"] and entry["argv"] == ["-m", "grd.cli", "--raw", "doctor", "--runtime", _CODEX_RUNTIME_NAME, "--local"]
    ]
    ...
    install_index = next(
        index
        for index, entry in enumerate(entries)
        if entry["managed"] and entry["argv"] == ["-m", "grd.cli", "install", _CODEX_RUNTIME_NAME, "--local"]
    )
...
def test_bootstrap_uninstall_routes_to_runtime_uninstall(tmp_path: Path) -> None:
    ...
    managed_runtime_doctor = [
        entry for entry in entries if entry["managed"] and entry["argv"][:4] == ["-m", "grd.cli", "--raw", "doctor"]
    ]
...
def test_bootstrap_install_blocks_when_selected_runtime_launcher_is_missing(tmp_path: Path) -> None:
    ...
    assert f"`grd doctor --runtime {_CODEX_RUNTIME_NAME} --local`" in result.stdout
...
def test_bootstrap_install_blocks_when_target_dir_is_not_writable(tmp_path: Path) -> None:
    ...
    assert f"`grd doctor --runtime {_CODEX_RUNTIME_NAME} --local --target-dir " in result.stdout
