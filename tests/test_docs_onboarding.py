...
from grd.core.onboarding_surfaces import beginner_runtime_surfaces
...
def test_runtime_quickstarts_surface_the_beginner_next_steps(surface) -> None:
    ...
    assert "Back to the onboarding hub: [GRD Onboarding Hub](./README.md)." in content
...
def test_os_quickstarts_install_matrix_matches_runtime_catalog(doc_name: str) -> None:
    ...
    install_section = _markdown_section(content, "## Install GRD")
...
def test_progress_docs_do_not_reference_nonexistent_list_todos_command() -> None:
    command = _read("src/grd/commands/progress.md")
    workflow = _read("src/grd/specs/workflows/progress.md")

    assert "list-todos" not in command
    assert "list-todos" not in workflow
    assert "@{GRD_INSTALL_DIR}/workflows/progress.md" in command
    assert "grd --raw init todos" in workflow


def test_progress_workflow_reconcile_mode_uses_supported_state_snapshot_fields() -> None:
    content = _read("src/grd/specs/workflows/progress.md")

    assert "grd --raw state snapshot" in content
