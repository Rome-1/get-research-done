"""Edge-case tests for GRD install system — 10 scenarios from team lead.

1. Install to read-only directory
2. Install with corrupted package data (missing commands/)
3. Install preserves non-GRD files in commands/
4. GRD_MODEL=invalid:model doesn't affect install
...
def _seed_ambiguous_install_target(target: Path, *, manifest_state: str) -> None:
    ...
    (target / "commands" / "grd" / "help.md").write_text("help\n", encoding="utf-8")
    (target / "get-research-done").mkdir(parents=True, exist_ok=True)
    (target / "get-research-done" / "VERSION").write_text("1.0\n", encoding="utf-8")
...
class TestNonGpdFilesPreserved:
    ...
    def test_install_preserves_non_grd_commands(self, tmp_path: Path) -> None:
        ...
        # Pre-populate with non-GRD commands
...
# =========================================================================
# 5. GRD_MODEL=invalid:model doesn't affect install
# =========================================================================
...
class TestMultiRuntimeSameTarget:
    ...
    def test_both_runtimes_leave_valid_structure(self, tmp_path: Path) -> None:
        ...
        assert (target_gem / "commands" / "grd").is_dir()
...
class TestExpandAtIncludesCircular:
    ...
    def test_three_way_cycle(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(
            tmp_path,
            {
                ...
                "c.md": f"gamma\n@{tmp_path}/get-research-done/a.md",
            },
        )
    ...
    def test_diamond_dependency(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(
            ...
            {
                "d.md": "diamond-leaf",
                "b.md": f"branch-b\n@{tmp_path}/get-research-done/d.md",
                "c.md": f"branch-c\n@{tmp_path}/get-research-done/d.md",
            },
        )
    ...
    def test_include_read_error(self, tmp_path: Path) -> None:
        ...
        bad_file = tmp_path / "get-research-done" / "binary.md"
        bad_file.write_bytes(b"\xff\xfe\x00\x01 invalid utf8")

        content = f"@{tmp_path}/get-research-done/binary.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
    ...
    def test_max_depth_produces_comment(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(
            tmp_path,
            {
                "deep.md": f"deep-content\n@{tmp_path}/get-research-done/deeper.md",
        ...
        result = expand_at_includes(content, str(grd_dir), "~/.test/", depth=9)
