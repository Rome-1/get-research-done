...
def _runtime_literal_patterns() -> list[str]:
    ...
    for descriptor in _RUNTIME_DESCRIPTORS:
        ...
        for value in descriptor.selection_aliases:
            patterns.add(re.escape(value))
            if descriptor.command_prefix == "$grd-":
...
_WOLFRAM_INTEGRATION_BOUNDARY_FILES = {
    "src/grd/core/tool_preflight.py",
    "src/grd/cli.py",
    "src/grd/mcp/managed_integrations.py",
}
_WOLFRAM_INTEGRATION_BOUNDARY_PREFIXES = (
    "src/grd/adapters/",
    "src/grd/mcp/integrations/",
)
...
_ALLOWED_RUNTIME_ADAPTER_FILES = {
    *(f"src/grd/adapters/{adapter.__class__.__module__.rsplit('.', 1)[-1]}.py" for adapter in iter_adapters()),
...
_SHARED_BOOTSTRAP_COMMAND_PATTERN = re.compile(
    r"(\bnpx\b|\bnpm\b|\buvx\b|\bpip\b|\bpipx\b|\bbunx\b|get-research-done)"
)
...
def test_runtime_public_command_prefixes_use_descriptor_public_surface(monkeypatch) -> None:
    ...
    monkeypatch.setattr("grd.adapters.runtime_catalog.iter_runtime_descriptors", lambda: descriptors)
...
def test_shared_mcp_python_surfaces_do_not_hardcode_runtime_command_prefixes() -> None:
    leaks = _scan_paths_for_pattern((REPO_ROOT / "src/grd/mcp",), _SHARED_COMMAND_SURFACE_PATTERN)
...
def test_shared_python_surfaces_do_not_hardcode_runtime_command_prefixes() -> None:
    leaks = _scan_paths_for_pattern((REPO_ROOT / "src/grd",), _SHARED_COMMAND_SURFACE_PATTERN)
    leaks = [
        (path, line_no, snippet)
...
def test_shared_source_surfaces_do_not_hardcode_runtime_tool_alias_literals() -> None:
    ...
    leaks = _scan_paths_for_pattern((REPO_ROOT / "src/grd",), alias_pattern)
    leaks = [
        (path, line_no, snippet)
        for path, line_no, snippet in leaks
        if path.suffix in {".py", ".md", ".json", ".toml"}
        and not path.as_posix().startswith("src/grd/adapters/")
        and path.as_posix() != "src/grd/hooks/runtime_detect.py"
    ]
...
def test_shared_builtin_server_descriptors_do_not_hardcode_bootstrap_commands() -> None:
    leaks = _scan_paths_for_pattern((REPO_ROOT / "src/grd/mcp/builtin_servers.py",), _SHARED_BOOTSTRAP_COMMAND_PATTERN)
...
def test_shared_runtime_docs_do_not_rebuild_install_metadata_literals() -> None:
    doc_paths = (
        REPO_ROOT / "src/grd/specs/workflows/update.md",
        REPO_ROOT / "src/grd/specs/workflows/reapply-patches.md",
        REPO_ROOT / "src/grd/specs/references/tooling/runtime-config-guide.md",
    )
    disallowed_literals = (
...
def test_shared_python_modules_keep_wolfram_integration_tokens_out_of_non_boundary_files() -> None:
    wolfram_pattern = re.compile(
        r"(grd-wolfram|grd-mcp-wolfram|GPD_WOLFRAM_MCP_API_KEY|GPD_WOLFRAM_MCP_ENDPOINT|WOLFRAM_MCP_SERVICE_API_KEY)"
    )
    leaks = [
        (path, line_no, snippet)
...
def test_user_facing_runtime_command_hints_use_runtime_placeholder() -> None:
    leaks = _scan_paths_for_pattern(
        (
            REPO_ROOT / "src/grd/cli.py",
            REPO_ROOT / "src/grd/core/health.py",
            REPO_ROOT / "src/grd/specs/workflows/new-project.md",
        ),
