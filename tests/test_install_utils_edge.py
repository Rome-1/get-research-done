...
def test_replace_placeholders_unknown_runtime_raises_keyerror() -> None:
    with pytest.raises(KeyError, match="Unknown runtime"):
        replace_placeholders("{GRD_RUNTIME_FLAG}", "/custom/", "bogus-runtime")
...
class TestExpandAtIncludes:
    ...
    def _make_src(self, tmp_path: Path, files: dict[str, str]) -> Path:
        """Create a source tree under tmp_path/get-research-done/ and return the grd dir."""
        grd_dir = tmp_path / "get-research-done"
    ...
    def test_simple_include(self, tmp_path: Path) -> None:
        grd_dir = self._make_src(tmp_path, {"sub.md": "included body"})
        content = f"@{tmp_path}/get-research-done/sub.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
    ...
    def test_nested_includes(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(
            tmp_path,
            {
                "c.md": "leaf content",
                "b.md": f"before\n@{tmp_path}/get-research-done/c.md\nafter",
                "a.md": f"top\n@{tmp_path}/get-research-done/b.md\nbottom",
            },
        )
        content = f"@{tmp_path}/get-research-done/b.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
    ...
    def test_cycle_detection(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(
            tmp_path,
            {
                "a.md": f"@{tmp_path}/get-research-done/b.md",
                "b.md": f"@{tmp_path}/get-research-done/a.md",
            },
        )
        content = f"@{tmp_path}/get-research-done/a.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
    ...
    def test_self_cycle_detection(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(
            tmp_path,
            {"self.md": f"@{tmp_path}/get-research-done/self.md"},
        )
        content = f"@{tmp_path}/get-research-done/self.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
        assert "cycle detected" in result

    def test_code_fence_protection(self, tmp_path: Path) -> None:
        """@include inside code fences should NOT be expanded."""
        grd_dir = self._make_src(tmp_path, {"sub.md": "should not appear"})
        content = f"before\n```\n@{tmp_path}/get-research-done/sub.md\n```\nafter"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
        # Original @include line should remain unchanged
        assert f"@{tmp_path}/get-research-done/sub.md" in result
    ...
    def test_depth_limit(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(tmp_path, {"deep.md": "deep content"})
        content = f"@{tmp_path}/get-research-done/deep.md"
        # Start at depth = MAX (10)
        result = expand_at_includes(content, str(grd_dir), "~/.test/", depth=10)
    ...
    def test_bibtex_not_expanded(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(tmp_path, {})
    ...
    def test_missing_file_comment(self, tmp_path: Path) -> None:
        ...
        grd_dir = self._make_src(tmp_path, {})
        content = f"@{tmp_path}/get-research-done/missing.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
    ...
    def test_frontmatter_stripped(self, tmp_path: Path) -> None:
        ...
        content = f"@{tmp_path}/get-research-done/fm.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
    ...
    def test_path_replacement_in_included(self, tmp_path: Path) -> None:
        """Included files should have canonical GRD placeholders replaced."""
        grd_dir = self._make_src(
            tmp_path,
            {"paths.md": "dir={GRD_INSTALL_DIR} config={GRD_CONFIG_DIR}/foo"},
        )
        content = f"@{tmp_path}/get-research-done/paths.md"
        result = expand_at_includes(content, str(grd_dir), "/custom/", runtime="claude-code")
        assert "dir=/custom/get-research-done" in result
        assert "config=/custom/foo" in result

    def test_planning_paths_skipped(self, tmp_path: Path) -> None:
        """.grd/ paths are project-specific, should not be expanded."""
        grd_dir = self._make_src(tmp_path, {})
        content = "@.grd/research/notes.md"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
        assert result == content
    ...
    def test_bullet_list_include_is_expanded(self, tmp_path: Path) -> None:
        grd_dir = self._make_src(tmp_path, {"workflow.md": "workflow body"})
        content = f"- @{tmp_path}/get-research-done/workflow.md (main workflow)"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
    ...
    def test_backticked_bullet_include_with_annotation_is_expanded(self, tmp_path: Path) -> None:
        grd_dir = self._make_src(tmp_path, {"reference.md": "reference body"})
        content = f"- `@{tmp_path}/get-research-done/reference.md` -- Shared reference"
        result = expand_at_includes(content, str(grd_dir), "~/.test/")
        assert "reference body" in result
        assert "<!-- [included: reference.md] -->" in result

    def test_grd_agents_dir_include_resolves_from_specs_root(self, tmp_path: Path) -> None:
        grd_dir = self._make_src(
            tmp_path,
            {
                "agents/grd-shared.md": "---\ndescription: shared\n---\nShared agent body\n",
        ...
        assert "<!-- [included: grd-shared.md] -->" in result

    def test_installed_style_nested_get_research_done_includes_resolve_against_specs_root(self, tmp_path: Path) -> None:
        grd_dir = self._make_src(
            tmp_path,
            {
                "templates/schema.md": "Canonical schema body\n",
                "workflows/verify.md": "@{GRD_INSTALL_DIR}/templates/schema.md\n",
            },
        )

        result = expand_at_includes(
            f"@{tmp_path}/runtime/get-research-done/workflows/verify.md",
            grd_dir,
            f"{tmp_path}/runtime/",
            runtime="codex",
        )
...
class TestCommandVisibilityInjection:
    ...
    def test_requires_only_section_is_injected_once_before_body(self) -> None:
        ...
        assert "GRD/ROADMAP.md" in result
    ...
    def test_review_contract_injection_preserves_conditional_requirements(self) -> None:
        ...
        assert "GRD/review/PROOF-REDTEAM{round_suffix}.md" in result
...
class TestBuildHookCommand:
    ...
    def test_grd_python_override_beats_other_resolution(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("GRD_PYTHON", "/env/override/python")
        monkeypatch.setattr("grd.adapters.install_utils.sys.executable", "/ambient/python")

        assert hook_python_interpreter() == "/env/override/python"

    def test_defaults_to_hidden_home_venv_when_grd_home_is_unset(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.delenv("GRD_PYTHON", raising=False)
        monkeypatch.delenv("GRD_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: fake_home)
        monkeypatch.setattr("grd.adapters.install_utils.sys.executable", "/ambient/python")
        monkeypatch.setattr("grd.version.checkout_root", lambda start=None: None)
        monkeypatch.setattr("grd.version.resolve_checkout_python", lambda start=None, fallback=None: "/ambient/python")

        assert hook_python_interpreter() == str(managed_python)

    def test_prefers_managed_grd_python_outside_checkout(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.delenv("GRD_PYTHON", raising=False)
        monkeypatch.setenv("GRD_HOME", str(managed_home))
        monkeypatch.setattr("grd.adapters.install_utils.sys.executable", "/ambient/python")
        monkeypatch.setattr("grd.version.checkout_root", lambda start=None: None)
    ...
    def test_outside_checkout_ignores_checkout_python_resolution_semantics(
        ...
        monkeypatch.delenv("GRD_PYTHON", raising=False)
        monkeypatch.setenv("GRD_HOME", str(managed_home))
        monkeypatch.setattr("grd.adapters.install_utils.sys.executable", "/repo/.venv/bin/python")
        monkeypatch.setattr("grd.version.checkout_root", lambda start=None: Path("/repo"))
    ...
    def test_checkout_prefers_checkout_virtualenv_python_over_stale_managed_env(
        ...
        monkeypatch.setattr("grd.adapters.install_utils.sys.executable", "/managed/grd/venv/bin/python")
        monkeypatch.setattr("grd.version.checkout_root", lambda start=None: checkout_root)
        monkeypatch.setattr(
            "grd.version.resolve_checkout_python",
    ...
    def test_checkout_falls_back_to_active_python_when_checkout_virtualenv_is_missing(
        ...
        monkeypatch.setenv("GPD_HOME", str(managed_home))
        monkeypatch.setattr("grd.adapters.install_utils.sys.executable", "/ambient/python")
        monkeypatch.setattr("grd.version.checkout_root", lambda start=None: tmp_path / "repo")
        monkeypatch.setattr(
            "grd.version.resolve_checkout_python",
...
class TestCopyWithPathReplacement:
    ...
    def _make_src(self, tmp_path: Path) -> Path:
        ...
        (src / "readme.md").write_text(
            "Config: {GRD_CONFIG_DIR}/foo\nDir: {GRD_INSTALL_DIR}/bar\nAgents: {GRD_AGENTS_DIR}/baz",
            encoding="utf-8",
        )
    ...
    def test_basic_copy(self, tmp_path: Path) -> None:
        ...
        assert "/custom/get-research-done/bar" in md_content
    ...
    def test_codex_slash_command_conversion(self, tmp_path: Path) -> None:
        """Codex runtime should convert /grd: to $grd-."""
        src = tmp_path / "src"
        src.mkdir()
        (src / "commands.md").write_text("Use /grd:execute-phase to run.", encoding="utf-8")

        dest = tmp_path / "dest"
        copy_with_path_replacement(src, dest, "/custom/", "codex")

        content = (dest / "commands.md").read_text(encoding="utf-8")
        assert "$grd-execute-phase" in content
    ...
    def test_opencode_runtime_translates_shared_markdown_content(self, tmp_path: Path) -> None:
        ...
        assert "/grd-plan-phase 3" in content
    ...
    def test_rollback_on_copy_failure(self, tmp_path: Path) -> None:
        ...
        with patch(
            "grd.adapters.install_utils._copy_dir_contents",
    ...
    def test_nested_directories_copied(self, tmp_path: Path) -> None:
        ...
        sub = src / "sub" / "deep"
        sub.mkdir(parents=True)
        (sub / "nested.md").write_text("{GRD_CONFIG_DIR}/test", encoding="utf-8")
...
class TestInstallBackupSafety:
    def test_write_manifest_tracks_hooks(self, tmp_path: Path) -> None:
        ...
        (config_dir / "get-research-done").mkdir(parents=True)
        (config_dir / "get-research-done" / "VERSION").write_text("1.0.0", encoding="utf-8")
    ...
    def test_pre_install_cleanup_backs_up_modified_hook_files(self, tmp_path: Path) -> None:
        ...
        backup_path = config_dir / "grd-local-patches" / "hooks" / "statusline.py"
    ...
    def test_opencode_manifest_tracks_hooks(self, tmp_path: Path) -> None:
        from grd.adapters.opencode import write_manifest as write_opencode_manifest
