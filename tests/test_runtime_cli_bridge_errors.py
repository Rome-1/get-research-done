...
def test_runtime_cli_allows_version_passthrough_as_root_flag(
    ...
    assert exit_code == 0
    assert observed["argv"] == ["grd", root_flag]
...
def test_runtime_cli_repair_command_projection_respects_local_and_custom_global_targets(
    ...
    monkeypatch.setattr("grd.runtime_cli.Path.home", lambda: home_dir)
...
def test_runtime_cli_rejects_manifest_install_scope_mismatch(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert "GRD runtime bridge scope mismatch" in captured.err
...
def test_runtime_cli_rejects_missing_bridge_arguments_without_argparse_abort(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert "GRD runtime bridge rejected malformed bridge invocation." in captured.err
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "unrecognized forwarded grd root flag: --bogus",
        ),
        (
            [
                "--runtime",
            ...
            "unrecognized forwarded grd root flag: --configdir",
        ),
    ],
)
