...
def _mark_complete_install(
    ...
    *,
    ...
    result = adapter.install(GRD_ROOT, config_dir, is_global=install_scope == "global")
...
def _run_runtime_cli_with_recording(
    ...
    *,
    ...
    monkeypatch.delenv(ENV_GRD_ACTIVE_RUNTIME, raising=False)
    monkeypatch.delenv(ENV_GRD_DISABLE_CHECKOUT_REEXEC, raising=False)
    monkeypatch.setattr(adapter, "missing_install_artifacts", record_missing_install_artifacts)
    monkeypatch.setattr("grd.runtime_cli.get_adapter", lambda runtime_name: adapter)
...
def test_runtime_cli_fails_cleanly_for_incomplete_install(tmp_path: Path, capsys, descriptor) -> None:
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert f"GRD runtime install incomplete for {adapter.display_name}" in captured.err
    assert "`grd-file-manifest.json`" in captured.err
    assert "`get-research-done`" in captured.err
    assert f"npx -y get-research-done {adapter.install_flag} --local" in captured.err
...
def test_runtime_cli_fails_when_manifest_runtime_is_missing_or_unrecognized(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert "GRD runtime bridge rejected malformed install manifest" in captured.err
...
def test_runtime_cli_fails_when_manifest_runtime_field_is_missing(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert "GRD runtime bridge rejected incomplete install manifest" in captured.err
...
def test_runtime_cli_preserves_subcommand_runtime_flags(monkeypatch, tmp_path: Path, descriptor) -> None:
    ...
    assert exit_code == 0
    assert observed["argv"] == ["grd", "resolve-model", "grd-executor", "--runtime", foreign_runtime]
    assert observed["runtime"] == descriptor.runtime_name
...
def test_runtime_cli_bridge_parse_preserves_passthrough_after_double_dash() -> None:
    ...
    assert grd_args == ["--raw", "state", "load"]
...
def test_runtime_cli_preserves_root_global_flags_before_subcommand(
    ...
    assert exit_code == 0
    assert observed["argv"] == ["grd", "--raw", "--cwd", str(forwarded_cwd), "state", "load"]
...
def test_runtime_cli_preserves_forwarded_nested_cwd_for_init_resume(
    ...
    assert exit_code == 0
    assert observed["argv"] == ["grd", "--raw", "--cwd", str(forwarded_cwd), "init", "resume"]
...
def test_runtime_cli_keeps_double_dash_passthrough_arguments_verbatim(
    ...
    assert exit_code == 0
    assert observed["argv"] == ["grd", "state", "load", "--", "--raw", "--cwd", "literal"]
...
def test_runtime_cli_reexecs_from_installed_package_using_forwarded_cli_cwd(
    ...
    (checkout_src / "grd").mkdir(parents=True)
    ...
    forwarded_cwd.mkdir(parents=True)
    installed_grd = tmp_path / "site-packages" / "grd"
    installed_grd.mkdir(parents=True)
    ...
    monkeypatch.setattr(runtime_cli, "__file__", str(installed_grd / "runtime_cli.py"))
    monkeypatch.setattr("grd.version.checkout_root", fake_checkout_root)
    monkeypatch.setattr("grd.runtime_cli.os.execve", fake_execve)
    ...
    assert observed["execve_env"][ENV_GRD_DISABLE_CHECKOUT_REEXEC] == "1"
...
def test_runtime_cli_rejects_local_candidate_with_file_prefixes_but_no_runtime(
    ...
    (candidate / "grd-file-manifest.json").write_text(
        json.dumps(
            {
                "install_scope": "local",
...
def test_runtime_cli_rejects_corrupt_manifest_before_dispatch(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert "GRD runtime bridge rejected unreadable install manifest" in captured.err
...
def test_runtime_cli_rejects_missing_manifest_before_dispatch(
    ...
    config_dir = tmp_path / descriptor.config_dir_name
    _mark_complete_install(config_dir, runtime=descriptor.runtime_name)
    (config_dir / "grd-file-manifest.json").unlink()
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("grd.version.checkout_root", lambda start=None: None)
    monkeypatch.setattr(
        "grd.cli.entrypoint",
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert "GRD runtime bridge rejected missing install manifest" in captured.err
    assert "`grd-file-manifest.json`" in captured.err
...
def test_runtime_cli_prefers_manifest_backed_ancestor_over_nearer_marker_only_candidate(
    ...
    (nested_cwd / GRD_INSTALL_DIR_NAME).mkdir(parents=True)
    monkeypatch.chdir(nested_cwd)
    monkeypatch.setattr("grd.version.checkout_root", lambda start=None: None)
    monkeypatch.setattr(
        "grd.cli.entrypoint",
...
def test_runtime_cli_rejects_manifestless_torn_managed_surface_before_dispatch(
    ...
    assert str(config_dir) in captured.err
    assert str(nested_cwd / descriptor.config_dir_name) not in captured.err
    assert "GRD runtime bridge rejected incomplete install manifest" not in captured.err
...
def test_runtime_cli_rejects_non_utf8_manifest_before_dispatch(
    ...
    config_dir = tmp_path / descriptor.config_dir_name
    _mark_complete_install(config_dir, runtime=descriptor.runtime_name)
    (config_dir / "grd-file-manifest.json").write_bytes(b"\xff")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("grd.version.checkout_root", lambda start=None: None)
    monkeypatch.setattr(
        "grd.cli.entrypoint",
...
def test_runtime_cli_resolves_local_config_dir_from_forwarded_cli_cwd(
    ...
    assert observed["argv"] == ["grd", "state", "load", "--cwd", str(forwarded_cwd)]
    assert observed["runtime"] == descriptor.runtime_name
    assert observed["disable_reexec"] == "1"
...
def test_runtime_cli_uses_last_repeated_forwarded_cli_cwd_for_bridge_resolution(
    ...
    assert exit_code == 0
    assert observed["argv"] == ["grd", "state", "load", "--cwd", str(ignored_cwd), "--cwd", str(final_cwd)]
    assert observed["runtime"] == descriptor.runtime_name
    assert observed["disable_reexec"] == "1"
...
def test_runtime_cli_fails_when_resolved_local_config_dir_manifest_runtime_mismatches(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert f"GRD runtime bridge mismatch for {adapter.display_name}" in captured.err
    assert f"{get_adapter(foreign_runtime).display_name} (`{foreign_runtime}`)" in captured.err
    assert f"--target-dir {config_dir}" in captured.err
    assert f"npx -y get-research-done {get_adapter(foreign_runtime).install_flag} --local" in captured.err
...
def test_runtime_cli_uses_manifest_owner_scope_for_mismatch_repair_guidance(
    ...
    captured = capsys.readouterr()
    assert exit_code == 127
    assert f"GRD runtime bridge mismatch for {get_adapter(descriptor.runtime_name).display_name}" in captured.err
    assert f"{get_adapter(foreign_runtime).display_name} (`{foreign_runtime}`)" in captured.err
    assert f"npx -y get-research-done {get_adapter(foreign_runtime).install_flag}" in captured.err
    assert "--global" in captured.err
