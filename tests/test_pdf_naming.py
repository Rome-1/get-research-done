...
from grd.mcp.paper.compiler import CompilationResult
from grd.mcp.paper.models import Author, PaperConfig, Section, derive_output_filename
...
def _journal_spec():
    from grd.mcp.paper.models import JournalSpec
...
class TestBuildPaperNaming:
    def test_build_paper_uses_derived_filename_for_tex(self, tmp_path: Path) -> None:
        ...
        with (
            patch("grd.mcp.paper.compiler.render_paper", return_value="\\documentclass{article}"),
            patch("grd.mcp.paper.compiler.get_journal_spec", return_value=_journal_spec()),
            patch("grd.mcp.paper.compiler.check_journal_dependencies", return_value=(False, ["missing"])),
            patch("grd.mcp.paper.compiler.build_artifact_manifest", return_value=None),
            patch("grd.mcp.paper.compiler.write_artifact_manifest"),
        ):
    ...
    def test_build_paper_surfaces_actual_output_paths_without_main_compatibility_copies(self, tmp_path: Path) -> None:
        ...
        with (
            patch("grd.mcp.paper.compiler.render_paper", return_value="\\documentclass{article}"),
            patch("grd.mcp.paper.compiler.get_journal_spec", return_value=_journal_spec()),
            patch("grd.mcp.paper.compiler.check_journal_dependencies", return_value=(True, [])),
            patch("grd.mcp.paper.compiler.build_artifact_manifest", return_value=None),
            patch("grd.mcp.paper.compiler.write_artifact_manifest"),
            patch("grd.mcp.paper.compiler.compile_paper", side_effect=mock_compile),
        ):
            from grd.mcp.paper.compiler import build_paper
