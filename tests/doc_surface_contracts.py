...
def assert_help_command_single_command_extract_contract(content: str) -> None:
    ...
    _assert_contains_any(
        content,
        (
            "Accept either a bare command name such as `plan-phase` or a canonical runtime command such as `grd:plan-phase`.",
            "bare command name",
    ...
    _assert_contains_any(
        content,
        (
            "If the lookup includes inline flags or arguments such as `grd:new-project --minimal`, normalize it to the base command block",
...
def assert_tour_read_only_teaching_contract(content: str) -> None:
    _assert_contains_any(
        ...
        (
            ...
            "read-only tour of the main GRD commands",
    ...
    _assert_contains_any(
        ...
        (
            ...
            "what GRD can do before choosing",
...
def assert_tour_command_surface_contract(content: str) -> None:
    ...
    _assert_contains_any(
        ...
        (
            ...
            "the normal terminal, where you install GRD",
            "the runtime, where you use the GRD command prefix",
        ),
...
def assert_beginner_startup_routing_contract(content: str) -> None:
    ...
    startup_markers = (
        ladder,
        ladder.strip("`"),
        "If you just installed GRD, use this order first:",
...
def assert_start_workflow_router_contract(content: str) -> None:
    _assert_contains_any(
        content,
        (
            "Give a first-run chooser for people who may not know GRD yet.",
            "first-run chooser",
        ),
    ...
    *,
    ...
    if allow_explicit_alias_examples:
        _assert_contains_any(
            ...
            (
                compatibility_note,
                "Compatibility-only backend intake (`grd init resume` only):",
            ),
        ...
        assert "Compatibility-only backend intake (`grd init resume` only):" not in content
...
def assert_settings_local_terminal_follow_up_contract(content: str) -> None:
    ...
    _assert_contains_any(
        ...
        (
            _unattended_readiness_surface(),
            "grd validate unattended-readiness --runtime <runtime> --autonomy <mode>",
        ),
    ...
    _assert_contains_any(
        ...
        (
            _permissions_status_surface(),
            _permissions_sync_surface(),
            "grd permissions sync --runtime <runtime> --autonomy <mode>",
        ),
        label="settings runtime-permission follow-up surface",
    )
    assert "grd cost" in content
...
def assert_optional_paper_workflow_guidance_contract(content: str) -> None:
    ...
    _assert_contains_any(
        content,
        (
            "Missing preset tooling degrades that preset; it does not block the base GRD install.",
            "degrade `write-paper`",
...
def assert_publication_toolchain_boundary_contract(content: str) -> None:
    _assert_contains_any(
        content,
        (
            "Use `grd paper-build` to judge whether the manuscript scaffold is buildable.",
...
def assert_workflow_preset_surface_contract(content: str) -> None:
    assert "grd presets list" in content
    assert "grd presets show <preset>" in content
    assert "grd presets apply <preset>" in content
    _assert_contains_any(
        content,
        (
...
def assert_wolfram_plan_boundary_contract(content: str) -> None:
    ...
    _assert_contains_any(
        ...
        (
            ...
            "plan gate",
            "does not replace `grd validate plan-preflight <PLAN.md>`",
            "stays separate from `grd validate plan-preflight <PLAN.md>`",
        ),
