...
from grd.hooks import runtime_detect as runtime_detect_module
...
def test_latest_update_cache_prefers_considered_candidate_order(tmp_path: Path) -> None:
    ...
    for candidates, payloads, expected_cache, expected_index in cases:
        ...
        with (
            patch("grd.hooks.runtime_detect.get_update_cache_candidates", return_value=list(candidates)),
...
def test_primary_update_cache_file_falls_back_to_home_data_root_cache(tmp_path: Path) -> None:
    ...
    assert cache_file == home / ".grd" / "cache" / "grd-update-check.json"


def test_home_update_cache_path_comes_from_one_helper_for_lookup_and_write_paths(tmp_path: Path) -> None:
    helper_path = tmp_path / ".grd" / "cache" / "grd-update-check.json"
...
def test_latest_update_cache_uses_shared_cache_constants_for_self_owned_install_and_workspace_precedence(
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    workspace_cache = workspace / ".claude" / "cache" / "grd-update-check.json"
    ...
    for patch_values, expected_cache_path in cases:
        with (
            patch("grd.hooks.install_context.detect_self_owned_install", return_value=patch_values["self_install"]),
            patch("grd.hooks.runtime_detect.detect_active_runtime", return_value=patch_values["active_runtime"]),
            patch("grd.hooks.runtime_detect.detect_active_runtime_with_grd_install", return_value=patch_values["active_runtime"]),
            patch(
...
def test_latest_update_cache_falls_back_when_self_owned_cache_is_missing(tmp_path: Path) -> None:
    ...
    self_install = SimpleNamespace(
        cache_file=self_cache_dir / "cache" / "grd-update-check.json",
    ...
    with (
        ...
        patch("grd.hooks.install_context.should_prefer_self_owned_install", return_value=True),
        patch(
            "grd.hooks.update_resolution.resolve_update_cache_inputs",
            return_value=(tmp_path, tmp_path / "home", "codex", "codex"),
        ),
        patch("grd.hooks.update_resolution.ordered_update_cache_candidates", return_value=[fallback_candidate]),
    ):
        cache, candidate = latest_update_cache(hook_file=__file__, cwd=str(tmp_path), debug=_noop_debug)
...
def test_update_command_for_candidate_prefers_expected_resolution_modes(tmp_path: Path) -> None:
    ...
    cases = (
        ...
        (
            type("Candidate", (), {"path": local_runtime_dir / "cache" / "grd-update-check.json", "runtime": "codex", "scope": "local"})(),
            None,
            _repair_command("codex", install_scope="local", target_dir=local_runtime_dir, explicit_target=False),
            workspace,
            (("grd.hooks.runtime_detect._runtime_dir_has_grd_install", True),),
        ),
        (
            SimpleNamespace(
                path=global_runtime_dir / "cache" / "grd-update-check.json",
                runtime="codex",
                scope="global",
            ),
            None,
            ...
            workspace,
            (
                ("grd.hooks.install_context.resolve_hook_lookup_context", SimpleNamespace(lookup_cwd=workspace, resolved_home=resolved_home, active_runtime=None, preferred_runtime=None)),
                ("grd.hooks.runtime_detect.Path.home", resolved_home),
            ),
        ),
        (
    ...
    for candidate, self_install_value, expected, cwd, extra_patches in cases:
        ...
        with patch("grd.hooks.install_context.detect_self_owned_install", return_value=self_install_value):
            if candidate.scope == "global":
                with (
                    ...
                    patch("grd.hooks.runtime_detect.Path.home", return_value=resolved_home),
                ):
                ...
                with patch("grd.hooks.runtime_detect._runtime_dir_has_grd_install", return_value=True):
                    command = update_command_for_candidate(candidate, hook_file=__file__, cwd=str(cwd))
            else:
                command = update_command_for_candidate(candidate, hook_file=__file__, cwd=str(cwd))
