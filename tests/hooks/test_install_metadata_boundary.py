...
from grd.hooks.runtime_detect import _manifest_runtime_status as runtime_detect_manifest_runtime_status
from grd.runtime_cli import _manifest_runtime_status as runtime_cli_manifest_runtime_status
        ...
        json.dumps(
            {
                "install_scope": "local",
                "files": {
                    "skills/grd-help/SKILL.md": "legacy-hint",
                },
            }
        ),
        encoding="utf-8",
...
def test_config_dir_has_managed_install_markers_ignores_empty_managed_dirs(tmp_path: Path) -> None:
    ...
    (config_dir / "commands" / "grd").mkdir(parents=True, exist_ok=True)
...
def test_assess_install_target_classifies_owned_complete_and_incomplete_install(
    ...
    monkeypatch.setattr("grd.hooks.install_metadata.get_adapter", lambda runtime: _FakeAdapter(()))
    complete = assess_install_target(config_dir)

    monkeypatch.setattr(
        "grd.hooks.install_metadata.get_adapter",
        lambda runtime: _FakeAdapter(("agents/grd-help/SKILL.md",)),
    )
    ...
    assert incomplete.missing_install_artifacts == ("agents/grd-help/SKILL.md",)
...
def test_assess_install_target_classifies_foreign_and_untrusted_manifests(
    ...
    (foreign_dir / "grd-file-manifest.json").write_text(
        json.dumps({"install_scope": "local", "runtime": "codex"}),
        encoding="utf-8",
    )
    ...
    (untrusted_dir / "grd-file-manifest.json").write_text(
...
def test_runtime_detect_manifest_helper_signature_drops_unused_cwd_and_home() -> None:
    from grd.hooks.runtime_detect import _runtime_from_manifest_or_path
...
def test_runtime_detect_install_helper_signature_drops_unused_cwd_and_home() -> None:
    from grd.hooks.runtime_detect import _has_grd_install

    params = inspect.signature(_has_grd_install).parameters
...
def test_install_metadata_keeps_manifest_boundary_free_of_install_utils_imports() -> None:
    import grd.hooks.install_metadata as install_metadata

    source = inspect.getsource(install_metadata)

    assert "from grd.adapters.install_utils import" not in source
