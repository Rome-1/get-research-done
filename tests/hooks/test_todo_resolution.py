...
from grd.hooks.install_context import SelfOwnedInstallContext, ordered_todo_lookup_candidates
...
def test_ordered_todo_lookup_candidates_ignores_runtime_less_explicit_target_hook_dir(
    ...
    with patch("grd.hooks.runtime_detect.get_todo_candidates", return_value=[]):
...
def test_ordered_todo_lookup_candidates_uses_non_project_cwd_for_runtime_preference_lookup(tmp_path: Path) -> None:
    ...
    with (
        patch("grd.hooks.install_context.resolve_project_root", return_value=None),
        patch("grd.hooks.install_context.detect_self_owned_install", return_value=None),
        patch("grd.hooks.runtime_detect.detect_active_runtime_with_grd_install", return_value="unknown"),
        patch("grd.hooks.runtime_detect.detect_runtime_for_grd_use", return_value="codex") as mock_preferred,
        patch("grd.hooks.runtime_detect.get_todo_candidates", return_value=[]),
        patch("grd.hooks.runtime_detect.should_consider_todo_candidate", return_value=True),
    ):
