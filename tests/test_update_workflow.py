...
def test_update_workflow_uses_current_runtime_agnostic_contract() -> None:
    ...
    assert 'PYTHON_BIN="${GRD_PYTHON:-}"' in content
...
def test_reapply_patches_workflow_uses_runtime_config_placeholders() -> None:
    ...
    for descriptor in _RUNTIME_DESCRIPTORS:
        runtime_patch_path = f"~/{descriptor.config_dir_name}/grd-local-patches"
        workspace_patch_path = f"./{descriptor.config_dir_name}/grd-local-patches"
...
def test_default_local_install_keeps_local_update_scope_and_manifest(
    ...
    _install_and_finalize(adapter, GRD_ROOT, target, is_global=False, **_install_kwargs_for_descriptor(descriptor, tmp_path))

    update_content = (target / "get-research-done" / "workflows" / "update.md").read_text(encoding="utf-8")
    reapply_content = (target / "get-research-done" / "workflows" / "reapply-patches.md").read_text(
        encoding="utf-8"
    )
    ...
    assert f'GRD_CONFIG_DIR="{target.as_posix()}"' in update_content
    assert f'GRD_INSTALL_DIR="{(target / "get-research-done").as_posix()}"' in update_content
    assert f'PATCHES_DIR="{target.as_posix()}/grd-local-patches"' in reapply_content
    if "skills/" in descriptor.manifest_file_prefixes:
...
def test_explicit_target_local_install_keeps_local_update_scope(tmp_path: Path, descriptor) -> None:
    ...
    assert 'TARGET_DIR_ARG=$("$PYTHON_BIN" - "$INSTALL_SCOPE" "$GRD_CONFIG_DIR" "$GRD_GLOBAL_CONFIG_DIR"' in content
...
def test_explicit_target_global_install_keeps_global_update_scope(
    ...
    target = tmp_path / "explicit-global" / f"{descriptor.runtime_name}-config"
    target.mkdir(parents=True)
    monkeypatch.setattr("grd.adapters.install_utils.Path.home", lambda: tmp_path / "ambient-home")
    ...
    assert f'GRD_CONFIG_DIR="{target.as_posix()}"' in content
    assert f'GRD_GLOBAL_CONFIG_DIR="{canonical_global.as_posix()}"' in content
    assert "{GRD_INSTALL_SCOPE_FLAG}" not in content
    assert 'TARGET_DIR_ARG=$("$PYTHON_BIN" - "$INSTALL_SCOPE" "$GRD_CONFIG_DIR" "$GRD_GLOBAL_CONFIG_DIR"' in content
...
def test_global_install_without_explicit_target_returns_no_trusted_update_command(
    ...
    with monkeypatch.context() as ctx:
        ctx.setattr("grd.hooks.install_metadata.Path.home", lambda: home_dir)
        _install_and_finalize(adapter, GRD_ROOT, canonical_target, **install_kwargs)

    manifest_path = canonical_target / "grd-file-manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest.pop("explicit_target", None)
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
...
def test_global_install_materializes_authoritative_paths_for_custom_home(
    ...
    with monkeypatch.context() as ctx:
        ctx.setattr("grd.adapters.install_utils.Path.home", lambda: tmp_path / "ambient-home")
